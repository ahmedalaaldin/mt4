//+------------------------------------------------------------------+
//|   Buy & Sell EA v295 - Longs Only (Max_Sells=0)                  |
//|   BASE: v293/v280 CHAMPION (Calmar=18.43, Return=220.62%, 637 T) |
//|   CHANGE: Max_Sells = 0 (only LONG trades taken)                 |
//|   RATIONALE:                                                      |
//|     Data from v293 shows asymmetric performance:                 |
//|       Long (BUY)  win rate: 95.68%  (509/532 won)               |
//|       Short (SELL) win rate: 89.84% (128/143 won)               |
//|     Shorts have 5.84pp LOWER win rate and 2.4× MORE losses       |
//|     per trade than longs. Removing shorts eliminates the         |
//|     lowest-quality trade segment without changing entry logic.   |
//|     Gold's structural bull trend (2025-2026) means mean-         |
//|     reversion BUYS at the lower BB band have better alignment    |
//|     with the dominant directional bias. Mean-reversion SELLS     |
//|     at the upper BB band fight the trend more often.             |
//|   HYPOTHESIS: ~532 longs-only, win rate ~95.7%, MaxDD drops     |
//|     to ~9-11%, Calmar improves above 18.43.                     |
//|   SCOREBOARD:                                                     |
//|     v280: Calmar=18.36 (champion baseline)                       |
//|     v290: Calmar=14.90 (aggressive AM — WORSE)                  |
//|     v291: Calmar=7.72  (BBW<8.0 — DISASTER)                     |
//|     v292: Calmar=9.02  (SL=0.6×ATR — DISASTER)                 |
//|     v293: Calmar=18.43 (Max_Bars=30 — marginal +0.07)           |
//|     v294: Calmar=11.57 (Session 7-22 — DISASTER)                |
//|     v295: Longs Only — TARGET >20                                |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "295.0"
#property strict

#include <stdlib.mqh>

//--- Session (same as v280/v293 champion: 9-21)
...
//--- Exit Settings (Max_Bars=30 from v293)
...
input int    Magic_Number       = 2952057;
input string Order_Comment      = "BSv295";
...
void ManageExits()
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      // Note: SELL management kept in case manually placed orders exist,
      // but no new SELL orders will be opened (Max_Sells=0)
...
void OnTick()
{
   ...
   // h1Bear kept in code but Max_Sells=0 prevents any sell orders
   ...
   //=== BUY ONLY (Max_Sells=0 prevents any SELL orders) ===
   if(h1Bull && stochBuy)
   {
   ...
   //=== SELL === DISABLED (Max_Sells=0: CountTradesByMagic < 0 is always false)
   // Sell block kept for reference but never executes:
   /*
   if(h1Bear && stochSell)
   {
      ... sell logic (unreachable with Max_Sells=0) ...
   }
   */
}
//+------------------------------------------------------------------+
