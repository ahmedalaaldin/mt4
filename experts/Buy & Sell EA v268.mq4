//+------------------------------------------------------------------+
//|   Buy & Sell EA v268 - Remove H1 EMA direction filter too        |
//|   Framework: H1 RSI range only / M5 Entry                        |
//|   HYPOTHESIS: H1 EMA direction gate also over-restrictive        |
//|     v267 champion: H1 EMA direction gate only (H4 already gone)  |
//|     v268 test: Remove H1 EMA direction too, only H1 RSI in range |
//|     Expected: More trades, test if H1 direction is necessary     |
//|     Both H4_EMA and H1_EMA still present (periods NON-NEG)       |
//|     but neither used as directional entry gate                   |
//|   ALL OTHER PARAMS IDENTICAL TO v267 (CURRENT CHAMPION)         |
//|     Return=162.35%, MaxDD=11.32%, Calmar=14.34, 609 trades       |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "268.0"
#property strict
...
input int    H1_EMA_Period      = 200;            // Period NON-NEGOTIABLE (not used as gate in v268)

//--- H1 RSI Gate (v186 CONFIRMED OPTIMUM - direction gate removed, RSI range kept)
...
input int    Magic_Number       = 2682057;
input string Order_Comment      = "BSv268";
...
void OnTick()
{
   ...
   // v268: Both H4 and H1 EMA direction gates REMOVED
   // H4_EMA_Period and H1_EMA_Period still present as inputs (NON-NEGOTIABLE periods)
   // but neither is used as a directional entry gate
   ...
   // v268: Only H1 RSI range kept as multi-TF filter (no EMA direction)
   // bool h1Bull = (iClose(NULL, PERIOD_H1, 1) > h1_ema)    // REMOVED
   //            && (h1_rsi1 >= H1_RSI_Min) && (h1_rsi1 <= H1_RSI_Max);
   // bool h1Bear = (iClose(NULL, PERIOD_H1, 1) < h1_ema)    // REMOVED
   //            && (h1_rsi1 >= H1_RSI_Min_Sell) && (h1_rsi1 <= H1_RSI_Max_Sell);
   bool h1RsiBuy  = (h1_rsi1 >= H1_RSI_Min)      && (h1_rsi1 <= H1_RSI_Max);
   bool h1RsiSell = (h1_rsi1 >= H1_RSI_Min_Sell) && (h1_rsi1 <= H1_RSI_Max_Sell);
   ...
   //=== BUY: Both H4 and H1 EMA direction removed, H1 RSI range only ===
   if(h1RsiBuy && stochBuy)
   {
   ...
   //=== SELL: Both H4 and H1 EMA direction removed, H1 RSI range only ===
   if(h1RsiSell && stochSell)
   {
