//+------------------------------------------------------------------+
//|   Buy & Sell EA v264 - TP 5.0->10.0 + Max_Bars fix (!partialDone)|
//|   Framework: H4+H1 Direction / M5 Entry (v186 CONFIRMED BEST)   |
//|   LESSONS LEARNED:                                               |
//|     v258: Partial_Pct=0.20 -> Calmar=10.62 *** NEW BEST ***     |
//|     v259: Partial_Pct=0.10 -> Calmar=10.56 (worse than v258)    |
//|     v260: Trail=0.3 -> Calmar=10.75 *** NEW BEST ***            |
//|     v261: Trail=0.1 -> Calmar=10.91 *** CHAMPION ***            |
//|       Return=111.08%, MaxDD=10.18%, 470 trades, Win=94.26%       |
//|     v262: Trail=0.0 -> Calmar=10.79 (worse than v261)           |
//|       Trail_ATR_Multi=0.1 CONFIRMED OPTIMAL                      |
//|     v263: ATR_TP_Multi=5.0 -> Calmar=10.98 *** NEW BEST ***     |
//|       Return=111.58%, MaxDD=10.16%, 472 trades, Win=94.28%       |
//|       Larger TP lets strong trending moves run further            |
//|                                                                  |
//|   v264 HYPOTHESIS:                                               |
//|     1) ATR_TP_Multi 5.0->10.0: effectively removes hard TP cap   |
//|        Trail at bbMid-0.1*ATR is the REAL exit mechanism         |
//|        TP=10.0 only fires on extreme momentum moves              |
//|     2) Max_Bars fix: only time-exit if !partialDone              |
//|        Profitable trailing positions (SL>=entry) run indefinitely |
//|        Only stagnating pre-partial positions get time-cut         |
//|        Avoids killing winning trails; cuts stagnating losers      |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "264.0"
#property strict
...
input double ATR_TP_Multi       = 10.0;           // v264: 5.0->10.0, bbMid trail is real exit
input double Trail_ATR_Multi    = 0.1;            // v261 CONFIRMED OPTIMAL
input double Partial_Pct        = 0.20;           // v258 CONFIRMED OPTIMAL
input int    Max_Bars           = 100;            // v264: only applies when !partialDone
...
input int    Magic_Number       = 2642057;
input string Order_Comment      = "BSv264";
...
void ManageExits()
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(type == OP_BUY)
      {
         ...
         // v264 KEY FIX: only time-exit stagnating pre-partial positions
         // Profitable trailing positions (partialDone) are never time-stopped
         if(!partialDone && bars >= Max_Bars)
