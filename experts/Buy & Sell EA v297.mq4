//+------------------------------------------------------------------+
//|   Buy & Sell EA v297 - Stronger Body Filter (Body_Pct 0.2→0.4)  |
//|   BASE: v293/v280 CHAMPION (Calmar=18.43, Return=220.62%, 637 T) |
//|   CHANGE: Body_Pct = 0.40 (was 0.20)                            |
//|   RATIONALE:                                                      |
//|     The body pct filter requires: body >= Body_Pct * totalRange  |
//|     At 0.20: a doji candle with 20% body qualifies → weak signal |
//|     At 0.40: only strong directional candles qualify → cleaner   |
//|     signal with clear bullish/bearish commitment.               |
//|     False reversals often appear as small-body candles at BB     |
//|     extremes — they touch the band but have no strong intent.   |
//|     Stronger body = market committed to the reversal direction.  |
//|   HYPOTHESIS: Remove ~15% of weak-body trades → 540-560 trades,  |
//|     win rate rises from 94.51% to ~97%,                         |
//|     MaxDD similar (keeps both longs+shorts diversification),     |
//|     Calmar could reach 20+.                                     |
//|   SCOREBOARD:                                                    |
//|     v293: Calmar=18.43 (champion — Max_Bars=30)                 |
//|     v296: Calmar=18.14 (Partial 35% — dead end)                 |
//|     v295: Calmar=10.11 (Longs only — lost diversification)      |
//|     v294: Calmar=11.57 (Session 7-22 — DISASTER)               |
//|     v297: Body 40% — TARGET Calmar >20                          |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "297.0"
#property strict
...
input double Body_Pct           = 0.40;   // KEY CHANGE: was 0.20, now 0.40
...
input int    Magic_Number       = 2972057;
input string Order_Comment      = "BSv297";
...
void OnTick()
{
   ...
   //=== BUY === (Body_Pct=0.40: require stronger bullish body)
   if(h1Bull && stochBuy)
   {
      ...
      bool bullBody  = (candleBodyB > 0) && (candleRange > 0)
                    && (candleBodyB >= Body_Pct * candleRange); // KEY: 40% body required
   ...
   //=== SELL === (Body_Pct=0.40: require stronger bearish body)
   if(h1Bear && stochSell)
   {
      ...
      bool bearBody   = (candleBodyS > 0) && (candleRange > 0)
                     && (candleBodyS >= Body_Pct * candleRange); // KEY: 40% body required
