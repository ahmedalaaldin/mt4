//+------------------------------------------------------------------+
//|   Buy & Sell EA v141 - BB(2.0) + Fixed Daily DD Guard            |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry         |
//|   Base: v140 (with critical bug fixes)                           |
//|   Changes vs v140:                                               |
//|     1. FIXED: Circuit breaker removed (caused deadlock -         |
//|        once fired with no open trades, equity can never          |
//|        recover → permanent shutdown)                             |
//|     2. FIXED: H1 RSI gate widened for bull market:              |
//|        H1_RSI_Min: 45→35, H1_RSI_Max: 75→85 for buys           |
//|        H1_RSI_Max_Sell: 55→65 for sells                         |
//|        (v140 had 0 sells all year: gold >H4 EMA always)         |
//|     3. Daily DD guard tightened: 15% → 8% (safe daily stop      |
//|        that resets each day, no deadlock possible)               |
//|     4. Scale_High: 1.75 → 1.5 (max 7.5% risk per trade)        |
//|     5. RSI_Buy: 50 → 45 (slightly tighter for BB 2.0 quality)  |
//|     6. RSI_Sell: 50 → 55 (symmetric)                           |
//|   Unchanged from v140:                                           |
//|     BB_Dev=2.0, Max_Buys=3, Max_Sells=3                        |
//|     Risk_Pct=5%, anti-martingale WinStreak_Mid=3/High=6        |
//|     Scale_Mid=1.5, ATR_SL_Multi=1.0                            |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "141.0"
#property strict
...
//--- H1 RSI Gate (widened for trending markets)
input string H1RSI_SET          = "============"; //====== H1 RSI Gate ======
input int    H1_RSI_Period      = 14;             //H1 RSI period
input double H1_RSI_Min         = 35.0;           //H1 RSI min for BUY (was 45)
input double H1_RSI_Max         = 85.0;           //H1 RSI max for BUY (was 75)
input double H1_RSI_Min_Sell    = 15.0;           //H1 RSI min for SELL (was 25)
input double H1_RSI_Max_Sell    = 65.0;           //H1 RSI max for SELL (was 55)

//--- Bollinger Bands (M5)
input string BB_SET             = "============"; //====== BB Settings ======
input int    BB_Period          = 20;             //BB period
input double BB_Dev             = 2.0;            //BB SD: 2.0 for higher frequency

//--- Signal: M5 BB Touch
input string S1_SET             = "============"; //====== Signal: BB Touch ======
input int    RSI_Period         = 14;             //M5 RSI period
input double RSI_Buy            = 45.0;           //M5 RSI max for BUY
input double RSI_Sell           = 55.0;           //M5 RSI min for SELL
...
input double Scale_High         = 1.5;            //Scale at high win streak (7.5% risk max)
...
input string Order_Comment      = "BSv141";       //Order comment
input double Max_Daily_DD_Pct   = 8.0;            //Daily DD stop (% of day open equity)
...
void OnTick()
{
   ...
   // Daily DD guard (8% per day - resets each morning, no deadlock)
