//+------------------------------------------------------------------+
//|   Buy & Sell EA v284 - Three-Bar Breakdown-Recovery Pattern      |
//|   Framework: H1 EMA direction + H1 RSI gate / M5 Entry          |
//|   HYPOTHESIS: v280's two-bar filter improved Calmar from 17.77   |
//|     to 18.36 by requiring bar[2] to have closed beyond the BB.   |
//|     A three-bar version strengthens this further:                 |
//|     - bar[3]: Low[3] <= bbLower3 (first touch/breach of band)    |
//|     - bar[2]: Close[2] <= bbLower2 (confirmed close below band)  |
//|     - bar[1]: recovery (our entry bar)                           |
//|     The three-bar pattern means price was pressed below the band  |
//|     for 2 consecutive bars before buyers defended it. This is a  |
//|     very strong reversal signal. Hypothesis: further improves    |
//|     win rate and Calmar but fewer trades (acceptable if <20%     |
//|     reduction vs v280, which had 637 trades).                    |
//|   BASE: v280 NEW CHAMPION (Calmar=18.36, Return=220%, 637 trades)|
//|   CHANGE: Added bar[3] low <= bbLower3 requirement               |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "284.0"
#property strict
...
//--- Three-bar pattern settings (NEW in v284, extends v280's two-bar)
input string THREEBARSET        = "============"; //====== Three-Bar Pattern ======
input bool   Use_ThreeBar       = true;  // bar[3] low must have touched BB, bar[2] closed below
...
input int    Magic_Number       = 2842057;
input string Order_Comment      = "BSv284";
...
void OnTick()
{
   ...
   double bbUpper3 = iBands(NULL, 0, BB_Period, BB_Dev, 0, PRICE_CLOSE, MODE_UPPER, 3);
   double bbLower3 = iBands(NULL, 0, BB_Period, BB_Dev, 0, PRICE_CLOSE, MODE_LOWER, 3);
   ...
   //=== BUY: Three-bar pattern — bar[3] low touched, bar[2] closed below, bar[1] recovers ===
   if(h1Bull && stochBuy)
   {
      ...
      // Three-bar: bar[3] low touched band AND bar[2] closed below band
      bool bbThreeBar = (!Use_ThreeBar) || ((Low[3] <= bbLower3) && (Close[2] <= bbLower2));

      if(bbTouch && rsiOS && rsiRising && bullBody && bbThreeBar)
      {
   ...
   //=== SELL: Three-bar pattern — bar[3] high touched, bar[2] closed above, bar[1] recovers ===
   if(h1Bear && stochSell)
   {
      ...
      // Three-bar: bar[3] high touched band AND bar[2] closed above band
      bool bbThreeBar = (!Use_ThreeBar) || ((High[3] >= bbUpper3) && (Close[2] >= bbUpper2));

      if(bbTouchUp && rsiOB && rsiFalling && bearBody && bbThreeBar)
      {
