//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)+------------------------------------------------------------------+
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals) (sweep: 0.5→0.35→0.25→0.15)|
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|   BASE: v411 ALL-TIME BEST (ratio=9.50, MaxDD=4.32%) ✓          |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|                                                                   |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|   SCOREBOARD (all M5 unless noted):                              |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|     v342: M5+M15 → +29.51%, MaxDD=4.74%, WR=86.90%, 756t ✓    |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|     v343: M5+M15+M30 → +30.67%, MaxDD=4.87%, WR=85.14%, 814t ✓|
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|     v354: M5+M15+M30, ATR_Spike=2.0 → +30.33%, MaxDD=4.65%✅   |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|     v355: ATR_Spike=1.5 → +0.73%, WR=70.16%, 305t  FAIL ✗      |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|     v356: Session 7-23 → +8.32%, MaxDD=5.33%❌, 619t FAIL ✗     |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|     v357: M5+M15 only → +28.66%, WR=87.05%✓, MaxDD=4.84% ✗    |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|                                                                   |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|   v357 FINDING: M30 signals lower WR but reduce MaxDD and add   |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|   recovery volume. Removing M30 → higher WR but worse overall.  |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|   v354 (M5+M15+M30+Spike=2.0) remains the best configuration.  |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|                                                                   |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|   v358 HYPOTHESIS: Add H4_EMA(50) as additional medium-term     |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|   trend gate on top of v354. The H4 50-period EMA (~200 hours   |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|   = ~8 trading days) tracks the medium-term trend. When H4      |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|   price dips below H4_EMA(50) but H1 is still bullish, those   |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|   entries may be fighting a medium-term downswing. Filtering    |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|   them out could improve WR and reduce MaxDD with minimal        |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|   trade count impact (H4 EMA(50) filters rarely in bull mkt).  |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|                                                                   |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|   v358 CHANGES from v354:                                        |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|     Added: H4_EMA_Period_Fast = 50 filter (Close[H4,1]>EMA)     |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|     All M5+M15+M30 sessions require BOTH H1 AND H4 to be        |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|     bullish. H4_EMA_Period=200 (existing, unused) left as-is.   |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|                                                                   |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|   NON-NEGOTIABLES (ALL unchanged):                               |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|     BB_Dev=2.5, BB_Period=20, TwoBar_Lookback=1                  |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|     Session=9-21, Max_Sells=0, ATR_SL_Multi=0.80                |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|     Partial_Pct=0.20, Trail_ATR_Multi=0.5, Hard_DD_Stop=0.0     |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|     RSI_Buy=45.0, RSI_Period=14, Body_Pct=0.20                   |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)|     Risk_Pct=0.85, DD_Budget=3.8, H1_EMA_Period=200              |
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "424.0"
#property strict

#include <stdlib.mqh>

//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)--- Session (LOCKED at 9-21)
input string SESSION_SET        = "============"; //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)====== Session Settings ======
input int    Session_Start_Hour = 9;
input int    Session_End_Hour   = 21;

//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)--- Trend Filter
input string TREND_SET          = "============"; //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)====== Trend Filter ======
input int    H4_EMA_Period      = 200;
input int    H4_EMA_Fast_Period = 60;             //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals) v374: confirmed optimal
input int    H1_EMA_Period      = 200;            //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals) NON-NEGOTIABLE: H1 EMA filter CRITICAL

//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)--- H1 RSI Gate
input string H1RSI_SET          = "============"; //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)====== H1 RSI Gate ======
...
//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)--- BB Entry Signal
input string S1_SET             = "============"; //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)====== BB Signal ======
input int    BB_Period          = 24;
input double BB_Dev             = 2.5;            //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals) NON-NEGOTIABLE (LOCKED)
input int    RSI_Period         = 14;             //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals) CONFIRMED BEST
input double RSI_Buy            = 45.0;           //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals) NON-NEGOTIABLE
input double RSI_Sell           = 60.0;
input double Body_Pct           = 0.20;           //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals) NON-NEGOTIABLE

//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)--- Two-bar pattern
input string TWOBAR_SET         = "============"; //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)====== Two-Bar Pattern ======
input bool   Use_TwoBar_Pattern = true;           //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals) NON-NEGOTIABLE

//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)--- Multi-Timeframe Settings
input string MTF_SET            = "============"; //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)====== Multi-TF Settings ======
input bool   Use_M15_Signals    = true;
input bool   Use_M30_Signals    = true;

//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)--- Volatility Filter
input string VOL_SET            = "============"; //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)====== Volatility Filter ======
input double Max_ATR_Pct        = 0.0;
input int    ATR_Spike_MA_Period = 50;            //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals) 50-bar rolling ATR average (~4h)
input double ATR_Spike_Multi    = 2.0;            //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals) v354 confirmed optimal

//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)--- Exit Settings
input string EXIT_SET           = "============"; //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)====== Exit Settings ======
input int    ATR_Period         = 14;
input double ATR_SL_Multi       = 0.8;            //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals) NON-NEGOTIABLE
input double ATR_TP_Multi       = 5.0;            //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals) TP never hit; safety only
input double Trail_ATR_Multi    = 0.15;           //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals) v412: sweep 0.5→0.35→0.25→0.15
input double Partial_Pct        = 0.20;           //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals) NON-NEGOTIABLE
input int    Max_Bars           = 22;             //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals) v381: fine-tune above 20

//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)--- Money Management
input string MONEY_SET          = "============"; //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)====== Money Management ======
input double Risk_Pct           = 0.85;           //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals) NON-NEGOTIABLE
input double Fixed_Lot          = 0.0;
input int    Max_Buys           = 1;
input int    Max_Sells          = 0;              //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals) NON-NEGOTIABLE (DISABLED)

//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)--- DD-Budget Scaling
input string DDB_SET            = "============"; //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)====== DD-Budget Scale ======
input double DD_Budget          = 3.8;
input double Min_Scale          = 0.01;

//|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)--- Safety
input string SAFETY_SET        = "============"; //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)====== Safety Settings ======
input int    MaxSpread          = 35;
input int    Slippage           = 30;
input int    Magic_Number       = 4242057;        //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals) v412 unique magic
input string Order_Comment      = "BSv424";
input double Max_Daily_DD_Pct   = 8.0;
input int    Max_Trades_Day     = 300;
input double Hard_DD_Stop       = 0.0;            //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals) NON-NEGOTIABLE (DISABLED)
...
void OnTick()
{
   ...
   //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals) v358: H4 EMA(50) medium-term trend gate
   ...
   //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals) ATR spike filter — skip when ATR > 2× rolling average (v354 confirmed optimal)
   ...
   //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)=== M5 BUY ===
   if(h1Bull)
   {
   ...
   //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)=== M15 BUY ===
   ...
   //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)=== M30 BUY ===
   ...
   //|   Buy & Sell EA v424 - BB_Period=24 (control, fewer signals)=== SELL (DISABLED - Max_Sells=0 NON-NEGOTIABLE) ===
