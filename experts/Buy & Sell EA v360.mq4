//+------------------------------------------------------------------+
//|   Buy & Sell EA v360 - H4 EMA(50) slope confirmation            |
//|   BASE: v358 (H4_EMA50 + ATR_Spike=2.0 + M5+M15+M30) BEST ✓   |
//|                                                                   |
//|   SCOREBOARD (all M5 unless noted):                              |
//|     v354: M5+M15+M30, ATR_Spike=2.0 → +30.33%, MaxDD=4.65%✅   |
//|     v358: +H4_EMA(50) → +31.23%, MaxDD=4.61%✅ ratio=6.77 BEST |
//|     v359: Max_Buys=2 → +24.95%, MaxDD=5.05%❌ FAIL ✗            |
//|                                                                   |
//|   v359 FINDING: Max_Buys=2 failed because:                      |
//|     1. BB touches co-occur across timeframes simultaneously       |
//|        → 2nd position adds risk w/o a genuinely new opportunity  |
//|     2. Trade count increased only 25 (806→831, not 2×)          |
//|     3. MaxDD exceeded 5% limit, WR dropped to 83.87%             |
//|   Conclusion: Max_Buys=1 is optimal for this signal structure.  |
//|                                                                   |
//|   v360 HYPOTHESIS: H4_EMA(50) in v358 checks price > EMA but    |
//|   doesn't verify the EMA itself is RISING. When H4 EMA is flat  |
//|   or declining but price just touched above it, that could be    |
//|   a "false" bullish signal — the medium-term trend hasn't        |
//|   actually turned bullish yet. Adding a slope condition          |
//|   (h4_ema_fast > h4_ema_fast_prev, i.e. EMA bar[1] > EMA bar[2])|
//|   ensures we only enter when the H4 trend is ACTIVELY rising,   |
//|   not just when price happens to be above a flat or falling EMA. |
//|                                                                   |
//|   v360 CHANGES from v358:                                        |
//|     h4Bull now requires BOTH:                                    |
//|       (1) Close[H4,1] > H4_EMA_Fast — price above EMA (v358)   |
//|       (2) H4_EMA_Fast[1] > H4_EMA_Fast[2] — EMA is rising NEW  |
//|     Max_Buys: 2 → 1 (back to optimal single-position setting)   |
//|                                                                   |
...
input int    H4_EMA_Fast_Period = 50;             // v358+: H4 medium-term trend gate
...
input int    Max_Buys           = 1;              // back to optimal
...
input int    Magic_Number       = 3602057;        // v360 unique magic
input string Order_Comment      = "BSv360";
...
void OnTick()
{
   ...
   // v360: H4 EMA(50) with slope confirmation — EMA must be rising AND price above EMA
   double h4_ema_fast      = iMA(NULL, PERIOD_H4, H4_EMA_Fast_Period, 0, MODE_EMA, PRICE_CLOSE, 1);
   double h4_ema_fast_prev = iMA(NULL, PERIOD_H4, H4_EMA_Fast_Period, 0, MODE_EMA, PRICE_CLOSE, 2);
   bool h4Bull = (iClose(NULL, PERIOD_H4, 1) > h4_ema_fast)
              && (h4_ema_fast > h4_ema_fast_prev);  // EMA itself must be rising
