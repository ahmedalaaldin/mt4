//+------------------------------------------------------------------+
//|   Buy & Sell EA v332 - ATR_SL_Multi isolation test               |
//|   BASE: v328 (best performing) + single change: ATR_SL_Multi     |
//|                                                                   |
//|   v331 POST-MORTEM (364 trades, +17.20%, MaxDD=5.01%):           |
//|     FAILURE: WR=77.47% collapsed below 83% threshold ✗           |
//|     FAILURE: MaxDD=5.01% barely above 5% limit ✗                 |
//|     LESSON: Partial_Pct=0.40 degrades WR. Keep 0.20 (v328).     |
//|     MORE PARTIAL CLOSE → WORSE WR (counter-intuitive but proven) |
//|                                                                   |
//|   v332 HYPOTHESIS: Only change ATR_SL_Multi 0.80 → 0.65          |
//|     Partial_Pct REVERTED to 0.20 (v328 value, NON-NEGOTIABLE)    |
//|     Rationale: Tighter SL → same dollar risk → LARGER LOTS       |
//|       → bigger dollar winners → better return.                   |
//|     SL distance 0.65/0.80 = 81.25% of v328 SL                   |
//|     Lot size increases by 0.80/0.65 = 1.23x vs v328             |
//|     Expected: Return ~20-21% (1.23x v328's 16.25%)              |
//|       WR might drop slightly from 83.47% due to tighter SL       |
//|       MaxDD should stay near 4.99% (same dollar risk logic)      |
//|                                                                   |
//|   v332 SINGLE CHANGE from v328:                                  |
//|     CHANGE: ATR_SL_Multi 0.80 → 0.65                            |
//|       Tighter stop-loss → larger position size for same risk     |
//|       Remainder trails with BB_mid - 0.5*ATR (unchanged)        |
//|                                                                   |
//|   SUCCESS CRITERIA:                                               |
//|     Return > 16.25% AND MaxDD < 5% AND WR > 83% → IMPROVEMENT   |
//|     Any metric worse than v328 → revert                          |
//|                                                                   |
...
//|                                                                   |
//|   SCOREBOARD:                                                     |
//|     v324: +15%, 4.60% MaxDD, 488 trades — Trail fixed! ✓        |
//|     v326: +18%, 5.39% MaxDD, 482 trades — over 5%               |
//|     v327: +17%, 5.06% MaxDD, 495 trades — 0.06% over target     |
//|     v328: +16%, 4.99% MaxDD, 496 trades — MaxDD PASS! ✓ BEST   |
//|     v329: +2%, 6.06% MaxDD, 614 trades — BB_Dev=2.0 fail ✗      |
//|     v330: +13%, 5.14% MaxDD, 313 trades — exit combo fail ✗     |
//|     v331: +17%, 5.01% MaxDD, 364 trades — Partial=0.40 fail ✗   |
//|     v332: This version — ATR_SL_Multi=0.65, Partial=0.20 (clean) |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "332.0"
#property strict
...
//--- BB Entry Signal (unchanged from v328)
...
//--- Exit Settings
// v332 SINGLE CHANGE: ATR_SL_Multi 0.80 → 0.65
// Partial_Pct RESTORED to v328 value (0.20)
// Trail: BB_mid-based (NON-NEGOTIABLE, identical to v328)
input string EXIT_SET           = "============"; //====== Exit Settings ======
input int    ATR_Period         = 14;
input double ATR_SL_Multi       = 0.65;           // v332: 0.80 → 0.65 (SINGLE CHANGE)
input double ATR_TP_Multi       = 5.0;
input double Trail_ATR_Multi    = 0.5;            // NON-NEGOTIABLE
input double Partial_Pct        = 0.20;           // RESTORED: v328 value (v331 proved 0.40 hurts WR)
...
input int    Magic_Number       = 3322057;
input string Order_Comment      = "BSv332";
input double Max_Daily_DD_Pct   = 8.0;
input int    Max_Trades_Day     = 300;
input double Hard_DD_Stop       = 0.0;
...
void ManageExits()
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(type == OP_BUY)
      {
         ...
         if(partialDone)
         {
            // v332: BB_mid-based trail — IDENTICAL to v328 (NON-NEGOTIABLE)
