//+------------------------------------------------------------------+
//|   Buy & Sell EA v108 - AllBuysRiskFree + H1 RSI Rising          |
//|   Framework: 4H Direction / 1H Structure / 5M Entry             |
//|   Base: v104 AllBuysRiskFree gate                               |
//|   Innovation:                                                    |
//|     - H1 RSI must be RISING (h1_rsi[1] > h1_rsi[2])            |
//|       Retains v106 quality gate, eliminates weak H1 momentum    |
//|     - H4 candle filter REMOVED: restores trade frequency        |
//|     - H1_RSI_Min = 45 (more entries vs v106's 50)              |
//|     - Min_Bar_Gap = 1 (faster re-entry vs v106's 2)            |
//|     - Max_Buys = 2 with AllBuysRiskFree gate (v104 proven)      |
//|       2nd pos only when 1st is at breakeven (risk-free)         |
//|   Goal: Blend v104 returns (~150%+) with v106 WR (~95%)         |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "118.0"
#property strict
...
input int    Magic_Number       = 1182025;        //Magic Number
input string Order_Comment      = "BSv118";       //Order comment
...
void OnTick()
{
   ...
   // 4H: Direction (price above EMA — no H4 candle filter for more signals)
   ...
   if(bbTouch && rsiOS && rsiRising && stochOS && bullBody)
   {
      ...
      if(tpDist >= minTP)
      {
         ...
         // Gate: 2nd position only when ALL existing buys are risk-free (SL >= openPrice)
