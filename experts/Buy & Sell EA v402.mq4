//+------------------------------------------------------------------+
//|   Buy & Sell EA v402 - v393 + parametric 2nd partial (Second_Partial_Pct2=0.0 = no close, just SL lock, 80% in trail)|
...
input double Second_Partial_Pct2 = 0.0;          // v402: 2nd partial fraction (0=SL-lock only, 0.375=v393 default)
...
input int    Magic_Number       = 4022057;        // v402 unique magic
input string Order_Comment      = "BSv402";
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(type == OP_BUY)
      {
         ...
         if(partialDone && !partial2Done && Bid >= bbMid + Trail_ATR_Multi * atr)
         {
            ...
            if(Second_Partial_Pct2 > 0)
            {
               double stepLot2  = MarketInfo(Symbol(), MODE_LOTSTEP);
               double minLot2   = MarketInfo(Symbol(), MODE_MINLOT);
               double partLots2 = MathFloor(OrderLots() * Second_Partial_Pct2 / stepLot2) * stepLot2;
               if(partLots2 >= minLot2 && partLots2 < OrderLots())
               {
            ...
            {
               // Second_Partial_Pct2=0: just lock SL to entry+0.5*ATR, keep all lots in trail
               if(newSL2 > currentSL)
                  OrderModify(tkt, openPrice, newSL2, 0, 0, clrBlue);
               // Fall through to trail (no continue)
            }
         }
         if(partialDone)
         {
