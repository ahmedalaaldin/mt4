//+------------------------------------------------------------------+
//|   Buy & Sell EA v291 - BB Width Filter (Lenient Threshold=8.0)   |
//|   BASE: v280 CHAMPION (Calmar=18.36, Return=220%, 637 trades)    |
//|   CHANGE: BB Width filter threshold raised from 5.5 to 8.0       |
//|   RATIONALE:                                                      |
//|     v289 (BBW<5.5) → 40% trade reduction, too many removed       |
//|       Result: Return=144.58%, MaxDD=10.41%, Calmar=13.89         |
//|     v290 (aggressive AM) → Calmar 14.90, worse than v280         |
//|     HYPOTHESIS: BBW<8.0 removes only the WORST trending-market    |
//|     entries (top 10-15% widest bands) while keeping ~87-90% of   |
//|     trades. These are the entries most likely to become MaxDD      |
//|     contributors. Expected: 550-580 trades, MaxDD ~10-11%,        |
//|     Return ~190-200%, Calmar ~18-20 (improvement over v280).     |
//|     MaxConsecLoss should improve (currently 8 in v280).           |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "291.0"
#property strict
...
//--- H1 RSI Gate (v269 confirmed optimal: 40-80/20-60)
...
input bool   Use_TwoBar_Pattern = true;  // Require bar[2] to have closed beyond BB band

//--- BB Width Filter
//    BB_Width = BB_Upper - BB_Lower. Normalize by ATR(14).
//    Narrow bands = ranging market = mean reversion works
//    Wide bands = trending market = mean reversion fails
//    Typical ranging: BB_Width/ATR ~ 3-5
//    Typical trending: BB_Width/ATR ~ 6-12
//    v289 used 5.5 → removed 40% of trades (too strict)
//    v291 uses 8.0 → targets ~10-15% removal (the worst trending entries)
input string BBW_SET            = "============"; //====== BB Width Filter ======
input bool   Use_BBW_Filter     = true;   // Block entries when bands are too wide
input double BB_Width_ATR_Max   = 8.0;   // Skip when BB_Width > 8.0 × ATR(14) (was 5.5 in v289)
...
//--- Anti-Martingale (same as v280 champion - v290 aggressive AM failed)
...
input int    Magic_Number       = 2912057;
input string Order_Comment      = "BSv291";
...
void OnTick()
{
   ...
   //--- BB Width Filter (v291: lenient threshold=8.0, was 5.5 in v289)
   //    Normalized BB width = BB_Width / ATR
   //    Low ratio = ranging market (good for mean reversion)
   //    High ratio = trending market (bad for mean reversion)
   //    8.0 targets only the most extreme trending sessions
   if(Use_BBW_Filter)
   {
      double bbW_Upper = iBands(NULL, 0, BB_Period, BB_Dev, 0, PRICE_CLOSE, MODE_UPPER, 1);
      double bbW_Lower = iBands(NULL, 0, BB_Period, BB_Dev, 0, PRICE_CLOSE, MODE_LOWER, 1);
      double bbWidth   = bbW_Upper - bbW_Lower;
      if(atr1 > 0 && bbWidth / atr1 >= BB_Width_ATR_Max) return;
   }
   ...
   if(h1Bull && stochBuy)
   {
      ...
      // Two-bar pattern: bar[2] CLOSED BELOW lower BB (breakdown confirmed)
   ...
   if(h1Bear && stochSell)
   {
      ...
      // Two-bar pattern: bar[2] CLOSED ABOVE upper BB (breakdown confirmed)
