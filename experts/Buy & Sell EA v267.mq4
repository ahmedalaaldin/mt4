//+------------------------------------------------------------------+
//|   Buy & Sell EA v267 - Remove H4 EMA direction filter           |
//|   Framework: H1 Direction only / M5 Entry                       |
//|   HYPOTHESIS: H4 EMA direction filter may be over-restrictive   |
//|     v263 champion: H4+H1 EMA required for entry                 |
//|     v267 test: Remove H4 requirement, only H1 direction used    |
//|     Expected: More trades (+40-60%), test if H4 is necessary    |
//|     H4_EMA_Period=200 still calculated but not used as gate     |
//|   ALL OTHER PARAMS IDENTICAL TO v263 (CURRENT CHAMPION)         |
//|     Return=111.58%, MaxDD=10.16%, Calmar=10.98, 472 trades      |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "267.0"
#property strict
...
input int    H4_EMA_Period      = 200;            // Period NON-NEGOTIABLE (not used as gate in v267)
...
input int    Magic_Number       = 2672057;
input string Order_Comment      = "BSv267";
...
void OnTick()
{
   ...
   // H4 EMA still computed (period NON-NEGOTIABLE=200) but NOT used as directional gate
   ...
   // v267: H4 direction REMOVED - only H1 direction gate remains
   ...
   //=== BUY: H4 direction removed, H1 Bull + Stoch gate only ===
   if(h1Bull && stochBuy)
   {
   ...
   //=== SELL: H4 direction removed, H1 Bear + Stoch gate only ===
   if(h1Bear && stochSell)
   {
