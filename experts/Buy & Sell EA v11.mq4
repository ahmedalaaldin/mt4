//+------------------------------------------------------------------+
//|   Buy & Sell EA v22 - BB Touch + Fixed 2:1 RR + BE Stop        |
//|   BUY: BB lower touch + bullish close + above H1 EMA200        |
//|   SELL: BB upper touch + bearish close + below H1 EMA200       |
//|   TP = 2xATR fixed (2:1 RR, breakeven at 33% WR)              |
//|   BE Stop: SL moves to entry when profit >= 0.5xATR            |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
...
input double ATR_TP_Multi       = 2.0;            //Take Profit = ATR x this (2:1 RR)
input double BE_Trigger_ATR     = 0.5;            //Move SL to BE when profit >= ATR x this
...
input int    Magic_Number       = 222025;         //Magic Number
input string Order_Comment      = "BSv22";        //Order comment
...
void OnTick()
{
   ...
   //--- BUY: above H1 EMA200, bar touched BB lower, bullish recovery close, RSI not overbought
   if(Close[1] > h1_ema && rsi1 < RSI_Skip_OB)
   {
      bool bbTouch = (Low[1] <= bbLower1) && (Close[1] > bbLower1) && (Close[1] > Open[1]);
   ...
   //--- SELL: below H1 EMA200, bar touched BB upper, bearish recovery close, RSI not oversold
   if(Close[1] < h1_ema && rsi1 > RSI_Skip_OS)
   {
      bool bbTouch = (High[1] >= bbUpper1) && (Close[1] < bbUpper1) && (Close[1] < Open[1]);
