//+------------------------------------------------------------------+
//|   Buy & Sell EA v357 - M5+M15 only + ATR_Spike=2.0 (combo)       |
//|   BASE: v354 (ATR_Spike=2.0) BEST ✓                              |
//|                                                                   |
...
//|                                                                   |
//|   v356 FINDING: Session 9-21 is the OPTIMAL window. Extending   |
//|   to 7-23 causes early trades to block quality 9-10 AM signals.  |
//|                                                                   |
//|   v357 HYPOTHESIS: Combine v342's M5+M15 (highest WR=86.90%)   |
//|   with v354's ATR_Spike_Multi=2.0 (best MaxDD=4.65%). These     |
//|   two improvements haven't been combined yet. M30 signals in     |
//|   v343/v354 provide more trades (+58 vs v342) but at slightly   |
//|   lower quality (WR drops from 86.90%→85.14%). Removing M30     |
//|   while keeping the ATR spike filter should give:               |
//|   - Higher WR (possibly 87%+)                                   |
//|   - Lower MaxDD (M30 signals may be the source of some losses)  |
//|   - Fewer trades (~756 like v342 vs 810 for v354)               |
//|   NET: Better Return/MaxDD ratio despite fewer trades            |
//|                                                                   |
//|   v357 CHANGES from v354:                                        |
//|     Use_M30_Signals: true → false                               |
//|     (ATR_Spike_Multi=2.0, Session=9-21 all unchanged)           |
//|                                                                   |
...
input bool   Use_M30_Signals    = false;          // v357: DISABLED (M5+M15 only, highest WR config)
...
input int    Magic_Number       = 3572057;        // v357 unique magic
input string Order_Comment      = "BSv357";
...
void OnTick()
{
   ...
   //=== M30 BUY (DISABLED in v357 — testing M5+M15 only for higher WR) ===
