//+------------------------------------------------------------------+
//|   Buy & Sell EA v110 - v104 Base + H1_RSI_Min=45               |
//|   Framework: 4H Direction / 1H Structure / 5M Entry             |
//|   Base: v104 AllBuysRiskFree gate (173.83% return proven)       |
//|   Innovation:                                                    |
//|     - Near-BE stop REMOVED: v107 and v109 confirmed it kills    |
//|       WR by triggering on normal M5 pullbacks after 0.5 ATR     |
//|       gain (mean-reversion needs room to develop)               |
//|     - H1 RSI rising REMOVED: halved trades in v108 for only     |
//|       +1.3% WR gain — not worth the trade frequency cost        |
//|     - H1_RSI_Min = 45 (vs v104=50): captures entries in the    |
//|       45-50 RSI transition zone during bullish pullbacks         |
//|     - AllBuysRiskFree + Max_Buys=2: proven compounding gate     |
//|     - Min_Bar_Gap = 1 (vs v104=2): faster re-entry              |
//|   Goal: Exceed v104's 173% return with slightly more trades     |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "120.0"
#property strict
...
input int    Min_Bar_Gap        = 1;              //Min bars between entries (vs v104=2)
...
input int    Magic_Number       = 1202025;        //Magic Number
input string Order_Comment      = "BSv120";       //Order comment
...
void OnTick()
{
   ...
   // 1H: Structure + RSI in zone (no RSI rising — restores v104 trade frequency)
