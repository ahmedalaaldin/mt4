//+------------------------------------------------------------------+
//|   Buy & Sell EA v149 - ATR Volatility Filter                     |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry          |
//|   Base: v144 (207.03% / 43.70% DD / 85.53% WR / 456 trades)    |
//|   Changes vs v144:                                               |
//|     1. ATR% Volatility Filter: skip entries when M5 ATR >        |
//|        Max_ATR_Pct % of current price                            |
//|        Root cause of 43.70% DD: large losses occur during        |
//|        high-volatility periods where ATR-based SL is very wide   |
//|        → position size smaller (risk capped) BUT price gaps       |
//|        can breach SL creating outsized losses                    |
//|        Example: ATR=50pts → SL=50pts below → 2x normal loss     |
//|        Filter: skip entry if ATR/Price > 0.45%                   |
//|        At XAUUSD ~$3300: 0.45% = $14.85. Normal ATR = $8-12.   |
//|        Filters top ~15-20% most volatile bars.                   |
//|     2. Peak Protection: reduce scale when equity drops           |
//|        significantly from its highest-ever peak                  |
//|        ddFromPeak > 15%: scale capped at 1.0x (no anti-mart)    |
//|        ddFromPeak > 25%: scale capped at 0.5x                   |
//|   All other params unchanged from v144:                          |
//|     BB_Dev=2.5, RSI_Buy=40, Body_Pct=0.2                       |
//|     Max_Buys=2, Risk_Pct=5%, Scale_High=1.85                   |
//|     WinStreak_High=5, Scale_Mid=1.25                            |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "149.0"
#property strict
...
input double BB_Dev             = 2.5;            //BB SD: 2.5 (proven high WR)

//--- Signal: M5 BB Touch
input string S1_SET             = "============"; //====== Signal: BB Touch ======
input int    RSI_Period         = 14;             //M5 RSI period
input double RSI_Buy            = 40.0;           //M5 RSI max for BUY (proven)
input double RSI_Sell           = 60.0;           //M5 RSI min for SELL (proven)
input double Body_Pct           = 0.2;            //Min body fraction (proven optimal)
input int    Min_Bar_Gap        = 1;              //Min M5 bars between entries

//--- Volatility Filter (NEW in v149)
input string VOL_SET            = "============"; //====== Volatility Filter ======
input double Max_ATR_Pct        = 0.45;           //Max ATR as % of price (0=disabled)
input double DD_Peak_Cap1_Pct   = 15.0;           //DD from peak % to cap scale at 1.0x
input double DD_Peak_Cap2_Pct   = 25.0;           //DD from peak % to cap scale at 0.5x

//--- Exit Settings
input string EXIT_SET           = "============"; //====== Exit Settings ======
input int    ATR_Period         = 14;             //ATR period (M5)
input double ATR_SL_Multi       = 1.0;            //SL = ATR x this
input double ATR_TP_Multi       = 2.0;            //TP = ATR x this
...
input double Risk_Pct           = 5.0;            //Base risk % (proven v144 level)
...
input double Scale_High         = 1.85;           //Scale at high streak (proven)
...
input int    Magic_Number       = 1492025;        //Magic Number
input string Order_Comment      = "BSv149";       //Order comment
...
double   g_EquityPeak   = 0;   // Peak equity tracker for DD protection
...
//+------------------------------------------------------------------+
// Peak Protection: cap risk scale when in deep drawdown from equity peak
double GetPeakProtectedScale(double riskScale)
{
   if(g_EquityPeak <= 0) return riskScale;
   double ddFromPeak = (g_EquityPeak - AccountEquity()) / g_EquityPeak * 100.0;
   if(ddFromPeak <= 0) return riskScale;
   if(ddFromPeak >= DD_Peak_Cap2_Pct) return MathMin(riskScale, 0.5);
   if(ddFromPeak >= DD_Peak_Cap1_Pct) return MathMin(riskScale, 1.0);
   return riskScale;
}
...
void OnTick()
{
   ...
   // Track equity peak for peak protection
   if(AccountEquity() > g_EquityPeak) g_EquityPeak = AccountEquity();
   ...
   // === ATR% Volatility Filter (v149 new) ===
   // Skip entry when market is unusually volatile (ATR > X% of price)
   // Avoids the high-ATR entries that generate the largest SL losses
   if(Max_ATR_Pct > 0)
   {
      double price1 = iClose(NULL, 0, 1);
      if(price1 > 0)
      {
         double atrPct = (atr1 / price1) * 100.0;
         if(atrPct > Max_ATR_Pct) return;
      }
   }
   ...
   // === Peak Protection (v149 new): cap scale when in deep DD from peak ===
   double riskScale = GetPeakProtectedScale(GetRiskScale());
