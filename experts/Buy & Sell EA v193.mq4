//+------------------------------------------------------------------+
//|   Buy & Sell EA v193 - Relaxed BB/RSI/Stoch for more trades      |
...
//|            FAIL: guard*anti-martingale = 0.25*0.5=0.125x lots    |
//|     v191: Peak only when flat  -> 89.37% ret, 16.38% DD, Cal=5.46|
//|            FAIL: Identical to v190. Diagnosis was wrong.          |
//|     v192: Guard caps win-scaling -> 228.49%, 18.88%, Cal=12.09   |
//|            FAIL: Guard still hurts returns, same DD as v186       |
//|            Conclusion: guard is counterproductive. Remove it.     |
//|   ROOT CAUSE: Guard never reduces DD (DD comes from anti-martin.) |
//|     Anti-martingale handles loss streaks. Guard only adds penalty.|
//|   v193 STRATEGY: Return to v186 base (no guard).                 |
//|     Relax entry thresholds to INCREASE TRADE FREQUENCY:           |
//|       BB_Dev: 2.5 -> 2.0 (signals ~4x more frequent)             |
//|       RSI_Buy: 40 -> 45, RSI_Sell: 60 -> 55 (wider OS/OB zone)   |
//|       Stoch_OS: 25 -> 30, Stoch_OB: 75 -> 70 (wider gate)        |
//|       Risk_Pct: 7.0 -> 6.5 (modest reduction for looser signals)  |
//|   Target: More trades (toward 20+/day), same proven framework     |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "193.0"
#property strict
...
input double BB_Dev             = 2.0;            //BB SD: 2.0 (v193: more signals vs 2.5)

//--- Signal: M5 BB Touch
input string S1_SET             = "============"; //====== Signal: BB Touch ======
input int    RSI_Period         = 14;             //M5 RSI period
input double RSI_Buy            = 45.0;           //M5 RSI max for BUY (v193: 45 vs 40)
input double RSI_Sell           = 55.0;           //M5 RSI min for SELL (v193: 55 vs 60)
...
input double Stoch_OS           = 30.0;           //Stoch oversold: BUY if K < this (v193: 30 vs 25)
input double Stoch_OB           = 70.0;           //Stoch overbought: SELL if K > this (v193: 70 vs 75)
...
input double Risk_Pct           = 6.5;            //Risk%: 6.5 (v193: modest reduction from 7.0)
...
input int    Magic_Number       = 1932025;        //Magic Number
input string Order_Comment      = "BSv193";       //Order comment
