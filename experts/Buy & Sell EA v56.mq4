//+------------------------------------------------------------------+
//|   Buy & Sell EA v67 - v51 + TP = BB upper (full band width)     |
//|   Entry: H1 EMA200 + H1 RSI 50-75 + BB 2.5SD lower touch       |
//|          + M5 RSI<40 + RSI rising + Stoch K&D<25 + Bull 20%    |
//|   Exit: TP = BB upper, SL = 1xATR                               |
//|   Changes: TP target = BB upper (from BB mid), Max_Bars=70      |
//|   Goal: 2x profit per winner, improving PF and return            |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "67.0"
#property strict
...
input double TP_Min_ATR         = 1.0;            //Min TP distance as ATR multiple (higher for BB upper target)
...
input int    Max_Bars           = 70;             //Force close after N bars (longer for BB upper target)
...
input int    Magic_Number       = 672025;         //Magic Number
input string Order_Comment      = "BSv67";        //Order comment
...
void OnTick()
{
   ...
   if(bbTouch && rsiOS && rsiRising && stochOS && strongBull)
   {
      ...
      double tpDist = bbUpper0 - entry;   // TP = BB upper (full band width target)
