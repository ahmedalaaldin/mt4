...
#property copyright ""
#property link      ""
#property version   "513.0"  // v513: v512 + ADX(H1) trend-strength filter (skip chop) to lift WR & cut DD
#property strict
...
//--- ADX Trend-Strength Filter (v513)
input string ADX_SET            = "============"; //====== ADX Filter ======
input bool   Use_ADX            = true;           // v513: gate entries on H1 trend strength
input int    ADX_Period         = 14;
input double ADX_Min            = 20.0;           // v513: only trade when H1 ADX >= this (skip chop)
...
input int    Max_Sells          = 1;              // v512: ENABLE short side (orthogonal edge; capture gold downtrends)
...
input int    Magic_Number       = 5132057;        // v513 unique magic
input string Order_Comment      = "BSv513";
...
void OnTick()
{
   ...
   // v513: ADX trend-strength gate on H1 (skip weak/choppy trends where pullbacks whipsaw)
   double adx_h1     = iADX(NULL, PERIOD_H1, ADX_Period, PRICE_CLOSE, MODE_MAIN, 1);
   bool   adxStrong  = (!Use_ADX) || (adx_h1 >= ADX_Min);
