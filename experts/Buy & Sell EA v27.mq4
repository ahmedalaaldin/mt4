//+------------------------------------------------------------------+
//|   Buy & Sell EA v38 - Bidirectional BB Mean Reversion           |
//|   BUY:  H1 price > EMA200, H1 RSI 50-75, M5 BB lower touch    |
//|         + M5 RSI<40, Stoch K<25, bullish close                 |
//|   SELL: H1 price < EMA200, H1 RSI 25-50, M5 BB upper touch    |
//|         + M5 RSI>60, Stoch K>75, bearish close                 |
//|   BB_Dev=2.5 hardcoded (optimal from testing)                  |
//|   Goal: 2x trade frequency vs v35, maintain quality            |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
...
input double BB_Dev             = 2.5;            //BB standard deviations (optimised at 2.5)
...
input double RSI_Buy            = 40.0;           //M5 RSI must be below (BUY)
input double RSI_Sell           = 60.0;           //M5 RSI must be above (SELL)
input int    Stoch_K            = 5;              //Stochastic K period
input int    Stoch_D            = 3;              //Stochastic D period
input int    Stoch_Slowing      = 3;              //Stochastic slowing
input double Stoch_Oversold     = 25.0;           //Stochastic oversold level (BUY)
input double Stoch_Overbought   = 75.0;           //Stochastic overbought level (SELL)
...
input int    Magic_Number       = 382025;         //Magic Number
input string Order_Comment      = "BSv38";        //Order comment
...
void OnTick()
{
   ...
   double bbLower0 = iBands(NULL, 0, BB_Period, BB_Dev, 0, PRICE_CLOSE, MODE_LOWER, 0);
   ...
   if(h1Bull)
   {
      ...
      bool stochOS   = (stochK1 < Stoch_Oversold) && (stochD1 < Stoch_Oversold);
   ...
   if(h1Bear)
   {
      ...
      bool stochOB    = (stochK1 > Stoch_Overbought) && (stochD1 > Stoch_Overbought);
      bool bearClose  = (Close[1] < Open[1]);

      if(bbTouchUp && rsiOB && stochOB && bearClose)
      {
