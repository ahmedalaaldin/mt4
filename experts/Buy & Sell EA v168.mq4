//+------------------------------------------------------------------+
//|   Buy & Sell EA v168 - ATR_SL sweep: 0.7 (between 0.6→BAD, 0.8→BEST) |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry          |
//|   Best settings confirmed (v165): WS=1, Scale=1.90, Risk=6%    |
//|     v162: SL=1.0 → ratio 11.29                                   |
//|     v165: SL=0.8 → ratio 63.48 ← CURRENT BEST                  |
//|     v166: SL=0.6 → ratio 16.00 (WR collapses 85%→78%)          |
//|   v168: ATR SL interpolation test                                 |
//|     ATR_SL_Multi: 0.8 → 0.7 (between 0.6 bad and 0.8 optimal)  |
//|     RR: 0.7 SL → TP=2.0 → RR=2.857:1                           |
//|     EV: if WR stays ≥77.1%, v168 ≥ v165 EV                     |
//|     v166 at 0.6 still had 78.17% WR, so 0.7 likely ≥81% WR    |
//|   Changes vs v165:                                                |
//|     1. ATR_SL_Multi: 0.8 → 0.7                                  |
//|   All other params identical to v165:                             |
//|     Risk=6%, TP=2x, ATR=0.60%, WS=1, Scale=1.90, Session=7-20  |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "168.0"
#property strict
...
input double ATR_SL_Multi       = 0.7;            //SL = ATR x 0.7 (sweep: 0.6→BAD, 0.7→?, 0.8→BEST)
...
input int    Magic_Number       = 1682025;        //Magic Number
input string Order_Comment      = "BSv168";       //Order comment
