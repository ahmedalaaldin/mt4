//+------------------------------------------------------------------+
//|   Buy & Sell EA v303 - Activate H4 EMA200 Trend Filter           |
//|   BASE: v300 (Calmar=20.84, Return=258.39%, MaxDD=12.40%, 1040T) |
//|   CHANGE: Wire up H4 EMA200 filter that existed in inputs but    |
//|           was NEVER USED in v293-v302 entry logic               |
//|   RATIONALE:                                                      |
//|     v300-v302 all declared H4_EMA_Period as an input parameter  |
//|     but the entry conditions only used the H1 EMA/RSI gate.     |
//|     The H4 EMA200 represents a ~33-day macro trend.             |
//|     The worst losing clusters (14 consecutive losses) likely    |
//|     occurred when M5 mean-reversion signals fired but the macro |
//|     H4 trend was strongly against the position direction.       |
//|     Adding genuine H4 EMA200 alignment:                         |
//|       BUY:  h1Bull  AND h4Bull  (both H1+H4 in uptrend)        |
//|       SELL: h1Bear  AND h4Bear  (both H1+H4 in downtrend)      |
//|     should filter the counter-macro-trend entries and reduce    |
//|     the losing clusters while preserving quality signals.       |
//|   RISK: If H4 was mostly bullish 2025-2026 (gold uptrend),     |
//|     shorts may be significantly reduced → test diversification. |
//|   HYPOTHESIS: Trades ~600-800, Win rate ~95-96%, MaxDD ~7-10%, |
//|     Return ~180-220%, Calmar ~22-28                            |
//|                                                                   |
//|   SCOREBOARD:                                                    |
//|     v293: Calmar=18.43, Return=220.62%, MaxDD=11.97%, 637 T     |
//|     v300: Calmar=20.84, Return=258.39%, MaxDD=12.40%, 1040 T    |
//|     v301: FAILED (binary circuit breaker)                        |
//|     v302: Calmar=5.75, Return=82.22%, MaxDD=14.29% — FAILED     |
//|     v303: H4 EMA200 filter — TARGET Calmar>22, MaxDD<10%        |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "303.0"
#property strict
...
//--- Trend Filter (H4 EMA200 NOW ACTIVE — v303 key change)
input string TREND_SET          = "============"; //====== Trend Filter (H4 ACTIVE) ======
...
//--- Stochastic (NOT used as gate — v300 approach)
...
input int    Magic_Number       = 3032057;
input string Order_Comment      = "BSv303";
...
void OnTick()
{
   ...
   //--- v303 KEY CHANGE: H4 EMA200 now ACTIVE in entry conditions
   ...
   bool h4Bull = (iClose(NULL, PERIOD_H4, 1) > h4_ema);   // H4 macro uptrend
   bool h4Bear = (iClose(NULL, PERIOD_H4, 1) < h4_ema);   // H4 macro downtrend
   ...
   //=== BUY === v303 KEY CHANGE: require h1Bull AND h4Bull
   if(h1Bull && h4Bull)
   {
   ...
   //=== SELL === v303 KEY CHANGE: require h1Bear AND h4Bear
   if(h1Bear && h4Bear)
   {
