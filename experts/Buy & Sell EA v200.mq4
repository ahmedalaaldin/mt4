//+------------------------------------------------------------------+
//|   Buy & Sell EA v200 - BB Period = 30 on M5                      |
...
//|       - Guards (v192): useless                                     |
//|       - BB relaxation (v193): catastrophic (BB_Dev=2.0)           |
//|       - Faster scaling (v194): worse                               |
//|       - H4 removal (v195): higher trades but DD explodes           |
//|       - SL/TP change (v196): WR destruction                        |
//|       - Trail change (v197): breakeven exits, avg win collapses    |
//|     v198: M15 timeframe -> -23.04% loss! Only 25 trades, WR 64%  |
//|       LESSON: Strategy is fundamentally M5-native                  |
//|     v199: BB_Period=14 on M5 -> -10.63% loss, 83.33% WR, 108 tr |
//|       Fewer AND worse-quality signals than BB(20) — counterintuitive|
//|       LESSON: Shorter BB period hurts WR AND reduces partial exit  |
//|       profit (midline is closer to entry)                          |
//|       BB_Period=20 is OPTIMAL for both entry quality and exits     |
//|   v200 HYPOTHESIS: BB_Period=30 on M5                             |
//|     Rationale: Longer BB lookback -> more extreme signals          |
//|       BB(30) lower band = 2.5 SD below 30-bar mean = rarer/cleaner|
//|       BB(30) midline for partial exit = further from entry = more  |
//|       profit captured per winning trade                            |
//|       Expected WR: possibly 92-95% (more extreme entry condition)  |
//|       Expected trades: possibly ~150-200/year (fewer than BB20)    |
//|       Expected avg win: higher (midline further from entry)        |
//|     All other parameters identical to v186 (proven optimal).       |
//|     Run with PERIOD = M5 in Strategy Tester.                       |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "200.0"
#property strict
...
input int    BB_Period          = 30;             //BB period: 30 (v200: longer lookback for quality)
...
//--- Anti-Martingale (v186 optimal: 5/10 thresholds)
...
input int    Magic_Number       = 2002030;        //Magic Number
input string Order_Comment      = "BSv200";       //Order comment
...
void OnTick()
{
   ...
   //=== Signal BUY: H4 bullish + H1 bullish + BB(30,2.5) touch + RSI<40 + Stoch<25
   ...
   //=== Signal SELL: H4 bearish + H1 bearish + BB(30,2.5) touch + RSI>60 + Stoch>75
