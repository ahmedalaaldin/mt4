//+------------------------------------------------------------------+
//|   Buy & Sell EA v90 - v77 + breakeven trailing stop             |
//|   Entry: H4 EMA200 + H1 EMA200 + H1 RSI 50-75                  |
//|          + BB 2.5SD lower touch + M5 RSI<40 + RSI rising        |
//|          + Stoch K&D<25 + Bull body 20%                         |
//|   Exit: TP = BB mid, SL = 1xATR, BE when +0.5xATR profit       |
//|   Change: ManageExits runs every tick for breakeven management  |
//|   Goal: Reduce consecutive losses from 4 to 1-2, cut DD <5%    |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "90.0"
#property strict
...
input int    Magic_Number       = 902025;         //Magic Number
input string Order_Comment      = "BSv90";        //Order comment
...
void OnTick()
{
   ...
   ManageExits(); // runs every tick for breakeven SL management
...
void ManageExits()
{
   double atrNow = iATR(NULL, 0, ATR_Period, 1);
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      // Breakeven: when Bid profit >= BE_Trigger_ATR * ATR, move SL to entry
      if(atrNow > 0 && BE_Trigger_ATR > 0)
      {
         double beTrigger = BE_Trigger_ATR * atrNow;
         double beLevel   = NormalizeDouble(OrderOpenPrice(), Digits);
         if(Bid >= OrderOpenPrice() + beTrigger && OrderStopLoss() < beLevel)
            OrderModify(OrderTicket(), OrderOpenPrice(), beLevel, OrderTakeProfit(), 0, clrBlue);
      }

      // Time-based forced close
