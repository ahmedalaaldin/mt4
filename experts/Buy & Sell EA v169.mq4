//+------------------------------------------------------------------+
//|   Buy & Sell EA v169 - Risk_Pct reduction: 6.0 → 1.5            |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry          |
//|   SL sweep COMPLETE: ATR_SL_Multi=0.8 is optimal (ratio 63.48)  |
//|     v165: SL=0.8, Risk=6% → 1168%/18.40% DD (ratio 63.48) BEST |
//|     v166: SL=0.6 → ratio 16.00 (WR collapses)                  |
//|     v168: SL=0.7 → ratio 24.66 (worse, DD 39.87%)              |
//|   v169: Risk scaling test for DD < 5% target                     |
//|     Return/DD ratio is fixed at ~63 for this strategy            |
//|     To hit both targets: 200% < return < 315%, 3.17% < DD < 5% |
//|     At Risk=1.5%: est. return ~292%, est. DD ~4.6% (< 5%) ✓   |
//|     Linear scaling: 1168%*(1.5/6)=292%, 18.40%*(1.5/6)=4.6%   |
//|   Changes vs v165:                                                |
//|     1. Risk_Pct: 6.0 → 1.5                                      |
//|   All other params identical to v165:                             |
//|     SL=0.8, TP=2x, ATR=0.60%, WS=1, Scale=1.90, Session=7-20  |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "169.0"
#property strict
...
input double Risk_Pct           = 1.5;            //Risk%: 6.0→1.5 (target DD<5%, Return>200%)
...
input int    WinStreak_Mid      = 1;              //Consec wins for Scale_Mid (superseded by High=1)
input int    WinStreak_High     = 1;              //Consec wins for Scale_High: immediate onset (proven)
input double Scale_Mid          = 1.25;           //Scale at Scale_Mid threshold (now unused)
input double Scale_High         = 1.90;           //Scale at 1+ consecutive win: proven optimal
...
input int    Magic_Number       = 1692025;        //Magic Number
input string Order_Comment      = "BSv169";       //Order comment
input double Max_Daily_DD_Pct   = 15.0;           //Daily DD guard (rarely fires at 1.5% risk)
