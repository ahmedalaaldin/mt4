...
#property copyright ""
#property link      ""
#property version   "500.0"  // [RECONSTRUCTED 2026-05-31: v499 + Risk_Pct 1.5->3.0]
#property strict
...
input double Risk_Pct           = 3.0;            // v500: 1.5->3.0 (recover return; clean exit compensates DD) [RECONSTRUCTED]
...
input int    Magic_Number       = 5002057;        // v500 unique magic
input string Order_Comment      = "BSv500";
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(type == OP_BUY)
      {
         ...
         if(partialDone && !partial2Done && Bid >= bbMid + Trail_ATR_Multi * atr)
         {
            double stepLot   = MarketInfo(Symbol(), MODE_LOTSTEP);
            double minLot    = MarketInfo(Symbol(), MODE_MINLOT);
            double partLots2 = MathFloor(OrderLots() * Partial_Pct / stepLot) * stepLot;
