//+------------------------------------------------------------------+
//|   Buy & Sell EA v270 - Dual BB entry (2.5σ + 2.0σ secondary)    |
//|   Framework: H1 Direction / M5 Entry / Dual Band                 |
//|   HYPOTHESIS: Adding 2.0σ BB as secondary entry increases trades |
//|     Primary: BB_Dev=2.5 touch + RSI < 40 (same as v267)         |
//|     Secondary: BB_Dev=2.0 touch + RSI < 38 (stricter RSI)       |
//|     Both use same H1 gate, Stoch, body filter                    |
//|     BB_Dev=2.5 NON-NEGOTIABLE for primary; 2.0 is additive only  |
//|   BASE: v267 CHAMPION (Calmar=14.34, 609 trades)                 |
//|     H4 direction removed, H1 EMA direction KEPT                  |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "270.0"
#property strict
...
//--- BB Secondary Entry Signal (additive 2.0σ band)
...
input double ATR_TP_Multi       = 5.0;            // v263 confirmed (trail is real exit)
...
//--- Anti-Martingale (flat wins, only loss downscaling - v250 confirmed)
...
input int    Magic_Number       = 2702057;
input string Order_Comment      = "BSv270";
...
void OnTick()
{
   ...
   // v270: H4 EMA direction removed (from v267), H1 EMA direction KEPT
   ...
   // Secondary BB (2.0σ - additive secondary entry)
   ...
   {
      // Primary entry: 2.5σ BB touch + RSI < 40
      bool primaryBuy = (Low[1] <= bbLower1) && (Close[1] > bbLower1)
                     && (rsi1 < RSI_Buy) && (rsi1 > rsi2) && bullBody;

      // Secondary entry: 2.0σ BB touch + RSI < 38 (stricter)
      // Only fires if price touched 2.0σ but NOT 2.5σ (additive, non-overlapping)
      bool secondaryBuy = (Low[1] <= bbLower1b) && (Close[1] > bbLower1b)
                       && (Low[1] > bbLower1)           // did NOT reach 2.5σ band
