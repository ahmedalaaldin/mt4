//+------------------------------------------------------------------+
//|   Buy & Sell EA v125 - v120 Base, No Stochastic Filter          |
//|   Framework: 4H Direction / 1H Structure / 5M Entry             |
//|   Base: v120 (224.50%, 14.95% DD, 15.0x ratio, gold standard)  |
//|   Changes vs v120:                                               |
//|     - Stochastic filter REMOVED                                  |
//|       Stoch required K<25 & D<25 for buys (K>75 & D>75 sells)   |
//|       This was the most restrictive M5 confirmation filter.      |
//|       Removing it should add ~40-60% more trades.               |
//|   All other parameters IDENTICAL to v120:                        |
//|     - BB_Dev=2.5, H1 RSI 50-75/25-50, Risk_Pct=5%              |
//|     - Trail_ATR_Multi=0.8, 50% partial close, Max_Bars=100      |
//|   Why: v124 showed wider RSI + 75% partial + circuit breaker   |
//|   all backfired. Cleanest possible one-variable test.           |
//|   Goal: more trades with near-v120 WR to push Return/DD ratio. |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "135.0"
#property strict
...
input double H1_RSI_Min         = 50.0;           //H1 RSI min for BUY zone [v120 value]
input double H1_RSI_Max         = 75.0;           //H1 RSI max for BUY [v120 value]
input double H1_RSI_Min_Sell    = 25.0;           //H1 RSI min for SELL [v120 value]
input double H1_RSI_Max_Sell    = 50.0;           //H1 RSI max for SELL [v120 value]

//--- Bollinger Bands (M5)
input string BB_SET             = "============"; //====== Bollinger Band Settings ======
input int    BB_Period          = 20;             //BB period
input double BB_Dev             = 2.5;            //BB SD [v120 optimal]
input double TP_Min_ATR         = 0.5;            //Min TP distance as ATR multiple

//--- Entry Signal (M5) — Stochastic filter removed vs v120
...
input double Price_Gap_ATR      = 0.3;            //Min ATR gap between entries
...
input double Risk_Pct           = 5.0;            //Risk % per trade [v120 value]
...
input int    Magic_Number       = 1352025;        //Magic Number
input string Order_Comment      = "BSv135";       //Order comment
input double Max_Daily_DD_Pct   = 15.0;           //Daily DD guard (% of equity) [v120 value]
...
void OnTick()
{
   ...
   //--- BUY: H4 bull + H1 above EMA + H1 RSI in zone (v120 values) + M5 signals (no stoch)
   ...
   if(h4Bull && h1Bull)
   {
      ...
      if(bbTouch && rsiOS && rsiRising && bullBody)
      {
         ...
         if(tpDist >= minTP)
         {
            ...
            if(openBuys == 0 && barsSince >= Min_Bar_Gap)
            {
   ...
   //--- SELL: H4 bear + H1 below EMA + H1 RSI in zone (v120 values) + M5 signals (no stoch)
   ...
   if(h4Bear && h1Bear)
   {
      ...
      if(bbTouchUp && rsiOB && rsiFalling && bearBody)
      {
         ...
         if(tpDist >= minTP)
         {
            ...
            if(openSells == 0 && barsSince >= Min_Bar_Gap)
            {
               double lots = CalcLots(slDist, riskScale);
               if(lots > 0)
               {
