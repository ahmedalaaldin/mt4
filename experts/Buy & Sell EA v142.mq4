//+------------------------------------------------------------------+
//|   Buy & Sell EA v142 - v139 + Tighter Anti-Martingale Cap        |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry         |
//|   Base: v139 (proven 217.62% / 85.65% WR)                       |
//|   Changes vs v139:                                               |
//|     1. Scale_High: 2.0 → 1.5 (max risk 10% → 7.5%)             |
//|        Reduces worst-case single trade loss by 25%               |
//|     2. Scale_Mid: 1.5 → 1.25 (mid risk 7.5% → 6.25%)           |
//|     3. WinStreak_High: 6 → 4 (reach 1.5x faster; EA has         |
//|        avg 16 consec wins so most time spent at Scale_High)      |
//|     4. WinStreak_Mid: 3 → 2 (reach 1.25x faster)               |
//|   Unchanged from v139:                                           |
//|     BB_Dev=2.5, RSI_Buy=40 (proven 85% WR formula)             |
//|     Risk_Pct=5%, Max_Buys=2, Max_Sells=2                       |
//|     Loss scale: 1 loss=0.5x, 2=0.25x, 3+=0.1x                 |
//|     Partial close at BB midline + trailing stop                 |
//|   Rationale:                                                     |
//|     v139: 218% return, 24% DD (max trade 10% risk).             |
//|     v141: BB(2.0) proved wider bands destroy the WR edge.       |
//|     v142: Keep the proven formula, just cap the maximum scale.  |
//|     With avg 16 win streak, reaching 1.5x at 4 wins vs 6 wins  |
//|     means EA spends same/more time at Scale_High but at lower   |
//|     exposure (7.5% vs 10%). Target: ~190% return / ~18% DD.    |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "142.0"
#property strict
...
//--- H1 RSI Gate (v139 proven values)
...
input double ATR_SL_Multi       = 1.0;            //SL = ATR x this (proven)
...
//--- Anti-Martingale (tighter than v139)
input string AM_SET             = "============"; //====== Anti-Martingale Settings ======
input int    WinStreak_Mid      = 2;              //Consec wins for Scale_Mid (was 3)
input int    WinStreak_High     = 4;              //Consec wins for Scale_High (was 6)
input double Scale_Mid          = 1.25;           //Scale at mid streak (was 1.5)
input double Scale_High         = 1.5;            //Scale at high streak (was 2.0)
...
input string Order_Comment      = "BSv142";       //Order comment
...
double GetRiskScale()
{
   ...
   if(streak >= WinStreak_High)     return Scale_High;  // 1.5× at 4+ wins
   if(streak >= WinStreak_Mid)      return Scale_Mid;   // 1.25× at 2-3 wins
