//+------------------------------------------------------------------+
//|   Buy & Sell EA v274 - Extended session + looser ATR spike       |
//|   Framework: H1 EMA direction + wider H1 RSI / M5 Entry         |
//|   HYPOTHESIS: Two changes to unlock more trades:                 |
//|     1. Session extended: 9-21 → 8-22 (2 more hours)            |
//|        Asian close (8AM) + late NY extension (21-22PM)          |
//|     2. ATR_Spike_Multi: 1.8 → 2.5 (allow moderate volatility)  |
//|        More bars qualify when BB signal fires                    |
//|     If DD stays controlled, we get more trades at same quality   |
//|   BASE: v269 CHAMPION (Calmar=17.77, Return=209%, 707 trades)   |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "274.0"
#property strict

#include <stdlib.mqh>

//--- Session - EXTENDED from 9-21 to 8-22
input string SESSION_SET        = "============"; //====== Session Settings ======
input int    Session_Start_Hour = 8;              // v274: extended from 9 to 8
input int    Session_End_Hour   = 22;             // v274: extended from 21 to 22
...
input int    Magic_Number       = 2742057;
input string Order_Comment      = "BSv274";
...
void OnTick()
{
   ...
   // v274: H4 EMA direction removed (same as v269), H1 EMA direction KEPT
   // Session extended 8-22, ATR spike loosened to 2.5
   ...
   //=== BUY: Session 8-22, ATR spike 2.5 ===
   if(h1Bull && stochBuy)
   {
   ...
   //=== SELL: Session 8-22, ATR spike 2.5 ===
   if(h1Bear && stochSell)
   {
