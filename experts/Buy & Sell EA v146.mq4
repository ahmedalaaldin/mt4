//+------------------------------------------------------------------+
//|   Buy & Sell EA v146 - Body_Pct 0.2→0.1 (trade frequency test)  |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry          |
//|   Base: v144 (207.03% / 43.70% DD / 85.53% WR / 456 trades)    |
//|   Changes vs v144:                                               |
//|     1. Body_Pct: 0.2 → 0.1                                      |
//|        Relaxes the bullish body requirement. Now allows entries  |
//|        when candle body >= 10% of total range (was 20%).         |
//|        Includes near-doji candles with small bodies.             |
//|        Expected: +30-80% more trades. WR may drop 1-3%.         |
//|   v145 lesson: M15 dual-TF approach FAILED (123% return, 58%DD) |
//|     Root cause: M15 trades occupied M5 position slots + shared  |
//|     anti-martingale streak contamination. Back to pure M5.      |
//|   All other params unchanged from v144:                          |
//|     BB_Dev=2.5, RSI_Buy=40 (proven 85% WR formula)             |
//|     Risk_Pct=5%, Max_Buys=2, Max_Sells=2                       |
//|     Scale_High=1.85, WinStreak_High=5                           |
//|   Goal: Find if body filter materially limits trade frequency    |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "146.0"
#property strict
...
input double Body_Pct           = 0.1;            //Min body fraction (0.1 vs 0.2 in v144)
...
input int    Magic_Number       = 1462025;        //Magic Number
input string Order_Comment      = "BSv146";       //Order comment
...
void OnTick()
{
   ...
   if(h4Bull && h1Bull)
   {
      ...
      bool bullBody  = (candleBodyB > 0) && (candleRange > 0)
                    && (candleBodyB >= Body_Pct * candleRange); // 0.1 vs 0.2
   ...
   if(h4Bear && h1Bear)
   {
      ...
      bool bearBody   = (candleBodyS > 0) && (candleRange > 0)
                     && (candleBodyS >= Body_Pct * candleRange); // 0.1 vs 0.2
