//+------------------------------------------------------------------+
//|   Buy & Sell EA v68 - v51 + 2-bar BB touch look-back window     |
//|   Entry: H1 EMA200 + H1 RSI 50-75 + BB 2.5SD touch (bar1 OR 2) |
//|          + M5 RSI<40 + RSI rising + Stoch K&D<25 + Bull 20%    |
//|   Exit: TP = BB mid, SL = 1xATR                                 |
//|   Changes: BB touch window = 2 bars (bar[1] OR bar[2] touched)  |
//|   Goal: Catch delayed-confirmation signals, ~70-80 trades/year   |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "68.0"
#property strict
...
input int    Magic_Number       = 682025;         //Magic Number
input string Order_Comment      = "BSv68";        //Order comment
...
void OnTick()
{
   ...
   // 2-bar BB touch window: either bar[1] or bar[2] touched BB lower
   bool bbTouch1 = (Low[1] <= bbLower1) && (Close[1] > bbLower1);
   bool bbTouch2 = (Low[2] <= bbLower2) && (Close[2] > bbLower2);
   bool bbTouch  = bbTouch1 || bbTouch2;

   // All confirmation signals checked at bar[1] (current closed bar)
