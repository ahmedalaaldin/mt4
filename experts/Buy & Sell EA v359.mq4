//+------------------------------------------------------------------+
//|   Buy & Sell EA v359 - Max_Buys=2 (concurrent position test)     |
//|   BASE: v358 (H4_EMA50 + ATR_Spike=2.0 + M5+M15+M30)            |
//|                                                                   |
...
//|     v358: H4_EMA(50) filter → +31.23%, MaxDD=4.61%✅ NEW BEST ✓ |
//|                                                                   |
//|   v358 FINDING: H4 EMA(50) improves all 3 metrics simultaneously:|
//|     WR: 85.19%→85.48%, Return: 30.33%→31.23%, MaxDD: 4.65%→4.61%|
//|     Return/MaxDD ratio improved from 6.52 to 6.77 — new best.   |
//|                                                                   |
//|   v359 HYPOTHESIS: Max_Buys=1 is the primary bottleneck to       |
//|   trade frequency (~2.2 trades/day). With 3 timeframes           |
//|   (M5+M15+M30) firing independently, signals are often skipped  |
//|   because a position is already open. Allowing Max_Buys=2 lets  |
//|   an M15 or M30 signal fire even when M5 already has a trade.   |
//|   The H4 EMA(50) filter from v358 is retained to ensure only     |
//|   quality setups get through the doubled capacity.               |
//|   Expected: ~1.4-1.6x more trades, potentially higher returns    |
//|   if MaxDD stays < 5% (each position still sized at Risk_Pct).  |
//|                                                                   |
//|   v359 CHANGES from v358:                                        |
//|     Max_Buys: 1 → 2 (allow one more concurrent buy)             |
//|     All other settings identical to v358                         |
//|                                                                   |
...
input int    Max_Buys           = 2;              // v359: 1→2 (concurrent position test)
...
input int    Magic_Number       = 3592057;        // v359 unique magic
input string Order_Comment      = "BSv359";
...
void OnTick()
{
   ...
   // H4 EMA(50) medium-term trend gate (from v358)
