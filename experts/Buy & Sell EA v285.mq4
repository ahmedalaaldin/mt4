//+------------------------------------------------------------------+
//|   Buy & Sell EA v285 - M1 Timeframe with M5-Pinned SL/TP ATR    |
//|   Framework: H1 EMA direction + H1 RSI gate / M1 Entry          |
//|   HYPOTHESIS: v280's two-bar BB breakdown-recovery works on M5   |
//|     giving ~1.75 trades/day. Running the same logic on M1 should  |
//|     give ~5x more signals (~8-10 trades/day) since M1 has 5x     |
//|     more bars. KEY FIX: ATR for SL/TP uses PERIOD_M5 explicitly  |
//|     so stop distances remain meaningful. If M1 ATR were used,    |
//|     SL = 0.8 × M1_ATR ≈ 0.8 pips — smaller than the spread,     |
//|     causing immediate stop-outs on almost every trade.            |
//|     Entry signals (BB, RSI, Stoch, two-bar) are M1-based.        |
//|     H4/H1 trend/RSI filters remain explicitly timeframe-locked.  |
//|   BASE: v280 NEW CHAMPION (Calmar=18.36, Return=220%, 637 trades)|
//|   CHANGE: SL_TP_ATR_TF=PERIOD_M5 (new), Max_Bars=500 (5x M5)    |
//|   RUN ON M1 IN STRATEGY TESTER                                   |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "285.0"
#property strict
...
//--- Two-bar pattern (M1 bars)
input string TWOBAR_SET         = "============"; //====== Two-Bar Pattern (M1) ======
...
//--- Volatility Filter (uses SL_TP_ATR_TF for meaningful comparison)
...
input int    SL_TP_ATR_TF       = PERIOD_M5;   // NEW: ATR timeframe for SL/TP (M5 = 5)
input double ATR_SL_Multi       = 0.8;
input double ATR_TP_Multi       = 5.0;
input double Trail_ATR_Multi    = 0.1;
input double Partial_Pct        = 0.20;
input int    Max_Bars           = 500;          // 5x M5's 100 — same real time (500 min)
...
input int    Magic_Number       = 2852057;
input string Order_Comment      = "BSv285";
...
void ManageExits()
{
   // SL/TP ATR uses the fixed SL_TP_ATR_TF (M5 by default), NOT current TF
   double atr   = iATR(NULL, SL_TP_ATR_TF, ATR_Period, 1);
...
void OnTick()
{
   ...
   // ATR for SL/TP sizing: use M5 regardless of chart TF to avoid tiny M1 stops
   double atr1 = iATR(NULL, SL_TP_ATR_TF, ATR_Period, 1);
   if(atr1 <= 0) return;

   // Volatility spike filter: compare M5 ATR vs M5 slow ATR (both on same TF)
   if(Max_ATR_Pct > 0)
   {
      double price1 = iClose(NULL, SL_TP_ATR_TF, 1);
      if(price1 > 0 && (atr1 / price1 * 100.0) > Max_ATR_Pct) return;
   }
   if(ATR_Spike_Multi > 0)
   {
      double atr_slow = iATR(NULL, SL_TP_ATR_TF, ATR_Spike_MA_Period, 1);
      if(atr_slow > 0 && atr1 > ATR_Spike_Multi * atr_slow) return;
   }

   // Entry signals on current TF (M1 when running on M1)
   ...
   double slDist    = ATR_SL_Multi * atr1;   // M5 ATR-based
   double tpDist    = ATR_TP_Multi * atr1;   // M5 ATR-based
   ...
   if(h1Bull && stochBuy)
   {
      ...
      // Two-bar pattern: bar[2] CLOSED BELOW lower BB (M1 breakdown)
   ...
   if(h1Bear && stochSell)
   {
      ...
      // Two-bar pattern: bar[2] CLOSED ABOVE upper BB (M1 breakdown)
