//+------------------------------------------------------------------+
//|   Buy & Sell EA v92 - Symmetric Buy+Sell with H4+H1 filters     |
//|   BUY:  H4 above EMA200 + H1 above EMA200 + H1 RSI 50-75       |
//|         + BB 2.5SD lower touch + M5 RSI<40 + RSI rising         |
//|         + Stoch K&D<25 + Bull body 20%                          |
//|   SELL: H4 below EMA200 + H1 below EMA200 + H1 RSI 25-50       |
//|         + BB 2.5SD upper touch + M5 RSI>60 + RSI falling        |
//|         + Stoch K&D>75 + Bear body 20%                          |
//|   Exit: TP = BB mid, SL = 1xATR                                 |
//|   Goal: More trades + DD hedging through direction diversity    |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "92.0"
#property strict
...
input double H1_RSI_Bull_Min    = 50.0;           //H1 RSI min for BUY (uptrend zone)
...
input double H1_RSI_Bear_Max    = 50.0;           //H1 RSI max for SELL (downtrend zone)
...
input double Stoch_OS           = 25.0;           //Stochastic oversold (buy signal)
input double Stoch_OB           = 75.0;           //Stochastic overbought (sell signal)
...
input int    Magic_Number       = 922025;         //Magic Number
input string Order_Comment      = "BSv92";        //Order comment
input double Max_Daily_DD_Pct   = 15.0;           //Daily drawdown guard (% of equity)
input int    Max_Trades_Day     = 160;            //Max trades per day
...
void OnTick()
{
   ...
   // BUY signal: uptrend H4+H1, BB lower touch, oversold momentum
   bool h4Bull    = (Close[1] > h4_ema);
   bool h1Bull    = (Close[1] > h1_ema) && (h1_rsi >= H1_RSI_Bull_Min) && (h1_rsi <= H1_RSI_Bull_Max);
   bool bbBuyTouch = (Low[1] <= bbLower1) && (Close[1] > bbLower1);
   ...
   if(h4Bull && h1Bull && bbBuyTouch && rsiOS && rsiRising && stochOS && bullBody)
   {
   ...
   // SELL signal: downtrend H4+H1, BB upper touch, overbought momentum
   bool h4Bear     = (Close[1] < h4_ema);
   bool h1Bear     = (Close[1] < h1_ema) && (h1_rsi >= H1_RSI_Bear_Min) && (h1_rsi <= H1_RSI_Bear_Max);
   bool bbSellTouch = (High[1] >= bbUpper1) && (Close[1] < bbUpper1);
   ...
   if(h4Bear && h1Bear && bbSellTouch && rsiOB && rsiFalling && stochOB && bearBody)
   {
