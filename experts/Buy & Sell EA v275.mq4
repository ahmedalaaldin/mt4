//+------------------------------------------------------------------+
//|   Buy & Sell EA v275 - Faster Stochastic (3,2,2) for more sigs  |
//|   Framework: H1 EMA direction + wider H1 RSI / M5 Entry         |
//|   HYPOTHESIS: Stoch(5,3,3) is slow, fewer OS/OB crossings       |
//|     Stoch(3,2,2) oscillates faster → more qualifying bars       |
//|     Combined with session 8-22 and ATR spike 2.5 from v274      |
//|     Expected: significantly more trades than v269 (707)         |
//|     Risk: more noise in Stoch signal → lower win rate           |
//|   BASE: v274 (v269 + session 8-22 + ATR spike 2.5)             |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "275.0"
#property strict

#include <stdlib.mqh>

//--- Session (extended from v274: 8-22)
input string SESSION_SET        = "============"; //====== Session Settings ======
input int    Session_Start_Hour = 8;
input int    Session_End_Hour   = 22;
...
//--- Stochastic Gate - FASTER PERIODS for more signals
input string STOCH_SET          = "============"; //====== Stochastic Gate ======
input int    Stoch_K            = 3;              // v275: faster (was 5)
input int    Stoch_D            = 2;              // v275: faster (was 3)
input int    Stoch_Slow         = 2;              // v275: faster (was 3)
input double Stoch_OS           = 25.0;           // NON-NEGOTIABLE
input double Stoch_OB           = 75.0;           // NON-NEGOTIABLE

//--- Volatility Filter (loosened from v274)
input string VOL_SET            = "============"; //====== Volatility Filter ======
input double Max_ATR_Pct        = 0.60;
input int    ATR_Spike_MA_Period = 50;
input double ATR_Spike_Multi    = 2.5;
...
input int    Magic_Number       = 2752057;
input string Order_Comment      = "BSv275";
...
void OnTick()
{
   ...
   // v275: H1 EMA direction kept, Stoch(3,2,2) faster, session 8-22
   ...
   // v275: Faster Stochastic Stoch_K=3, Stoch_D=2, Stoch_Slow=2
   ...
   //=== SELL: Faster Stoch + session 8-22 ===
   if(h1Bear && stochSell)
   {
