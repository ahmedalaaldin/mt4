//+------------------------------------------------------------------+
//|   Buy & Sell EA v23 - BB+RSI Confluence + Min TP + BE Stop     |
//|   Signal: BB touch AND RSI<40/60 AND bullish/bearish close      |
//|   TP = BB midline (only when >= 1xATR away)                    |
//|   SL = 1xATR; BE Stop: SL->entry when profit >= 0.5xATR        |
//|   Combines v9 signal quality with v11 breakeven protection      |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
...
input double RSI_Buy            = 40.0;           //BUY: RSI must be below this (oversold)
input double RSI_Sell           = 60.0;           //SELL: RSI must be above this (overbought)
...
input double BE_Trigger_ATR     = 0.5;            //Move SL to entry when profit >= ATR x this
...
input int    Magic_Number       = 232025;         //Magic Number
input string Order_Comment      = "BSv23";        //Order comment
...
void OnTick()
{
   ...
   //--- BUY: above H1 EMA200, BB lower touch AND RSI oversold AND bullish close
   if(Close[1] > h1_ema)
   {
   ...
   //--- SELL: below H1 EMA200, BB upper touch AND RSI overbought AND bearish close
   if(Close[1] < h1_ema)
   {
...
void ManageBreakevenStops()
{
   ...
   if(atr0 <= 0) return;
   double beTrigger = BE_Trigger_ATR * atr0;
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(OrderType() == OP_BUY)
      {
         ...
         if(profit >= beTrigger && sl < openPrice)
            OrderModify(OrderTicket(), openPrice, NormalizeDouble(openPrice, Digits),
