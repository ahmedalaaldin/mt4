//+------------------------------------------------------------------+
//|   Buy & Sell EA v189 - Equity Peak Guard                         |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry         |
//|   LESSONS LEARNED:                                               |
//|     v183: Stoch gate          -> 90.50% WR, 15.50% DD, Cal=11.07|
//|     v185: Risk=6.5%           -> 90.62% WR, 19.42% DD, Cal=12.32|
//|     v186: 7.0%, Streak=5/10   -> 90.70% WR, 18.87% DD, Cal=12.60|
//|            BEST: delayed anti-martingale thresholds improve Calmar|
//|     v187: Risk=8.0%           -> 90.16% WR, 26.56% DD, Cal=10.28|
//|            FAIL: 8% base risk too aggressive, DD jumps +40%      |
//|     v188: Scale_Mid/High=1.0  -> 90.44% WR, 17.85% DD, Cal=11.00|
//|            FAIL: removing win scaling hurts return (-41%) more   |
//|            than it helps DD (-1%), net Calmar drop 12.60->11.00  |
//|   ROOT CAUSE OF DD: losses at elevated lot during/after streaks  |
//|   APPROACH v189:                                                  |
//|     Restore v186 win-scaling (Scale_Mid=1.10, Scale_High=1.25)   |
//|     Add Equity Peak Guard: if equity drops >X% from rolling      |
//|     peak, halve the effective lot size until equity recovers.    |
//|     This soft-clamps cascading losses during drawdown periods.   |
//|     All other v186 params identical (Risk=7%, Streak=5/10)       |
//|   Changes vs v183:                                                |
//|     1. New EQUITY_GUARD_SET section (2 new inputs)               |
//|     2. g_EquityPeak global + OnInit() init                       |
//|     3. Equity peak update on every tick (before isNewBar gate)   |
//|     4. Guard applied to riskScale at lot calculation             |
//|     5. Magic_Number: 1832025 -> 1892025                          |
//|     6. Order_Comment: BSv183 -> BSv189                           |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "189.0"
#property strict
...
input int    Magic_Number       = 1892025;        //Magic Number
input string Order_Comment      = "BSv189";       //Order comment
...
int OnInit()
{
   g_EquityPeak = AccountEquity();  // NEW v189: seed peak at start
...
void OnTick()
{
   ...
   // === NEW v189: Update equity peak on every tick ===
   // Must run before isNewBar gate so floating P&L peaks are captured
   ...
   // === NEW v189: Equity Peak Guard ===
   // If equity has dropped >Equity_DD_Guard_Pct% from rolling peak,
   // apply Equity_DD_Guard_Scale to lot size to soft-cap cascading losses.
   if(Equity_DD_Guard_Pct > 0 && g_EquityPeak > 0)
   {
      double ddFromPeak = (g_EquityPeak - curEquity) / g_EquityPeak * 100.0;
