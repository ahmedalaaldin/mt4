//+------------------------------------------------------------------+
//|   Buy & Sell EA v241 - High Base Risk, No Win-Streak Scaling     |
//|   Framework: H4+H1 Direction / M5 Entry                        |
//|   LESSONS LEARNED:                                              |
//|     v186: BB+RSI+Stoch(25/75)+H4+H1 -> Cal=12.60  *** BEST *** |
//|     v236: H4 RSI gate [40,70]/[30,60] -> Cal=9.36 Return=207%  |
//|     v237: H4 RSI + ATR_SL=0.6 -> Cal=2.04 TERRIBLE            |
//|     v238: H4 RSI + No Upscaling -> Cal=8.43 Return=179%        |
//|     v239: Tighter H4 RSI [45,65]/[35,55] -> Cal=8.25 Ret=159%  |
//|       Narrower range filters too many good trades               |
//|     v240: v186 + Risk_Pct=8.5% -> Cal=8.65 Return=242%         |
//|       MaxDD=28.05% — "TALL PEAK, BIG CRASH" PROBLEM            |
//|       60-win streak built equity to $47k, Scale_High=1.25       |
//|       means each loss = $47k × 10.625% = $5k. 4.5× amplif.     |
//|       Root cause: Scale_High × Risk creates (Scale_High)²        |
//|       compound effect: higher risk → higher peaks → 1.25× loss  |
//|                                                                 |
//|   v241 HYPOTHESIS: Break Tall Peak / Big Crash Pattern          |
//|     Remove win-streak UPSCALING: Scale_Mid=1.0, Scale_High=1.0  |
//|     Keep base Risk_Pct=8.5% for 200%+ return                   |
//|     Keep LOSS downscaling: 1 loss=0.5x, 2=0.25x, 3+=0.1x       |
//|     Without Scale_High, max loss from any state = 8.5% equity  |
//|     vs v240's 8.5% × 1.25 = 10.625% at peak                   |
//|     Eliminates (Scale_High)² compound amplification             |
//|     Expected: Return ~200-210%, MaxDD ~13-14%, Calmar ~14-15   |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "241.0"
#property strict
...
//--- Money Management (v241: Risk=8.5%)
input string MONEY_SET          = "============"; //====== Money Management ======
input double Risk_Pct           = 8.5;            // v241: 8.5% to push return above 200%
input double Fixed_Lot          = 0.0;
input int    Max_Buys           = 1;
input int    Max_Sells          = 1;

//--- Anti-Martingale (v241: NO win-streak upscaling, loss protection only)
input string AM_SET             = "============"; //====== Anti-Martingale ======
input int    WinStreak_Mid      = 5;
input int    WinStreak_High     = 10;
input double Scale_Mid          = 1.0;            // v241: NO upscaling on wins (was 1.10)
input double Scale_High         = 1.0;            // v241: NO upscaling on wins (was 1.25)
// Loss downscaling KEPT: 1 loss=0.5x, 2=0.25x, 3+=0.1x for protection

//--- Safety
input string SAFETY_SET         = "============"; //====== Safety Settings ======
input int    MaxSpread          = 35;
input int    Slippage           = 30;
input int    Magic_Number       = 2412051;
input string Order_Comment      = "BSv241";
