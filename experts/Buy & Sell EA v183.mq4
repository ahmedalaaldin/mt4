//+------------------------------------------------------------------+
//|   Buy & Sell EA v183 - Stochastic Confirmation Gate             |
...
//|            SUCCESS: DD halved! ATR spike filter proven           |
//|     v181: Risk=5%, Max_Buys=1      -> 232% ret, 19.17% DD, Cal=12.12 |
//|            NEW BEST: restored 200%+ return, Calmar improved      |
//|     v182: Daily DD guard 8%->5%   -> 182% ret, 26.06% DD, Cal=7.01  |
//|            FAIL: 5% guard with 5% risk fires after 1 loss,      |
//|            blocks afternoon recovery, DD worse not better         |
//|   ROOT CAUSE OF DD: consecutive losses (max 11) at 5% risk      |
//|   APPROACH v183:                                                  |
//|     Add Stochastic(5,3,3) confirmation gate:                     |
//|       BUY only when Stoch K < 25 (oversold) AND rising          |
//|       SELL only when Stoch K > 75 (overbought) AND falling      |
//|     This filters BB touches where momentum isn't actually        |
//|     reversing (trending moves, not mean-reversion setups)        |
//|     Expected: fewer trades (-15-25%) but WR 86%->90%+           |
//|     Lower consecutive losses -> lower DD                         |
//|   Changes vs v181:                                                |
//|     1. New Stochastic Gate section (5 new inputs)                |
//|     2. stochBuy/stochSell conditions added to entry logic        |
//|     3. Daily DD guard: back to 8% (5% was proven wrong)         |
//|     4. Magic_Number: 1812025 -> 1832025                          |
//|     5. Order_Comment: BSv181 -> BSv183                           |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "183.0"
#property strict
...
input int    Magic_Number       = 1832025;        //Magic Number
input string Order_Comment      = "BSv183";       //Order comment
input double Max_Daily_DD_Pct   = 8.0;            //Daily DD guard: 8% (5% proven wrong with 5% risk)
...
void OnTick()
{
   ...
   // === STOCHASTIC GATE (NEW v183) ===
   // Fast Stochastic(5,3,3): confirms momentum reversal at extremes
   ...
   bool stochBuy  = (stoch_k1 < Stoch_OS) && (stoch_k1 > stoch_k2);  // oversold + turning up
   bool stochSell = (stoch_k1 > Stoch_OB) && (stoch_k1 < stoch_k2);  // overbought + turning down
