//+------------------------------------------------------------------+
//|   Buy & Sell EA v27 - EMA Dip-to-Trend Scalper (Buy Only)      |
//|   BUY: H1 EMA200 + M15 EMA20 + M5 EMA8>EMA21 all bullish      |
//|         + price dips to M5 EMA8 then recovers + RSI 35-65      |
//|   TP = 1.5xATR, SL = 0.8xATR (1.875:1 RR)                    |
//|   Trend-following dip entry for XAUUSD bullish 2025-2026        |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
...
//--- Trend Filters
input string TREND_SET          = "============"; //====== Trend Filters ======
input int    H1_EMA_Period      = 200;            //H1 EMA period (macro)
input int    M15_EMA_Period     = 20;             //M15 EMA period (intermediate)
input int    Fast_EMA_Period    = 8;              //M5 fast EMA period (micro)
input int    Slow_EMA_Period    = 21;             //M5 slow EMA period (micro trend)

//--- Entry Signal
input string ENTRY_SET          = "============"; //====== Entry Settings ======
input int    RSI_Period         = 14;             //RSI period
input double RSI_Min            = 35.0;           //RSI lower bound (avoid deep oversold)
input double RSI_Max            = 65.0;           //RSI upper bound (avoid overbought)
...
input double ATR_SL_Multi       = 0.8;            //Stop Loss = ATR x this
input double ATR_TP_Multi       = 1.5;            //Take Profit = ATR x this (1.875:1 RR)
...
input int    Magic_Number       = 272025;         //Magic Number
input string Order_Comment      = "BSv27";        //Order comment
...
void OnTick()
{
   ...
   double m15_ema   = iMA(NULL, PERIOD_M15, M15_EMA_Period,  0, MODE_EMA, PRICE_CLOSE, 0);

   // M5 micro-trend EMAs
   double fast_ema1 = iMA(NULL, 0, Fast_EMA_Period, 0, MODE_EMA, PRICE_CLOSE, 1);
   double slow_ema1 = iMA(NULL, 0, Slow_EMA_Period, 0, MODE_EMA, PRICE_CLOSE, 1);
   ...
   //--- BUY: all trend filters aligned + dip to M5 fast EMA + recovery
   if(Close[1] > h1_ema && Close[1] > m15_ema && fast_ema1 > slow_ema1)
   {
      bool dipToEMA  = (Low[1] <= fast_ema1) && (Close[1] > fast_ema1);
      bool bullClose = (Close[1] > Open[1]);
      bool rsiOK     = (rsi1 >= RSI_Min) && (rsi1 <= RSI_Max);

      if(dipToEMA && bullClose && rsiOK)
      {
