//+------------------------------------------------------------------+
//|   Buy & Sell EA v308 - Slower/Wider Bollinger Bands              |
//|   BASE: v300 CHAMPION (Calmar=20.84, Return=258.39%, 1040 T)      |
//|   CHANGE: BB_Period 20 → 30                                       |
//|   RATIONALE:                                                       |
//|     v306 tested BB_Period=14 (faster) → catastrophic failure     |
//|     (Return=36.96%, Calmar=1.49). Going the other direction:     |
//|     BB_Period=30 creates WIDER, SLOWER bands. Only very extreme  |
//|     price deviations (stronger statistical outliers) trigger      |
//|     signals. Hypothesis: higher-quality entries with better win  |
//|     rate and improved per-trade profitability. The fewer but      |
//|     stronger signals may compensate for reduced trade count.     |
//|   HYPOTHESIS: ~700-900 trades, win rate ~94-96%, similar MaxDD, |
//|     Return 200-260%, Calmar 18-25                                |
//|   DEAD ENDS SO FAR (25 confirmed):                               |
...
//|     v308: TARGET higher quality signals via wider BB period      |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "308.0"
#property strict
...
//--- BB Entry Signal — KEY CHANGE: BB_Period=30 (was 20)
input string S1_SET             = "============"; //====== BB Signal ======
input int    BB_Period          = 30;             // v308: 20 → 30 (wider/slower bands)
...
input int    Magic_Number       = 3082057;
input string Order_Comment      = "BSv308";
...
void OnTick()
{
   ...
   // v308: BB_Period=30 — wider bands, more extreme deviations required
