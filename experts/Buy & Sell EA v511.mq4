//+------------------------------------------------------------------+
//|   Buy & Sell EA v411 - Trail_ATR_Multi=0.25 (sweep: 0.5→0.35→0.25)|
//|   BASE: v406 NEW BEST (ratio=9.10, MaxDD=4.38%) ✓               |
//|                                                                   |
...
#property copyright ""
#property link      ""
#property version   "511.0"  // v511: v411 + M1 entry signals (frequency experiment, targets >=5 trades/day)
#property strict
...
input bool   Use_M1_Signals     = true;            // v511: add M1 entries to lift trades/day toward >=5
...
input double Trail_ATR_Multi    = 0.25;           // v411: sweep 0.5→0.35→0.25
...
input int    Magic_Number       = 5112057;        // v511 unique magic
input string Order_Comment      = "BSv511";
...
void OnTick()
{
   ...
   //=== M1 BUY (v511: add M1 frequency — same BB+RSI mean-reversion on M1, same H1/H4 gates) ===
   if(Use_M1_Signals && h1Bull)
   {
      double m1_bbLower1 = iBands(NULL, PERIOD_M1, BB_Period, BB_Dev, 0, PRICE_CLOSE, MODE_LOWER, 1);
      double m1_bbLower2 = iBands(NULL, PERIOD_M1, BB_Period, BB_Dev, 0, PRICE_CLOSE, MODE_LOWER, 2);
      double m1_rsi1     = iRSI(NULL, PERIOD_M1, RSI_Period, PRICE_CLOSE, 1);
      double m1_rsi2     = iRSI(NULL, PERIOD_M1, RSI_Period, PRICE_CLOSE, 2);

      double m1_high1  = iHigh(NULL, PERIOD_M1, 1);
      double m1_low1   = iLow(NULL, PERIOD_M1, 1);
      double m1_close1 = iClose(NULL, PERIOD_M1, 1);
      double m1_open1  = iOpen(NULL, PERIOD_M1, 1);
      double m1_close2 = iClose(NULL, PERIOD_M1, 2);
