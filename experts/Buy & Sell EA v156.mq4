//+------------------------------------------------------------------+
//|   Buy & Sell EA v156 - TP 1.5x ATR (Closer TP Test)             |
...
//|     v155: Risk=6%, TP=3x → 361.22% / 50.00% DD (ratio 7.22)   |
//|   Key findings:                                                   |
//|     - Risk=6% is optimal (7% triggers daily DD guard too often)  |
//|     - Return/DD peaks at 6% risk (7.32 ratio)                   |
//|     - TP=3x WORSE than TP=2x (fewer second-half hits)           |
//|   v156 Hypothesis: TP 2.0x → 1.5x ATR at Risk=6%              |
//|     If TP=3x > TP=2x fails (it did), try TP=1.5x               |
//|     Smaller TP = higher hit rate for second half after partial   |
//|     Expected: more TP hits, smaller avg win but higher count     |
//|     If hit rate gain > avg win loss → higher return              |
//|   Changes vs v152:                                               |
//|     1. ATR_TP_Multi: 2.0 → 1.5                                 |
//|   All other params identical to v152 (best performer):           |
//|     Risk=6%, ATR=0.60%, Scale_High=1.85, WinStreak_High=5      |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "156.0"
#property strict
...
input double ATR_TP_Multi       = 1.5;            //TP = ATR x this: 2.0→1.5 (closer TP)
...
input int    Magic_Number       = 1562025;        //Magic Number
input string Order_Comment      = "BSv156";       //Order comment
...
void OnTick()
{
   ...
   double tpDist    = ATR_TP_Multi * atr1;   // Now 1.5x ATR
