//+------------------------------------------------------------------+
//|   Buy & Sell EA v212 - Tighter H1 RSI Buy Gate (Min=50)          |
...
//|   v212 HYPOTHESIS: H1_RSI_Min = 50.0 (tighter uptrend gate)       |
//|     Currently: Buy if H1 RSI in [45, 75].                         |
//|     v212 tries: Buy only if H1 RSI in [50, 75].                   |
//|     Rationale: RSI=50 is the neutral midpoint on H1.              |
//|       - RSI [45,50] = borderline/barely bullish H1 momentum        |
//|       - RSI [50,75] = confirmed bullish momentum on H1             |
//|       - Excluding borderline cases should improve buy quality       |
//|       - May reduce trade count slightly but improve win rate        |
//|       - Target: higher Calmar via fewer but higher-quality buys    |
//|     ALL other parameters = v186 GLOBAL OPTIMUM.                   |
//|     Run with PERIOD = M5 in Strategy Tester.                      |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "212.0"
#property strict
...
//--- H1 RSI Gate (v212 CHANGE: H1_RSI_Min = 50.0, was 45.0)
input string H1RSI_SET          = "============"; //====== H1 RSI Gate ======
input int    H1_RSI_Period      = 14;             //H1 RSI period
input double H1_RSI_Min         = 50.0;           //v212 CHANGE: BUY only if H1 RSI >= 50 [50,75]
input double H1_RSI_Max         = 75.0;           //BUY: H1 RSI maximum
input double H1_RSI_Min_Sell    = 25.0;           //SELL: H1 RSI minimum [25,55] (unchanged)
input double H1_RSI_Max_Sell    = 55.0;           //SELL: H1 RSI maximum (unchanged)
...
//--- Exit Settings (v186 GLOBAL OPTIMA — ATR_SL_Multi restored to 0.8)
input string EXIT_SET           = "============"; //====== Exit Settings ======
input int    ATR_Period         = 14;             //ATR period
input double ATR_SL_Multi       = 0.8;            //SL = ATR x 0.8 (v186 GLOBAL OPTIMUM — restored)
...
input int    Magic_Number       = 2122038;        //Magic Number
input string Order_Comment      = "BSv212";       //Order comment
...
void OnTick()
{
   ...
   //v212: H1 RSI Buy gate tightened: [50,75] instead of [45,75]
   ...
   double slDist    = ATR_SL_Multi * atr1;   // v186 optimum: 0.8 * ATR (restored)
   ...
   //=== Signal BUY: H4 bull + H1 bull(EMA+RSI[50,75]) + BB(20,2.5) + RSI<40 + Stoch<25
