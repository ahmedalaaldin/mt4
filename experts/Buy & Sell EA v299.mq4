//+------------------------------------------------------------------+
//|   Buy & Sell EA v299 - BB_Dev 2.5→3.0 (deeper extremes)        |
//|   BASE: v293/v280 CHAMPION (Calmar=18.43, Return=220.62%, 637 T) |
//|   CHANGE: BB_Dev = 3.0 (was 2.5)                                |
//|   RATIONALE:                                                      |
//|     v286 tested BB_Dev=2.0 (LESS extreme) → catastrophic:        |
//|     price crosses 2σ too frequently, lots of noise trades.       |
//|     v299 tests BB_Dev=3.0 (MORE extreme) — the opposite logic:   |
//|     At 3σ, only 0.27% of a normal distribution lies beyond.     |
//|     These are genuinely rare, extreme deviations from mean.      |
//|     XAUUSD at 3σ BB extremes has very high mean-reversion odds.  |
//|     Expected: ~370-450 trades (-30-40%), win rate rises to 97%+, |
//|     fewer trades caught in trending conditions → lower MaxDD.    |
//|     Per-trade profit similar or higher (deeper entry = more       |
//|     room to BB_mid). Total return should stay near 200%+.        |
//|   DEAD ENDS CONFIRMED:                                           |
//|     - Max_Bars: irrelevant (trades exit before time limit)       |
//|     - Body_Pct=0.40: filters quality trades, -58% return        |
//|     - Session/partial/longs-only: all tested dead ends           |
//|   SCOREBOARD:                                                     |
//|     v293: Calmar=18.43 (champion — BB_Dev=2.5, 637 trades)      |
//|     v298: Calmar=18.35 (Max_Bars=20 — dead end, identical)      |
//|     v297: Calmar=6.46  (Body_Pct=40% — disaster)               |
//|     v299: BB_Dev=3.0   — TARGET Calmar 20+                      |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "299.0"
#property strict
...
//--- BB Entry Signal — KEY CHANGE: BB_Dev = 3.0 (was 2.5)
input string S1_SET             = "============"; //====== BB Signal ======
input int    BB_Period          = 20;
input double BB_Dev             = 3.0;   // KEY CHANGE: was 2.5, now 3.0
input int    RSI_Period         = 14;
input double RSI_Buy            = 40.0;
input double RSI_Sell           = 60.0;
input double Body_Pct           = 0.20;   // v293 value (unchanged)
...
input int    Magic_Number       = 2992057;
input string Order_Comment      = "BSv299";
...
void OnTick()
{
   ...
   // BB bands at 3.0σ (KEY CHANGE from 2.5σ)
