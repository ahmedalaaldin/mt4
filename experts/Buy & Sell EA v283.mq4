//+------------------------------------------------------------------+
//|   Buy & Sell EA v283 - Two-Bar Pattern + Max_Buys=2             |
//|   Framework: H1 EMA direction + H1 RSI gate / M5 Entry          |
//|   HYPOTHESIS: Combine v280's two-bar BB breakdown-recovery       |
//|     filter (bar[2] closes beyond band, bar[1] recovers) with     |
//|     v282's Max_Buys=2/Max_Sells=2 (allow second position when   |
//|     conditions retrigger while first is open).                   |
//|     V280 improved quality (94.51% win, Calmar=18.36) but        |
//|     reduced trades (637 vs 707). Adding Max_Buys=2 should        |
//|     recover some trade count while keeping two-bar quality.      |
//|   BASE: v280 NEW CHAMPION (Calmar=18.36, Return=220%, 637 trades)|
//|   CHANGE: Max_Buys=2, Max_Sells=2 (was 1, 1 in v280)            |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "283.0"
#property strict
...
//--- Two-bar pattern settings (from v280)
...
input int    Max_Buys           = 2;   // v283: increased from 1 to 2
input int    Max_Sells          = 2;   // v283: increased from 1 to 2
...
input int    Magic_Number       = 2832057;
input string Order_Comment      = "BSv283";
...
void OnTick()
{
   ...
   //=== BUY: Two-bar pattern + allow up to Max_Buys=2 concurrent positions ===
   if(h1Bull && stochBuy)
   {
   ...
   //=== SELL: Two-bar pattern + allow up to Max_Sells=2 concurrent positions ===
   if(h1Bear && stochSell)
   {
