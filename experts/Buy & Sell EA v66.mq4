//+------------------------------------------------------------------+
//|   Buy & Sell EA v77 - v58 + ATR trailing stop                   |
//|   Entry: H1 EMA200 + H1 RSI 50-75 + BB 2.5SD lower touch       |
//|          + M5 RSI<40 + RSI rising + Stoch K&D<25 + Bull 20%    |
//|   Exit: Trail SL at (Bid - Trail_ATR x ATR), TP = BB mid       |
//|   Changes: ATR trailing stop kicks in from open (no fixed SL)   |
//|   Goal: Higher WR by cutting losers + bigger wins on trends     |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "77.0"
#property strict
...
input double ATR_Trail_Multi    = 0.8;            //Trail SL at (Bid - ATR x this)
input int    Max_Bars           = 40;             //Force close after N bars

//--- Money Management
input string MONEY_SET          = "============"; //====== Money Management ======
input double Risk_Pct           = 7.0;            //Risk % per trade (proven best)
...
input int    Magic_Number       = 772025;         //Magic Number
input string Order_Comment      = "BSv77";        //Order comment
...
{
   if(atr <= 0) return;
   double trailDist = ATR_Trail_Multi * atr;
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      // Only move SL up, never down; keep minimum 1 point away from bid
