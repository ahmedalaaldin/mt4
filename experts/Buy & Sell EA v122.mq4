//+------------------------------------------------------------------+
//|   Buy & Sell EA v122 - No H1 RSI Zone Filter                     |
//|   Framework: 4H Direction / 1H Structure / 5M Entry             |
//|   Base: v120 (224.50%, 14.95% DD, 15.0x ratio, best so far)     |
//|   Changes vs v120:                                               |
//|     - Removed H1 RSI bounds check: only need H1 close >/<        |
//|       H1 EMA200 (drop the RSI 50-75/25-50 zone requirement)      |
//|     - Trail_ATR_Multi: 0.8 (confirmed optimal from v120)         |
//|     - Max_Bars: 100 (from v120)                                  |
//|   Hypothesis: H1 RSI zone filter was rejecting valid setups;     |
//|   removing it should increase trade count while H4+H1 EMA200     |
//|   still provide quality trend alignment                          |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
...
input int    H1_RSI_Period      = 14;             //H1 RSI period (kept for reference, not used in filter)
...
input int    Magic_Number       = 1322025;        //Magic Number
...
void OnTick()
{
   ...
   //--- BUY logic: H4 bull + H1 price above EMA (no RSI zone filter)
   ...
   //--- SELL logic: H4 bear + H1 price below EMA (no RSI zone filter)
