//+------------------------------------------------------------------+
//|   Buy & Sell EA v335 - RSI_Buy 45 → 55                          |
//|   BASE: v328 (BEST BASELINE: +16.25%, MaxDD=4.99%, WR=83.47%)   |
//|                                                                   |
//|   v335 HYPOTHESIS:                                               |
//|   BB_Dev=2.5 already ensures statistical extremity (2.5σ touch). |
//|   RSI<45 may be OVER-FILTERING — trades near RSI=45–55 at a     |
//|   2.5σ BB touch may still be high-probability mean reversions.   |
//|   Goal: increase trade frequency ~50-100% with minimal WR cost.  |
//|                                                                   |
//|   v335 CHANGE (1 parameter only):                                |
//|     RSI_Buy: 45.0 → 55.0                                        |
//|     Allows entries when RSI[1] is between 45–55 (neutral zone    |
//|     that was previously filtered out).                           |
//|     All other parameters IDENTICAL to v328.                      |
//|                                                                   |
//|   SCOREBOARD:                                                     |
//|     v328: +16.25%, MaxDD=4.99%, WR=83.47%, 380 trades  BEST ✓  |
//|     v329: BB_Dev=2.0 → WR~50%                          FAIL ✗  |
//|     v331: Partial=0.40 → WR=77.47%                    FAIL ✗  |
//|     v332: SL=0.65 → WR=70.09%                         FAIL ✗  |
//|     v333: SL=1.0 → +8.08%, 543 trades (lots shrink)   FAIL ✗  |
//|     v334: BB_Period=14 → WR=64.54%                    FAIL ✗  |
//|     v335: RSI_Buy=55 → ??? (testing more signals)             |
//|                                                                   |
...
input double RSI_Buy            = 55.0;           // v335 CHANGE: 45 → 55
...
input double ATR_SL_Multi       = 0.8;            // NON-NEGOTIABLE (v328 optimal)
...
input int    Magic_Number       = 3352057;
input string Order_Comment      = "BSv335";
...
void OnTick()
{
   ...
   if(h1Bull)
   {
      ...
      bool rsiOS       = (rsi1 < RSI_Buy);             // v335: RSI < 55 (was < 45)
