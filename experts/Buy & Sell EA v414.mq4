//+------------------------------------------------------------------+
//|   Buy & Sell EA v412 - Trail_ATR_Multi=0.15 (sweep: 0.5â†’0.35â†’0.25â†’0.15)|
//|   BASE: v411 ALL-TIME BEST (ratio=9.50, MaxDD=4.32%) âœ“          |
//|                                                                   |
...
input double Trail_ATR_Multi    = 0.15;           // v412: sweep 0.5â†’0.35â†’0.25â†’0.15
...
input int    Magic_Number       = 4142057;        // v414 unique magic
input string Order_Comment      = "BSv414";
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(type == OP_BUY)
      {
         ...
         if(partialDone && !partial2Done && Bid >= bbMid + Trail_ATR_Multi * atr)
         {
            double stepLot   = MarketInfo(Symbol(), MODE_LOTSTEP);
            double minLot    = MarketInfo(Symbol(), MODE_MINLOT);
            double partLots2 = MathFloor(OrderLots() * 0.500 / stepLot) * stepLot;
