//+------------------------------------------------------------------+
//|   Buy & Sell EA v177 - Higher Risk + Bigger TP (RSI=40 Restored) |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry          |
//|   LESSONS LEARNED:                                                |
//|     v170: BB 2.5SD, RSI<40, 358 trades, 81.28% WR, 100% return  |
//|     v175: H1 BB layer = ZERO extra trades (correlated signals)   |
//|     v176: RSI 40->45 = 739 trades BUT return FELL 100%->90%      |
//|            Extra RSI 40-45 trades are NET LOSERS (smaller reverts)|
//|            Calmar dropped: 6.97 -> 3.64. RSI relaxation = bad.   |
//|   FINDING: BB(20,2.5SD)+RSI<40 ceiling = ~358 quality trades/yr  |
//|     Relaxing RSI adds quantity but destroys quality R:R.          |
//|     RSI 40-45 zone = weaker pullbacks = smaller BB midline profit |
//|   APPROACH v177:                                                  |
//|     Restore RSI_Buy=40 / RSI_Sell=60 (v170 proven quality)       |
//|     Increase Risk_Pct: 2.0% -> 3.5% (push for higher return)     |
//|     Increase ATR_TP_Multi: 2.0 -> 2.5 (bigger TP on trailing 50%)|
//|     Keep BB_Dev=2.5, Max_Buys=2 (all proven optimal)             |
//|     Expected: ~358 trades, 81% WR, ~175-200% return, ~25% DD     |
//|   Changes vs v170:                                                |
//|     1. Risk_Pct: 2.0 -> 3.5 (larger position size per trade)     |
//|     2. ATR_TP_Multi: 2.0 -> 2.5 (bigger TP on remaining 50%)     |
//|     3. Magic_Number: 1702025 -> 1772025                           |
//|     4. Order_Comment: BSv170 -> BSv177                            |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "179.0"
#property strict
...
input double RSI_Buy            = 40.0;           //M5 RSI max for BUY: 40 (v170 proven quality restored)
input double RSI_Sell           = 60.0;           //M5 RSI min for SELL: 60 (v170 proven quality restored)
...
input double ATR_SL_Multi       = 0.6;            //SL = ATR x 0.6 (tighter: larger lots -> bigger wins, test Calmar impact)
...
input double Risk_Pct           = 4.0;            //Risk%: 4.0 (same as v178, combined with tighter SL)
...
input int    Magic_Number       = 1792025;        //Magic Number
input string Order_Comment      = "BSv179";       //Order comment
...
void OnTick()
{
   ...
   if(h4Bull && h1Bull)
   {
      ...
      bool rsiOS     = (rsi1 < RSI_Buy);       // RSI<40 (v170 quality restored)
   ...
   if(h4Bear && h1Bear)
   {
      ...
      bool rsiOB      = (rsi1 > RSI_Sell);     // RSI>60 (v170 quality restored)
