//+------------------------------------------------------------------+
//|   Buy & Sell EA v143 - Scale_High=1.75 (between v142 and v139)   |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry          |
//|   Base: v142 (169.89% / 39.93% rel DD, 85.37% WR)               |
//|   Changes vs v142:                                                |
//|     1. Scale_High: 1.5 → 1.75 (8.75% risk at high streak)       |
//|        Targets 200%+ return (interpolating v142=170% / v139=218%)|
//|     2. WinStreak_High: 4 → 5 (slightly slower ramp to peak scale)|
//|        With avg 16 consec wins, EA still spends majority of time  |
//|        at Scale_High — slight delay increases protection.         |
//|   Unchanged from v142:                                            |
//|     BB_Dev=2.5, RSI_Buy=40 (proven 85% WR formula)              |
//|     Risk_Pct=5%, Max_Buys=2, Max_Sells=2                        |
//|     Scale_Mid=1.25 at WinStreak_Mid=2                            |
//|     Loss scale: 1 loss=0.5x, 2=0.25x, 3+=0.1x                  |
//|     Partial close at BB midline + trailing stop                  |
//|     H1 RSI gate: Min=45, Max=75 (proven v139 values)            |
//|     Max_Daily_DD_Pct=15%                                         |
//|   Expected result: ~190-210% return, ~42-44% relative DD        |
//|   Scale ladder: v142(1.5)=170% / v143(1.75)=~200% / v139(2.0)=218%|
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "143.0"
#property strict
...
//--- Anti-Martingale (between v142 and v139)
input string AM_SET             = "============"; //====== Anti-Martingale Settings ======
input int    WinStreak_Mid      = 2;              //Consec wins for Scale_Mid
input int    WinStreak_High     = 5;              //Consec wins for Scale_High (was 4)
input double Scale_Mid          = 1.25;           //Scale at mid streak
input double Scale_High         = 1.75;           //Scale at high streak (was 1.5, v139=2.0)
...
input int    Magic_Number       = 1432025;        //Magic Number
input string Order_Comment      = "BSv143";       //Order comment
...
double GetRiskScale()
{
   ...
   if(streak >= WinStreak_High)     return Scale_High;  // 1.75× at 5+ wins
   if(streak >= WinStreak_Mid)      return Scale_Mid;   // 1.25× at 2-4 wins
