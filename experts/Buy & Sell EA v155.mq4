//+------------------------------------------------------------------+
//|   Buy & Sell EA v155 - TP 3x ATR + Risk 6% (Win Size Boost)     |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry          |
//|   Risk/Return table (all ATR=0.60%):                             |
//|     v150: Risk=5%, TP=2x → 280.79% / 43.70% DD (ratio 6.43)   |
//|     v152: Risk=6%, TP=2x → 366.16% / 50.00% DD (ratio 7.32) ✓ |
//|     v153: Risk=5%, Scale=2.5 → 334.25% / 51.43% DD (ratio 6.55)|
//|     v154: Risk=7%, TP=2x → 359.20% / 57.75% DD (ratio 6.22)   |
//|   Key findings:                                                   |
//|     - Risk=6% is optimal (7% triggers daily DD guard too often)  |
//|     - Return/DD peaks at 6% risk (7.32 ratio)                   |
//|     - Higher Scale_High < Higher Risk_Pct for efficiency         |
//|   v155 Hypothesis: TP 2.0x → 3.0x ATR at Risk=6%              |
//|     After partial close at BB midline (SL at breakeven),         |
//|     second half now targets 3x ATR (vs 2x). The second half     |
//|     has zero downside (SL at entry = breakeven).                 |
//|     Expected: larger avg wins, lower hit rate, similar WR        |
//|     If avg win grows more than hit-rate drop → higher return     |
//|     DD impact: same SL, same lot sizes → DD unchanged (?)        |
//|   Changes vs v152:                                               |
//|     1. ATR_TP_Multi: 2.0 → 3.0                                 |
//|   All other params identical to v152 (best performer):           |
//|     Risk=6%, ATR=0.60%, Scale_High=1.85, WinStreak_High=5      |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "155.0"
#property strict
...
input double ATR_SL_Multi       = 1.0;            //SL = ATR x this (unchanged)
input double ATR_TP_Multi       = 3.0;            //TP = ATR x this: 2.0→3.0 (50% bigger)
...
input double Risk_Pct           = 6.0;            //Base risk % (proven optimal)
...
input int    Magic_Number       = 1552025;        //Magic Number
input string Order_Comment      = "BSv155";       //Order comment
...
void OnTick()
{
   ...
   double tpDist    = ATR_TP_Multi * atr1;   // Now 3.0x ATR
