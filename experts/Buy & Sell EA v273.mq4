//+------------------------------------------------------------------+
//|   Buy & Sell EA v273 - Remove H1 RSI range, keep H1 EMA dir only |
//|   Framework: H1 EMA direction only (no RSI range gate) / M5 Entry|
//|   HYPOTHESIS: H1 RSI range may be over-restricting qualifying hrs  |
//|     H1 RSI range 40-80 (v269) adds quality but also cuts trades   |
//|     v273: Keep ONLY H1 EMA direction, remove all H1 RSI filtering  |
//|     Different from v268: v268 removed H1 EMA direction (bad)      |
//|     v273 KEEPS H1 EMA direction but removes H1 RSI range          |
//|     Expected: More trades than v269, test if RSI range is needed  |
//|   BASE: v269 CHAMPION (Calmar=17.77, Return=209%, 707 trades)    |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "273.0"
#property strict
...
//--- H1 RSI inputs (kept as parameters but NOT used for filtering in v273)
input string H1RSI_SET          = "============"; //====== H1 RSI Gate ======
input int    H1_RSI_Period      = 14;
input double H1_RSI_Min         = 40.0;           // NOT USED - H1 RSI gate removed
input double H1_RSI_Max         = 80.0;           // NOT USED - H1 RSI gate removed
input double H1_RSI_Min_Sell    = 20.0;           // NOT USED
input double H1_RSI_Max_Sell    = 60.0;           // NOT USED
...
input int    Magic_Number       = 2732057;
input string Order_Comment      = "BSv273";
...
int OnInit() { g_InitialBalance = AccountBalance(); return(INIT_SUCCEEDED); }
void OnDeinit(const int reason) {}

int CountLastStreak()
{
   datetime prevTime = TimeCurrent() + 1;
   int maxLook = 50; bool isWin = false; bool firstFound = false; int count = 0;
   for(int pass = 0; pass < maxLook; pass++)
   {
      datetime bestTime = 0; bool bestResult = false;
      for(int i = OrdersHistoryTotal() - 1; i >= 0; i--)
      {
         ...
         { bestTime = OrderCloseTime(); bestResult = (OrderProfit() + OrderCommission() + OrderSwap() >= 0); }
      }
      if(bestTime == 0) break;
...
double GetRiskScale()
{
   ...
   if(streak > 0) return 1.0; if(streak == 0) return 1.0;
   int losses = -streak;
   if(losses == 1) return 0.5; if(losses == 2) return 0.25; return 0.1;
}
...
void CloseAllTrades()
{
   ...
   { if(!OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) continue;
     if(OrderSymbol() != Symbol() || OrderMagicNumber() != Magic_Number) continue;
     if(OrderType() == OP_BUY) OrderClose(OrderTicket(), OrderLots(), Bid, Slippage, clrOrange);
     else if(OrderType() == OP_SELL) OrderClose(OrderTicket(), OrderLots(), Ask, Slippage, clrOrange); }
}
...
void ManageExits()
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      if(!OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) continue;
      if(OrderSymbol() != Symbol() || OrderMagicNumber() != Magic_Number) continue;
      int tkt = OrderTicket(); double openPrice = OrderOpenPrice();
      double currentSL = OrderStopLoss(); int type = OrderType();
      ...
      if(type == OP_BUY)
      {
         ...
         if(!partialDone && Bid >= bbMid)
         {
            ...
            else { if(currentSL < openPrice) OrderModify(tkt, openPrice, NormalizeDouble(openPrice, Digits), 0, 0, clrBlue); }
            continue;
         }
         if(partialDone)
         { double trailSL = NormalizeDouble(bbMid - Trail_ATR_Multi * atr, Digits);
           if(trailSL > currentSL) OrderModify(tkt, openPrice, trailSL, 0, 0, clrBlue); }
      ...
      else if(type == OP_SELL)
      {
         ...
         if(!partialDone && Ask <= bbMid)
         {
            ...
            { if(OrderClose(tkt, partLots, Ask, Slippage, clrCyan))
                if(OrderSelect(tkt, SELECT_BY_TICKET, MODE_TRADES))
                   OrderModify(tkt, openPrice, NormalizeDouble(openPrice, Digits), 0, 0, clrBlue); }
            else { if(currentSL > openPrice || currentSL == 0) OrderModify(tkt, openPrice, NormalizeDouble(openPrice, Digits), 0, 0, clrBlue); }
            continue;
         }
         if(partialDone)
         { double trailSL = NormalizeDouble(bbMid + Trail_ATR_Multi * atr, Digits);
...
void OnTick()
{
   ...
   if(today != g_LastDay)
   { g_DayOpenEquity = curBalance; g_DailyDDHit = false; g_TradesToday = 0; g_LastDay = today; }
   ...
   // v273: H1 EMA direction KEPT, H1 RSI range gate REMOVED entirely
   // Only H1 EMA direction used as multi-TF filter
   ...
   //=== BUY: H1 EMA direction only, NO H1 RSI range gate ===
   if(h1Bull && stochBuy)
   {
      bool bbTouch   = (Low[1] <= bbLower1) && (Close[1] > bbLower1);
      bool rsiOS     = (rsi1 < RSI_Buy);
      bool rsiRising = (rsi1 > rsi2);
      bool bullBody  = (candleBodyB > 0) && (candleRange > 0) && (candleBodyB >= Body_Pct * candleRange);
   ...
   //=== SELL: H1 EMA direction only, NO H1 RSI range gate ===
   if(h1Bear && stochSell)
   {
      bool bbTouchUp  = (High[1] >= bbUpper1) && (Close[1] < bbUpper1);
      bool rsiOB      = (rsi1 > RSI_Sell);
      bool rsiFalling = (rsi1 < rsi2);
      bool bearBody   = (candleBodyS > 0) && (candleRange > 0) && (candleBodyS >= Body_Pct * candleRange);
