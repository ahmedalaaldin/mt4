//+------------------------------------------------------------------+
//|   Buy & Sell EA v398 - v393 + re-entry after partial close: extra entry when first trade at breakeven|
...
input int    Magic_Number       = 3982057;        // v398 unique magic
input string Order_Comment      = "BSv398";
...
void OnTick()
{
   ...
   if(h1Bull)
   {
      ...
      {
         // Allow re-entry if existing trade has SL at breakeven (protected after partial close)
         int effectiveMaxBuys = Max_Buys;
         for(int re = OrdersTotal()-1; re >= 0; re--)
         {
            if(!OrderSelect(re, SELECT_BY_POS, MODE_TRADES)) continue;
            if(OrderMagicNumber() != Magic_Number) continue;
            if(OrderType() != OP_BUY) continue;
            if(OrderStopLoss() >= OrderOpenPrice() - Point)
            {
               effectiveMaxBuys = Max_Buys + 1;
               break;
            }
         }
         if(CountTradesByMagic(OP_BUY, Magic_Number) < effectiveMaxBuys)
         {
