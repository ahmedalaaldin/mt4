//+------------------------------------------------------------------+
//|   Buy & Sell EA v115 - Fixed TP (No Partial Close / No Trail)    |
//|   Framework: 4H Direction / 1H Structure / 5M Entry             |
//|   Base: v114 entry logic                                         |
//|   Key change:                                                    |
//|     - Fixed TP = ATR_TP_Multi * ATR (set directly in OrderSend) |
//|     - Eliminates partial close + trailing stop entirely          |
//|     - Equity peaks capped at entry + 1.5*ATR (vs full BB mid)   |
//|     - Shorter trade duration = less equity exposure per trade    |
//|     - Goal: break the DD–risk% linear relationship by removing  |
//|       large unrealized-profit peaks from the equity curve        |
//|   Expected: higher WR, faster turnover, lower equity DD         |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
...
input double ATR_TP_Multi       = 1.5;            //TP = ATR x this (fixed take profit)
...
input int    Magic_Number       = 1252025;        //Magic Number
input string Order_Comment      = "BSv125";       //Order comment
...
void ManageExits()
{
   // Only handle timeout — TP and SL are managed by the broker
