//+------------------------------------------------------------------+
//|   Buy & Sell EA v157 - Tighter ATR Filter 0.45% (vs 0.60%)      |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry          |
//|   Risk/Return table (all Risk=6%):                               |
//|     v152: ATR=0.60%, TP=2x → 366.16% / 50.00% DD (ratio 7.32) ✓|
//|     v155: ATR=0.60%, TP=3x → 361.22% / 50.00% DD (ratio 7.22)  |
//|     v156: ATR=0.60%, TP=1.5x→ 227.16% / 46.47% DD (ratio 4.89) |
//|   TP Conclusion: TP=2.0x is the proven optimal exit multiplier   |
//|   Key findings:                                                   |
//|     - Risk=6% is optimal risk level                              |
//|     - TP=2.0x ATR is confirmed optimal (1.5x and 3.0x worse)    |
//|     - BB midline partial close + breakeven SL is the key exit    |
//|   v157 Hypothesis: ATR filter 0.60% → 0.45% (tighter)          |
//|     Filters top ~15-20% most volatile M5 bars (vs ~10% at 0.60%)|
//|     Expected: fewer but higher-quality entries, lower DD         |
//|     Higher quality = fewer whipsaw traps = better Return/DD      |
//|     If Return/DD > 7.32 → new best configuration                |
//|   Changes vs v152:                                               |
//|     1. Max_ATR_Pct: 0.60 → 0.45                                |
//|   All other params identical to v152 (best performer):           |
//|     Risk=6%, TP=2x, Scale_High=1.85, WinStreak_High=5           |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "157.0"
#property strict
...
input double Max_ATR_Pct        = 0.45;           //Max ATR as % of price: 0.60→0.45 (tighter)
...
input int    Magic_Number       = 1572025;        //Magic Number
input string Order_Comment      = "BSv157";       //Order comment
...
void OnTick()
{
   ...
   // === ATR% Volatility Filter (tighter: 0.45% vs 0.60%) ===
   if(Max_ATR_Pct > 0)
   {
