//+------------------------------------------------------------------+
//|   Buy & Sell EA v159 - WinStreak_High=3 (faster scale onset)     |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry          |
//|   Risk/Return table (all Risk=6%, ATR=0.60%, TP=2x):            |
//|     v152: Scale=1.85, WS=5 → 366.16% / 50.00% DD (ratio 7.32) ✓|
//|     v155: TP=3.0x        → 361.22% / 50.00% DD (ratio 7.22)    |
//|     v157: ATR=0.45%      → 366.14% / 50.00% DD (ratio 7.32) =  |
//|     v158: Scale=2.0,WS=5 → 337.73% / 52.01% DD (ratio 6.49)   |
//|   Confirmed optimal: Risk=6%, TP=2x, Scale=1.85, ATR=0.60%     |
//|   Key findings:                                                   |
//|     - Scale_High=1.85 is the exact sweet spot (2.0 too high)    |
//|     - TP=2.0x, ATR=0.60% confirmed optimal on both sides        |
//|     - v152 holds at Return/DD=7.32 ceiling with current params   |
//|   v159 Hypothesis: WinStreak_High 5 → 3 (scale starts sooner)  |
//|     Current: scale kicks in at 5+ consecutive wins              |
//|     v159: scale kicks in at 3+ consecutive wins                 |
//|     86% WR means 3-win streaks are common (~every 2-3 trades)   |
//|     Expected: more trades at Scale_High → higher compounding    |
//|     Risk: more exposure at high scale → potential DD increase    |
//|     If compounding gain > DD cost → ratio > 7.32                |
//|   Changes vs v152:                                               |
//|     1. WinStreak_High: 5 → 3                                   |
//|   All other params identical to v152:                            |
//|     Risk=6%, TP=2x, ATR=0.60%, Scale_High=1.85                 |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "159.0"
#property strict
...
input int    WinStreak_High     = 3;              //Consec wins for Scale_High: 5→3 (faster onset)
input double Scale_Mid          = 1.25;           //Scale at mid streak
input double Scale_High         = 1.85;           //Scale at high streak (proven optimal)
...
input int    Magic_Number       = 1592025;        //Magic Number
input string Order_Comment      = "BSv159";       //Order comment
