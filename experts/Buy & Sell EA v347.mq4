//+------------------------------------------------------------------+
//|   Buy & Sell EA v347 - Max_Buys=2, Risk_Pct=0.45                 |
...
//|                                                                   |
//|   v347 HYPOTHESIS: Max_Buys=2 + Risk_Pct=0.45                   |
//|   With Max_Buys=1, an M15 or M30 signal is SKIPPED if an M5     |
//|   trade is already open. With Max_Buys=2 at Risk_Pct=0.45%,     |
//|   two concurrent trades can be open simultaneously.              |
//|   Total max risk = 2 x 0.45% = 0.90% (vs current 0.85%).       |
//|   When M5 is open and M15 signals, we can take BOTH.            |
//|   Expected: ~900-1000 trades, similar MaxDD, higher return.      |
//|                                                                   |
//|   v347 CHANGES from v343:                                        |
//|     Max_Buys: 1 → 2                                              |
//|     Risk_Pct: 0.85 → 0.45 (half per trade, ~same total risk)    |
//|                                                                   |
...
input double Risk_Pct           = 0.45;           // v347: halved (2 concurrent = 0.90% total)
input double Fixed_Lot          = 0.0;
input int    Max_Buys           = 2;              // v347: allow 2 concurrent buys
...
input int    Magic_Number       = 3472057;        // v347 unique magic
input string Order_Comment      = "BSv347";
