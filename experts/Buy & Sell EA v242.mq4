//+------------------------------------------------------------------+
//|   Buy & Sell EA v242 - Optimal Risk, No Win-Streak Scaling       |
...
//|     v240: Risk=8.5% + Scale=1.25 -> Cal=8.65 Return=242%       |
//|       MaxDD=28.05% — TALL PEAK / BIG CRASH                     |
//|     v241: Risk=8.5% + Scale=1.0 -> Cal=6.31 Return=172%        |
//|       MaxDD=27.20% — still TALL PEAK at 8.5% base risk         |
//|       KEY LESSON: 8.5% risk itself is too high for Calmar       |
//|       Even without upscaling, high base risk grows too fast     |
//|       then proportional losses from peak stay large             |
//|                                                                 |
//|   v242 HYPOTHESIS: 7% Risk (v186 optimal) + No Win-Streak      |
//|     Scaling (Scale_Mid=1.0, Scale_High=1.0) + No H4 RSI gate   |
//|     This combination has NOT been tested: v186 had 7%+scaling, |
//|     v238 had 7%+no-scaling+H4 RSI gate                         |
//|     EXPECTED: Return ~165-175%, MaxDD ~11-12%, Calmar ~14-15   |
//|     If MaxDD drops from v186's 14.21% to ~11-12% while         |
//|     return stays ~165%+, Calmar beats 12.60 → new record       |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "242.0"
#property strict
...
//--- Money Management (v242: Risk=7.0% — back to v186 OPTIMUM)
input string MONEY_SET          = "============"; //====== Money Management ======
input double Risk_Pct           = 7.0;            // v242: 7.0% (v186 CONFIRMED OPTIMUM)
input double Fixed_Lot          = 0.0;
input int    Max_Buys           = 1;
input int    Max_Sells          = 1;

//--- Anti-Martingale (v242: NO win-streak upscaling, loss protection only)
input string AM_SET             = "============"; //====== Anti-Martingale ======
input int    WinStreak_Mid      = 5;
input int    WinStreak_High     = 10;
input double Scale_Mid          = 1.0;            // v242: NO upscaling (was 1.10 in v186)
input double Scale_High         = 1.0;            // v242: NO upscaling (was 1.25 in v186)
...
input int    Magic_Number       = 2422052;
input string Order_Comment      = "BSv242";
