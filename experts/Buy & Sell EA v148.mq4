//+------------------------------------------------------------------+
//|   Buy & Sell EA v148 - Max_Buys=1, Risk_Pct=5% (clean test)     |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry          |
//|   Base: v147 (260.07% / 50.00% DD / 85.81% WR / 465 trades)    |
//|   Changes vs v147:                                               |
//|     1. Risk_Pct: 6% → 5%  (return to v144 base risk)           |
//|        v147 muddied the test by raising Risk_Pct simultaneously  |
//|        This version cleanly isolates Max_Buys=1 effect:         |
//|        Same as v144 EXCEPT Max_Buys=1 (was 2 in v144)          |
//|   Hypothesis: Max_Buys=1 + 5% risk should reduce DD vs v144    |
//|     v144: Max_Buys=2, 5% risk → 207% return, 43.70% DD        |
//|     v148: Max_Buys=1, 5% risk → ? return, ? DD                |
//|   If DD drops significantly → simultaneous positions cause DD   |
//|   If DD unchanged → anti-martingale scaling itself causes DD    |
...
input double Risk_Pct           = 5.0;            //Base risk % (back to v144 level)
input double Fixed_Lot          = 0.0;            //Fixed lot (0 = use Risk_Pct)
input int    Max_Buys           = 1;              //Max concurrent BUY (1 = no simultaneous)
input int    Max_Sells          = 1;              //Max concurrent SELL (1 = no simultaneous)
...
input int    Magic_Number       = 1482025;        //Magic Number
input string Order_Comment      = "BSv148";       //Order comment
