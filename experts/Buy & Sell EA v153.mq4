//+------------------------------------------------------------------+
//|   Buy & Sell EA v153 - Scale_High=2.5 (Amplify Win Streaks)     |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry          |
//|   Base: v150 (280.79% / 43.70% DD / 86.03% WR / 458 trades)    |
//|   v152: Risk=6%+ATR=0.60% → 366.16% / 50.00% DD                |
//|     ATR filter adds +106% return at 6% risk (260→366%)          |
//|     But DD stays at 50% (same as 6% risk without ATR)           |
//|     DD is driven by base risk %, not ATR filter                  |
//|   v153 Hypothesis: Scale_High 1.85→2.5 (win-streak amplifier)  |
//|     Keep base risk at 5% (to preserve 43.7% DD target range)    |
//|     Boost peak anti-martingale multiplier by 35%                 |
//|     Win streaks (5+ wins) now scale at 2.5x instead of 1.85x   |
//|     Expected: 330-370% return, 44-48% DD                        |
//|     Risk: First loss after high win streak is at 2.5x scale      |
//|       → may slightly increase DD vs v150                         |
//|     But returns should gain proportionally from long win streaks  |
//|   Changes vs v150:                                               |
//|     1. Scale_High: 1.85 → 2.5                                   |
//|   All other params identical to v150:                            |
//|     BB_Dev=2.5, RSI_Buy=40, ATR=0.60%, Risk_Pct=5%             |
//|     WinStreak_High=5, WinStreak_Mid=2, Scale_Mid=1.25           |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "153.0"
#property strict
...
input double Risk_Pct           = 5.0;            //Base risk % (v150 proven level)
...
input double Scale_Mid          = 1.25;           //Scale at mid streak (unchanged)
input double Scale_High         = 2.5;            //Scale at high streak: 1.85→2.5
...
input int    Magic_Number       = 1532025;        //Magic Number
input string Order_Comment      = "BSv153";       //Order comment
