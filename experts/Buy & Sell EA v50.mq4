//+------------------------------------------------------------------+
//|   Buy & Sell EA v61 - v47 Best + Break-Even Trailing Stop       |
//|   Entry: H1 EMA200 + H1 RSI 50-75 + BB 2.5SD lower touch       |
//|          + M5 RSI<40 + RSI rising + Stoch K&D<25 + Bull 20%    |
//|   Exit: TP = BB mid, SL = 1xATR + BE after 50% move to TP      |
//|   Changes: Add break-even stop when price moves 50% to TP       |
//|   Goal: Reduce DD by eliminating reversal losses                 |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "61.0"
#property strict
...
input double BE_Trigger_Pct     = 0.5;            //Move SL to BE after this % of TP reached

//--- Money Management
input string MONEY_SET          = "============"; //====== Money Management ======
input double Risk_Pct           = 8.0;            //Risk % per trade
...
input int    Magic_Number       = 612025;         //Magic Number
input string Order_Comment      = "BSv61";        //Order comment
...
void ManageExits()
{
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      // Break-even: once price moves BE_Trigger_Pct of the way to TP, move SL to open
      if(tp > openPrice && sl < openPrice)
      {
         double tpDist     = tp - openPrice;
         double beTrigger  = openPrice + BE_Trigger_Pct * tpDist;
         double beLevel    = NormalizeDouble(openPrice + Point, Digits);
         if(Bid >= beTrigger)
            OrderModify(OrderTicket(), openPrice, beLevel, tp, 0, clrBlue);
      }
