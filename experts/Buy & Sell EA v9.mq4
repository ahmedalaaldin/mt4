//+------------------------------------------------------------------+
//|   Buy & Sell EA v20 - BB + RSI Confluence Scalper               |
//|   BUY: BB lower touch AND RSI<40 (double oversold confluence)   |
//|   SELL: BB upper touch AND RSI>60 (double overbought)           |
//|   TP = BB midline, minimum 1.0×ATR enforced                     |
//|   Only enters when TP distance >= SL distance (positive RR)     |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
...
input double TP_Min_ATR         = 1.0;            //Min TP distance as ATR multiple (skip if too close)
...
input int    Magic_Number       = 202025;         //Magic Number
input string Order_Comment      = "BSv20";        //Order comment
...
void OnTick()
{
   ...
   //--- BUY: above H1 EMA200, bar dipped to/below BB lower, RSI below buy threshold
   if(Close[1] > h1_ema)
   {
      bool bbTouch   = (Low[1] <= bbLower1) && (Close[1] > bbLower1);
      bool rsiOversold = (rsi1 < RSI_Buy);
      bool bullClose = (Close[1] > Open[1]);

      if(bbTouch && rsiOversold && bullClose)
      {
         double entry = NormalizeDouble(Ask, Digits);
         double tpDist = bbMid0 - entry;

         //--- Only enter if TP distance >= minimum (ensures positive RR)
         if(tpDist >= minTP)
         {
   ...
   //--- SELL: below H1 EMA200, bar spiked to/above BB upper, RSI above sell threshold
   if(Close[1] < h1_ema)
   {
      bool bbTouch     = (High[1] >= bbUpper1) && (Close[1] < bbUpper1);
      bool rsiOverbought = (rsi1 > RSI_Sell);
      bool bearClose   = (Close[1] < Open[1]);

      if(bbTouch && rsiOverbought && bearClose)
      {
