//+------------------------------------------------------------------+
//|   Buy & Sell EA v293 - Faster Exit (Max_Bars 100 → 30)           |
//|   BASE: v280 CHAMPION (Calmar=18.36, Return=220%, 637 trades)    |
//|   CHANGE: Max_Bars reduced from 100 to 30 (5 bars = 150 min)    |
//|   RATIONALE:                                                      |
//|     All entry/exit tested variants have failed to beat v280:     |
//|       v292 (SL=0.6×ATR) → Calmar=9.02, win rate dropped to 91%  |
//|       v291 (BBW<8.0)    → Calmar=7.72, MaxDD=18.18%             |
//|       v290 (aggressive AM) → Calmar=14.90                        |
//|       v289 (BBW<5.5)    → Calmar=13.89                           |
//|                                                                   |
//|   HYPOTHESIS: Mean reversion either works within 2.5 hours (30   |
//|   M5 bars) or it fails. Trades still open after 30 bars are      |
//|   slow-drifters — they haven't reversed quickly, suggesting the  |
//|   signal was a false reversal in a trending market. Closing them  |
//|   at bar 30 produces smaller exits (price hasn't drifted as far  |
//|   yet), frees the slot for fresh high-quality signals sooner,    |
//|   and reduces exposure time. Expected: same 637 trades or more,  |
//|   slightly smaller per-trade losses on stuck trades, similar or  |
//|   improved MaxDD vs v280.                                         |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "293.0"
#property strict
...
//--- Exit Settings (KEY CHANGE: Max_Bars 100 → 30)
//    Mean reversion on M5: if price hasn't reached BB_mid in 30 bars
//    (150 minutes), the reversal likely failed. Close and recycle slot.
...
input int    Max_Bars           = 30;   // KEY CHANGE: was 100 → now 30 bars (150 min)
...
input int    Magic_Number       = 2932057;
input string Order_Comment      = "BSv293";
...
void ManageExits()
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(type == OP_BUY)
      {
         ...
         // KEY: 30-bar timeout instead of 100 bars
