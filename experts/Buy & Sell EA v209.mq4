//+------------------------------------------------------------------+
//|   Buy & Sell EA v209 - Earlier Streak Scaling Thresholds         |
...
//|   v209 HYPOTHESIS: Earlier scaling activation thresholds           |
//|     v186 scales up at: 1.10x after 5 wins, 1.25x after 10 wins   |
//|     v209 tries:       1.10x after 3 wins, 1.25x after 8 wins     |
//|     Rationale: average streak is ~20 wins (from v208 data).       |
//|     Most streaks hit 3+ wins before 5+, meaning more time spent   |
//|     at 1.10x and 1.25x scale without increasing the max risk cap. |
//|     Winning streaks of 8+ are common; triggering 1.25x sooner     |
//|     at win #8 vs #10 could boost returns without raising max DD.  |
//|     Scale_High RESTORED to v186 optimum (1.25x, not 1.50x).      |
//|     ALL other parameters = v186 GLOBAL OPTIMUM:                   |
//|       Max_Buys=1, Max_Sells=1, Risk_Pct=7.0%, H1 RSI gate ON     |
//|       Max_Daily_DD_Pct=8.0% (v186 confirmed optimum)             |
//|     Run with PERIOD = M5 in Strategy Tester.                      |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "209.0"
#property strict
...
//--- Anti-Martingale (v209 HYPOTHESIS: earlier scaling thresholds)
input string AM_SET             = "============"; //====== Anti-Martingale ======
input int    WinStreak_Mid      = 3;              //v209 CHANGE: 1.10x after 3 wins (was 5 in v186)
input int    WinStreak_High     = 8;              //v209 CHANGE: 1.25x after 8 wins (was 10 in v186)
input double Scale_Mid          = 1.10;           //1.10x at WinStreak_Mid+ wins (v186 optimum)
input double Scale_High         = 1.25;           //v209 RESTORED: 1.25x at WinStreak_High+ (v186 optimum)
...
input int    Magic_Number       = 2092038;        //Magic Number
input string Order_Comment      = "BSv209";       //Order comment
input double Max_Daily_DD_Pct   = 8.0;            //Daily DD guard: 8% (v186 GLOBAL OPTIMUM — DO NOT CHANGE)
...
double GetRiskScale()
{
   ...
   if(streak >= WinStreak_High) return Scale_High;  // 1.25x at 8+ wins (v209: was 10)
   if(streak >= WinStreak_Mid)  return Scale_Mid;   // 1.10x at 3+ wins (v209: was 5)
