//+------------------------------------------------------------------+
//|   Buy & Sell EA v129 - Dual Signal (BB Touch + EMA Cross) v2     |
//|   Framework: 4H Direction / 1H Structure / 5M Entry             |
//|   Base: v128                                                     |
//|   Changes vs v128:                                               |
//|     - FIXED: Signal 2 TP check was always blocking entry         |
//|       bbMid0 - entry <= 0 when EMA cross fires in trend          |
//|       Now uses ATR-based TP (entry +/- TP2_ATR_Multi * ATR)      |
//|     - Signal 2: explicit TP set in OrderSend (not trail only)    |
//|     - H1 RSI filter widened: 40-85 buys / 15-60 sells            |
//|     - M5 Stochastic widened: OS 30 / OB 70                       |
//|     - M5 RSI widened: Buy<45, Sell>55                             |
//|     - SL tightened: ATR_SL_Multi 1.0 → 0.7                       |
//|     - Risk reduced: Risk_Pct 5.0 → 3.5                            |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
...
input double H1_RSI_Min         = 40.0;           //H1 RSI min for BUY zone (was 50)
input double H1_RSI_Max         = 85.0;           //H1 RSI max for BUY zone (was 75)
input double H1_RSI_Min_Sell    = 15.0;           //H1 RSI min for SELL zone (was 25)
input double H1_RSI_Max_Sell    = 60.0;           //H1 RSI max for SELL zone (was 50)
...
//--- Entry Signal 1: BB Touch confirmation
input string ENTRY_SET          = "============"; //====== BB Touch Filters ======
input int    RSI_Period         = 14;             //M5 RSI period
input double RSI_Buy            = 45.0;           //M5 RSI max for BUY oversold (was 40)
input double RSI_Sell           = 55.0;           //M5 RSI min for SELL overbought (was 60)
input int    Stoch_K            = 5;              //Stochastic K period
input int    Stoch_D            = 3;              //Stochastic D period
input int    Stoch_Slowing      = 3;              //Stochastic slowing
input double Stoch_OS           = 30.0;           //Stochastic oversold level buy (was 25)
input double Stoch_OB           = 70.0;           //Stochastic overbought level sell (was 75)
...
input double ATR_SL_Multi       = 0.7;            //SL = ATR x this (was 1.0)
...
input double Risk_Pct           = 3.5;            //Risk % per trade (was 5.0)
...
input int    Magic_Number       = 1392025;        //Magic Number (Signal 1)
...
void OnTick()
{
   ...
   //=== Signal 1: BB Touch (mean-reversion, unchanged quality filters) ===
   if(h4Bull && h1Bull)
   {
   ...
   //=== Signal 2: EMA Crossover (Magic_Number+1, ATR-based TP - fixed) ===
   if(Use_EMA_Cross)
   {
      ...
      // H1 structure only (no RSI restriction — momentum can be anywhere)
...
void ManageExits()
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      // Signal 2 has its own TP set in OrderSend — only apply time-based exit
