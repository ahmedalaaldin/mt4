//+------------------------------------------------------------------+
//|   Buy & Sell EA v174 - BB 2.5 SD + Relaxed Entry Spacing        |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry         |
//|   LESSONS LEARNED:                                               |
//|     v170: BB 2.5 SD, M5, 2.0% risk -> 100% return, 14.34% DD   |
//|     v171: No H4/H1 EMA filter -> WR 81%->73%, WORSE DD          |
//|     v172: M1 timeframe -> WR 81%->56%, -41% return (FAILED)     |
//|     v173: BB 2.0 SD -> WR 75%, poor R:R (avg win < avg loss)    |
//|     ROOT CAUSE v173: smaller SD = smaller midline profit, same   |
//|     ATR-based SL -> effective R:R=0.40 (terrible)               |
//|   APPROACH v174:                                                  |
//|     Return to BB 2.5 SD (proven R:R and 81% WR)                 |
//|     Increase trade count by: Min_Bar_Gap=0 (no forced wait)     |
//|     Allow 3 concurrent positions (Max_Buys/Sells = 3)           |
//|     Risk_Pct = 1.5% (compromise between 2.0% v170 and 1.0% v173)|
//|   Changes vs v170:                                               |
//|     1. Min_Bar_Gap: 1 -> 0 (entries on consecutive bars allowed) |
//|     2. Max_Buys: 2 -> 3, Max_Sells: 2 -> 3 (more slots)        |
//|     3. Risk_Pct: 2.0 -> 1.5 (DD control)                       |
//|     4. Magic_Number: 1702025 -> 1742025                         |
//|     5. Order_Comment: BSv170 -> BSv174                          |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "174.0"
#property strict
...
input int    Min_Bar_Gap        = 0;              //Min bars between entries: 1->0
...
input double Risk_Pct           = 1.5;            //Risk%: 2.0->1.5 (DD control)
input double Fixed_Lot          = 0.0;            //Fixed lot (0 = use Risk_Pct)
input int    Max_Buys           = 3;              //Max concurrent BUY: 2->3
input int    Max_Sells          = 3;              //Max concurrent SELL: 2->3
...
input int    Magic_Number       = 1742025;        //Magic Number
input string Order_Comment      = "BSv174";       //Order comment
...
void OnTick()
{
   ...
   if(h4Bull && h1Bull)
   {
      ...
      if(bbTouch && rsiOS && rsiRising && bullBody)
      {
         ...
         int barsSince = (Min_Bar_Gap == 0) ? Min_Bar_Gap
                       : (int)((Time[0] - g_LastBuyTime) / PeriodSeconds());
   ...
   if(h4Bear && h1Bear)
   {
      ...
      if(bbTouchUp && rsiOB && rsiFalling && bearBody)
      {
         ...
         int barsSince = (Min_Bar_Gap == 0) ? Min_Bar_Gap
                       : (int)((Time[0] - g_LastSellTime) / PeriodSeconds());
