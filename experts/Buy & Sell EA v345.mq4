//+------------------------------------------------------------------+
//|   Buy & Sell EA v345 - RSI_Buy=50 (wider RSI gate)               |
...
//|     v344: No TwoBar → WR=75.88%, +9.18%             FAIL ✗     |
//|                                                                   |
//|   v345 HYPOTHESIS: Test RSI_Buy=50 (wider RSI gate)              |
//|   RSI_Buy=45 was confirmed best when testing LOWER (42 failed).  |
//|   But RSI_Buy=50 was NEVER tested. Cases where recovery bar       |
//|   pushes RSI from ~35 to ~47 (45-50 range) = strong recoveries  |
//|   currently skipped. These might be high-quality signals.        |
//|   Expected: +20-40% more trades, WR stays above 76.8% breakeven.|
//|                                                                   |
//|   v345 CHANGES from v343:                                        |
//|     RSI_Buy = 50.0 (was 45.0)                                    |
//|     Use_TwoBar_Pattern = true (CRITICAL - confirmed non-negot.)  |
//|     All other settings identical to v343                         |
//|                                                                   |
//|   NON-NEGOTIABLES (ALL unchanged from v343):                     |
//|     BB_Dev=2.5, BB_Period=20, Use_TwoBar_Pattern=true            |
...
input double RSI_Buy            = 50.0;           // v345: test wider gate (was 45.0)
input double RSI_Sell           = 60.0;
input double Body_Pct           = 0.20;

//--- Two-bar pattern (CONFIRMED NON-NEGOTIABLE - v344 proved it critical)
...
input int    Magic_Number       = 3452057;        // v345 unique magic
input string Order_Comment      = "BSv345";
...
void OnTick()
{
   ...
   //=== M15 BUY (v342+: same pattern on M15, RSI_Buy=50) ===
   if(Use_M15_Signals && isNewM15Bar && h1Bull)
   {
      ...
      bool m15_rsiOS       = (m15_rsi1 < RSI_Buy);    // v345: RSI_Buy=50
   ...
   //=== M30 BUY (v343+: same pattern on M30, RSI_Buy=50) ===
   if(Use_M30_Signals && isNewM30Bar && h1Bull)
   {
      ...
      bool m30_rsiOS       = (m30_rsi1 < RSI_Buy);    // v345: RSI_Buy=50
