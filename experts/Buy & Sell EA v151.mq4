//+------------------------------------------------------------------+
//|   Buy & Sell EA v151 - ATR Filter + Higher WinStreak_High        |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry          |
//|   Base: v150 (280.79% / 43.70% DD / 86.03% WR / 458 trades)    |
//|   Key finding from v150:                                          |
//|     ATR 0.60% filter: DRAMATICALLY improved return 207%→281%    |
//|     - Filtered out weak volatile entries, kept high-quality      |
//|     - WR improved 85.53%→86.03% (tiny % but huge compound)     |
//|     - Avg win improved $153→$170 (11% better quality trades)    |
//|     - Anti-martingale amplified these improvements →+74% return  |
//|   But: DD stayed exactly 43.70% (same $6,151 event persists)   |
//|     The 43.7% DD comes from a specific early-period event when   |
//|     equity was ~$14K and a 7-loss cluster hit the strategy.      |
//|     This cluster has LOW ATR (not filtered) but the mean-        |
//|     reversion fails anyway (likely a strong sustained trend).   |
//|   Changes vs v150:                                               |
//|     1. WinStreak_High: 5 → 7                                    |
//|        Need 7 consecutive wins to reach Scale_High=1.85         |
//|        Theory: reaching Scale_High less frequently = fewer       |
//|        large-scale losses = potentially lower DD                 |
//|        Cost: slightly lower return (less time at max scale)     |
//|        Benefit: 7-win requirement means we're in a VERY strong  |
//|        trend when we scale up → higher confidence               |
//|     2. Scale_High: 1.85 → 2.0 (compensate for harder-to-reach) |
//|        Higher peak scale when streaks DO reach 7                |
//|        Net peak exposure: similar to v150 on average            |
//|   Expected: ~250-280% return, potentially ~38-42% DD           |
//|   All other params identical to v150:                            |
//|     ATR filter 0.60%, BB_Dev=2.5, RSI_Buy=40, Body_Pct=0.2    |
//|     Risk_Pct=5%, WinStreak_Mid=2, Scale_Mid=1.25               |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "151.0"
#property strict
...
input double Max_ATR_Pct        = 0.60;           //Max ATR% of price (proven from v150)
...
input double Risk_Pct           = 5.0;            //Base risk %
...
input int    WinStreak_High     = 7;              //Consec wins for Scale_High (was 5, now 7)
input double Scale_Mid          = 1.25;           //Scale at mid streak
input double Scale_High         = 2.0;            //Scale at high streak (was 1.85, now 2.0)
...
input int    Magic_Number       = 1512025;        //Magic Number
input string Order_Comment      = "BSv151";       //Order comment
...
void OnTick()
{
   ...
   // ATR% Volatility Filter (proven from v150)
   if(Max_ATR_Pct > 0)
   {
