//+------------------------------------------------------------------+
//|   Buy & Sell EA v114 - Single Position Per Direction             |
//|   Framework: 4H Direction / 1H Structure / 5M Entry             |
//|   Base: v113 (253.80%, 16.26% DD proven dual-direction)          |
//|   Changes:                                                       |
//|     - Max_Buys=1, Max_Sells=1 (no P2 compounding)                |
//|     - Risk_Pct=5.0% (was 5.5%)                                   |
//|     - Simplified entry gate: openBuys==0 / openSells==0          |
//|     - Removes AllPositionsRiskFree overhead (H4 ensures           |
//|       buy/sell mutual exclusivity; Max=1 prevents stacking)      |
//|   Goal: Reduce DD from 16.26% toward <8% while preserving 200%+ |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "124.0"
#property strict
...
input double Risk_Pct           = 5.0;            //Risk % per trade (was 5.5 in v113)
input double Fixed_Lot          = 0.0;            //Fixed lot (0 = use Risk_Pct)
input int    Max_Buys           = 1;              //Max concurrent BUY positions (no P2)
input int    Max_Sells          = 1;              //Max concurrent SELL positions (no P2)
...
input int    Magic_Number       = 1242025;        //Magic Number
input string Order_Comment      = "BSv124";       //Order comment
...
void OnTick()
{
   ...
   //--- BUY logic: H4 bull + H1 bull + single-position gate
   ...
   if(h4Bull && h1Bull)
   {
      ...
      if(bbTouch && rsiOS && rsiRising && stochOS && bullBody)
      {
         ...
         if(tpDist >= minTP)
         {
            ...
            // Single-position gate: open only when no buy is open
   ...
   //--- SELL logic: H4 bear + H1 bear + single-position gate
   ...
   if(h4Bear && h1Bear)
   {
      ...
      if(bbTouchUp && rsiOB && rsiFalling && stochOB && bearBody)
      {
         ...
         if(tpDist >= minTP)
         {
            ...
            // Single-position gate: open only when no sell is open
