//+------------------------------------------------------------------+
//|   Buy & Sell EA v118 - More Frequent Signals                     |
//|   Framework: 4H Direction / 1H Structure / 5M Entry             |
//|   Base: v114 (best performer: 217.78%, 14.93% DD, 327T, 93%WR)  |
//|   Changes from v114:                                             |
//|     - BB_Dev: 2.0 (was 2.5) → more frequent BB touches          |
//|     - RSI_Buy: 45 (was 40) → less extreme oversold threshold    |
//|     - RSI_Sell: 55 (was 60) → less extreme overbought threshold |
//|     - Stoch_OS: 30 (was 25) → slightly less restrictive         |
//|     - Stoch_OB: 70 (was 75) → slightly less restrictive         |
//|     - Risk_Pct: 4.5% (was 5.0%) → compensate for lower quality  |
//|   Hypothesis: 2× trade count, ~90% WR, similar or higher return |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "128.0"
#property strict
...
input double BB_Dev             = 2.0;            //BB SD (was 2.5 - more frequent touches)
...
input double RSI_Buy            = 45.0;           //M5 RSI max for BUY (was 40 - less restrictive)
input double RSI_Sell           = 55.0;           //M5 RSI min for SELL (was 60 - less restrictive)
input int    Stoch_K            = 5;              //Stochastic K period
input int    Stoch_D            = 3;              //Stochastic D period
input int    Stoch_Slowing      = 3;              //Stochastic slowing
input double Stoch_OS           = 30.0;           //Stochastic oversold level (was 25)
input double Stoch_OB           = 70.0;           //Stochastic overbought level (was 75)
...
input double Risk_Pct           = 4.5;            //Risk % per trade (was 5.0 - reduced for quality)
...
input int    Magic_Number       = 1282025;        //Magic Number
input string Order_Comment      = "BSv128";       //Order comment
