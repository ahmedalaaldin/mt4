//+------------------------------------------------------------------+
//|   Buy & Sell EA v271 - Combine v269 + v270 (widened RSI + dual BB)|
//|   Framework: H1 Direction / M5 Entry / Wide H1 RSI / Dual Band   |
//|   HYPOTHESIS: Combine both improvements for maximum trades+quality |
//|     From v269: H1 RSI range widened to 40-80/20-60               |
//|     From v270: Secondary BB at 2.0σ + RSI < 38 stricter filter   |
//|     Expected: More trades than v269 with similar quality          |
//|   BASE: v269 CHAMPION (Calmar=17.77, Return=209%, 707 trades)    |
//|     H4 direction removed, H1 EMA direction KEPT                  |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "271.0"
#property strict
...
input int    H4_EMA_Period      = 200;            // Period NON-NEGOTIABLE (not used as gate)
input int    H1_EMA_Period      = 200;

//--- H1 RSI Gate - WIDENED (v269 confirmed improvement: 40-80/20-60)
input string H1RSI_SET          = "============"; //====== H1 RSI Gate ======
input int    H1_RSI_Period      = 14;
input double H1_RSI_Min         = 40.0;           // v269: widened from 45 to 40
input double H1_RSI_Max         = 80.0;           // v269: widened from 75 to 80
input double H1_RSI_Min_Sell    = 20.0;           // v269: widened from 25 to 20
input double H1_RSI_Max_Sell    = 60.0;           // v269: widened from 55 to 60

//--- BB Entry Signal - Primary (BB_Dev=2.5 NON-NEGOTIABLE)
input string S1_SET             = "============"; //====== BB Primary Signal ======
input int    BB_Period          = 20;
input double BB_Dev             = 2.5;            // NON-NEGOTIABLE primary band
input int    RSI_Period         = 14;
input double RSI_Buy            = 40.0;           // v250 confirmed optimal (primary)
input double RSI_Sell           = 60.0;           // v250 confirmed optimal (primary)
input double Body_Pct           = 0.2;

//--- BB Secondary Entry Signal (additive 2.0σ band - from v270)
input string S2_SET             = "============"; //====== BB Secondary Signal ======
input double BB_Dev2            = 2.0;            // Secondary band - softer entry
input double RSI_Buy2           = 38.0;           // Stricter than primary (38 vs 40)
input double RSI_Sell2          = 62.0;           // Stricter than primary (62 vs 60)
...
input double Trail_ATR_Multi    = 0.1;            // v261 CONFIRMED OPTIMAL
input double Partial_Pct        = 0.20;           // v258 CONFIRMED OPTIMAL
...
//--- Anti-Martingale (flat wins, only loss downscaling)
...
input int    Magic_Number       = 2712057;
input string Order_Comment      = "BSv271";
...
void OnTick()
{
   ...
   // v271: H4 EMA direction removed, H1 EMA direction KEPT, H1 RSI range WIDENED (v269)
   ...
   // Primary BB (2.5σ - NON-NEGOTIABLE)
   ...
   // Secondary BB (2.0σ - additive, v270)
   double bbUpper1b = iBands(NULL, 0, BB_Period, BB_Dev2, 0, PRICE_CLOSE, MODE_UPPER, 1);
   double bbLower1b = iBands(NULL, 0, BB_Period, BB_Dev2, 0, PRICE_CLOSE, MODE_LOWER, 1);
   ...
   //=== BUY ENTRY: Wide H1 RSI (v269) + Dual BB (v270) ===
   if(h1Bull && stochBuy && CountTradesByMagic(OP_BUY, Magic_Number) < Max_Buys)
   {
      // Primary: 2.5σ BB touch + RSI < 40
      bool primaryBuy = (Low[1] <= bbLower1) && (Close[1] > bbLower1)
                     && (rsi1 < RSI_Buy) && (rsi1 > rsi2) && bullBody;

      // Secondary: 2.0σ touch (not reaching 2.5σ) + RSI < 38 (stricter)
      bool secondaryBuy = (Low[1] <= bbLower1b) && (Close[1] > bbLower1b)
                       && (Low[1] > bbLower1)
                       && (rsi1 < RSI_Buy2) && (rsi1 > rsi2) && bullBody;

      if(primaryBuy || secondaryBuy)
      {
   ...
   //=== SELL ENTRY: Wide H1 RSI (v269) + Dual BB (v270) ===
   if(h1Bear && stochSell && CountTradesByMagic(OP_SELL, Magic_Number) < Max_Sells)
   {
      // Primary: 2.5σ BB touch + RSI > 60
      bool primarySell = (High[1] >= bbUpper1) && (Close[1] < bbUpper1)
                      && (rsi1 > RSI_Sell) && (rsi1 < rsi2) && bearBody;

      // Secondary: 2.0σ touch (not reaching 2.5σ) + RSI > 62 (stricter)
      bool secondarySell = (High[1] >= bbUpper1b) && (Close[1] < bbUpper1b)
                        && (High[1] < bbUpper1)
                        && (rsi1 > RSI_Sell2) && (rsi1 < rsi2) && bearBody;

      if(primarySell || secondarySell)
      {
