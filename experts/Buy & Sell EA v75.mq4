//+------------------------------------------------------------------+
//|   Buy & Sell EA v86 - Dual BB level quality entry               |
//|   Entry: H1 EMA200 + H1 RSI 50-75                              |
//|          + EITHER BB 2.5SD touch (RSI<40, Stoch<25)            |
//|          + OR    BB 2.0SD touch (RSI<35, Stoch<20) - tighter   |
//|          + RSI rising + Bull body 20%                           |
//|   Exit: TP = BB mid, SL = 1xATR                                 |
//|   Goal: Get 100-150 trades/yr keeping 52%+ WR (vs 50 at 56%)  |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "86.0"
#property strict
...
input int    BB_Period          = 20;             //BB period (both levels)
input double BB_Dev_High        = 2.5;            //Primary BB SD (quality entry)
input double BB_Dev_Low         = 2.0;            //Secondary BB SD (needs tighter RSI/Stoch)
...
input double RSI_Buy_High       = 40.0;           //RSI max for primary (BB 2.5SD) entry
input double RSI_Buy_Low        = 35.0;           //RSI max for secondary (BB 2.0SD) entry
input int    Stoch_K            = 5;              //Stochastic K period
input int    Stoch_D            = 3;              //Stochastic D period
input int    Stoch_Slowing      = 3;              //Stochastic slowing
input double Stoch_OS_High      = 25.0;           //Stoch oversold for primary (BB 2.5SD)
input double Stoch_OS_Low       = 20.0;           //Stoch oversold for secondary (BB 2.0SD)
...
input int    Magic_Number       = 862025;         //Magic Number
input string Order_Comment      = "BSv86";        //Order comment
...
void OnTick()
{
   ...
   // Primary BB: 2.5SD lower band (extreme oversold zone)
   double bbMid0    = iBands(NULL, 0, BB_Period, BB_Dev_High, 0, PRICE_CLOSE, MODE_MAIN,  0);
   double bbLower25 = iBands(NULL, 0, BB_Period, BB_Dev_High, 0, PRICE_CLOSE, MODE_LOWER, 1);
   // Secondary BB: 2.0SD lower band (less extreme)
   double bbLower20 = iBands(NULL, 0, BB_Period, BB_Dev_Low,  0, PRICE_CLOSE, MODE_LOWER, 1);
   ...
   // Primary: BB 2.5SD touch + RSI<40 + Stoch<25 (proven 56% WR setup)
   bool primarySignal = (Low[1] <= bbLower25) && (Close[1] > bbLower25) &&
                        (rsi1 < RSI_Buy_High) && (stochK1 < Stoch_OS_High) && (stochD1 < Stoch_OS_High);

   // Secondary: BB 2.0SD touch (but NOT 2.5SD) + RSI<35 + Stoch<20 (tighter to compensate looser BB)
   bool secondarySignal = (Low[1] <= bbLower20) && (Low[1] > bbLower25) && (Close[1] > bbLower20) &&
                          (rsi1 < RSI_Buy_Low) && (stochK1 < Stoch_OS_Low) && (stochD1 < Stoch_OS_Low);

   if((primarySignal || secondarySignal) && rsiRising && strongBull)
   {
