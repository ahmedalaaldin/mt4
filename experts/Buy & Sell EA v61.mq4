//+------------------------------------------------------------------+
//|   Buy & Sell EA v72 - M1 high-frequency scalping                 |
//|   Entry: H1 EMA200 + H1 RSI 50-75 + BB 2.5SD lower touch (M1)   |
//|          + M1 RSI<40 + RSI rising + Stoch K&D<25 + Bull 10%     |
//|   Exit: TP = BB mid, SL = 1xATR                                  |
//|   Changes: M1 chart, Body_Pct=10%, Min_Bar_Gap=1, Max_Buys=5    |
//|   Goal: 20+ trades/day by catching M1 BB touches                 |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
...
input double TP_Min_ATR         = 0.3;            //Min TP distance as ATR multiple (lower for M1)

//--- Entry Signal
input string ENTRY_SET          = "============"; //====== Entry Settings ======
input int    RSI_Period         = 14;             //M1 RSI period
input double RSI_Buy            = 40.0;           //M1 RSI must be below this
...
input double Body_Pct           = 0.1;            //Min bull body as fraction of range (10% for M1)
input int    Min_Bar_Gap        = 1;              //Min bars between entries (1 for M1 frequency)
...
input int    Max_Bars           = 200;            //Force close after N bars (200 M1 bars = ~3hrs)
...
input int    Magic_Number       = 722025;         //Magic Number
input string Order_Comment      = "BSv72";        //Order comment
input double Max_Daily_DD_Pct   = 15.0;           //Daily drawdown guard (% of equity)
input int    Max_Trades_Day     = 999;            //Max trades per day (high for M1 frequency)
