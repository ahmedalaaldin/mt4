//+------------------------------------------------------------------+
//|   Buy & Sell EA v170 - W-L drain fix + Risk_Pct boost            |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry          |
//|   ROOT CAUSE ANALYSIS (v169 → v170):                             |
//|     v169: 131% return, 14.94% DD — both targets missed           |
//|     Root cause: Scale_High=1.90 after any win → W-L drain       |
//|       Each loss at 1.90x, each recovery win at 0.5x              |
//|       W-L cycle net: +0.75%*2.5 - 1.90*1.5% = -0.975%/cycle   |
//|       Extended W-L phases → ~10% DD even with 81% WR            |
//|   MATHEMATICAL FIX:                                               |
//|     Scale_High = 0.5 × RR_ratio = 0.5 × 2.5 = 1.25              |
//|     → W-L cycle net: 0.5*1.25*2.5% - 1.25*2% = +0% (neutral)  |
//|     → Consecutive wins still generate big profits                  |
//|     → 8-loss worst case DD: 2.5+1+0.5+1.0 = 5.0% (target<5%)  |
//|   Changes vs v169:                                                |
//|     1. Scale_High: 1.90 → 1.25 (W-L drain elimination)          |
//|     2. Scale_Mid: 1.25 → 1.10 (proportional reduction)          |
//|     3. Risk_Pct: 1.5 → 2.0 (compensate for lower scale)         |
//|     4. Magic_Number: 1692025 → 1702025                           |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "170.0"
#property strict
...
input double Risk_Pct           = 2.0;            //Risk%: 1.5→2.0 (compensate for Scale_High reduction)
...
input int    WinStreak_High     = 1;              //Consec wins for Scale_High: immediate onset
input double Scale_Mid          = 1.10;           //Scale_Mid: 1.25→1.10 (proportional reduction)
input double Scale_High         = 1.25;           //Scale_High: 1.90→1.25 (WL-drain fix: =0.5*RR)
...
input int    Magic_Number       = 1702025;        //Magic Number
input string Order_Comment      = "BSv170";       //Order comment
