//+------------------------------------------------------------------+
//|   Buy & Sell EA v290 - Aggressive Anti-Martingale                |
//|   BASE: v280 CHAMPION (Calmar=18.36, Return=220%, 637 trades)    |
//|   CHANGE: More aggressive loss downscaling after consecutive      |
//|   losses. Current (hardcoded): 0.5x→0.25x→0.1x                  |
//|   New: 0.25x→0.1x→0.05x                                         |
//|   RATIONALE: MaxDD is dominated by first 2 losses in any streak. |
//|     Loss1: always 7% of equity (unchanged)                       |
//|     Loss2 current:  3.5%×0.93 = 3.255% → Total 2-streak=10.26%  |
//|     Loss2 new:      1.75%×0.93 = 1.628% → Total 2-streak=8.63%  |
//|     This change does NOT affect trade count or win rate.         |
//|     After any win, risk resets to 7% (same as before).          |
//|   HYPOTHESIS: MaxDD drops from 12% to ~9% (-25%). Calmar jumps  |
//|     from 18.36 to ~24.4. Return stays ~200%+.                    |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "290.0"
#property strict
...
//--- Anti-Martingale - AGGRESSIVE VERSION (NEW in v290)
//    v280 hardcoded: 0.5x / 0.25x / 0.1x after 1/2/3+ losses
//    v290 parameterized with more aggressive defaults:
//      0.25x after 1 loss → loss2 only costs 1.75% risk (vs 3.5%)
//      0.1x  after 2 losses → loss3+ tiny
//      0.05x after 3+ losses → minimal damage from extended losing streaks
//    Win streak scaling: unchanged (Scale_Mid=Scale_High=1.0)
...
input double AM_Scale_Loss1     = 0.25;  // Scale after 1 consecutive loss (was 0.5)
input double AM_Scale_Loss2     = 0.10;  // Scale after 2 consecutive losses (was 0.25)
input double AM_Scale_Loss3     = 0.05;  // Scale after 3+ consecutive losses (was 0.1)

//--- Safety
input string SAFETY_SET        = "============"; //====== Safety Settings ======
input int    MaxSpread          = 35;
input int    Slippage           = 30;
input int    Magic_Number       = 2902057;
input string Order_Comment      = "BSv290";
...
double GetRiskScale()
{
   ...
   if(streak == 0)              return 1.0;
   // AGGRESSIVE anti-martingale loss scaling (v290)
   int losses = -streak;
   if(losses == 1) return AM_Scale_Loss1;  // 0.25 (was 0.5)
   if(losses == 2) return AM_Scale_Loss2;  // 0.10 (was 0.25)
   return AM_Scale_Loss3;                  // 0.05 (was 0.10)
}
