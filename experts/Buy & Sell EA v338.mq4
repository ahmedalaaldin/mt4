//+------------------------------------------------------------------+
//|   Buy & Sell EA v338 - Dual RSI Filter                           |
//|   BASE: v336 (RSI_Period=7 breakthrough: +11.62%, WR=82.38%)     |
//|                                                                   |
//|   v337 POST-MORTEM (435 trades, MaxDD=4.98%):                    |
//|     FAILURE: RSI_Buy=42 WORSE than RSI_Buy=45                    |
//|     WR=81.15% (dropped 1.23% from v336's 82.38%)                 |
//|     Trades=435 (70 FEWER than v336's 505)                        |
//|     Key insight: RSI(7) 42-45 range has HIGHER WR than RSI<42    |
//|     Extreme RSI(7) oversold = strong downtrend = lower WR        |
//|                                                                   |
//|   v338 HYPOTHESIS:                                               |
//|   RSI_Period=7 gave +33% more trades vs RSI_Period=14, BUT the   |
//|   extra 125 trades are net-LOSING (avg -3.72/trade).             |
//|   These bad signals occur when RSI(7) briefly dips below 45      |
//|   while RSI(14) is still ABOVE 55 (market not genuinely OS).     |
//|   Adding RSI(14)<55 gate should filter these false RSI(7) dips   |
//|   while keeping genuine dual-RSI oversold entries.               |
//|                                                                   |
//|   v338 CHANGES from v336:                                        |
//|     RSI_Buy=45 restored (from v337's 42 back to v336's 45)       |
//|     ADD: RSI_Slow_Max=55 (RSI(14) must be < 55 to confirm)       |
//|   Expected: ~380-420 trades, WR hopefully back above 83%         |
//|                                                                   |
//|   SCOREBOARD:                                                     |
//|     v328: +16.25%, MaxDD=4.99%, WR=83.47%, 380 trades  BEST ✓  |
//|     v335: RSI_Buy=55 → WR=71.12%                    FAIL ✗    |
//|     v336: RSI_Period=7 → +11.62%, WR=82.38%, 505t   NEAR ✓    |
//|     v337: RSI_Period=7 + RSI_Buy=42 → WR=81.15%, 435t FAIL ✗  |
//|     v338: RSI_Period=7 + RSI(14)<55 gate → ???                   |
//|                                                                   |
...
input double RSI_Buy            = 45.0;           // RESTORED to v336 level (v337 proved 42 is worse)
input double RSI_Slow_Max       = 55.0;           // v338 NEW: RSI(14) must be < 55 (genuine oversold gate)
...
input int    Magic_Number       = 3382057;
input string Order_Comment      = "BSv338";
...
void OnTick()
{
   ...
   double rsi1     = iRSI(NULL, 0, RSI_Period, PRICE_CLOSE, 1);  // RSI(7) fast signal
   double rsi2     = iRSI(NULL, 0, RSI_Period, PRICE_CLOSE, 2);  // RSI(7)
   double rsiSlow1 = iRSI(NULL, 0, 14, PRICE_CLOSE, 1);          // RSI(14) confirmation gate
   ...
   if(h1Bull)
   {
      ...
      bool rsiOS       = (rsi1 < RSI_Buy);                // RSI(7) < 45 (fast signal)
      bool rsiSlowOS   = (rsiSlow1 < RSI_Slow_Max);       // RSI(14) < 55 (slow confirmation)
      ...
      if(bbTouch && rsiOS && rsiSlowOS && rsiRising && bullBody && bbPrevBreak)
      {
