//+------------------------------------------------------------------+
//|   Buy & Sell EA v57 - Bidirectional + 20% Body + 5% Risk       |
//|   BUY:  H1 EMA200 up + H1 RSI 50-75 + BB 2.5SD lower touch    |
//|          + M5 RSI<40 + RSI rising + Stoch K&D<25 + Bull 20%    |
//|   SELL: H1 EMA200 dn + H1 RSI 25-50 + BB 2.5SD upper touch    |
//|          + M5 RSI>60 + RSI falling + Stoch K&D>75 + Bear 20%   |
//|   Exit: TP = BB mid, SL = 1xATR                                |
//|   Goal: Double trade count via bidirectional + looser body      |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
...
input double H1_RSI_BuyMin      = 50.0;           //H1 RSI min for BUY
input double H1_RSI_BuyMax      = 75.0;           //H1 RSI max for BUY
input double H1_RSI_SellMin     = 25.0;           //H1 RSI min for SELL
input double H1_RSI_SellMax     = 50.0;           //H1 RSI max for SELL
...
input double RSI_Buy            = 40.0;           //M5 RSI must be below this (buy)
input double RSI_Sell           = 60.0;           //M5 RSI must be above this (sell)
input int    Stoch_K            = 5;              //Stochastic K period
input int    Stoch_D            = 3;              //Stochastic D period
input int    Stoch_Slowing      = 3;              //Stochastic slowing
input double Stoch_OB           = 25.0;           //Stochastic oversold (buy)
input double Stoch_OS           = 75.0;           //Stochastic overbought (sell)
...
input int    Magic_Number       = 572025;         //Magic Number
input string Order_Comment      = "BSv57";        //Order comment
...
void OnTick()
{
   ...
   double bullBody  = Close[1] - Open[1];
   double bearBody  = Open[1] - Close[1];

   //--- BUY signal
   bool h1BullTrend = (Close[1] > h1_ema) && (h1_rsi >= H1_RSI_BuyMin) && (h1_rsi <= H1_RSI_BuyMax);
   bool bbTouchLow  = (Low[1] <= bbLower1) && (Close[1] > bbLower1);
   bool rsiOSBuy    = (rsi1 < RSI_Buy);
   bool rsiRising   = (rsi1 > rsi2);
   bool stochOSBuy  = (stochK1 < Stoch_OB) && (stochD1 < Stoch_OB);
   bool strongBull  = (bullBody > 0) && (bullBody >= Body_Pct * candleRange);

   if(h1BullTrend && bbTouchLow && rsiOSBuy && rsiRising && stochOSBuy && strongBull)
   {
   ...
   //--- SELL signal
   bool h1BearTrend = (Close[1] < h1_ema) && (h1_rsi >= H1_RSI_SellMin) && (h1_rsi <= H1_RSI_SellMax);
   bool bbTouchHigh = (High[1] >= bbUpper1) && (Close[1] < bbUpper1);
   bool rsiOBSell   = (rsi1 > RSI_Sell);
   bool rsiFalling  = (rsi1 < rsi2);
   bool stochOBSell = (stochK1 > Stoch_OS) && (stochD1 > Stoch_OS);
   bool strongBear  = (bearBody > 0) && (bearBody >= Body_Pct * candleRange);

   if(h1BearTrend && bbTouchHigh && rsiOBSell && rsiFalling && stochOBSell && strongBear)
   {
