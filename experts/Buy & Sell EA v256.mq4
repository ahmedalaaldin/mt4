//+------------------------------------------------------------------+
//|   Buy & Sell EA v256 - Smaller Partial (50%→30%, 70% Continues) |
...
//|     v255: RSI_Buy=35 → Calmar=0.84 (catastrophic: only 66 trades)|
//|       RSI_Buy=40 was perfectly calibrated — too tight kills it   |
//|                                                                  |
//|   v256 HYPOTHESIS: Reduce partial close from 50% to 30%         |
//|     More lots (70%) ride the continuation vs current 50%         |
//|     SL still moves to entry on partial (breakeven protection     |
//|       preserved regardless of partial size)                      |
//|     If BB_mid rises after partial: 70% continuation captures     |
//|       more upside than 50% → higher avg win per trade            |
//|     If BB_mid stays flat: both 70% and 50% exit near entry       |
//|       (trail at bbMid-0.5×ATR is above entry either way)        |
//|     Trade count UNCHANGED (same entries)                         |
//|     Win rate PRESERVED (breakeven lock still fires on partial)   |
//|     All exit parameters: BB_mid trail at 0.5×ATR (v253 best)    |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "256.0"
#property strict
...
input double Partial_Pct        = 0.30;           // v256: 50% → 30% (70% rides continuation)
...
input int    Magic_Number       = 2562057;
input string Order_Comment      = "BSv256";
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(type == OP_BUY)
      {
         ...
         if(!partialDone && Bid >= bbMid)
         {
            double stepLot  = MarketInfo(Symbol(), MODE_LOTSTEP);
            double minLot   = MarketInfo(Symbol(), MODE_MINLOT);
            // v256: Partial_Pct=0.30 → close only 30%, let 70% continue
         ...
         if(partialDone)
         {
            // BB_mid trail at 0.5×ATR (v253 confirmed marginally best)
            // 70% of position rides here — more upside captured vs 50%
      ...
      else if(type == OP_SELL)
      {
         ...
         if(partialDone)
         {
            // v256: 70% continuation rides trail
            double trailSL = NormalizeDouble(bbMid + Trail_ATR_Multi * atr, Digits);
            if(trailSL < currentSL)
               OrderModify(tkt, openPrice, trailSL, 0, 0, clrBlue);
         }
