//+------------------------------------------------------------------+
//|   Buy & Sell EA v219 - Tighter Daily DD Circuit Breaker           |
...
//|       LESSON: Price bounces immediately — limits miss winning fills.|
//|       Market orders = OPTIMAL. Limits hurt both return AND DD.    |
//|     v217: Early BE (0.5 ATR) -> BUGGY - only 53 trades, 37.7% WR |
//|       BUG: Early BE set partialDone=TRUE before partial close,    |
//|       causing trail to fire with bbMid >> current price.          |
//|       LESSON: Early BE breaks the partialDone state machine.      |
//|     v218: 75% partial -> +154.55%, DD 22.51%, Cal=6.87, 160 tr   |
//|       LESSON: 75% partial at midline WORSE than 50%. Smaller      |
//|       trailing portion (25%) fails to generate enough profit.     |
//|       50% partial (v186) confirmed optimal for partial exit %.    |
//|   v219 HYPOTHESIS: Tighter Daily DD Circuit Breaker               |
//|     Max_Daily_DD_Pct = 4.0% (was 8.0% in v186)                   |
//|     Partial_Pct back to 0.50 (v186 confirmed optimum)             |
//|     Rationale: MaxDD 22-25% has persisted across ALL v186+        |
//|     versions. Cutting each losing day earlier at 4% may prevent   |
//|     multi-day compounding of drawdowns without hurting return.    |
//|     At peak equity $25k: 4% = $1000/day max loss (vs $2000 at 8%)|
//|     If Calmar > 12.60: daily DD tightening beats v186.            |
//|     If Calmar < 12.60: circuit breaker cuts too many recovery     |
//|     trades; 8% was the right balance.                             |
//|     ALL other parameters = v186 GLOBAL OPTIMA.                    |
//|     Run with PERIOD = M5 in Strategy Tester.                       |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "219.0"
#property strict
...
//--- Exit Settings (v186 GLOBAL OPTIMA — Partial_Pct restored to 50%)
...
input double Partial_Pct        = 0.50;           //v219: 50% partial (v186 confirmed optimum; 75% was WORSE)
...
input int    Magic_Number       = 2192038;        //Magic Number
input string Order_Comment      = "BSv219";       //Order comment
input double Max_Daily_DD_Pct   = 4.0;            //v219: Tighter daily DD guard: 4% (was 8% in v186)
