//+------------------------------------------------------------------+
//|   Buy & Sell EA v197 - Wider Trailing Stop Experiment            |
//|   Framework: H4+H1 Direction / M5 Entry                          |
//|   LESSONS LEARNED:                                                |
//|     v186: 7.0%, Streak=5/10   -> 237.84% ret, 18.87% DD, Cal=12.60|
//|            BEST Calmar so far: ALL parameters optimal              |
//|     v192: Guard caps win-scaling -> 228.49%, 18.88%, Cal=12.09    |
//|            Guard hurts returns, same DD -> guards useless           |
//|     v193: BB 2.0 SD relaxed -> 79.74%, 50.07%, Cal=1.59 FAIL     |
//|            BB 2.5 + Stoch 25/75 quality gate is NON-NEGOTIABLE    |
//|     v194: Faster streak Mid=3/High=6 -> 181.43%, Cal=10.67        |
//|            v186 streak thresholds (5/10) already optimal           |
//|     v195: H4 filter removed -> 281.38%, 27.36% DD, Cal=10.28      |
//|            H4 IS important quality control -> RESTORE              |
//|     v196: SL=0.7 ATR, TP=3.0 ATR -> 121.52%, 23.42% DD, Cal=5.19 |
//|            WR dropped 90.7%->85.7%! SL=0.8 and TP=2.5 BOTH optimal|
//|   v197 HYPOTHESIS: Wider trailing stop for remaining 50%           |
//|     Trail_ATR_Multi: 0.8 -> 1.2 (50% more room after partial close)|
//|     Rationale: After 50% partial at BB midline + SL to breakeven,  |
//|       current 0.8 ATR trail may exit too early on genuine moves.   |
//|       At 1.2 ATR trail, worst case on remaining 50% = breakeven.   |
//|       Best case: captures larger trend extensions for bigger wins.  |
//|     All other params: IDENTICAL to v186 (proven optimal)           |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "197.0"
#property strict
...
input double Trail_ATR_Multi    = 1.2;            //Trail SL offset: 1.2 (v197: wider trail)
input int    Max_Bars           = 100;            //Force-close after N bars

//--- Money Management
input string MONEY_SET          = "============"; //====== Money Management ======
input double Risk_Pct           = 7.0;            //Risk%: 7.0 (same as v186)
...
//--- Anti-Martingale (same as v186: proven optimal 5/10 thresholds)
...
input int    Magic_Number       = 1972027;        //Magic Number
input string Order_Comment      = "BSv197";       //Order comment
...
void ManageExits()
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(type == OP_BUY)
      {
         ...
         if(partialDone)
         {
            // Trail at bbMid - 1.2*ATR (wider than v186's 0.8 to ride bigger moves)
...
void OnTick()
{
   ...
   double slDist    = ATR_SL_Multi * atr1;   // 0.8 ATR (v186 optimal)
   double tpDist    = ATR_TP_Multi * atr1;   // 2.5 ATR (v186 optimal)
