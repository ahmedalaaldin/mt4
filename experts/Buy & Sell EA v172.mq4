//+------------------------------------------------------------------+
//|   Buy & Sell EA v172 - M1 Timeframe Test                         |
//|   Framework: H4 Direction / H1 RSI Structure / M1 Entry         |
//|   LESSON FROM v171:                                              |
//|     v171: Removed H4/H1 EMA filter => WR 81%->73%, DD 14%->18% |
//|     Conclusion: H4/H1 EMA filter is CRITICAL for signal quality  |
//|     Must keep the filter intact.                                 |
//|   APPROACH v172:                                                  |
//|     Keep all v170 logic (H4/H1 EMA + H1 RSI gate) UNCHANGED    |
//|     Test on M1 period in Strategy Tester (not M5)               |
//|     M1 = 5x more bars -> expect ~1800 trades/yr (~7/day)        |
//|     Risk_Pct reduced 2.0->0.7 to control DD with higher freq   |
//|     Theory: 100% * (0.7/2.0) * 5 = ~175% return, DD ~5%        |
//|   Changes vs v170:                                               |
//|     1. Risk_Pct: 2.0 -> 0.7 (tuned for M1 trade frequency)     |
//|     2. Magic_Number: 1702025 -> 1722025                         |
//|     3. Order_Comment: BSv170 -> BSv172                          |
//|     4. Test in Strategy Tester on M1 Period (not M5)            |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "172.0"
#property strict
...
input int    RSI_Period         = 14;             //M1 RSI period
input double RSI_Buy            = 40.0;           //M1 RSI max for BUY (proven)
input double RSI_Sell           = 60.0;           //M1 RSI min for SELL (proven)
...
input double Risk_Pct           = 0.7;            //Risk%: 2.0->0.7 (M1 has ~5x trades)
...
input int    Magic_Number       = 1722025;        //Magic Number
input string Order_Comment      = "BSv172";       //Order comment
