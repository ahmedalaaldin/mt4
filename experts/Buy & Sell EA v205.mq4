//+------------------------------------------------------------------+
//|   Buy & Sell EA v205 - M1 Timeframe Test                         |
//|   Framework: H4+H1 Direction / M1 Entry                          |
//|   LESSONS LEARNED:                                                |
...
//|     v198: M15 timeframe -> catastrophic loss                       |
...
//|       NY session confirmed PROFITABLE, full 09:00-21:00 is optimal|
//|     v202: ATR_Spike_Multi=1.5 -> worse (WR 88.60%, DD 31.18%)    |
//|       LESSON: High-volatility XAUUSD = BEST BB mean reversion     |
//|       conditions. Tighter spike filter removes GOOD trades.        |
//|       ATR_Spike_Multi=1.8 confirmed GLOBAL OPTIMUM.               |
//|     v203: Scale_Mid=1.0, Scale_High=1.0 (flat wins) ->           |
//|       +147.99%, DD 21.48%, Calmar 6.89                            |
//|       LESSON: Win-streak scaling IMPROVES Calmar. v186 optimal.   |
//|     v204: Max_Daily_DD_Pct=5.0 (from 8.0) ->                     |
//|       +66.68%, DD 19.14%, Calmar 3.48                             |
//|       LESSON: Daily DD guard at 5% blocks high-volatility          |
//|       recovery sessions — same lesson as ATR_Spike_Multi=1.5.     |
//|       8% daily guard in v186 is GLOBAL OPTIMUM.                   |
//|   v205 HYPOTHESIS: M1 timeframe to increase trade count           |
//|     Single-symbol M5 gives ~250 trades/year = 0.7/day (target 20+)|
//|     M15 (v198) was catastrophic — signals too noisy on slow TF    |
//|     M1 is the OPPOSITE direction: faster = 5x more opportunities  |
//|     BB(20, 2.5SD) + RSI + Stoch on M1 may still find valid        |
//|     mean-reversion entries in XAUUSD's intraday swings            |
//|     Adjustments for M1:                                            |
//|       Max_Bars: 100->500 (same time: 100×M5=500×M1=500min)        |
//|       Min_Bar_Gap: 1->3 (prevent entries every single minute)     |
//|     All other parameters identical to v186 (proven optimal).       |
//|     CRITICAL: Run with PERIOD = M1 in Strategy Tester.            |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "205.0"
#property strict
...
input int    RSI_Period         = 14;             //RSI period (M1 bars)
input double RSI_Buy            = 40.0;           //RSI max for BUY
input double RSI_Sell           = 60.0;           //RSI min for SELL
input double Body_Pct           = 0.2;            //Min body fraction
input int    Min_Bar_Gap        = 3;              //v205: M1 min bars between entries (3 min)
...
//--- Exit Settings (v186 optimal values)
input string EXIT_SET           = "============"; //====== Exit Settings ======
input int    ATR_Period         = 14;             //ATR period
input double ATR_SL_Multi       = 0.8;            //SL = ATR x 0.8 (v186 optimal)
input double ATR_TP_Multi       = 2.5;            //TP = ATR x 2.5 (v186 optimal)
input double Trail_ATR_Multi    = 0.8;            //Trail SL offset: 0.8 (v186 optimal)
input int    Max_Bars           = 500;            //v205: M1 force-close: 500 bars = ~500 min = 8h

//--- Money Management (v186 optimal)
input string MONEY_SET          = "============"; //====== Money Management ======
input double Risk_Pct           = 7.0;            //Risk%: 7.0 (v186 optimal)
...
//--- Anti-Martingale (v186 optimal: scale up on wins, protect on losses)
input string AM_SET             = "============"; //====== Anti-Martingale ======
input int    WinStreak_Mid      = 5;              //Wins before mid scaling
input int    WinStreak_High     = 10;             //Wins before high scaling
input double Scale_Mid          = 1.10;           //v186 optimal: 1.10x at 5+ wins
input double Scale_High         = 1.25;           //v186 optimal: 1.25x at 10+ wins
...
input int    Magic_Number       = 2052035;        //Magic Number
input string Order_Comment      = "BSv205";       //Order comment
...
double GetRiskScale()
{
   ...
   if(streak >= WinStreak_High) return Scale_High;  // 1.25x at 10+ wins (v186)
   if(streak >= WinStreak_Mid)  return Scale_Mid;   // 1.10x at 5+ wins (v186)
...
void OnTick()
{
   ...
   // H4 + H1 direction filter (both required — same as v186)
   ...
   //=== Signal BUY: H4 bullish + H1 bullish + BB(20,2.5) touch + RSI<40 + Stoch<25
   ...
   //=== Signal SELL: H4 bearish + H1 bearish + BB(20,2.5) touch + RSI>60 + Stoch>75
