//+------------------------------------------------------------------+
//|   Buy & Sell EA v222 - H1 ADX Ranging-Market Filter               |
...
//|     v221: Scale 1.05/1.10  -> Cal=7.06   WORSE (lower peak = less |
//|       equity buffer, % MaxDD HIGHER despite smaller absolute DD;   |
//|       confirmed Scale_Mid=1.10/Scale_High=1.25 are GLOBAL OPTIMA) |
//|   v222 HYPOTHESIS: H1 ADX Ranging-Market Filter                   |
//|     BB+RSI is a mean-reversion strategy. It works in RANGING       |
//|     markets and fails in TRENDING markets (price keeps moving away |
//|     from BB outer band, causing consecutive SL hits = MaxDD events)|
//|     Adding H1 ADX < 25 ensures entries only fire when the market  |
//|     is ranging/sideways — the natural habitat of mean-reversion.  |
//|     Expected effect:                                               |
//|     - Fewer total trades (skip trending-market false setups)       |
//|     - Higher WR (only range-bound entries)                         |
//|     - Reduced consecutive loss streaks (trend entries were losses) |
//|     - Lower MaxDD with maintained return → higher Calmar           |
//|   Scale params restored to v186 GLOBAL OPTIMA (1.10/1.25).        |
//|   ALL other parameters = v186 GLOBAL OPTIMA.                      |
//|   Run with PERIOD = M5 in Strategy Tester.                        |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "222.0"
#property strict
...
//--- ADX Ranging Filter (v222 NEW)
input string ADX_SET            = "============"; //====== ADX Ranging Filter (v222) ======
input int    ADX_Period         = 14;             //ADX period (standard)
input double ADX_Max            = 25.0;           //Skip entry if H1 ADX >= this (market trending)
...
input double Scale_Mid          = 1.10;           //1.10x after 5 wins (v186 GLOBAL OPTIMUM restored)
input double Scale_High         = 1.25;           //1.25x after 10 wins (v186 GLOBAL OPTIMUM restored)
...
input int    Magic_Number       = 2222038;        //Magic Number
input string Order_Comment      = "BSv222";       //Order comment
...
double GetRiskScale()
{
   ...
   // v186 GLOBAL OPTIMA restored: 1.10x at 5 wins, 1.25x at 10 wins
...
void OnTick()
{
   ...
   //--- v222: H1 ADX ranging-market filter
   //--- Only enter when H1 ADX < ADX_Max (market is ranging, not trending)
   //--- Mean-reversion entries fail in trending markets; ADX filter removes those setups
   double h1_adx = iADX(NULL, PERIOD_H1, ADX_Period, PRICE_CLOSE, MODE_MAIN, 1);
   if(h1_adx <= 0) return;                 // data not ready
   if(h1_adx >= ADX_Max) return;           // market trending — skip all entries
