//+------------------------------------------------------------------+
//|   Buy & Sell EA v126 - M15 Confluence Filter                     |
//|   Framework: 4H Direction / 1H Structure / 5M Entry             |
//|   Base: v120 (gold standard 15.0x Return/DD ratio)              |
//|   Changes vs v120:                                               |
//|     - M15 BB midline + M15 RSI added as intermediate filter     |
//|       BUY: M15 close < M15 BB mid AND M15 RSI < 50              |
//|       SELL: M15 close > M15 BB mid AND M15 RSI > 50             |
//|   Hypothesis: M5+M15 confluence pushes WR toward 95-97%,        |
//|   reduces max consec losses, lowers DD while preserving return   |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
...
//--- M15 Confluence Filter (NEW in v126)
input string M15_SET            = "============"; //====== M15 Confluence Filter ======
input int    M15_BB_Period      = 20;             //M15 BB period
input double M15_BB_Dev         = 2.0;            //M15 BB deviation
input int    M15_RSI_Period     = 14;             //M15 RSI period
input double M15_RSI_Buy        = 50.0;           //M15 RSI max for BUY (below midline)
input double M15_RSI_Sell       = 50.0;           //M15 RSI min for SELL (above midline)
...
input int    Magic_Number       = 1362025;        //Magic Number
...
void OnTick()
{
   ...
   //--- M15 confluence indicators (NEW in v126)
   double m15_bb_mid = iBands(NULL, PERIOD_M15, M15_BB_Period, M15_BB_Dev, 0, PRICE_CLOSE, MODE_MAIN, 1);
   double m15_rsi1   = iRSI(NULL, PERIOD_M15, M15_RSI_Period, PRICE_CLOSE, 1);
   double m15_close  = iClose(NULL, PERIOD_M15, 1);
   ...
   bool m15Bull  = (m15_close < m15_bb_mid) && (m15_rsi1 < M15_RSI_Buy);  // M15 still bearish/oversold

   if(h4Bull && h1Bull && m15Bull)
   {
   ...
   bool m15Bear  = (m15_close > m15_bb_mid) && (m15_rsi1 > M15_RSI_Sell);  // M15 still bullish/overbought

   if(h4Bear && h1Bear && m15Bear)
   {
