//+------------------------------------------------------------------+
//|   Buy & Sell EA v150 - ATR Filter Only (Peak Prot removed)       |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry          |
//|   Base: v149 (138.17% / 36.37% DD / 84.76% WR / 420 trades)    |
//|   Analysis of v149:                                              |
//|     - ATR filter at 0.45%: filtered too many bars (trades -8%)   |
//|       avg win dropped $153→$105 AND avg loss dropped $570→$367   |
//|       The loss reduction IS working (36% drop in avg loss)       |
//|     - Peak Protection was TOO aggressive: capped scale at 1.0x   |
//|       when >15% below peak. With natural 43.7% DD, strategy      |
//|       spends >50% of time in protection mode → kills anti-mart   |
//|       wins → return fell 207%→138%.                              |
//|   Changes vs v149:                                               |
//|     1. Remove Peak Protection (set DD caps to 100% = disabled)  |
//|     2. Relax ATR filter from 0.45% → 0.60% of price             |
//|        0.60% at $3300 = $19.80 ATR threshold                    |
//|        Filters only top ~5-10% most volatile bars (vs ~30-40%)  |
//|        Should preserve most valid entries while eliminating       |
//|        the extreme-volatility outliers driving big losses.       |
//|   Expected: 190-205% return, 37-42% DD                         |
//|   Hypothesis: ATR filter at 0.60% improves DD slightly with     |
//|   minimal return sacrifice vs v144's 43.70% DD baseline         |
//|   All other params unchanged from v144:                          |
//|     BB_Dev=2.5, RSI_Buy=40, Body_Pct=0.2, Max_Buys=2           |
//|     Risk_Pct=5%, Scale_High=1.85, WinStreak_High=5             |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
...
input double Max_ATR_Pct        = 0.60;           //Max ATR as % of price (0=disabled)
// Peak Protection removed - was killing anti-martingale returns in v149
...
input int    Magic_Number       = 1502025;        //Magic Number
...
void OnTick()
{
   ...
   // Skip entry when M5 ATR exceeds Max_ATR_Pct% of current price
   // 0.60% at $3300 gold = $19.80 threshold (filters top ~5-10% most volatile bars)
   // Avoids entries with extremely wide ATR-based SLs that generate outsized losses
   if(Max_ATR_Pct > 0)
   {
