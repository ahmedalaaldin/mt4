//+------------------------------------------------------------------+
//|   Buy & Sell EA v75 - Dual direction BB mean reversion          |
//|   BUY:  H1 price>EMA200 + H1 RSI 50-75 + BB 2.5SD lower touch  |
//|         + M5 RSI<40 + RSI rising + Stoch K&D<25 + Bull 20%     |
//|   SELL: H1 price<EMA200 + H1 RSI 25-50 + BB 2.5SD upper touch  |
//|         + M5 RSI>60 + RSI falling + Stoch K&D>75 + Bear 20%    |
//|   Exit: TP = BB mid (SMA20), SL = 1xATR                        |
//|   Goal: ~100 trades/year by trading both directions              |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "75.0"
#property strict
...
input double H1_RSI_Bull_Min    = 50.0;           //H1 RSI min for uptrend (BUY)
input double H1_RSI_Bull_Max    = 75.0;           //H1 RSI max for uptrend (BUY)
input double H1_RSI_Bear_Min    = 25.0;           //H1 RSI min for downtrend (SELL)
input double H1_RSI_Bear_Max    = 50.0;           //H1 RSI max for downtrend (SELL)
...
input double RSI_Buy            = 40.0;           //M5 RSI must be BELOW for BUY
input double RSI_Sell           = 60.0;           //M5 RSI must be ABOVE for SELL
input int    Stoch_K            = 5;              //Stochastic K period
input int    Stoch_D            = 3;              //Stochastic D period
input int    Stoch_Slowing      = 3;              //Stochastic slowing
input double Stoch_OS           = 25.0;           //Stochastic oversold (for BUY)
input double Stoch_OB           = 75.0;           //Stochastic overbought (for SELL)
input double Body_Pct           = 0.2;            //Min body as fraction of range
input int    Min_Bar_Gap        = 2;              //Min bars between same-dir entries
...
input int    Magic_Number       = 752025;         //Magic Number
input string Order_Comment      = "BSv75";        //Order comment
...
void OnTick()
{
   ...
   //--- BUY Logic (identical to v58/BSv69)
   bool h1Bull    = (Close[1] > h1_ema) && (h1_rsi >= H1_RSI_Bull_Min) && (h1_rsi <= H1_RSI_Bull_Max);
   bool bbTouchLo = (Low[1] <= bbLower1) && (Close[1] > bbLower1);
   ...
   if(h1Bull && bbTouchLo && rsiOS && rsiRising && stochOS && bullBody)
   {
   ...
   //--- SELL Logic (symmetric: BB upper touch in downtrend)
   bool h1Bear    = (Close[1] < h1_ema) && (h1_rsi >= H1_RSI_Bear_Min) && (h1_rsi <= H1_RSI_Bear_Max);
   bool bbTouchHi = (High[1] >= bbUpper1) && (Close[1] < bbUpper1);
   ...
   if(h1Bear && bbTouchHi && rsiOB && rsiFalling && stochOB && bearBody)
   {
