//+------------------------------------------------------------------+
//|   Buy & Sell EA v363 - H1 Stochastic Dead-Zone Exclusion Filter  |
//|   BASE: v358 (H4_EMA50 + M5+M15+M30 + ATR_Spike=2.0) BEST ✓    |
//|                                                                   |
...
//|     v358: +H4_EMA(50) → +31.23%, MaxDD=4.61%✅, WR=85.48% BEST|
//|     v359: Max_Buys=2 → +24.95%, MaxDD=5.05%❌  FAIL             |
//|     v360: H4 slope → identical to v358 (slope always true)       |
//|                                                                   |
//|   v361 HYPOTHESIS: Add H1 Stochastic %K oversold filter.         |
//|   Current WR=85.48%, avg_loss=-$50.94, 117 losses/806 trades.    |
//|   Losing trades occur when BB_lower is touched but price          |
//|   continues lower (bearish continuation). A Stochastic %K below  |
//|   threshold on H1 confirms oversold momentum at the higher TF,   |
//|   increasing probability that the reversal is genuine.            |
//|                                                                   |
//|   v361 CHANGES from v358:                                         |
//|     Added: H1 Stochastic %K < H1_Stoch_OS (default 35)           |
//|     This filters entries where H1 momentum is not yet oversold    |
//|     even though price has reached the BB_lower band.              |
//|     Expected: Higher WR, fewer trades, lower avg_loss             |
//|                                                                   |
...
input int    H4_EMA_Fast_Period = 50;             // v358: medium-term H4 trend gate
...
//--- H1 Stochastic Gate (v363: dead-zone exclusion)
input string H1STOCH_SET        = "============"; //====== H1 Stochastic Gate ======
input int    H1_Stoch_K         = 5;              // Stochastic %K period
input int    H1_Stoch_D         = 3;              // Stochastic %D smoothing
input int    H1_Stoch_Slow      = 3;              // Stochastic slowing
input double H1_Stoch_Dead_Low  = 35.0;           // Dead zone lower — skip when %K >= this
input double H1_Stoch_Dead_High = 60.0;           // Dead zone upper — skip when %K <= this
// Logic: ALLOW trades when Stoch<Dead_Low (deep oversold) OR Stoch>Dead_High (strong trend dip)
// SKIP trades when Stoch is between 35-60 (worst quality "uncertain momentum" zone)
...
input int    Magic_Number       = 3632057;        // v361 unique magic
input string Order_Comment      = "BSv363";
...
void OnTick()
{
   ...
   // v363: Dead-zone exclusion — skip trades when H1 Stoch is in the 35-60 "uncertain momentum" zone
   // Data shows: Stoch<35 = +$4.26/trade, Stoch 35-60 = -$0.91/trade (NEGATIVE!), Stoch>60 = +$5.69/trade
   double h1_stoch_k = iStochastic(NULL, PERIOD_H1, H1_Stoch_K, H1_Stoch_D, H1_Stoch_Slow,
                                    MODE_SMA, 0, MODE_MAIN, 1);
   bool h1StochOS = (h1_stoch_k < H1_Stoch_Dead_Low) || (h1_stoch_k > H1_Stoch_Dead_High);

   bool h1Bull = h4Bull
              && (iClose(NULL, PERIOD_H1, 1) > h1_ema)
              && (h1_rsi1 >= H1_RSI_Min) && (h1_rsi1 <= H1_RSI_Max)
              && h1StochOS;          // v361: require H1 stochastic oversold
