//+------------------------------------------------------------------+
//|   Buy & Sell EA v70 - v58 + Close strength filter               |
//|   Entry: H1 EMA200 + H1 RSI 50-75 + BB 2.5SD lower touch       |
//|          + M5 RSI<40 + RSI rising + Stoch K&D<25 + Bull 20%    |
//|          + Close in top 50% of candle range (strong close)      |
//|   Exit: TP = BB mid, SL = 1xATR                                 |
//|   Changes: Risk=8%, close must be in upper 50% of bar range     |
//|   Goal: Push WR above 60% by requiring stronger reversal candles |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "70.0"
#property strict
...
input double Close_Pct          = 0.5;            //Close must be in top X% of candle range
...
input int    Magic_Number       = 702025;         //Magic Number
input string Order_Comment      = "BSv70";        //Order comment
...
void OnTick()
{
   ...
   // Close must be in upper portion of candle range (strong reversal close)
   bool strongClose = (candleRange > 0) && ((Close[1] - Low[1]) >= Close_Pct * candleRange);

   if(bbTouch && rsiOS && rsiRising && stochOS && strongBull && strongClose)
   {
