//+------------------------------------------------------------------+
//|   Buy & Sell EA v292 - Tighter SL (0.6×ATR vs 0.8×ATR)          |
//|   BASE: v280 CHAMPION (Calmar=18.36, Return=220%, 637 trades)    |
//|   CHANGE: ATR_SL_Multi reduced from 0.8 to 0.6                  |
//|   RATIONALE:                                                      |
//|     All filter approaches have been exhausted:                    |
//|       v286 (BB_Dev=2.0)  → DISASTER (-6%)                       |
//|       v287 (M5 ADX<25)  → 85% trade reduction                   |
//|       v288 (H1 ADX<30)  → 37% reduction, Calmar=12.71           |
//|       v289 (BBW<5.5)    → 40% reduction, Calmar=13.89           |
//|       v290 (aggressive AM) → Calmar=14.90, worse than v280      |
//|       v291 (BBW<8.0)    → DISASTER (Calmar=7.72, MaxDD=18.18%)  |
//|     INSIGHT: v280 natural self-selection is superior to any      |
//|     filter. Middle-band (BBW 5.5-8.0) trades have 86% win rate  |
//|     vs 97% for narrow-band trades.                               |
//|                                                                   |
//|   HYPOTHESIS: Tighter SL (0.6×ATR) increases lot size by 33%   |
//|   since sizing is fixed-dollar-risk based. All profits amplified |
//|   by 1.333× while all losses remain at same dollar amount.      |
//|   The 2-bar confirmation + RSI/Stoch gates should keep win rate  |
//|   above 90% break-even threshold (needs > 89.6% to be net +).  |
//|   If win rate holds: Return ~295%, MaxDD ~12%, Calmar ~24.6.    |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "292.0"
#property strict
...
//--- Exit Settings (KEY CHANGE: ATR_SL_Multi 0.8 → 0.6)
//    Tighter SL → larger lot size (fixed dollar risk / smaller SL distance)
//    Result: same $ loss when stopped, 33% more $ profit when winning
//    TP:SL ratio improves from 6.25× to 8.33× (5.0/0.6)
input string EXIT_SET           = "============"; //====== Exit Settings ======
input int    ATR_Period         = 14;
input double ATR_SL_Multi       = 0.6;   // KEY CHANGE: was 0.8 in v280 → tighter SL, bigger lots
...
//--- Anti-Martingale (same as v280 champion)
...
input int    Magic_Number       = 2922057;
input string Order_Comment      = "BSv292";
...
void OnTick()
{
   ...
   double slDist    = ATR_SL_Multi * atr1;   // KEY: 0.6×ATR (tighter → larger lots)
