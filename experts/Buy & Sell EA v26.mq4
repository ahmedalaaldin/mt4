//+------------------------------------------------------------------+
//|   Buy & Sell EA v37 - Partial Close Strategy                    |
//|   Entry: v35 signal (BB lower + RSI<40 + Stoch K<25)           |
//|   At BB midline: close 50%, move SL to breakeven               |
//|   Let remaining 50% run to BB upper band                        |
//|   Goal: higher effective WR, lower DD, better avg profit        |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
...
input int    Magic_Number       = 372025;         //Magic Number
input string Order_Comment      = "BSv37";        //Order comment
...
void OnTick()
{
   ...
   // Partial close management runs every tick
   ManagePartialClose();
   ...
   if(h1Trend)
   {
      ...
      if(bbTouch && rsiOS && stochOS && bullClose)
      {
         ...
         if(tpDist >= minTP)
         {
            ...
            if(openBuys < Max_Buys && barsSince >= Min_Bar_Gap)
            {
               if(openBuys == 0 || HasEnoughDistance(OP_BUY, atr1))
               {
                  ...
                  // Ensure we can split into two halves (minimum 2× min lot)
                  double minLot  = MarketInfo(Symbol(), MODE_MINLOT);
                  double stepLot = MarketInfo(Symbol(), MODE_LOTSTEP);
                  if(lots >= 2.0 * minLot)
                  {
                     // TP initially at BB midline (first half target)
                  ...
                  {
                     // Too small to split; trade normally to BB midline
                     int tkt = OrderSend(Symbol(), OP_BUY, lots, entry, Slippage,
                                         NormalizeDouble(entry - slDist, Digits),
                                         NormalizeDouble(bbMid0, Digits),
                                         Order_Comment, Magic_Number, 0, clrGreen);
...
//+------------------------------------------------------------------+
// When Bid >= BB midline (original TP):
//   1. Close half the position (lock profit)
//   2. Move SL of remainder to entry (breakeven)
//   3. Extend TP to BB upper band
// Trigger guard: TP still near BB midline (not yet extended to upper)
void ManagePartialClose()
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      // Only trigger once: when TP is still at BB midline and price reached it
      if(Bid >= bbMid && curTP < bbMid + 10 * Point)
      {
         double minLot  = MarketInfo(Symbol(), MODE_MINLOT);
         double stepLot = MarketInfo(Symbol(), MODE_LOTSTEP);

         double halfLots = MathFloor(curLots / 2.0 / stepLot) * stepLot;
         halfLots = MathMax(halfLots, minLot);

         if(halfLots < curLots)
         {
            // Close half position
            bool closed = OrderClose(OrderTicket(), halfLots, Bid, Slippage, clrBlue);

            if(closed)
            {
               // Now find the remaining order (same ticket if partial close)
               // Re-select since lot size changed
               if(OrderSelect(i, SELECT_BY_POS, MODE_TRADES))
               {
                  if(OrderSymbol() == Symbol() && OrderMagicNumber() == Magic_Number)
                  {
                     // Move SL to breakeven, extend TP to BB upper
                     double newSL = NormalizeDouble(openPrice + Point, Digits);
                     double newTP = NormalizeDouble(bbUpper, Digits);
         ...
         {
            // Position too small to split — just extend TP to BB upper and move SL to BE
            double newSL = NormalizeDouble(openPrice + Point, Digits);
            double newTP = NormalizeDouble(bbUpper, Digits);
            if(newSL > OrderStopLoss() + Point)
               OrderModify(OrderTicket(), openPrice, newSL, newTP, 0, clrAqua);
