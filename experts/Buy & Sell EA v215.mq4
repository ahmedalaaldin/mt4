//+------------------------------------------------------------------+
//|   Buy & Sell EA v215 - Add CCI(14) Oversold/Overbought Filter     |
...
//|   v215 HYPOTHESIS: Add CCI(14) confirmation filter                |
//|     CCI(Commodity Channel Index, 14) measures price deviation from  |
//|     statistical mean. CCI < -100 = extreme oversold (4th filter).  |
//|     BUY if: existing conditions + CCI(14,M5) < CCI_OS (-100)      |
//|     SELL if: existing conditions + CCI(14,M5) > CCI_OB (+100)     |
//|     Rationale: CCI < -100 alongside BB lower + RSI<40 + Stoch<25  |
//|       = 4-layer oversold confirmation = potentially higher quality. |
//|     If Calmar > 12.60: CCI confirms higher-quality extreme setups  |
//|     If Calmar < 12.60: CCI over-filters — each filter independently|
//|       identifies the same quality trades (expected from prior tests)|
//|     ALL other parameters = v186 GLOBAL OPTIMUM.                   |
//|     Run with PERIOD = M5 in Strategy Tester.                      |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "215.0"
#property strict
...
//--- CCI Filter (v215 NEW: 4th oscillator confirmation)
input string CCI_SET            = "============"; //====== CCI Filter ======
input int    CCI_Period         = 14;             //CCI period (M5)
input double CCI_OS             = -100.0;         //BUY only if CCI < -100 (extreme oversold)
input double CCI_OB             = 100.0;          //SELL only if CCI > +100 (extreme overbought)
...
input int    Max_Sells          = 1;              //1 concurrent SELL — v186 GLOBAL OPTIMUM (restored)
...
input int    Magic_Number       = 2152038;        //Magic Number
input string Order_Comment      = "BSv215";       //Order comment
...
void OnTick()
{
   ...
   // v215 NEW: CCI(14) 4th oscillator confirmation
   ...
   //=== Signal BUY: H4 bull + H1 bull(EMA+RSI[45,75]) + BB(20,2.5) + RSI<40 + Stoch<25 rising + CCI<-100
   if(h4Bull && h1Bull && stochBuy && cci1 < CCI_OS)
   {
   ...
   //=== Signal SELL: H4 bear + H1 bear(EMA+RSI[25,55]) + BB(20,2.5) + RSI>60 + Stoch>75 falling + CCI>+100
   if(h4Bear && h1Bear && stochSell && cci1 > CCI_OB)
   {
