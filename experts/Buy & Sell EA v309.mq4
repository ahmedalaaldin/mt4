//+------------------------------------------------------------------+
//|   Buy & Sell EA v309 - Scale-In: 2 Simultaneous Positions        |
//|   BASE: v300 CHAMPION (Calmar=20.84, Return=258.39%, 1040 T)      |
//|   CHANGES from v300:                                              |
//|     Max_Buys  = 2 (was 1)                                         |
//|     Max_Sells = 2 (was 1)                                         |
//|     Risk_Pct  = 3.5 (was 7.0) — halved to keep same peak exposure |
//|   RATIONALE:                                                       |
//|     Allowing 2 concurrent positions per direction "scales in"     |
//|     at progressively more extreme BB deviations. The 2nd entry    |
//|     fires only when a new signal triggers while the 1st is still  |
//|     open — meaning price has dipped even deeper into oversold/OB. |
//|     Risk halved (3.5%) so max simultaneous exposure = 2x3.5%=7%  |
//|     (same total cap as v300's single 7% position). Expected:      |
//|     ~30-60% more trades, similar MaxDD, higher average entry      |
//|     quality on 2nd position due to more extreme entry level.      |
//|   HYPOTHESIS: ~1300-1700 trades, WR ~92-95%, MaxDD ~10-14%,      |
//|     Return 220-300%, Calmar 18-28                                 |
//|   DEAD ENDS SO FAR (26 confirmed):                                |
...
//|   SCOREBOARD:                                                      |
//|     v300: Calmar=20.84 (champion — stoch gate removed)            |
//|     v309: TARGET more trades via scale-in with halved risk         |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "309.0"
#property strict
...
//--- BB Entry Signal — v309: BB_Period=20 (v300 champion value)
input string S1_SET             = "============"; //====== BB Signal ======
input int    BB_Period          = 20;             // v309: restored to v300 champion value
...
//--- Money Management — KEY CHANGES: Max_Buys/Sells=2, Risk_Pct=3.5
input string MONEY_SET          = "============"; //====== Money Management ======
input double Risk_Pct           = 3.5;            // v309: 7.0 → 3.5 (halved; 2 positions × 3.5% = 7% max)
input double Fixed_Lot          = 0.0;
input int    Max_Buys           = 2;              // v309: 1 → 2 (scale-in allowed)
input int    Max_Sells          = 2;              // v309: 1 → 2 (scale-in allowed)
...
input int    Magic_Number       = 3092057;
input string Order_Comment      = "BSv309";
