//+------------------------------------------------------------------+
//|   Buy & Sell EA v300 - Remove Stochastic Gate                   |
//|   BASE: v293/v280 CHAMPION (Calmar=18.43, Return=220.62%, 637 T) |
//|   CHANGE: Stochastic gate removed (stochBuy=stochSell=always OK)|
//|   RATIONALE:                                                      |
//|     v293 requires stoch K < 25 AND K rising before entering.    |
//|     This blocks valid BB breakdown-recovery signals where RSI<40 |
//|     and BB touch are confirmed but stochastic hasn't yet         |
//|     dipped below 25 or turned up. Many "early" but valid         |
//|     mean-reversion setups are missed.                            |
//|     Removing stochastic while keeping all other filters:         |
//|     BB touch + RSI<40 + RSI rising + 20% body + two-bar         |
//|     + H1 trend (EMA+RSI) should increase trades significantly.  |
//|   HYPOTHESIS: ~900-1200 trades, win rate ~91-93%,               |
//|     higher total return, potentially better MaxDD diversification |
//|     (more trades = better statistical averaging of losses).      |
//|   DEAD ENDS SO FAR:                                             |
//|     - Max_Bars: irrelevant (never triggered)                     |
//|     - Body_Pct=0.40: filters quality trades                      |
//|     - BB_Dev=3.0: too few trades, loses shorts                   |
//|     - M15 timeframe: losing strategy (-10% return)              |
//|     - Session/partial/longs-only/ADX/BBW: all tested            |
//|   SCOREBOARD:                                                    |
//|     v293: Calmar=18.43 (champion — all filters active)          |
//|     v300: No stoch gate — TARGET higher trade count + Calmar    |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "300.0"
#property strict

#include <stdlib.mqh>

//--- Session (same as v293: 9-21)
...
input int    Magic_Number       = 3002057;
input string Order_Comment      = "BSv300";
...
void OnTick()
{
   ...
   // KEY CHANGE: Stochastic is NOT used as a gate in v300
   // stochBuy = true, stochSell = true always
   // All other filters remain identical to v293
   ...
   //=== BUY === (no stoch gate — only h1Bull required as outer gate)
   if(h1Bull)  // KEY CHANGE: removed stochBuy condition
   {
   ...
   //=== SELL === (no stoch gate — only h1Bear required as outer gate)
   if(h1Bear)  // KEY CHANGE: removed stochSell condition
   {
