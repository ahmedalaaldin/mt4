//+------------------------------------------------------------------+
//|   Buy & Sell EA v218 - 75% Partial Exit at BB Midline            |
...
//|       causing trail to fire with bbMid >> current price → immediate|
//|       stop-outs above entry. Exit logic architecture flaw.        |
//|       LESSON: Early BE breaks the partialDone state machine.      |
//|       Abandoning early BE concept — doesn't suit mean reversion.  |
//|   v218 HYPOTHESIS: 75% partial exit (up from 50%)                 |
//|     Lock MORE profit at BB midline: close 75% of position at BB   |
//|     midline (was 50%). Trail remaining 25% with 0.8 ATR.         |
//|     Rationale: Captures more profit early, reduces the 50% half   |
//|     that is exposed to reversal risk on the trailing portion.     |
//|     Higher profit lock = lower MaxDD (less equity swing from trail)|
//|     If Calmar > 12.60: 75% lock is optimal, 50% was leaving too  |
//|     much exposed. DD reduction > return reduction.                |
//|     If Calmar < 12.60: 25% trailing is too small; trailing half  |
//|     was contributing significant profit that the 75% gives up.   |
//|     ALL other parameters = v186 GLOBAL OPTIMUM.                   |
//|     Run with PERIOD = M5 in Strategy Tester.                       |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "218.0"
#property strict
...
//--- Exit Settings (v186 GLOBAL OPTIMA — partial % changed in v218)
...
input double Partial_Pct        = 0.75;           //v218: Close 75% at BB midline (was 50% in v186)
...
input int    Magic_Number       = 2182038;        //Magic Number
input string Order_Comment      = "BSv218";       //Order comment
...
void ManageExits()
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(type == OP_BUY)
      {
         // v218: partialDone = SL moved to entry (BE) — same logic as v186
         bool partialDone = (currentSL >= openPrice);
         if(!partialDone && Bid >= bbMid)
         {
            double stepLot    = MarketInfo(Symbol(), MODE_LOTSTEP);
            double minLot     = MarketInfo(Symbol(), MODE_MINLOT);
            // v218: close Partial_Pct (75%) instead of 50%
