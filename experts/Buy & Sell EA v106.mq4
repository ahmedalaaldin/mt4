//+------------------------------------------------------------------+
//|   Buy & Sell EA v106 - H1 RSI Rising Momentum Filter            |
//|   Framework: 4H Direction / 1H Structure / 5M Entry             |
//|   Base: v104 AllBuysRiskFree gate                               |
//|   Innovation:                                                    |
//|     - H1 RSI must be RISING (h1_rsi[1] > h1_rsi[2])            |
//|       Eliminates entries when H1 trend is actively weakening    |
//|     - H4 candle must be bullish (Close > Open on H4)            |
//|       Adds momentum confirmation at the 4H level                |
//|     - Reverts to standard trailing exit (BB upper exit removed) |
//|     - Max_Buys=2 with AllBuysRiskFree gate (v104 approach)      |
//|   Goal: Push WR toward 97%+, reduce consecutive losses          |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "116.0"
#property strict
...
input int    Magic_Number       = 1162025;        //Magic Number
input string Order_Comment      = "BSv116";       //Order comment
...
void OnTick()
{
   ...
   // 4H: Direction + bullish candle confirmation
   double h4_ema  = iMA(NULL, PERIOD_H4, H4_EMA_Period, 0, MODE_EMA, PRICE_CLOSE, 0);
   double h4_c1   = iClose(NULL, PERIOD_H4, 1);
   double h4_o1   = iOpen(NULL, PERIOD_H4, 1);
   bool   h4Bull  = (iClose(NULL, PERIOD_H4, 0) > h4_ema) && (h4_c1 >= h4_o1);
   if(!h4Bull) return;
   ...
   bool   h1Bull  = (iClose(NULL, PERIOD_H1, 1) > h1_ema)
                  ...
                  && (h1_rsi1 > h1_rsi2);     // H1 RSI must be rising
   if(!h1Bull) return;
   ...
   if(bbTouch && rsiOS && rsiRising && stochOS && bullBody)
   {
      ...
      if(tpDist >= minTP)
      {
         ...
         // Gate: 2nd+ position only when ALL existing buys are risk-free
