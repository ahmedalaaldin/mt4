//+------------------------------------------------------------------+
//|   Buy & Sell EA v175 - M5 + H1 Dual-Timeframe Signals           |
//|   Framework: H4 Direction / H1 RSI Structure / M5+H1 Entry      |
//|   LESSONS LEARNED:                                               |
//|     v170: BB 2.5 SD M5, 2.0% risk -> 100% return, 14% DD       |
//|     v173: BB 2.0 SD -> Poor R:R (avg win < avg loss)            |
//|     v174: Max_Buys=3 -> fewer trades (anti-martingale cascade)   |
//|     KEY: M5 BB(20,2.5SD)+RSI<40 fires ~350x/year - hard to     |
//|     increase without losing quality. Need second signal LAYER.   |
//|   APPROACH v175:                                                  |
//|     LAYER 1: M5 BB(20,2.5SD) lower touch + M5 RSI<40 (proven)  |
//|     LAYER 2: H1 BB(20,2.0SD) lower touch + H1 RSI<40 (new)     |
//|     Both layers use same H4/H1 EMA direction filter (MANDATORY) |
//|     Both layers use same exit logic (partial close + trailing)   |
//|     H1 signals fire on new H1 bar detection from M5 chart       |
//|     Expected: 350 M5 signals + ~100 H1 signals = ~450/year      |
//|   Changes vs v170:                                               |
//|     1. Added H1 BB(20,2.0SD) signal layer (new code)            |
//|     2. H1_BB_Dev input for H1 band width                        |
//|     3. g_LastH1Bar tracker for new-H1-bar detection             |
//|     4. Magic_Number: 1702025 -> 1752025                         |
//|     5. Order_Comment: BSv170 -> BSv175                          |
//|     6. Risk_Pct stays 2.0 (v170 level for max return)           |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "175.0"
#property strict
...
//--- Bollinger Bands M5 (Primary)
input string BB_SET             = "============"; //====== BB Settings M5 ======
input int    BB_Period          = 20;             //BB period (M5)
input double BB_Dev             = 2.5;            //BB SD: 2.5 (proven 81% WR on M5)

//--- Bollinger Bands H1 (Secondary)
input string BB_H1_SET          = "============"; //====== BB Settings H1 ======
input double H1_BB_Dev          = 2.0;            //H1 BB SD: 2.0 (reasonable for H1)

//--- Signal: M5 BB Touch (Primary)
input string S1_SET             = "============"; //====== Signal: M5 BB Touch ======
input int    RSI_Period         = 14;             //M5/H1 RSI period
...
input int    Magic_Number       = 1752025;        //Magic Number
input string Order_Comment      = "BSv175";       //Order comment
...
datetime g_LastH1Bar    = 0;  // H1 bar tracker for secondary signal
...
void OnTick()
{
   ...
   datetime h1Time = iTime(NULL, PERIOD_H1, 0);
   bool isNewH1Bar = (h1Time != g_LastH1Bar);
   if(isNewH1Bar) g_LastH1Bar = h1Time;
   ...
   // ============================================================
   // LAYER 1: M5 BB(20,2.5SD) Signal (original proven logic)
   // ============================================================
   ...
   // ============================================================
   // LAYER 2: H1 BB(20,2.0SD) Signal (additional higher-TF layer)
   // Fires once per H1 bar close - checked on first M5 bar of new H1
   // ============================================================
   if(isNewH1Bar)
   {
      double h1_bb_lower = iBands(NULL, PERIOD_H1, BB_Period, H1_BB_Dev, 0, PRICE_CLOSE, MODE_LOWER, 1);
      double h1_bb_upper = iBands(NULL, PERIOD_H1, BB_Period, H1_BB_Dev, 0, PRICE_CLOSE, MODE_UPPER, 1);
      double h1_rsi_sig  = iRSI(NULL, PERIOD_H1, RSI_Period, PRICE_CLOSE, 1);
      double h1_rsi_prev = iRSI(NULL, PERIOD_H1, RSI_Period, PRICE_CLOSE, 2);

      double h1_low   = iLow(NULL, PERIOD_H1, 1);
      double h1_close = iClose(NULL, PERIOD_H1, 1);
      double h1_open  = iOpen(NULL, PERIOD_H1, 1);
      double h1_high  = iHigh(NULL, PERIOD_H1, 1);
      double h1_range = h1_high - h1_low;
      double h1_bodyB = h1_close - h1_open;
      double h1_bodyS = h1_open  - h1_close;

      //=== H1 Signal BUY ===
      if(h4Bull && h1Bull)
      {
         bool h1bbTouch   = (h1_low <= h1_bb_lower) && (h1_close > h1_bb_lower);
         bool h1rsiOS     = (h1_rsi_sig < RSI_Buy);
         bool h1rsiRising = (h1_rsi_sig > h1_rsi_prev);
         bool h1bullBody  = (h1_bodyB > 0) && (h1_range > 0)
                         && (h1_bodyB >= Body_Pct * h1_range);

         if(h1bbTouch && h1rsiOS && h1rsiRising && h1bullBody)
         {
            int openBuys  = CountTradesByMagic(OP_BUY, Magic_Number);
            int barsSince = (int)((Time[0] - g_LastBuyTime) / PeriodSeconds());
            if(openBuys < Max_Buys && barsSince >= Min_Bar_Gap)
      ...
      if(h4Bear && h1Bear)
      {
         bool h1bbTouchUp  = (h1_high >= h1_bb_upper) && (h1_close < h1_bb_upper);
         bool h1rsiOB      = (h1_rsi_sig > RSI_Sell);
         bool h1rsiFalling = (h1_rsi_sig < h1_rsi_prev);
         bool h1bearBody   = (h1_bodyS > 0) && (h1_range > 0)
                          && (h1_bodyS >= Body_Pct * h1_range);

         if(h1bbTouchUp && h1rsiOB && h1rsiFalling && h1bearBody)
         {
            int openSells = CountTradesByMagic(OP_SELL, Magic_Number);
            int barsSince = (int)((Time[0] - g_LastSellTime) / PeriodSeconds());
            if(openSells < Max_Sells && barsSince >= Min_Bar_Gap)
