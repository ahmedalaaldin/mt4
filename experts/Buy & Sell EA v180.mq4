//+------------------------------------------------------------------+
//|   Buy & Sell EA v180 - ATR Spike Filter + Tighter Session        |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry         |
//|   LESSONS LEARNED:                                               |
//|     v178: Risk=4.0%, SL=0.8  -> 229.30% ret, 24.35% DD, Cal=9.42|
//|     v179: Risk=4.0%, SL=0.6  -> 244.52% ret, 26.43% DD, Cal=9.25|
//|            TIGHTER SL FAILS: WR 84%->78%, DD WORSE               |
//|            SL=0.6 causes stop-outs before price reverses          |
//|   FINDING: ATR_SL_Multi=0.8 is optimal. Do NOT tighten SL.       |
//|   ROOT CAUSE OF HIGH DD: Trending/news periods create losing      |
//|    streaks that overcome anti-martingale scaling. Need to          |
//|    filter out volatile/trending regimes at entry.                 |
//|   APPROACH v180:                                                  |
//|     Revert SL=0.8 (optimal, proven)                              |
//|     Add ATR SPIKE FILTER: skip if fast ATR > spike_multi * slow  |
//|       ATR. Detects news events / volatility spikes.              |
//|       atr_fast(14) vs atr_slow(50) as proxy for avg ATR          |
//|     Narrow session: 09:00-21:00 (skip thin early Asian session)  |
//|     Tighter daily DD guard: 15% -> 8%                            |
//|     Keep Risk=4.0%, RSI=40/60, BB=2.5, TP=2.5 (all proven)      |
//|   Changes vs v178:                                                |
//|     1. ATR_SL_Multi: 0.8 (unchanged from v177, reverted from v179)|
//|     2. Added ATR_Spike_MA_Period=50 + ATR_Spike_Multi=1.8        |
//|     3. Session_Start_Hour: 7->9 (skip thin Asian pre-session)    |
//|     4. Session_End_Hour: 20->21 (extend slightly into NY evening) |
//|     5. Max_Daily_DD_Pct: 15->8 (tighter equity protection)       |
//|     6. Magic_Number: 1782025 -> 1802025                           |
//|     7. Order_Comment: BSv178 -> BSv180                            |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "180.0"
#property strict
...
input double ATR_Spike_Multi    = 1.8;            //Skip if fast ATR > spike_multi * slow ATR

//--- Exit Settings
input string EXIT_SET           = "============"; //====== Exit Settings ======
input int    ATR_Period         = 14;             //ATR period
input double ATR_SL_Multi       = 0.8;            //SL = ATR x 0.8 (proven optimal - reverted from v179)
input double ATR_TP_Multi       = 2.5;            //TP = ATR x 2.5 (increased from 2.0 for bigger wins)
...
input double Risk_Pct           = 4.0;            //Risk%: 4.0 (proven 200%+ return target)
input double Fixed_Lot          = 0.0;            //Fixed lot (0 = use Risk_Pct)
input int    Max_Buys           = 2;              //Max concurrent BUY (proven: 3 caused cascade)
...
input int    Magic_Number       = 1802025;        //Magic Number
input string Order_Comment      = "BSv180";       //Order comment
input double Max_Daily_DD_Pct   = 8.0;            //Daily DD guard: 8% (tightened from 15%)
...
void OnTick()
{
   ...
   // === ATR SPIKE FILTER (NEW v180) ===
   // Skip if fast ATR is significantly elevated vs slow ATR.
   // This catches news events, flash crashes, earnings surprises.
   // atr_fast = 14-bar ATR (responsive), atr_slow = 50-bar ATR (baseline)
   ...
   if(h4Bull && h1Bull)
   {
      ...
      bool rsiOS     = (rsi1 < RSI_Buy);       // RSI<40 (proven quality)
   ...
   if(h4Bear && h1Bear)
   {
      ...
      bool rsiOB      = (rsi1 > RSI_Sell);     // RSI>60 (proven quality)
