//+------------------------------------------------------------------+
//|   Buy & Sell EA v339 - RSI Trough Confirmation                   |
//|   BASE: v336 (RSI_Period=7 breakthrough: +11.62%, WR=82.38%)     |
//|                                                                   |
//|   v338 POST-MORTEM (505 trades, MaxDD=4.97%):                    |
//|     IDENTICAL to v336 — RSI(14)<55 gate was completely redundant |
//|     When RSI(7)<45, RSI(14) is ALWAYS already <55                |
//|     The slow RSI threshold was too loose to filter anything       |
//|     Key insight: need a stronger or different quality gate        |
//|                                                                   |
//|   v339 HYPOTHESIS:                                               |
//|   The 125 extra trades from RSI_Period=7 vs RSI_Period=14 are    |
//|   "false troughs" where RSI(7) briefly dips below 45 at bar[1]  |
//|   but bar[2] was NOT a genuine RSI local minimum.               |
//|   Pattern: bar[3]→bar[2] RSI was RISING (not a real trough),    |
//|   then bar[2]→bar[1] RSI dipped slightly and is rising now.     |
//|   Fix: require rsi[2] < rsi[3] — RSI must have been DECLINING   |
//|   going INTO bar[2], confirming bar[2] is a true RSI trough.    |
//|   This matches the two-bar pattern: bar[2]=extreme, bar[1]=recovery.|
//|                                                                   |
//|   v339 CHANGES from v336:                                        |
//|     ADD: rsiTrough = (rsi[2] < rsi[3]) - confirms RSI minimum   |
//|     RSI_Buy=45 restored (v337 proved 42 is worse)               |
//|   Expected: ~420-460 trades, WR hopefully >83%                   |
//|                                                                   |
//|   SCOREBOARD:                                                     |
//|     v328: +16.25%, MaxDD=4.99%, WR=83.47%, 380 trades  BEST ✓  |
//|     v336: RSI_Period=7 → +11.62%, WR=82.38%, 505t   NEAR ✓    |
//|     v337: RSI_Period=7 + RSI_Buy=42 → WR=81.15%, 435t FAIL ✗  |
//|     v338: Dual RSI → identical to v336             SKIP ~      |
//|     v339: RSI trough gate (rsi[2]<rsi[3]) → ???                 |
//|                                                                   |
...
input int    RSI_Period         = 7;              // v336 breakthrough: faster RSI
input double RSI_Buy            = 45.0;           // NON-NEGOTIABLE (v335/v337 confirmed)
...
input int    Magic_Number       = 3392057;
input string Order_Comment      = "BSv339";
...
void OnTick()
{
   ...
   double rsi1 = iRSI(NULL, 0, RSI_Period, PRICE_CLOSE, 1);  // RSI(7) at bar[1] - recovery bar
   double rsi2 = iRSI(NULL, 0, RSI_Period, PRICE_CLOSE, 2);  // RSI(7) at bar[2] - trough bar
   double rsi3 = iRSI(NULL, 0, RSI_Period, PRICE_CLOSE, 3);  // RSI(7) at bar[3] - pre-trough bar
   ...
   if(h1Bull)
   {
      ...
      bool rsiOS       = (rsi1 < RSI_Buy);              // RSI(7) < 45 at recovery bar
      bool rsiRising   = (rsi1 > rsi2);                 // RSI rising from trough to recovery
      bool rsiTrough   = (rsi2 < rsi3);                 // v339 NEW: RSI was DECLINING into bar[2]
                                                        // Confirms bar[2] is a genuine local minimum
      ...
      if(bbTouch && rsiOS && rsiRising && rsiTrough && bullBody && bbPrevBreak)
      {
