//+------------------------------------------------------------------+
//|   Buy & Sell EA v47 - Relaxed H1 Filter + More Concurrent Pos  |
//|   Entry: H1 EMA200 ONLY (H1 RSI removed) + BB 2.5SD lower      |
//|          + M5 RSI<40 + Stoch K&D<25 + bullish close             |
//|   Exit: TP = BB mid, SL = 1xATR                                |
//|   Goal: More buy signals by removing H1 RSI 50-75 bottleneck   |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
...
input int    H1_EMA_Period      = 200;            //H1 EMA period (only filter: price > EMA200)
...
input int    Max_Buys           = 4;              //Max concurrent BUY positions (raised from 3)
...
input int    Magic_Number       = 472025;         //Magic Number
input string Order_Comment      = "BSv47";        //Order comment
...
void OnTick()
{
   ...
   // Only H1 EMA filter — H1 RSI removed to unlock more signals
   bool h1Trend = (Close[1] > h1_ema);
   if(!h1Trend) return;
