//+------------------------------------------------------------------+
//|   Buy & Sell EA v36 - Stoch K<40 + Smart TP Extension          |
//|   Same core: H1 EMA200 + H1 RSI 50-75 + M5 BB lower touch     |
//|         + M5 RSI<40 + Stoch K<40 (relaxed) + bullish close     |
//|   When Bid >= BB mid: extend TP to BB upper, lock SL at entry  |
//|   Goal: more trades than v35 while keeping improved PF         |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
...
input double Stoch_OB           = 40.0;           //Stochastic oversold level (relaxed from 25)
...
input double Lock_Profit_ATR    = 0.3;            //Lock-in SL above entry when TP extended
...
input int    Magic_Number       = 362025;         //Magic Number
input string Order_Comment      = "BSv36";        //Order comment
...
void OnTick()
{
   ...
   // Smart TP extension runs every tick
   ManageSmartExit();
...
//+------------------------------------------------------------------+
// When Bid >= current TP (BB midline), extend TP to BB upper and
// lock in profit by moving SL to entry + Lock_Profit_ATR × ATR.
// Fires only once per trade (detected by TP still being near BB mid).
void ManageSmartExit()
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      // Only trigger when TP is still at or near BB midline (not yet extended)
      // and price has reached or exceeded BB midline
      if(Bid >= bbMid && curTP < bbMid + 10 * Point)
      {
         ...
         if(newSL > curSL + Point && newTP > curTP + Point)
