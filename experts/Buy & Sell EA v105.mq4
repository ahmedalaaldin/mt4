//+------------------------------------------------------------------+
//|   Buy & Sell EA v105 - Max_Buys=1 + BB Upper Exit               |
//|   Framework: 4H Direction / 1H Structure / 5M Entry             |
//|   Base: v104 gated 2nd position                                  |
//|   Innovation:                                                    |
//|     - Max_Buys = 1: strict single position (no concurrent DD)   |
//|     - Phase 2 exit: close remaining 50% at BB upper (dynamic TP)|
//|     - Trail stop as fallback if BB upper not reached            |
//|     - H1_RSI_Min = 45 (from 50): slightly more entries          |
//|     - Min_Bar_Gap = 1 (from 2): faster re-entry                 |
//|   Goal: DD guaranteed < 5% (single loss = 4.9%), boost return   |
//|         by capturing full BB range (lower → upper)              |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "115.0"
#property strict
...
input double H1_RSI_Min         = 45.0;           //H1 RSI min (relaxed from 50 for more entries)
...
input int    Min_Bar_Gap        = 1;              //Min bars between entries (faster re-entry)
...
input double Trail_ATR_Multi    = 0.3;            //Trail SL fallback (ATR multiples below BB mid)
input int    Max_Bars           = 60;             //Force close after N bars

//--- Money Management
input string MONEY_SET          = "============"; //====== Money Management ======
input double Risk_Pct           = 4.9;            //Risk % per trade (single trade < 5% DD)
...
input int    Magic_Number       = 1152025;        //Magic Number
input string Order_Comment      = "BSv115";       //Order comment
...
void ManageExits()
{
   ...
   double bbUpr = iBands(NULL, 0, BB_Period, BB_Dev, 0, PRICE_CLOSE, MODE_UPPER, 0);
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      // Phase 2: Close remaining at BB upper (full mean-reversion TP)
      if(partialDone)
      {
         if(Bid >= bbUpr)
         {
            OrderClose(tkt, OrderLots(), Bid, Slippage, clrMagenta);
            continue;
         }
         // Fallback: trail SL = bbMid - offset (only moves up)
