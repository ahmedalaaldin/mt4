//+------------------------------------------------------------------+
//|   Buy & Sell EA v127 - Dual Signal (BB Touch + EMA Cross)        |
//|   Framework: 4H Direction / 1H Structure / 5M Entry             |
//|   Base: v120 (gold standard 15.0x Return/DD ratio)              |
//|   Changes vs v120:                                               |
//|     - Removed M15 confluence filter (v126 proved it hurts)       |
//|     - Added Signal 2: M5 EMA fast/slow crossover                 |
//|       BUY:  fast EMA crossed above slow + H4 bull + H1 above EMA |
//|       SELL: fast EMA crossed below slow + H4 bear + H1 below EMA |
//|     - Hypothesis: EMA crosses add 2-5 extra trades/day           |
//|       while H4 filter keeps trade quality high                   |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "137.0"
#property strict
...
input int    Magic_Number       = 1372025;        //Magic Number
input string Order_Comment      = "BSv137";       //Order comment
...
void OnTick()
{
   ...
   //=== Signal 1: BB Touch (v120 logic, unchanged) ===
   if(h4Bull && h1Bull)
   {
   ...
   //=== Signal 2: EMA Crossover (new in v127) ===
   if(Use_EMA_Cross)
   {
      ...
      // H1 structure check (no RSI filter for EMA cross — more permissive)
         ...
         if(tpDist >= minTP)
         {
            ...
            if(openBuys == 0 && barsSince >= Min_Bar_Gap)
            {
               ...
               if(lots > 0)
               {
                  int tkt = OrderSend(Symbol(), OP_BUY, lots, entry, Slippage,
                                      NormalizeDouble(entry - slDist, Digits),
                                      0, Order_Comment, Magic_Number, 0, clrLime);
                  if(tkt > 0) { g_TradesToday++; g_LastBuyTime = Time[0]; }
               }
            }
         }
         ...
         if(tpDist >= minTP)
         {
            ...
            if(openSells == 0 && barsSince >= Min_Bar_Gap)
            {
               ...
               if(lots > 0)
               {
                  int tkt = OrderSend(Symbol(), OP_SELL, lots, entry, Slippage,
                                      NormalizeDouble(entry + slDist, Digits),
                                      0, Order_Comment, Magic_Number, 0, clrOrangeRed);
                  if(tkt > 0) { g_TradesToday++; g_LastSellTime = Time[0]; }
               }
            }
         }
