//+------------------------------------------------------------------+
//|   Buy & Sell EA v353 - No H1 EMA filter                          |
...
//|                                                                   |
//|   v353 HYPOTHESIS: Remove H1 EMA trend filter.                   |
//|   Currently h1Bull requires price > H1_200_EMA. In the          |
//|   2025-2026 XAUUSD bull market, brief dips below H1_200_EMA    |
//|   may be the BEST mean-reversion entries (deeper pullbacks).    |
//|   v352 showed H1_RSI_Max=70 hurt (overbought H1 = good trades). |
//|   Testing the opposite end: allow ALL H1 conditions, even       |
//|   when H1 price is below its 200_EMA.                           |
//|   Expected: more trades, WR impact unknown (could go either     |
//|   way depending on H1 bearish periods in 2025-2026 data).       |
//|                                                                   |
//|   v353 CHANGES from v343:                                        |
//|     Use_H1_EMA_Filter: true → false (EMA condition removed)     |
//|     h1Bull = RSI_H1 in [H1_RSI_Min, H1_RSI_Max] only (no EMA)  |
//|                                                                   |
...
input bool   Use_H1_EMA_Filter  = false;          // v353: EMA filter DISABLED
...
input int    Magic_Number       = 3532057;        // v353 unique magic
input string Order_Comment      = "BSv353";
...
void OnTick()
{
   ...
   // v353: H1 EMA filter REMOVED — allow entries regardless of H1 trend direction
   // Only RSI gate remains (H1 RSI between 0 and 90 = almost always true)
   bool h1Bull = ((!Use_H1_EMA_Filter) || (iClose(NULL, PERIOD_H1, 1) > h1_ema))
