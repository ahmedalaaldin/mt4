//+------------------------------------------------------------------+
//|   Buy & Sell EA v279 - CCI Oversold/Overbought Filter            |
//|   Framework: H1 EMA direction + H1 RSI gate / M5 Entry          |
//|   HYPOTHESIS: Adding M5 CCI < -100 for buys and CCI > +100 for  |
//|     sells confirms the BB2.5σ deviation on a DIFFERENT metric.  |
//|     CCI measures deviation from the mean differently from RSI.   |
//|     When BOTH RSI (<40) AND CCI (<-100) are oversold, the       |
//|     pullback is confirmed by two independent oscillators,        |
//|     potentially improving trade quality over v269's single RSI.  |
//|   BASE: v269 CHAMPION (Calmar=17.77, Return=209%, 707 trades)   |
//|   CHANGE: Add M5 CCI < CCI_Buy threshold for buys,             |
//|     CCI > CCI_Sell threshold for sells                          |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "279.0"
#property strict
...
//--- CCI Filter (NEW in v279)
input string CCI_SET            = "============"; //====== CCI Filter ======
input int    CCI_Period         = 14;             // CCI lookback period
input double CCI_Buy            = -100.0;         // Buy only when M5 CCI < this (oversold)
input double CCI_Sell           = 100.0;          // Sell only when M5 CCI > this (overbought)
...
input int    Magic_Number       = 2792057;
input string Order_Comment      = "BSv279";
...
void OnTick()
{
   ...
   //--- CCI filter: additional oversold/overbought confirmation
   ...
   //=== BUY ===
   if(h1Bull && stochBuy && (cci1 < CCI_Buy))   // CCI oversold filter added
   {
   ...
   //=== SELL ===
   if(h1Bear && stochSell && (cci1 > CCI_Sell))  // CCI overbought filter added
   {
