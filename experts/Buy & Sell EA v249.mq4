//+------------------------------------------------------------------+
//|   Buy & Sell EA v249 - Cap Position Size to Initial Balance      |
...
//|     v244: NO STOCH = CATASTROPHIC                                |
//|     v245: BB_Dev=2.0 FAILED (24× worse Calmar)                   |
//|     v246: M1 TIMEFRAME FAILED                                    |
//|     v247: EQUITY GUARD FAILED — Return=23.73%, MaxDD=8.69%       |
//|       Calmar=2.73 (5× WORSE than v186)                           |
//|       LESSON: Equity guard fires on floating losses → kills gains |
//|     v248: NO EQUITY GUARD — Return=166.21%, MaxDD=23.18%         |
//|       Calmar=7.17, 255 trades, Win=89.41%, MaxConsecWins=57      |
//|       Equity guard removal CONFIRMED CORRECT (win rate restored)  |
//|       PROBLEM: Anti-martingale ballooned positions to $2,600+/   |
//|         trade on $32k balance after 57-win streak                |
//|         Avg loss=$1,126, MaxDD=$7,556 on peak $32,598            |
//|         7%×1.25×$32k=$2,800 → 1 trade = 8.6% of peak            |
//|       LESSON: Risk scales with EQUITY — after big win streak,    |
//|         a single loss exceeds the entire 5% MaxDD budget          |
//|                                                                  |
//|   v249 HYPOTHESIS: Cap risk calculation to INITIAL BALANCE       |
//|     Positions sized for initial $10k regardless of equity growth  |
//|     Anti-martingale still applies to initial-balance sizing       |
//|     Max position: 7%×1.25×$10k = $875 per trade regardless       |
//|     6 consecutive losses: $875+$437+$218+$87+$87+$87 = $1,791    |
//|     On peak $32k: MaxDD ≈ $1,791/$32k = 5.6% — near target!      |
//|     Expected return: somewhat lower (no compounding) but Calmar   |
//|       should improve substantially                                |
//|     All v186 entry conditions UNCHANGED (NON-NEGOTIABLE)         |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "249.0"
#property strict
...
input double Risk_Pct           = 7.0;            // v186 CONFIRMED
...
input double Scale_Mid          = 1.10;
input double Scale_High         = 1.25;

//--- Safety
input string SAFETY_SET         = "============"; //====== Safety Settings ======
input int    MaxSpread          = 35;
input int    Slippage           = 30;
input int    Magic_Number       = 2492057;
input string Order_Comment      = "BSv249";
...
double   g_InitialBalance = 0;    // NEW v249: snapshot at startup for position size cap

//+------------------------------------------------------------------+
int OnInit()
{
   g_InitialBalance = AccountBalance();   // Cap positions to this base — no runaway sizing
...
//+------------------------------------------------------------------+
// KEY v249 CHANGE: Risk amount capped to initial balance sizing.
// Prevents positions from ballooning as equity grows during win streaks.
// Anti-martingale riskScale still applies, but anchored to initial deposit,
// not current equity. Max single trade risk = 7% × 1.25 × InitialBalance.
double CalcLots(double slDist, double riskScale = 1.0)
{
   ...
   // ...but CAP it to initial balance so positions don't grow beyond $10k-calibrated sizes
   // This bounds max loss per trade regardless of how much the account has grown
...
void OnTick()
{
   ...
   // Daily DD gate — balance-based (v247 fix: prevents false triggers on open floating losses)
   ...
   double riskScale = GetRiskScale();   // Anti-martingale applied to initial-balance sizing
