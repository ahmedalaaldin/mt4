//+------------------------------------------------------------------+
//|   Buy & Sell EA v119 - Dual Concurrent Positions                 |
//|   Framework: 4H Direction / 1H Structure / 5M Entry             |
//|   Base: v114 (217.78%, 14.93% DD, 327T, 93.27% WR)             |
//|   Changes from v114:                                             |
//|     - Max_Buys: 2 (was 1) → allow 2nd position while 1st open   |
//|     - Max_Sells: 2 (was 1) → allow 2nd position while 1st open  |
//|     - Risk_Pct: 3.5% (was 5.0%) → per slot; max 7% concurrent  |
//|     - Gate: openBuys < Max_Buys (was openBuys == 0)             |
//|   All entry filters IDENTICAL to v114 (no threshold changes)    |
//|   Hypothesis: 30-50% more trades, WR stays ~93%, higher return  |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "129.0"
#property strict
...
input double BB_Dev             = 2.5;            //BB SD (v114 proven optimal)
...
input double RSI_Buy            = 40.0;           //M5 RSI max for BUY (v114 proven)
input double RSI_Sell           = 60.0;           //M5 RSI min for SELL (v114 proven)
input int    Stoch_K            = 5;              //Stochastic K period
input int    Stoch_D            = 3;              //Stochastic D period
input int    Stoch_Slowing      = 3;              //Stochastic slowing
input double Stoch_OS           = 25.0;           //Stochastic oversold level (v114 proven)
input double Stoch_OB           = 75.0;           //Stochastic overbought level (v114 proven)
input double Body_Pct           = 0.2;            //Min body as fraction of candle range
input int    Min_Bar_Gap        = 1;              //Min bars between entries (same direction)
...
input double Risk_Pct           = 3.5;            //Risk % per trade (3.5 x 2 slots = 7% max)
input double Fixed_Lot          = 0.0;            //Fixed lot (0 = use Risk_Pct)
input int    Max_Buys           = 2;              //Max concurrent BUY positions (was 1)
input int    Max_Sells          = 2;              //Max concurrent SELL positions (was 1)
...
input int    Magic_Number       = 1292025;        //Magic Number
input string Order_Comment      = "BSv129";       //Order comment
...
void OnTick()
{
   ...
   //--- BUY logic: H4 bull + H1 bull + up-to-Max_Buys gate
   ...
   if(h4Bull && h1Bull)
   {
      ...
      if(bbTouch && rsiOS && rsiRising && stochOS && bullBody)
      {
         ...
         if(tpDist >= minTP)
         {
            ...
            // Allow up to Max_Buys concurrent positions, spaced Min_Bar_Gap apart
   ...
   //--- SELL logic: H4 bear + H1 bear + up-to-Max_Sells gate
   ...
   if(h4Bear && h1Bear)
   {
      ...
      if(bbTouchUp && rsiOB && rsiFalling && stochOB && bearBody)
      {
         ...
         if(tpDist >= minTP)
         {
            ...
            // Allow up to Max_Sells concurrent positions, spaced Min_Bar_Gap apart
