//+------------------------------------------------------------------+
//|   Buy & Sell EA v196 - H4 Restored + SL/TP Ratio Optimized      |
...
//|            More trades (+28%) but WR 88.82% vs 90.7%, DD too high  |
//|            H4 filter IS important quality control -> RESTORE        |
//|   v196 HYPOTHESIS: Restore H4 + optimize SL/TP ratio              |
//|     SL = 0.7 ATR (vs 0.8): smaller loss per stop, lower DD        |
//|     TP = 3.0 ATR (vs 2.5): bigger win per winner, better R:R      |
//|     R:R = 4.29:1 (vs 3.125:1): 37% improvement                    |
//|     EV at 90.7% WR: 0.907*3.0 - 0.093*0.7 = 2.656 ATR/trade      |
//|       vs current: 0.907*2.5 - 0.093*0.8 = 2.193 ATR/trade (+21%) |
//|     Smaller SL: fewer $ lost per stop = lower DD episodes          |
//|     Larger TP: harder to reach = WR may drop slightly              |
//|     Net hypothesis: lower DD + higher return = better Calmar       |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "196.0"
#property strict
...
input double ATR_SL_Multi       = 0.7;            //SL = ATR x 0.7 (v196: tighter SL for lower DD)
input double ATR_TP_Multi       = 3.0;            //TP = ATR x 3.0 (v196: larger TP for better R:R)
input double Trail_ATR_Multi    = 0.7;            //Trail SL offset (matches new SL size)
...
input int    Magic_Number       = 1962026;        //Magic Number
input string Order_Comment      = "BSv196";       //Order comment
...
void OnTick()
{
   ...
   double slDist    = ATR_SL_Multi * atr1;   // 0.7 ATR (v196: tighter)
   double tpDist    = ATR_TP_Multi * atr1;   // 3.0 ATR (v196: larger)
