//+------------------------------------------------------------------+
//|   Buy & Sell EA v128 - Dual Signal (BB Touch + EMA Cross) Fixed  |
//|   Framework: 4H Direction / 1H Structure / 5M Entry             |
//|   Base: v127                                                     |
//|   Changes vs v127:                                               |
//|     - FIXED: Signal 2 now uses Magic_Number+1 independently      |
//|       In v127 both signals shared the same magic counter so      |
//|       Signal 2 never entered (Signal 1 always blocked it)        |
//|     - Signal 1 (BB Touch): Magic_Number   → same as v120         |
//|     - Signal 2 (EMA Cross): Magic_Number+1 → fully independent   |
//|     - Separate last-entry trackers per signal                    |
//|     - ManageExits/CloseAll handles both magic numbers            |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
...
input int    Magic_Number       = 1382025;        //Magic Number (Signal 1)
...
int CountConsecLosses()
{
   ...
   for(int pass = 0; pass < 6; pass++)
   {
      ...
      for(int i = OrdersHistoryTotal() - 1; i >= 0; i--)
      {
         ...
         // Count losses from BOTH signals
...
void OnTick()
{
   ...
   //=== Signal 2: EMA Crossover (Magic_Number+1, fully independent) ===
   if(Use_EMA_Cross)
   {
      ...
      // H1 structure only (no RSI restriction — more permissive than Signal 1)
         ...
         if(tpDist >= minTP)
         {
            ...
            if(openBuys < Max_Buys && barsSince >= Min_Bar_Gap2)
            {
               ...
               if(lots > 0)
               {
                  int tkt = OrderSend(Symbol(), OP_BUY, lots, entry, Slippage,
                                      NormalizeDouble(entry - slDist, Digits),
                                      0, Order_Comment, magic2, 0, clrLime);
         ...
         if(tpDist >= minTP)
         {
            ...
            if(openSells < Max_Sells && barsSince >= Min_Bar_Gap2)
            {
               ...
               if(lots > 0)
               {
                  int tkt = OrderSend(Symbol(), OP_SELL, lots, entry, Slippage,
                                      NormalizeDouble(entry + slDist, Digits),
                                      0, Order_Comment, magic2, 0, clrOrangeRed);
...
void ManageExits()
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(OrderSymbol() != Symbol()) continue;
      // Manage BOTH signal 1 and signal 2 positions
      if(OrderMagicNumber() != Magic_Number && OrderMagicNumber() != Magic_Number + 1) continue;
