//+------------------------------------------------------------------+
//|   Buy & Sell EA v34 - v18 Signal + ATR Trailing Stop            |
//|   Same proven signal: H1 EMA200 + H1 RSI 50-75 + M5 BB lower  |
//|         + M5 RSI<40 + bullish close                             |
//|   TP: BB upper band (wider target, let winners run)             |
//|   Trailing stop activates once profit >= 1xATR                  |
//|   Goal: same WR as v18, significantly higher avg win            |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
...
input double H1_RSI_Max         = 75.0;           //H1 RSI max (same as v18)

//--- Bollinger Bands
input string BB_SET             = "============"; //====== Bollinger Band Settings ======
input int    BB_Period          = 20;             //BB period
input double BB_Dev             = 2.0;            //BB standard deviations
input double TP_Min_ATR         = 1.0;            //Min initial TP distance as ATR multiple

//--- Entry Signal
input string ENTRY_SET          = "============"; //====== Entry Settings ======
input int    RSI_Period         = 14;             //M5 RSI period
input double RSI_Buy            = 40.0;           //BUY: M5 RSI must be below this (same v18)
...
input double Trail_Activate_ATR = 1.0;            //Activate trail when profit >= ATR x this
input double Trail_Distance_ATR = 0.8;            //Trail SL distance below price = ATR x this
input int    Max_Bars           = 80;             //Force close after N bars (wider for trend)

//--- Money Management
input string MONEY_SET          = "============"; //====== Money Management ======
input double Risk_Pct           = 1.5;            //Risk per trade (% of equity) - same v18
...
input int    Magic_Number       = 342025;         //Magic Number
input string Order_Comment      = "BSv34";        //Order comment
...
void OnTick()
{
   ...
   if(h1Trend)
   {
      ...
      if(bbTouch && rsiOS && bullClose)
      {
         double entry = NormalizeDouble(Ask, Digits);
         // Use BB midline as initial TP - trailing stop will capture more if trend continues
...
{
   double atrCur = iATR(NULL, 0, ATR_Period, 1);
   if(atrCur <= 0) return;

   double activateProfit = Trail_Activate_ATR * atrCur;
   double trailDist      = Trail_Distance_ATR * atrCur;
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      // Only activate trail once profit threshold is reached
      if(profit < activateProfit) continue;

      // New trailing SL: Bid - trailDist
      double newSL = NormalizeDouble(Bid - trailDist, Digits);

      // Only move SL upward, never down; must be above current SL and above entry
      if(newSL > curSL + Point && newSL > openPrice)
      {
         // Also extend TP to give room for trend to continue (BB upper direction)
         double bbUpper = iBands(NULL, 0, BB_Period, BB_Dev, 0, PRICE_CLOSE, MODE_UPPER, 0);
         double newTP = (bbUpper > curTP) ? NormalizeDouble(bbUpper, Digits) : curTP;
         OrderModify(OrderTicket(), openPrice, newSL, newTP, 0, clrAqua);
      }
   }
}
