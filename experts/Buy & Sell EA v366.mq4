//+------------------------------------------------------------------+
//|   Buy & Sell EA v365 - ATR_Spike_Multi=3.0 (from 2.0 in v358)         |
//|   BASE: v354 (ATR_Spike=2.0, M5+M15+M30) BEST âœ“                 |
//|                                                                   |
//|   SCOREBOARD (all M5 unless noted):                              |
//|     v342: M5+M15 â†’ +29.51%, MaxDD=4.74%, WR=86.90%, 756t âœ“    |
//|     v343: M5+M15+M30 â†’ +30.67%, MaxDD=4.87%, WR=85.14%, 814t âœ“|
//|     v354: M5+M15+M30, ATR_Spike=2.0 â†’ +30.33%, MaxDD=4.65%âœ…   |
//|     v355: ATR_Spike=1.5 â†’ +0.73%, WR=70.16%, 305t  FAIL âœ—      |
//|     v356: Session 7-23 â†’ +8.32%, MaxDD=5.33%âŒ, 619t FAIL âœ—     |
//|     v357: M5+M15 only â†’ +28.66%, WR=87.05%âœ“, MaxDD=4.84% âœ—    |
//|                                                                   |
//|   v357 FINDING: M30 signals lower WR but reduce MaxDD and add   |
//|   recovery volume. Removing M30 â†’ higher WR but worse overall.  |
...
input int    H4_EMA_Fast_Period = 50;             // v358: NEW â€” medium-term H4 trend gate
...
input bool   Use_H1_Signals     = true;           // v366: H1 BB(20,2.5) entries as 4th TF
...
input double ATR_Spike_Multi    = 2.0;            // v358 CONFIRMED optimal (v365 3.0 was worse)
...
input int    Magic_Number       = 3662057;        // v366 unique magic
input string Order_Comment      = "BSv366";
...
void OnTick()
{
   ...
   datetime h1BarTime = iTime(NULL, PERIOD_H1, 0);
   bool isNewH1Bar = (h1BarTime != g_LastH1Bar);
   if(isNewH1Bar) g_LastH1Bar = h1BarTime;
   ...
   // ATR spike filter â€” skip when ATR > 2Ã— rolling average (v354 confirmed optimal)
   ...
   //=== H1 BUY (v366: 4th timeframe - H1 BB(20,2.5) touch) ===
   if(Use_H1_Signals && isNewH1Bar && h1Bull)
   {
      double h1_bbLower1 = iBands(NULL, PERIOD_H1, BB_Period, BB_Dev, 0, PRICE_CLOSE, MODE_LOWER, 1);
      double h1_bbLower2 = iBands(NULL, PERIOD_H1, BB_Period, BB_Dev, 0, PRICE_CLOSE, MODE_LOWER, 2);
      double h1_rsi1     = iRSI(NULL, PERIOD_H1, RSI_Period, PRICE_CLOSE, 1);
      double h1_rsi2     = iRSI(NULL, PERIOD_H1, RSI_Period, PRICE_CLOSE, 2);

      double h1_high1  = iHigh(NULL, PERIOD_H1, 1);
      double h1_low1   = iLow(NULL, PERIOD_H1, 1);
      double h1_close1 = iClose(NULL, PERIOD_H1, 1);
      double h1_open1  = iOpen(NULL, PERIOD_H1, 1);
      double h1_close2 = iClose(NULL, PERIOD_H1, 2);

      double h1_range = h1_high1 - h1_low1;
      double h1_bodyB = h1_close1 - h1_open1;

      bool h1_bbTouch     = (h1_low1 <= h1_bbLower1) && (h1_close1 > h1_bbLower1);
      bool h1_rsiOS       = (h1_rsi1 < RSI_Buy);
      bool h1_rsiRising   = (h1_rsi1 > h1_rsi2);
      bool h1_bullBody    = (h1_bodyB > 0) && (h1_range > 0)
                         && (h1_bodyB >= Body_Pct * h1_range);
      bool h1_bbPrevBreak = (!Use_TwoBar_Pattern) || (h1_close2 <= h1_bbLower2);

      if(h1_bbTouch && h1_rsiOS && h1_rsiRising && h1_bullBody && h1_bbPrevBreak)
      {
         if(CountTradesByMagic(OP_BUY, Magic_Number) < Max_Buys)
         {
            double h1_atr = iATR(NULL, PERIOD_H1, ATR_Period, 1);
            if(h1_atr > 0)
            {
               double h1_slDist = ATR_SL_Multi * h1_atr;
               double h1_tpDist = ATR_TP_Multi * h1_atr;
               double h1_lots   = CalcLots(h1_slDist, riskScale);
               double entry     = NormalizeDouble(Ask, Digits);
               double sl        = NormalizeDouble(entry - h1_slDist, Digits);
               double tp        = NormalizeDouble(entry + h1_tpDist, Digits);
               if(h1_lots > 0)
               {
                  int tkt = OrderSend(Symbol(), OP_BUY, h1_lots, entry, Slippage,
                                      sl, tp, Order_Comment, Magic_Number, 0, clrGold);
                  if(tkt > 0) g_TradesToday++;
               }
            }
         }
