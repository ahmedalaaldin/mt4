//+------------------------------------------------------------------+
//|   Buy & Sell EA v312 - Tighter Risk + More Signals (BB_Dev=2.0)  |
//|   BASE: v300 CHAMPION (Calmar=20.84, Return=258.39%, 1040 T)      |
//|   CHANGES from v300:                                              |
//|     BB_Dev    = 2.0  (was 2.5; more signals → ~1500-2000 trades)  |
//|     Risk_Pct  = 4.0  (was 7.0; max single loss = 4% of equity)   |
//|     LossScale_1=0.20 (was hardcoded 0.50; after 1 loss)          |
//|     LossScale_2=0.05 (was hardcoded 0.25; after 2 losses)        |
//|     LossScale_3=0.02 (was hardcoded 0.10; after 3+ losses)       |
//|   No ADX filter (v311 dead end — ADX killed returns, not MaxDD)   |
//|   RATIONALE:                                                       |
//|     v311 proved MaxDD is NOT caused by ADX>25 trending markets.   |
//|     True driver: single trade at 7% risk = 7% potential loss.     |
//|     Fix 1: Risk_Pct=4% → first loss = only 4% MaxDD.             |
//|     Fix 2: LossScale_1=0.20 → second loss = 0.8%. Total=4.8%<5%. |
//|     Fix 3: BB_Dev=2.0 → more M5 signals to recover lost return.  |
//|   MATH: (1800 trades × 4%/7%) × 258% ≈ 254% return est.         |
//|   HYPOTHESIS: ~1500-2000 trades, WR ~90-94%, MaxDD ~4-7%,        |
//|     Return 200-280%, Calmar 30-60                                 |
//|   DEAD ENDS SO FAR (29 confirmed):                                |
//|     v308: BB_Period=30 → Return=170.38%, MaxDD=25.53%, Calmar=6.67|
//|     v309: Max_Buys=2/Sells=2 → Return=128.73%, Calmar=13.07      |
//|     v310: M1 timeframe → Modelling 25%, total loss (dead end)     |
//|     v311: ADX<25 filter → Return=75.25%, MaxDD=12.25%, Calmar=6.14|
//|   SCOREBOARD:                                                      |
//|     v300: Calmar=20.84 (M5 champion — no ADX filter)             |
//|     v312: BB_Dev=2.0 + Risk=4% + aggressive AM scaling           |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "312.0"
#property strict
...
//--- BB Entry Signal — v312: BB_Dev=2.0 for more signals (~1.5-2x trades vs v300)
input string S1_SET             = "============"; //====== BB Signal ======
input int    BB_Period          = 20;             // v312: same as v300 champion
input double BB_Dev             = 2.0;            // v312: 2.5→2.0 (tighter bands = more touches)
...
//--- Stochastic settings (NOT used as gate — kept for reference)
...
//--- Money Management — v312: Risk=4% (was 7%) — limits single trade loss to 4% equity
input string MONEY_SET          = "============"; //====== Money Management ======
input double Risk_Pct           = 4.0;            // v312: 7.0→4.0 (max loss per trade = 4%)
input double Fixed_Lot          = 0.0;
input int    Max_Buys           = 1;
input int    Max_Sells          = 1;

//--- Anti-Martingale — v312: configurable loss scaling (more aggressive downscaling)
...
input double LossScale_1        = 0.20;           // v312: scale after 1 loss (was hardcoded 0.50)
input double LossScale_2        = 0.05;           // v312: scale after 2 losses (was hardcoded 0.25)
input double LossScale_3        = 0.02;           // v312: scale after 3+ losses (was hardcoded 0.10)

//--- Safety
input string SAFETY_SET        = "============"; //====== Safety Settings ======
input int    MaxSpread          = 35;
input int    Slippage           = 30;
input int    Magic_Number       = 3122057;
input string Order_Comment      = "BSv312";
...
double GetRiskScale()
{
   ...
   if(streak == 0)              return 1.0;
   // v312: configurable loss scaling (more aggressive anti-martingale)
   int losses = -streak;
   if(losses == 1) return LossScale_1;   // default 0.20 (was hardcoded 0.50)
   if(losses == 2) return LossScale_2;   // default 0.05 (was hardcoded 0.25)
   return LossScale_3;                    // default 0.02 (was hardcoded 0.10)
}
...
void OnTick()
{
   ...
   // v312: BB_Dev=2.0 (more touches), Risk=4% (reduces max loss to 4%), no ADX filter
