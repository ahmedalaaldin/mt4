//+------------------------------------------------------------------+
//|   Buy & Sell EA v281 - Pinbar Rejection Wick Filter              |
//|   Framework: H1 EMA direction + H1 RSI gate / M5 Entry          |
//|   HYPOTHESIS: At the BB lower band, a candle with a significant  |
//|     LOWER SHADOW (wick) demonstrates price REJECTION of the      |
//|     extreme level. Bears pushed price down, bulls defended it    |
//|     and closed near the high. Adding Lower_Shadow >= LW_Min_Pct  |
//|     of total range filters weak touches (where candle body is    |
//|     at the bottom = no real rejection) vs strong touches (long   |
//|     wick = clear rejection at support/resistance).               |
//|     Should improve win rate at cost of fewer trades.             |
//|   BASE: v269 CHAMPION (Calmar=17.77, Return=209%, 707 trades)   |
//|   CHANGE: Add lower shadow (for buys) / upper shadow (for sells) |
//|     minimum percentage of total candle range as rejection filter  |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "281.0"
#property strict
...
//--- Pinbar / Rejection Wick Filter (NEW in v281)
input string PINBAR_SET         = "============"; //====== Pinbar Filter ======
input double LW_Min_Pct         = 0.25;  // Lower wick must be >= this fraction of total range (for buys)
input double UW_Min_Pct         = 0.25;  // Upper wick must be >= this fraction of total range (for sells)
...
input int    Magic_Number       = 2812057;
input string Order_Comment      = "BSv281";
...
void OnTick()
{
   ...
   //--- Pinbar / rejection wick calculation
   // Lower wick = from Low to bottom of body = Low[1] to MathMin(Open[1],Close[1])
   double lowerWick = 0, upperWick = 0;
   if(candleRange > 0)
   {
      lowerWick = (MathMin(Open[1], Close[1]) - Low[1]) / candleRange;
      upperWick = (High[1] - MathMax(Open[1], Close[1])) / candleRange;
   }
   ...
   if(h1Bull && stochBuy)
   {
      ...
      bool pinbarOK  = (candleRange <= 0) || (lowerWick >= LW_Min_Pct);  // lower wick ≥ 25% of range

      if(bbTouch && rsiOS && rsiRising && bullBody && pinbarOK)
      {
   ...
   if(h1Bear && stochSell)
   {
      ...
      bool pinbarOK   = (candleRange <= 0) || (upperWick >= UW_Min_Pct);  // upper wick ≥ 25% of range

      if(bbTouchUp && rsiOB && rsiFalling && bearBody && pinbarOK)
      {
