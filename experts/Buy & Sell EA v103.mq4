//+------------------------------------------------------------------+
//|   Buy & Sell EA v103 - Bidirectional: BUY + SELL                 |
//|   Framework: 4H Bull / 1H RSI Structure / M5 Entry               |
//|   Base: v102 partial exit + trailing stop + streak scaling       |
//|   Innovation:                                                    |
//|     - BUY:  H4 Bull + H1 RSI 50-75 + M5 BB lower touch          |
//|     - SELL: H4 Bull + H1 RSI 25-50 + M5 BB upper touch          |
//|     - Both directions: partial close 50% at BB mid + trail       |
//|     - Risk_Pct = 4.5% (single trade DD stays below 5%)          |
//|     - Max_Buys=1, Max_Sells=1 (no concurrent same-dir DD)       |
//|   Goal: Double trade opportunities while cutting DD below 5%    |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "113.0"
#property strict
...
input double H1_RSI_Min         = 50.0;           //H1 RSI min for BUY (bullish zone)
input double H1_RSI_Max         = 75.0;           //H1 RSI max for BUY (not overextended)
input double H1_RSI_Sell_Min    = 25.0;           //H1 RSI min for SELL (bearish zone)
input double H1_RSI_Sell_Max    = 50.0;           //H1 RSI max for SELL (pullback zone)
...
input double Risk_Pct           = 4.5;            //Base risk % per trade (single trade < 5% DD)
input double Fixed_Lot          = 0.0;            //Fixed lot (0 = use Risk_Pct)
input int    Max_Buys           = 1;              //Max concurrent BUY positions
input int    Max_Sells          = 1;              //Max concurrent SELL positions
...
input int    Magic_Number       = 1132025;        //Magic Number
input string Order_Comment      = "BSv113";       //Order comment
...
void OnTick()
{
   ...
   if(!h4Bull) return; // Both buy and sell require H4 bullish backdrop
   ...
   // === BUY: H4 Bull + H1 Bull (RSI 50-75) + M5 oversold bounce ===
   ...
   // === SELL: H4 Bull + H1 pullback (RSI 25-50) + M5 overbought rejection ===
   bool h1SellOK = (h1_rsi >= H1_RSI_Sell_Min) && (h1_rsi <= H1_RSI_Sell_Max);
   if(h1SellOK)
   {
      double candleBody = Open[1] - Close[1]; // Bearish body magnitude
      bool bbTouch  = (High[1] >= bbUpper1) && (Close[1] < bbUpper1);
      bool rsiOB    = (rsi1 > RSI_Sell);
      bool rsiFall  = (rsi1 < rsi2);
      bool stochOB  = (stochK1 > Stoch_OB) && (stochD1 > Stoch_OB);
      bool bearBody = (candleBody > 0) && (candleRange > 0) && (candleBody >= Body_Pct * candleRange);

      if(bbTouch && rsiOB && rsiFall && stochOB && bearBody)
      {
...
void ManageExits()
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      {
         // For sells: SL starts above entry. partialDone = SL moved to breakeven (at or below entry).
         ...
         if(partialDone)
         {
            // Trail SL downward: only moves SL lower (toward and below current price)
            double trailSL = NormalizeDouble(bbMid + Trail_ATR_Multi * atr, Digits);
            if(trailSL < currentSL)
               OrderModify(tkt, openPrice, trailSL, 0, 0, clrBlue);
         }
