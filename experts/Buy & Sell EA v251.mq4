//+------------------------------------------------------------------+
//|   Buy & Sell EA v251 - Lower Risk + Larger TP                    |
...
//|       Calmar=7.17, PROBLEM: unbounded equity scaling              |
//|     v249: INITIAL BALANCE CAP — Return=119.29%, MaxDD=16.12%     |
//|       Calmar=7.40, 236 trades, Win=88.56%                        |
//|       PROBLEM: Scale_High=1.25 pushed first loss to 5.8% peak   |
//|     v250: FLAT WIN SCALING — Return=107.11%, MaxDD=10.37%        |
//|       Calmar=10.33 *** BEST *** , 234 trades, Win=88.46%        |
//|       PF=1.87, AvgWin=$111, AvgLoss=$457, MaxConsecL=6          |
//|       PROBLEM: AvgLoss=$457 >> AvgWin=$111 (4.1:1 ratio)        |
//|         Alternating L-W-L-W near peak: -$700+$111 = -$589/cycle |
//|         MaxDD still 10.37% despite good Calmar                  |
//|         Single trade $700 = 3.1% of $22k peak — OK per trade    |
//|         But L-W alternations create cumulative drawdown          |
//|                                                                  |
//|   v251 HYPOTHESIS: Lower Risk_Pct + Larger ATR_TP_Multi          |
//|     Risk_Pct: 7% → 5%  (max loss $700 → $500)                   |
//|     ATR_TP_Multi: 2.5 → 3.5 (larger TP target, +40%)            |
//|     Max single loss: 5% × $10k = $500 = 2.3% of $22k peak ✓    |
//|     6 consecutive losses: $500+$250+$125+$50+$50+$50 = $1,025   |
//|       = 4.7% of $22k peak ← just under 5% target ✓             |
//|     Larger TP should increase avg win, improving L-W asymmetry  |
//|     Expected: MaxDD < 8%, Calmar > 12, similar/better return    |
//|     All v186 entry conditions UNCHANGED (NON-NEGOTIABLE)         |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "251.0"
#property strict
...
input double ATR_TP_Multi       = 3.5;            // v251: 2.5 → 3.5 (+40%) to increase avg win
...
input double Risk_Pct           = 5.0;            // v251: 7.0 → 5.0 (max loss $500 not $700)
...
input int    Magic_Number       = 2512057;
input string Order_Comment      = "BSv251";
...
double   g_InitialBalance = 0;    // v249+: initial balance cap for position sizing

//+------------------------------------------------------------------+
int OnInit()
{
   g_InitialBalance = AccountBalance();   // Cap positions to initial deposit size
...
double GetRiskScale()
{
   ...
   // v250/v251: Scale_Mid=1.0 and Scale_High=1.0 — no win upscaling
   // Only loss recovery downscaling applies
...
//+------------------------------------------------------------------+
// v251 key change: Risk_Pct = 5.0% (was 7.0%)
// Max single loss: 5% × $10k × 1.0 = $500 = 2.3% of $22k peak ✓
// 6 consecutive: $500+$250+$125+$50+$50+$50 = $1,025 = 4.7% of $22k ✓
// Larger ATR_TP_Multi=3.5 (was 2.5) should increase average win size
double CalcLots(double slDist, double riskScale = 1.0)
{
   ...
   // ...but CAP to initial balance — no runaway sizing on win streaks
...
void OnTick()
{
   ...
   // Daily DD gate — balance-based (v247 fix: no false triggers on floating losses)
