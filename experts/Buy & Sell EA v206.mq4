//+------------------------------------------------------------------+
//|   Buy & Sell EA v206 - Remove H1 RSI Gate                        |
...
//|   v206 HYPOTHESIS: Remove H1 RSI gate to increase trade count     |
//|     Current H1 RSI gate: BUY if H1_RSI in [45,75]               |
//|                          SELL if H1_RSI in [25,55]               |
//|     This gate filters trades when H1 is:                          |
//|       - Deeply oversold (<45): could be GREAT BUY opportunities   |
//|         (H4 bull + H1 deeply oversold = prime mean-reversion)     |
//|       - Mildly overbought (>75): H4 bull continuation signals     |
//|     Removing gate tests whether H1 RSI adds value or blocks       |
//|     high-quality oversold entries in H4 uptrend context           |
//|     Expected: More trades, similar or better WR for oversold H1,  |
//|     potential Calmar improvement. H4+H1 EMA direction still kept. |
//|     All other parameters identical to v186 (proven optimal).       |
//|     Run with PERIOD = M5 in Strategy Tester.                       |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "206.0"
#property strict
...
//--- H1 RSI Gate (v206: REMOVED — bounds set to 0/100 = no constraint)
input string H1RSI_SET          = "============"; //====== H1 RSI Gate ======
input int    H1_RSI_Period      = 14;             //H1 RSI period
input double H1_RSI_Min         = 0.0;            //v206: NO gate — was 45.0
input double H1_RSI_Max         = 100.0;          //v206: NO gate — was 75.0
input double H1_RSI_Min_Sell    = 0.0;            //v206: NO gate — was 25.0
input double H1_RSI_Max_Sell    = 100.0;          //v206: NO gate — was 55.0
...
input int    Magic_Number       = 2062036;        //Magic Number
input string Order_Comment      = "BSv206";       //Order comment
...
void OnTick()
{
   ...
   // H4 + H1 EMA direction filter (both required — same as v186)
   // v206: H1 RSI gate REMOVED (bounds 0/100 = no constraint)
   ...
   // h1Bull: H1 close above H1 EMA + H1 RSI in [0,100] (no gate = just EMA direction)
   ...
   //=== Signal BUY: H4 bullish + H1 bullish(EMA only) + BB(20,2.5) touch + RSI<40 + Stoch<25
   ...
   //=== Signal SELL: H4 bearish + H1 bearish(EMA only) + BB(20,2.5) touch + RSI>60 + Stoch>75
