//+------------------------------------------------------------------+
//|   Buy & Sell EA v135 - BB_Dev=2.5 + RSI<40 + Clean TP/SL       |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry         |
//|   Base: v134                                                     |
//|   Changes vs v134:                                               |
//|     - BB_Dev: 2.0 → 2.5 (restored) — v134 proved that 2.0      |
//|       collapses WR from 84.76% to 34.25% because price at       |
//|       2.0 SD is NOT extreme enough for reliable mean reversion   |
//|       2.5 SD filters only genuine extremes = high WR            |
//|     - RSI_Buy: 45 → 40 (restored) — stricter oversold needed   |
//|     - RSI_Sell: 55 → 60 (restored) — stricter overbought       |
//|     - Clean TP/SL exits KEPT from v134 (no partial, no trail)   |
//|       v133 had inverted RR: wins $92 vs losses $350 from        |
//|       partial close cutting winners short                        |
//|       v135: TP=2xATR at order open, SL=1xATR = clean 2:1 RR   |
//|   Expected: ~430 trades/yr, WR ~84%, clean 2:1 RR              |
//|   Key hypothesis: same WR as v133 + better RR = much higher    |
//|   return and lower DD than v133's 31% DD                       |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
...
//--- H1 RSI Gate (proven values from v120 — unchanged)
...
//--- Bollinger Bands (M5) — restored to 2.5 SD for high WR
input string BB_SET             = "============"; //====== BB Settings ======
input int    BB_Period          = 20;             //BB period
input double BB_Dev             = 2.5;            //BB standard deviation (2.0 killed WR)

//--- Signal 1: M5 BB Touch + RSI — strict thresholds restored
input string S1_SET             = "============"; //====== Signal 1: BB Touch ======
input int    RSI_Period         = 14;             //M5 RSI period
input double RSI_Buy            = 40.0;           //M5 RSI max for BUY (40 = genuine oversold)
input double RSI_Sell           = 60.0;           //M5 RSI min for SELL (60 = genuine overbought)
input double Body_Pct           = 0.2;            //Min body fraction of candle range
input int    Min_Bar_Gap        = 1;              //Min M5 bars between entries

//--- Exit Settings — CLEAN TP/SL, no partial close, no trailing
input string EXIT_SET           = "============"; //====== Exit Settings ======
input int    ATR_Period         = 14;             //ATR period (M5)
input double ATR_SL_Multi       = 1.0;            //SL = ATR x this (1xATR)
...
//+------------------------------------------------------------------+
// Safety-only: force-close after Max_Bars. TP and SL handle all exits.
...
void OnTick()
{
   ...
   // BB 2.5 SD — proven extreme filter for high WR
   ...
   double tpDist    = ATR_TP_Multi * atr1;   // 2xATR = clean 2:1 RR
   ...
   if(h4Bull && h1Bull)
   {
      ...
      bool rsiOS     = (rsi1 < RSI_Buy);       // RSI<40 = genuine oversold
   ...
   if(h4Bear && h1Bear)
   {
      ...
      bool rsiOB      = (rsi1 > RSI_Sell);     // RSI>60 = genuine overbought
