//+------------------------------------------------------------------+
//|   Buy & Sell EA v139 - v133 + Anti-Martingale Win-Streak Scale  |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry         |
//|   Base: v133                                                     |
//|   Changes vs v133:                                               |
//|     - GetRiskScale replaced with two-way adaptive scale:         |
//|       WIN streaks  → scale UP (anti-martingale):                 |
//|         3-5 consecutive wins → 1.5× base risk                   |
//|         6+ consecutive wins  → 2.0× base risk                   |
//|       LOSS streaks → scale DOWN (same as v133):                 |
//|         1 consecutive loss → 0.5× base risk                     |
//|         2 consecutive losses → 0.25× base risk                  |
//|         3+ consecutive losses → 0.1× base risk                  |
//|     - Risk_Pct = 5% (unchanged from v133)                       |
//|       Max position = 2.0× × 5% = 10% risk (same ceiling as v138)|
//|       BUT: only occurs during confirmed win streaks (not always) |
//|       AND: after ANY loss, scale-down kicks in immediately       |
//|     - All other v133 parameters unchanged:                       |
//|       BB_Dev=2.5, RSI_Buy=40, ATR_SL_Multi=1.0                 |
//|       Partial close at BB midline + trail (Trail_ATR=0.8)        |
//|       H4/H1 gate, Max_Buys=2, Max_Sells=2                       |
//|   Rationale: v133 had max 33 consec wins, avg ~11. Scaling to   |
//|   2.0× during those streaks could push return from 107% to      |
//|   ~180-250% while keeping DD similar to v133 (~31%) since       |
//|   scale-down on losses is preserved.                             |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "139.0"
#property strict
...
input double BB_Dev             = 2.5;            //BB standard deviation (proven high WR)
...
input double ATR_SL_Multi       = 1.0;            //SL = ATR x this (1.0 proven)
...
input int    WinStreak_Mid      = 3;              //Consec wins for 1.5x scale-up
input int    WinStreak_High     = 6;              //Consec wins for 2.0x scale-up
input double Scale_Mid          = 1.5;            //Scale at mid win streak
input double Scale_High         = 2.0;            //Scale at high win streak
...
input string Order_Comment      = "BSv139";       //Order comment
...
//+------------------------------------------------------------------+
// Scan last N closed trades to determine consecutive wins or losses
// Returns: positive = consecutive wins count, negative = consecutive losses count, 0 = no history
int CountLastStreak()
{
   datetime prevTime = TimeCurrent() + 1;
   int maxLook = 40; // look back up to 40 trades
   bool isWin = false;
   ...
   for(int pass = 0; pass < maxLook; pass++)
   {
      ...
      if(!firstFound)
      {
         // First trade found — establishes the streak direction
         isWin      = bestResult;
         firstFound = true;
         count      = 1;
      }
...
double GetRiskScale()
{
   ...
   // WIN streak → scale UP (anti-martingale)
   if(streak >= WinStreak_High) return Scale_High;   // 2.0× at 6+ wins
   if(streak >= WinStreak_Mid)  return Scale_Mid;    // 1.5× at 3-5 wins
   if(streak > 0)               return 1.0;          // 1-2 wins: normal

   // LOSS streak → scale DOWN (same protection as v133)
   if(streak == 0) return 1.0;  // no history
   int losses = -streak;
...
void OnTick()
{
   ...
   double riskScale = GetRiskScale();   // Two-way: scales up on wins, down on losses
