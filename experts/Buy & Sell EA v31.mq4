//+------------------------------------------------------------------+
//|   Buy & Sell EA v42 - v35 Signal + TP at BB Upper              |
//|   Entry: H1 EMA200 + H1 RSI 50-75 + BB 2.5SD lower touch      |
//|          + M5 RSI<40 + Stoch K<25 + bullish close              |
//|   Exit: TP = BB upper (full band width)                        |
//|         When price crosses BB mid, SL moves to entry+0.3xATR  |
//|   Goal: Higher avg win by targeting BB upper, lock at BB mid   |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
...
input double Lock_Profit_ATR    = 0.3;            //Lock-in SL above entry when crossing BB mid
input int    Max_Bars           = 60;             //Force close after N bars (longer — targeting BB upper)
...
input int    Magic_Number       = 422025;         //Magic Number
input string Order_Comment      = "BSv42";        //Order comment
...
void OnTick()
{
   ...
   // Lock profit when price crosses BB mid (runs every tick)
   ManageLockAtMid();
...
//+------------------------------------------------------------------+
// When price crosses BB mid: lock SL to entry + Lock_Profit_ATR * ATR
// This fires only once per trade (SL will be above original entry SL after)
void ManageLockAtMid()
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      // Lock profit when price crosses BB mid — only once (guard: SL still below entry)
      if(Bid >= bbMid && curSL < openPrice - Point)
      {
         double newSL = NormalizeDouble(openPrice + Lock_Profit_ATR * atrCur, Digits);
         if(newSL > curSL + Point)
            OrderModify(OrderTicket(), openPrice, newSL, OrderTakeProfit(), 0, clrAqua);
      }
   }
}
