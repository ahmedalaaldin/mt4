//+------------------------------------------------------------------+
//|   Buy & Sell EA v26 - BUY ONLY Quality Filter                  |
//|   Signal: BB lower touch + RSI<45 + bullish close + H1 EMA200  |
//|   + strong candle body + TP >= 0.8xATR                         |
//|   Quality: Min_Bar_Gap=2, Max_Buys=3, Risk=1.0%               |
//|   Tight daily DD cap at 2.5% to protect equity                 |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
...
input double RSI_Buy            = 45.0;           //BUY: RSI must be below this (oversold)
input double Body_Min_ATR       = 0.2;            //Min candle body as ATR multiple (quality)
...
input double Risk_Pct           = 1.0;            //Risk per trade (% of equity)
...
input int    Magic_Number       = 262025;         //Magic Number
input string Order_Comment      = "BSv26";        //Order comment
input double Max_Daily_DD_Pct   = 2.5;            //Daily drawdown guard (% of equity)
...
void OnTick()
{
   ...
   double minBody = Body_Min_ATR * atr1;

   //--- BUY: above H1 EMA200, BB lower touch, RSI oversold, strong bullish close
   if(Close[1] > h1_ema)
   {
      bool bbTouch   = (Low[1] <= bbLower1) && (Close[1] > bbLower1);
      bool rsiOS     = (rsi1 < RSI_Buy);
      double body    = Close[1] - Open[1];
      bool strongBull = (body >= minBody);

      if(bbTouch && rsiOS && strongBull)
      {
