//+------------------------------------------------------------------+
//|   Buy & Sell EA v137 - v133 with SL=0.5xATR (double lot size)  |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry         |
//|   Base: v133                                                     |
//|   Changes vs v133:                                               |
//|     - ATR_SL_Multi: 1.0 → 0.5                                  |
//|       Halving SL distance doubles lot size (same dollar risk).   |
//|       Partial close at BB midline now earns ~2x dollar profit:   |
//|       v133: 1.0xATR SL → lots L → partial win ~$92             |
//|       v137: 0.5xATR SL → lots 2L → partial win ~$184           |
//|       Full SL dollar loss stays SAME: 0.5×ATR × 2L = 1.0×L    |
//|       If WR stays ≥75%: EV = 0.75×$184 - 0.25×$350 = $50/trd  |
//|       If WR stays ≥80%: EV = 0.80×$184 - 0.20×$350 = $77/trd  |
//|       Projected annual return: 200-334% vs v133's 107.86%       |
//|     - All other v133 logic unchanged:                            |
//|       BB_Dev=2.5, RSI_Buy=40, RSI_Sell=60, H1 RSI gate         |
//|       Partial close 50% at BB midline + trail                    |
//|       ATR_TP_Multi=2.0 (safety), Trail_ATR_Multi=0.8            |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
...
input double BB_Dev             = 2.5;            //BB standard deviation (2.5 = proven high WR)
...
input double ATR_SL_Multi       = 0.5;            //SL = ATR x this (0.5 = tighter → 2x lots)
...
void OnTick()
{
   ...
   double slDist    = ATR_SL_Multi * atr1;   // 0.5xATR → 2x lot size vs v133
