//+------------------------------------------------------------------+
//|   Buy & Sell EA v311 - ADX Trend Filter to Cut Worst Losses       |
//|   BASE: v300 CHAMPION (Calmar=20.84, Return=258.39%, 1040 T)      |
//|   CHANGES from v300:                                              |
//|     + ADX_Period = 14  (new parameter)                            |
//|     + Max_ADX    = 25.0 (skip entries when ADX > 25 = trending)   |
//|     Risk_Pct=7.0 (restored), Max_Buys=1/Max_Sells=1 (restored)   |
//|     BB_Dev=2.5 (restored to v300 champion value)                  |
//|   RATIONALE:                                                       |
//|     v300's MaxDD (12.4%) is driven by trend days when mean-       |
//|     reversion fails — BB touch during strong trend = trap.        |
//|     ADX < 25 = ranging/oscillating market (ideal for BB rev.)     |
//|     ADX > 25 = trending market (BB touch is continuation signal). |
//|     Filtering out high-ADX days should eliminate the worst        |
//|     losses while keeping most winning mean-reversion trades.      |
//|     Target: fewer trades but much higher Calmar (>30).            |
//|   HYPOTHESIS: ~600-900 trades, WR ~95-97%, MaxDD ~4-7%,          |
//|     Return 150-250%, Calmar 25-50                                 |
//|   DEAD ENDS SO FAR (28 confirmed):                                |
...
//|     v311: ADX<25 gate targeting MaxDD<5% + higher Calmar          |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "311.0"
#property strict
...
//--- BB Entry Signal — v311: BB_Dev=2.5 (v300 champion value restored)
input string S1_SET             = "============"; //====== BB Signal ======
input int    BB_Period          = 20;             // v311: v300 champion value
input double BB_Dev             = 2.5;            // v311: restored to v300 champion (2.5)
...
//--- ADX Trend Filter — v311: new — skip trending markets where BB mean-reversion fails
input string ADX_SET            = "============"; //====== ADX Trend Filter ======
input int    ADX_Period         = 14;             // v311: ADX lookback period
input double Max_ADX            = 25.0;           // v311: skip entry when ADX > 25 (trending)
...
//--- Money Management — v311: restored to v300 champion values (Risk=7%, Max=1/1)
input string MONEY_SET          = "============"; //====== Money Management ======
input double Risk_Pct           = 7.0;            // v311: restored to v300 champion (was 3.5 in v309)
input double Fixed_Lot          = 0.0;
input int    Max_Buys           = 1;              // v311: restored to 1 (was 2 in v309)
input int    Max_Sells          = 1;              // v311: restored to 1 (was 2 in v309)

//--- Anti-Martingale (same as v300)
...
input int    Magic_Number       = 3112057;
input string Order_Comment      = "BSv311";
...
void OnTick()
{
   ...
   // v311: ADX filter — skip entries when market is trending (ADX > Max_ADX)
   if(Max_ADX > 0)
   {
      double adx1 = iADX(NULL, 0, ADX_Period, PRICE_CLOSE, MODE_MAIN, 1);
      if(adx1 > Max_ADX) return;
   }

   // v311: BB_Period=20, BB_Dev=2.5 (v300 champion values — ADX filter is the only addition)
