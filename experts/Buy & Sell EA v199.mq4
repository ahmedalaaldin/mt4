//+------------------------------------------------------------------+
//|   Buy & Sell EA v199 - BB Period = 14 on M5                      |
...
//|       M15 BB signals are too sparse and WR collapses on M15       |
//|       LESSON: This strategy is fundamentally M5-native             |
//|   v199 HYPOTHESIS: BB_Period=14 instead of 20 on M5               |
//|     Rationale: Shorter BB lookback -> tighter, more reactive bands |
//|       BB(14) bands cross price more often -> more signals           |
//|       RSI<40 + Stoch<25 quality gates still filter noise           |
//|       Expected: ~1.5-2x more trades vs v186's 258                  |
//|       Expected WR: possibly 88-91% (vs 90.73% with BB20)          |
//|       If WR holds: more trades + similar WR = higher Calmar         |
//|     All other parameters identical to v186 (proven optimal).       |
//|     Run with PERIOD = M5 in Strategy Tester.                       |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "199.0"
#property strict
...
input int    BB_Period          = 14;             //BB period: 14 (v199: shorter lookback for more signals)
...
input int    Magic_Number       = 1992029;        //Magic Number
input string Order_Comment      = "BSv199";       //Order comment
...
void OnTick()
{
   ...
   //=== Signal BUY: H4 bullish + H1 bullish + BB(14,2.5) touch + RSI<40 + Stoch<25
   ...
   //=== Signal SELL: H4 bearish + H1 bearish + BB(14,2.5) touch + RSI>60 + Stoch>75
