//+------------------------------------------------------------------+
//|   Buy & Sell EA v343 - Multi-Timeframe M5+M15+M30 Signals        |
//|   BASE: v342 (M5+M15, WR=86.90%, +29.51%, 756t)                 |
//|                                                                   |
//|   SCOREBOARD (all M5 unless noted):                              |
//|     v328: +16.25%, MaxDD=4.99%, WR=83.47%, 380t   BEST M5 ✓    |
//|     v336: RSI_Period=7 → +11.62%, WR=82.38%, 505t  FAIL ✗      |
...
//|     v341: BUY_STOP → WR=64.29%, -$2.21 LOSS         FAIL ✗     |
//|     v342: M5+M15 → +29.51%, MaxDD=4.74%, WR=86.90%, 756t ✓    |
//|                                                                   |
//|   v343 HYPOTHESIS: Add M30 timeframe signals                     |
//|   Same BB breakdown-recovery pattern checked on M5, M15, M30.   |
//|   M30 signals fire once per new M30 bar (every 6 M5 bars).      |
//|   M30 ATR used for SL/TP sizing (wider stops → smaller lots).   |
//|   Expected: ~950-1000 trades/year, WR ~87-88%, return ~36-42%.  |
//|   H1 trend filter still required for all M30 entries.           |
//|                                                                   |
//|   v343 CHANGES from v342:                                        |
//|     New: Use_M30_Signals parameter (default true)                |
//|     New: M30 BB pattern check fires once per new M30 bar         |
//|     M30 entry uses M30 ATR for SL/TP sizing                      |
//|     All three TF trades share same Max_Buys limit                |
//|                                                                   |
//|   NON-NEGOTIABLES (ALL unchanged from v342):                     |
...
//--- v342+: Multi-Timeframe Settings
input string MTF_SET            = "============"; //====== Multi-TF Settings ======
input bool   Use_M15_Signals    = true;           // v342: also trade M15 BB pattern
input bool   Use_M30_Signals    = true;           // v343: also trade M30 BB pattern
...
input int    Magic_Number       = 3432057;        // v343 unique magic
input string Order_Comment      = "BSv343";
...
datetime g_LastM15Bar     = 0;   // v342: tracks last M15 bar for M15 signal
datetime g_LastM30Bar     = 0;   // v343: tracks last M30 bar for M30 signal
...
void OnTick()
{
   ...
   // v342: Track new M15 bar (fires once every 3 M5 bars)
   ...
   // v343: Track new M30 bar (fires once every 6 M5 bars)
   ...
   // === M5 ATR for volatility filters and M5-based trades ===
   ...
   //=== M5 BUY (exact v328 market order logic) ===
   if(h1Bull)
   {
   ...
   //=== M15 BUY (v342: same pattern on M15 timeframe) ===
   // Fires once per new M15 bar only.
   // Uses M15 ATR for SL/TP; exits managed by M5 ManageExits via M5 bbMid.
   ...
   //=== M30 BUY (v343 new: same pattern on M30 timeframe) ===
   // Fires once per new M30 bar only.
   // Uses M30 ATR for SL/TP (wider stops, smaller lots, higher quality signals).
   // Exits managed by M5 ManageExits via M5 bbMid.
