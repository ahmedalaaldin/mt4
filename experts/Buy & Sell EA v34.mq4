//+------------------------------------------------------------------+
//|   Buy & Sell EA v45 - v35 Signal + Two-Stage Exit (Trail)       |
//|   Entry: H1 EMA200 + H1 RSI 50-75 + BB 2.5SD lower touch       |
//|          + M5 RSI<40 + Stoch K&D<25 + bullish close            |
//|   Exit Stage 1: When Bid >= BB mid → move SL to breakeven       |
//|   Exit Stage 2: When Bid >= BB mid + 0.5xATR → SL to BB mid   |
//|   Final TP: BB upper (3xATR away), Max_Bars fallback           |
//|   Goal: Higher avg win by letting runners reach BB upper        |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
...
input double TP_Min_ATR         = 1.0;            //Min TP distance as ATR multiple
...
input double Stage2_ATR         = 0.5;            //ATR above BB mid for Stage 2 SL lock
...
input int    Magic_Number       = 452025;         //Magic Number
input string Order_Comment      = "BSv45";        //Order comment
...
void OnTick()
{
   ...
   // Two-stage exit runs every tick (not just on new bar)
   ManageTwoStageExit();
   ...
   if(bbTouch && rsiOS && stochOS && bullClose)
   {
      ...
      // Initial TP = BB upper (we manage the trail manually, but set BB upper as max target)
...
//+------------------------------------------------------------------+
// Two-stage exit management (runs every tick):
// Stage 1: Price crosses BB mid → move SL to breakeven (entry price)
// Stage 2: Price crosses BB mid + 0.5xATR → move SL to BB mid (lock partial profit)
// Final TP: BB upper (already set at entry)
void ManageTwoStageExit()
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      // Stage 1: crossed BB mid → move SL to breakeven
      if(Bid >= bbMid && curSL < openPrice - Point)
      {
         newSL = NormalizeDouble(openPrice, Digits);
      }

      // Stage 2: crossed BB mid + 0.5xATR → lock SL at BB mid
      double stage2Level = bbMid + Stage2_ATR * atrCur;
      if(Bid >= stage2Level && curSL < bbMid - Point)
      {
         newSL = NormalizeDouble(bbMid, Digits);
      }

      // Apply SL only if it's an improvement (higher than current)
      if(newSL > curSL + Point)
         OrderModify(OrderTicket(), openPrice, newSL, curTP, 0, clrAqua);
   }
}
