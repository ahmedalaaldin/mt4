//+------------------------------------------------------------------+
//|   Buy & Sell EA v24 - Triple TF BB Bounce                      |
//|   BUY: H1 EMA200 up + M15 EMA20 up + M5 BB lower touch        |
//|         + RSI<40 + bullish close + TP >= 1xATR                 |
//|   SELL: H1 EMA200 down + M15 EMA20 down + M5 BB upper touch   |
//|         + RSI>60 + bearish close + TP >= 1xATR                 |
//|   Multi-TF alignment filters weak bounces, improves WR         |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "24.0"
#property strict
...
input int    M15_EMA_Period     = 20;             //M15 EMA period (intermediate direction)
...
input double RSI_Buy            = 40.0;           //BUY: RSI must be below this
input double RSI_Sell           = 60.0;           //SELL: RSI must be above this
...
input int    Magic_Number       = 242025;         //Magic Number
input string Order_Comment      = "BSv24";        //Order comment
...
void OnTick()
{
   ...
   //--- BUY: H1 EMA200 up + M15 EMA20 up + M5 BB lower touch + RSI<40 + bullish
   if(Close[1] > h1_ema && Close[1] > m15_ema)
   {
   ...
   //--- SELL: H1 EMA200 down + M15 EMA20 down + M5 BB upper touch + RSI>60 + bearish
   if(Close[1] < h1_ema && Close[1] < m15_ema)
   {
