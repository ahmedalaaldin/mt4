//+------------------------------------------------------------------+
//|   Buy & Sell EA v194 - Faster Streak Scaling + Lower Base Risk   |
...
//|            Guard hurts returns, same DD -> guard is useless        |
//|     v193: BB 2.0 SD, relaxed RSI/Stoch -> 79.74%, 50.07%, Cal=1.6|
//|            CATASTROPHIC: lower WR (84%) + anti-martingale = 50% DD|
//|            BB 2.5 SD + Stoch 25/75 quality gate is NON-NEGOTIABLE |
//|   ROOT CAUSE ANALYSIS (DD):                                       |
//|     v186 DD driver: after 10-win streak (Scale=1.25, risk=8.75%), |
//|       1 full-SL loss = 8.75% → anti-martingale kicks in → 0.5x  |
//|       If 2nd loss: 3.5% more → total from peak: ~12%+             |
//|       DD accumulates across multiple such episodes over the year   |
//|   v194 STRATEGY: Reduce max effective risk by lowering base Risk_% |
//|     while reaching Scale_High FASTER to compensate for returns:    |
//|       Risk_Pct: 7.0% -> 6.0% (lower base = lower per-trade loss)  |
//|       WinStreak_Mid: 5 -> 3 (reach 1.10x after only 3 wins)       |
//|       WinStreak_High: 10 -> 6 (reach 1.25x after only 6 wins)     |
//|     Math: max effective risk at Scale_High = 6% x 1.25 = 7.5%    |
//|       vs v186: 7% x 1.25 = 8.75% → 14% reduction in peak risk     |
//|     At 90.7% WR, 6-win streaks happen every ~10 trades (common)   |
//|       vs 10-win streaks every ~20 trades → more time at 1.25x     |
//|     Estimated DD from 3 losses at max scale:                       |
//|       7.5% + 3% + 1.5% = 12% (vs v186: 8.75%+3.5%+1.75% = 14%) |
//|     Expected: lower DD (~15% vs 18.87%) with comparable return     |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "194.0"
#property strict
...
input double BB_Dev             = 2.5;            //BB SD: 2.5 (proven 90%+ WR - DO NOT CHANGE)

//--- Signal: M5 BB Touch
input string S1_SET             = "============"; //====== Signal: BB Touch ======
input int    RSI_Period         = 14;             //M5 RSI period
input double RSI_Buy            = 40.0;           //M5 RSI max for BUY (proven 90%+ WR)
...
input double Stoch_OS           = 25.0;           //Stoch oversold: BUY if K < this (proven optimal)
input double Stoch_OB           = 75.0;           //Stoch overbought: SELL if K > this (proven optimal)
...
input double ATR_TP_Multi       = 2.5;            //TP = ATR x 2.5 (proven optimal)
...
input double Risk_Pct           = 6.0;            //Risk%: 6.0 (v194: reduced from 7.0 for lower DD)
...
//--- Anti-Martingale (FASTER STREAK SCALING v194)
input string AM_SET             = "============"; //====== Anti-Martingale ======
input int    WinStreak_Mid      = 3;              //Wins for Scale_Mid (v194: 3 vs v186's 5)
input int    WinStreak_High     = 6;              //Wins for Scale_High (v194: 6 vs v186's 10)
input double Scale_Mid          = 1.10;           //Scale at mid streak (unchanged)
input double Scale_High         = 1.25;           //Scale at high streak (unchanged)
// v194 effect: At 90.7% WR, 6-win streaks happen every ~10 trades (very common)
// vs 10-win streaks every ~20 trades in v186. More time at 1.25x = better compounding.
// Max effective risk = 6% x 1.25 = 7.5% (vs v186: 7% x 1.25 = 8.75% = 14% lower peak risk)
...
input int    Magic_Number       = 1942025;        //Magic Number
input string Order_Comment      = "BSv194";       //Order comment
