//+------------------------------------------------------------------+
//|   Buy & Sell EA v211 - Wider Stop Loss (ATR_SL = 1.0)            |
...
//|   v211 HYPOTHESIS: ATR_SL_Multi = 1.0 (wider stop loss)           |
//|     Currently: SL = 0.8 × ATR. v211 tries: SL = 1.0 × ATR.      |
//|     Rationale: wider SL gives price more room to breathe before   |
//|     reversing toward the target. Key insight:                      |
//|       - Lot size = equity × 7% / SL_distance                     |
//|       - Wider SL → 20% smaller lots (risk % unchanged)            |
//|       - Price must travel 25% farther to hit the stop             |
//|       - Result: fewer false stop-outs, higher win rate             |
//|       - When stopped: still lose exactly 7% of equity             |
//|       - Net: higher win rate + same individual loss → better Calmar|
//|     ALL other parameters = v186 GLOBAL OPTIMUM.                   |
//|     Run with PERIOD = M5 in Strategy Tester.                      |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "211.0"
#property strict
...
//--- Exit Settings (v211 CHANGE: ATR_SL_Multi = 1.0)
input string EXIT_SET           = "============"; //====== Exit Settings ======
input int    ATR_Period         = 14;             //ATR period
input double ATR_SL_Multi       = 1.0;            //v211 CHANGE: SL = ATR x 1.0 (was 0.8)
...
input int    Magic_Number       = 2112038;        //Magic Number
input string Order_Comment      = "BSv211";       //Order comment
...
void OnTick()
{
   ...
   double slDist    = ATR_SL_Multi * atr1;   // v211: 1.0 * ATR (wider, was 0.8)
