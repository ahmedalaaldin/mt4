//+------------------------------------------------------------------+
//|   Buy & Sell EA v95 - v77 + ADX ranging-market filter           |
//|   Entry: H4 EMA200 + H1 EMA200 + H1 RSI 50-75                  |
//|          + BB 2.5SD lower touch + M5 RSI<40 + RSI rising        |
//|          + Stoch K&D<25 + Bull body 20%                         |
//|          + ADX < 25 (mean-reversion only in ranging markets)    |
//|   Exit: TP = BB mid, SL = 1xATR                                 |
//|   Goal: Filter trending breakouts that kill mean-reversion WR   |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "95.0"
#property strict
...
input double ADX_Max            = 25.0;           //Max ADX - skip entry if trending (>this)
...
input int    Magic_Number       = 952025;         //Magic Number
input string Order_Comment      = "BSv95";        //Order comment
...
void OnTick()
{
   ...
   // ADX ranging-market filter: skip if market is strongly trending
   double adx1 = iADX(NULL, 0, ADX_Period, PRICE_CLOSE, MODE_MAIN, 1);
   if(adx1 > ADX_Max) return;
