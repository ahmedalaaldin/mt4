//+------------------------------------------------------------------+
//|   Buy & Sell EA v198 - M15 Timeframe Test (same as v186 formula) |
//|   Framework: H4+H1 Direction / M15 Entry                         |
//|   LESSONS LEARNED:                                                |
...
//|       - Guards (v192): useless                                     |
//|       - BB relaxation (v193): catastrophic                         |
...
//|     CONCLUSION: v186 parameters are globally optimal for M5.       |
//|   v198 HYPOTHESIS: Same proven strategy on M15 timeframe           |
//|     Rationale: M15 candles contain 3× the data of M5 candles.     |
//|       BB(20) on M15 = 5 hours of smoothed price → less noise.      |
//|       RSI/Stoch on M15 = more reliable overbought/oversold signals. |
//|       Expected WR: possibly 92-95% (vs M5's 90.7%) due to filtering|
//|       Expected trades: ~80-100/year (1/3 of M5's 258)              |
//|       Expected ATR: √3× M5 ATR → larger $ moves per trade          |
//|       Risk management auto-adjusts via ATR-based lot sizing.        |
//|     Run with PERIOD = M15 in Strategy Tester.                       |
//|     All parameters same as v186 (proven optimal baseline).          |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "198.0"
#property strict
...
input int    RSI_Period         = 14;             //RSI period (M15 bars)
...
input int    Magic_Number       = 1982028;        //Magic Number
input string Order_Comment      = "BSv198";       //Order comment
...
void OnTick()
{
   ...
   //=== Signal BUY: H4 bullish + H1 bullish + BB2.5 touch + RSI<40 + Stoch<25
   ...
   //=== Signal SELL: H4 bearish + H1 bearish + BB2.5 touch + RSI>60 + Stoch>75
