//+------------------------------------------------------------------+
//|   Buy & Sell EA v277 - H1 ADX Range Filter                      |
//|   Framework: H1 EMA direction + H1 RSI gate / M5 Entry          |
//|   HYPOTHESIS: Many v269 losses happen during strong H1 trends    |
//|     (ADX high). BB mean-reversion fails when market is trending. |
//|     Adding H1 ADX < 25 filter ensures we only trade in           |
//|     consolidating/range-bound conditions where mean-reversion    |
//|     is most reliable. Expect: fewer trades, higher win%, lower   |
//|     MaxDD → significantly higher Calmar.                         |
//|   BASE: v269 CHAMPION (Calmar=17.77, Return=209%, 707 trades)   |
//|   CHANGE: Add H1 ADX < ADX_Max (default=25) gate               |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "277.0"
#property strict
...
//--- H1 ADX Filter (NEW in v277 - avoids strong trending markets)
input string ADX_SET            = "============"; //====== ADX Filter ======
input int    ADX_Period         = 14;             // ADX lookback period
input double ADX_Max            = 25.0;           // Only trade when H1 ADX < this (below = consolidating)
...
input int    Magic_Number       = 2772057;
input string Order_Comment      = "BSv277";
...
void OnTick()
{
   ...
   //--- H1 ADX Filter: only trade when H1 is ranging/consolidating (ADX < ADX_Max)
   double h1_adx = iADX(NULL, PERIOD_H1, ADX_Period, PRICE_CLOSE, MODE_MAIN, 1);
   bool adxFilter = (h1_adx < ADX_Max);
   if(!adxFilter) return;
