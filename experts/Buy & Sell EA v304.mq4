//+------------------------------------------------------------------+
//|   Buy & Sell EA v304 - Relaxed RSI Thresholds (45/55)           |
//|   BASE: v300 (Calmar=20.84, Return=258.39%, MaxDD=12.40%, 1040T) |
//|   CHANGE: RSI_Buy 40→45, RSI_Sell 60→55 (both sides relaxed)   |
//|   RATIONALE:                                                      |
//|     v301/v302/v303 all failed to improve Calmar beyond v300.    |
//|     The key lesson: longs+shorts diversification is ESSENTIAL.  |
//|     (v303 with H4 filter: 33 shorts → MaxDD=23.27% disaster).  |
//|     v302 HWM scaling: reduces wins as fast as losses → worse.  |
//|                                                                   |
//|     New approach: MORE TRADES via relaxed RSI gate.             |
//|     v300 used RSI_Buy<40 and RSI_Sell>60 (clear OB/OS).        |
//|     v304 uses RSI_Buy<45 and RSI_Sell>55 (approaching OB/OS).  |
//|     This captures "early mean-reversion" setups: RSI hasn't    |
//|     fully dipped to 40 yet but the BB touch + two-bar pattern  |
//|     + H1 trend all confirm the bounce starting.                |
//|                                                                   |
//|     Expected: ~1200-1400 trades vs 1040 in v300.               |
//|     More trades = more statistical diversification of losses.  |
//|     Both longs AND shorts increase proportionally.              |
//|     Win rate may drop slightly (~92% vs 93.37%).               |
//|     But total return should increase meaningfully (~290-320%). |
//|     MaxDD: ~11-13% (similar or slightly better diversification)|
//|     Calmar target: ~300/11 = 27 (vs 20.84 in v300)            |
//|                                                                   |
//|   SCOREBOARD:                                                    |
//|     v293: Calmar=18.43, Return=220.62%, MaxDD=11.97%, 637 T     |
//|     v300: Calmar=20.84, Return=258.39%, MaxDD=12.40%, 1040 T    |
//|     v301: FAILED  v302: Calmar=5.75  v303: Calmar=10.27         |
//|     v304: RSI 45/55 — TARGET Calmar>25                          |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "304.0"
#property strict
...
//--- BB Entry Signal — v304 KEY CHANGE: RSI_Buy=45, RSI_Sell=55
input string S1_SET             = "============"; //====== BB Signal ======
input int    BB_Period          = 20;
input double BB_Dev             = 2.5;
input int    RSI_Period         = 14;
input double RSI_Buy            = 45.0;   // v304: relaxed from 40 → 45
input double RSI_Sell           = 55.0;   // v304: relaxed from 60 → 55
...
input int    Magic_Number       = 3042057;
input string Order_Comment      = "BSv304";
...
void OnTick()
{
   ...
   //=== BUY === v304: RSI_Buy=45 (relaxed from 40)
   if(h1Bull)
   {
      bool bbTouch   = (Low[1] <= bbLower1) && (Close[1] > bbLower1);
      bool rsiOS     = (rsi1 < RSI_Buy);        // RSI < 45 (was < 40)
   ...
   //=== SELL === v304: RSI_Sell=55 (relaxed from 60)
   if(h1Bear)
   {
      bool bbTouchUp  = (High[1] >= bbUpper1) && (Close[1] < bbUpper1);
      bool rsiOB      = (rsi1 > RSI_Sell);      // RSI > 55 (was > 60)
