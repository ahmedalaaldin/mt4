//+------------------------------------------------------------------+
//|   Buy & Sell EA v33 - Dual BB Signal (Double Frequency)         |
//|   Signal A: BB lower touch + RSI<40 → TP at BB mid             |
//|   Signal B: BB mid cross-up + RSI 40-60 → TP at BB upper       |
//|   Both require H1 EMA200 + H1 RSI 50-75 bull trend             |
//|   Goal: ~2x trade frequency vs v18 with maintained WR          |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
...
input double H1_RSI_Max         = 75.0;           //H1 RSI max (same tight filter as v18)

//--- Bollinger Bands
input string BB_SET             = "============"; //====== Bollinger Band Settings ======
input int    BB_Period          = 20;             //BB period
input double BB_Dev             = 2.0;            //BB standard deviations
input double TP_Min_ATR         = 0.8;            //Min TP distance as ATR multiple

//--- Entry Signals
input string ENTRY_SET          = "============"; //====== Entry Settings ======
input int    RSI_Period         = 14;             //M5 RSI period
input double RSI_A_Max          = 40.0;           //Signal A: RSI must be below (oversold)
input double RSI_B_Min          = 40.0;           //Signal B: RSI min (not oversold)
input double RSI_B_Max          = 62.0;           //Signal B: RSI max (not overbought)
...
input int    Magic_Number       = 332025;         //Magic Number
input string Order_Comment      = "BSv33";        //Order comment
...
void OnTick()
{
   ...
   double bbMid2   = iBands(NULL, 0, BB_Period, BB_Dev, 0, PRICE_CLOSE, MODE_MAIN,  2);
   ...
   // Signal A: BB lower band touch (oversold bounce) → TP at BB midline
   bool sigA_bbTouch  = (Low[1] <= bbLower1) && (Close[1] > bbLower1);
   bool sigA_rsi      = (rsi1 < RSI_A_Max);
   bool sigA_bull     = (Close[1] > Open[1]);
   if(sigA_bbTouch && sigA_rsi && sigA_bull)
   {
      double tp = bbMid0;
      if(tp - entry >= minTP) { tpPrice = tp; signalFired = true; }
   }

   // Signal B: BB midline cross-up (momentum continuation) → TP at BB upper band
   bool sigB_cross = (Close[2] < bbMid2) && (Close[1] >= bbMid1) && (Close[1] > Open[1]);
   bool sigB_rsi   = (rsi1 >= RSI_B_Min) && (rsi1 <= RSI_B_Max);
   if(!signalFired && sigB_cross && sigB_rsi)
   {
      double tp = bbUpper1;
      if(tp - entry >= minTP) { tpPrice = tp; signalFired = true; }
   }

   if(signalFired && tpPrice > 0)
   {
