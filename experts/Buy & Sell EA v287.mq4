//+------------------------------------------------------------------+
//|   Buy & Sell EA v287 - ADX Trend Filter                          |
//|   BASE: v280 CHAMPION (Calmar=18.36, Return=220%, 637 trades)    |
//|   CHANGE: Add ADX filter - do not enter when ADX > ADX_Max.      |
//|     Mean reversion strategies fail in trending markets (ADX>25). |
//|     Consecutive losses in v280 cluster when market is trending.  |
//|     ADX<25 ensures we only trade in ranging/flat conditions where |
//|     BB band touches reliably snap back to the mean.              |
//|   HYPOTHESIS: ADX filter cuts MaxConsecLoss from 8→3-4,         |
//|     reducing MaxDD from ~12% toward 5-7%, improving Calmar.      |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "287.0"
#property strict
...
//--- ADX Trend Strength Filter (NEW in v287)
input string ADX_SET            = "============"; //====== ADX Filter ======
input bool   Use_ADX_Filter     = true;   // Block entries when market is strongly trending
input int    ADX_Period         = 14;     // ADX calculation period
input double ADX_Max            = 25.0;  // Max ADX allowed (>25 = trending = skip)
...
input int    Magic_Number       = 2872057;
input string Order_Comment      = "BSv287";
...
void OnTick()
{
   ...
   //--- ADX Trend Strength Filter (NEW v287)
   //    ADX > ADX_Max means the market is trending strongly - skip mean reversion entries
   double adx1 = iADX(NULL, 0, ADX_Period, PRICE_CLOSE, MODE_MAIN, 1);
   bool   adxOK = (!Use_ADX_Filter) || (adx1 > 0 && adx1 < ADX_Max);
   if(!adxOK) return;
