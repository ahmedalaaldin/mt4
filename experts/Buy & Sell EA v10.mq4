//+------------------------------------------------------------------+
//|   Buy & Sell EA v21 - BB/RSI OR Signal + Min TP Filter          |
//|   BUY: (BB lower touch OR RSI crossover up) + TP>=1xATR         |
//|   SELL: (BB upper touch OR RSI crossover down) + TP>=1xATR      |
//|   TP = BB midline, only when >=1xATR away (positive RR)         |
//|   SL = 0.8xATR (tighter SL for better RR)                      |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
...
input double RSI_Buy            = 40.0;           //BUY: RSI dip threshold (oversold)
input double RSI_Sell           = 60.0;           //SELL: RSI spike threshold (overbought)
input double RSI_Skip_OB        = 68.0;           //Skip BUY if RSI above this
input double RSI_Skip_OS        = 32.0;           //Skip SELL if RSI below this
...
input double ATR_SL_Multi       = 0.8;            //Stop Loss = ATR x this (tighter)
input int    Max_Bars           = 35;             //Force close after N bars
...
input int    Magic_Number       = 212025;         //Magic Number
input string Order_Comment      = "BSv21";        //Order comment
...
void OnTick()
{
   ...
   //--- BUY: above H1 EMA200
   if(Close[1] > h1_ema && rsi1 < RSI_Skip_OB)
   {
      // Signal A: bar dipped to/below BB lower band, bullish recovery close
      bool sigBB  = (Low[1] <= bbLower1) && (Close[1] > bbLower1) && (Close[1] > Open[1]);
      // Signal B: RSI crossed back above oversold threshold (bullish RSI recovery)
      bool sigRSI = (rsi2 < RSI_Buy) && (rsi1 >= RSI_Buy) && (Close[1] > Open[1]);
   ...
   //--- SELL: below H1 EMA200
   if(Close[1] < h1_ema && rsi1 > RSI_Skip_OS)
   {
      // Signal A: bar spiked to/above BB upper band, bearish recovery close
      bool sigBB  = (High[1] >= bbUpper1) && (Close[1] < bbUpper1) && (Close[1] < Open[1]);
      // Signal B: RSI crossed back below overbought threshold (bearish RSI reversal)
      bool sigRSI = (rsi2 > RSI_Sell) && (rsi1 <= RSI_Sell) && (Close[1] < Open[1]);
