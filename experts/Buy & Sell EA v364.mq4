//+------------------------------------------------------------------+
//|   Buy & Sell EA v364 - M1 BB signals added to M5+M15+M30+H4_EMA50 base |
//|   BASE: v358 (H4_EMA50 gate, M5+M15+M30, ATR_Spike=2.0) BEST ✓  |
//|                                                                   |
...
//|     v358: +H4_EMA(50) gate → pending backtest                   |
//|     v359: pending backtest                                       |
//|     v360: pending backtest                                       |
//|     v361: pending backtest                                       |
//|     v362: pending backtest                                       |
//|     v363: pending backtest                                       |
//|                                                                   |
//|   v364 HYPOTHESIS: Add M1 BB touch signals as additional entry  |
//|   type on top of v358 (M5+M15+M30+H4_EMA50 base). M1 bars fire  |
//|   5x more frequently than M5, potentially adding significant    |
//|   additional trades. Same two-bar BB(20,2.5) pattern, RSI<45,   |
//|   bullish body, and H1 Bull filter ensure quality. M1 ATR spike  |
//|   filter applied separately on M1 ATR series. OnTick is         |
//|   restructured so M1 checks occur between M5 bar boundaries,    |
//|   allowing M1 signals to fire on every new M1 bar without       |
//|   waiting for a new M5 bar.                                      |
//|                                                                   |
//|   v364 CHANGES from v358:                                        |
//|     Added: Use_M1_Signals input (default true)                   |
//|     Added: g_LastM1Bar global tracker                            |
//|     Restructured OnTick: M1 block runs on new M1 bars;          |
//|     M5/M15/M30 blocks guarded by separate isNewBar check after.  |
//|     M1 uses its own ATR series for SL/TP and spike filter.       |
//|     M1 OrderSend uses clrYellow color for visual distinction.    |
//|                                                                   |
...
input int    H4_EMA_Fast_Period = 50;             // v358: NEW — medium-term H4 trend gate
...
input bool   Use_M1_Signals     = true;           // v364: NEW — M1 BB touch signals
...
input int    Max_Bars           = 30;             // v343 baseline
...
input int    Magic_Number       = 3642057;        // v364 unique magic
input string Order_Comment      = "BSv364";
...
void OnTick()
{
   ...
   datetime m1BarTime = iTime(NULL, PERIOD_M1, 0);
   bool isNewM1Bar = (m1BarTime != g_LastM1Bar);
   if(isNewM1Bar) g_LastM1Bar = m1BarTime;
   ...
   ManageExits();
   bool anyNewBar = isNewBar || (Use_M1_Signals && isNewM1Bar);
   if(!anyNewBar) return;
   ...
   //--- Session + spread + trade count checks
   ...
   //--- Risk scale (shared by all TF blocks)
   double riskScale = GetRiskScale();

   //=== M1 BUY === (v364: fires on every new M1 bar, independent of M5)
   if(Use_M1_Signals && isNewM1Bar && h1Bull)
   {
      double m1_atr      = iATR(NULL, PERIOD_M1, ATR_Period, 1);
      double m1_atr_slow = iATR(NULL, PERIOD_M1, ATR_Spike_MA_Period, 1);

      if(m1_atr > 0)
      {
         // M1 ATR spike filter
         bool m1_spikeOk = true;
         if(ATR_Spike_Multi > 0 && m1_atr_slow > 0)
            m1_spikeOk = (m1_atr <= ATR_Spike_Multi * m1_atr_slow);

         if(m1_spikeOk)
         {
            int shift = 1;
            double m1_bbLower1 = iBands(NULL, PERIOD_M1, BB_Period, BB_Dev, 0, PRICE_CLOSE, MODE_LOWER, shift);
            double m1_bbLower2 = iBands(NULL, PERIOD_M1, BB_Period, BB_Dev, 0, PRICE_CLOSE, MODE_LOWER, shift + 1);
            double m1_rsi1     = iRSI(NULL, PERIOD_M1, RSI_Period, PRICE_CLOSE, shift);
            double m1_rsi2     = iRSI(NULL, PERIOD_M1, RSI_Period, PRICE_CLOSE, shift + 1);

            double m1_high1  = iHigh(NULL, PERIOD_M1, shift);
            double m1_low1   = iLow(NULL, PERIOD_M1, shift);
            double m1_close1 = iClose(NULL, PERIOD_M1, shift);
            double m1_open1  = iOpen(NULL, PERIOD_M1, shift);
            double m1_close2 = iClose(NULL, PERIOD_M1, shift + 1);

            double m1_range = m1_high1 - m1_low1;
            double m1_bodyB = m1_close1 - m1_open1;

            bool m1_bbTouch     = (m1_low1 <= m1_bbLower1) && (m1_close1 > m1_bbLower1);
            bool m1_rsiOS       = (m1_rsi1 < RSI_Buy);
            bool m1_rsiRising   = (m1_rsi1 > m1_rsi2);
            bool m1_bullBody    = (m1_bodyB > 0) && (m1_range > 0)
                                && (m1_bodyB >= Body_Pct * m1_range);
            bool m1_bbPrevBreak = (!Use_TwoBar_Pattern) || (m1_close2 <= m1_bbLower2);

            if(m1_bbTouch && m1_rsiOS && m1_rsiRising && m1_bullBody && m1_bbPrevBreak)
            {
               if(CountTradesByMagic(OP_BUY, Magic_Number) < Max_Buys)
               {
                  double m1_slDist = ATR_SL_Multi * m1_atr;
                  double m1_tpDist = ATR_TP_Multi * m1_atr;
                  double m1_lots   = CalcLots(m1_slDist, riskScale);
                  double entry     = NormalizeDouble(Ask, Digits);
                  double sl        = NormalizeDouble(entry - m1_slDist, Digits);
                  double tp        = NormalizeDouble(entry + m1_tpDist, Digits);
                  if(m1_lots > 0)
                  {
                     int tkt = OrderSend(Symbol(), OP_BUY, m1_lots, entry, Slippage,
                                         sl, tp, Order_Comment, Magic_Number, 0, clrYellow);
   ...
   // M5/M15/M30 signals: only on new M5 bar
   if(!isNewBar) return;
