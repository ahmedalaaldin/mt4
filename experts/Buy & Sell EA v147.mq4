//+------------------------------------------------------------------+
//|   Buy & Sell EA v147 - Max_Buys=1 (DD reduction test)           |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry          |
//|   Base: v144 (207.03% / 43.70% DD / 85.53% WR / 456 trades)    |
//|   Changes vs v144:                                               |
//|     1. Max_Buys: 2 → 1  Max_Sells: 2 → 1                       |
//|        Eliminate simultaneous positions that compound losses.    |
//|        Two concurrent BUY at 1.85× scale = 18.5% combined       |
//|        equity exposure. Both stopping simultaneously causes      |
//|        ~15-18% single-event equity drop driving the 43% DD.     |
//|        With Max_Buys=1: max single-event loss = 9.25% (1.85×)   |
//|     2. Risk_Pct: 5% → 6% (slight offset for fewer entries)      |
//|        With only 1 position allowed, some signals are missed     |
//|        while a position is open. Higher risk compensates.        |
//|   All other params unchanged from v144:                          |
//|     BB_Dev=2.5, RSI_Buy=40, Body_Pct=0.2                       |
//|     Scale_High=1.85, WinStreak_High=5, Scale_Mid=1.25           |
//|   Expected: ~170-190% return, ~20-28% relative DD              |
//|   v146 lesson: Body_Pct=0.1 adds trades but hurts anti-mart     |
//|     (disrupts win streaks, avg win falls $152→$122)             |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "147.0"
#property strict
...
input double Risk_Pct           = 6.0;            //Base risk % (6% vs 5% in v144)
input double Fixed_Lot          = 0.0;            //Fixed lot (0 = use Risk_Pct)
input int    Max_Buys           = 1;              //Max concurrent BUY (was 2, now 1)
input int    Max_Sells          = 1;              //Max concurrent SELL (was 2, now 1)
...
input int    Magic_Number       = 1472025;        //Magic Number
input string Order_Comment      = "BSv147";       //Order comment
...
void OnTick()
{
   ...
   if(h4Bull && h1Bull)
   {
      ...
      if(bbTouch && rsiOS && rsiRising && bullBody)
      {
         ...
         if(openBuys < Max_Buys && barsSince >= Min_Bar_Gap)  // Max_Buys=1
         {
   ...
   if(h4Bear && h1Bear)
   {
      ...
      if(bbTouchUp && rsiOB && rsiFalling && bearBody)
      {
         ...
         if(openSells < Max_Sells && barsSince >= Min_Bar_Gap)  // Max_Sells=1
         {
