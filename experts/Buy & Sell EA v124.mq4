//+------------------------------------------------------------------+
//|   Buy & Sell EA v124 - BB_Dev 2.5 + Wider H1 RSI + 75% Partial  |
//|   Framework: 4H Direction / 1H Structure / 5M Entry             |
//|   Base: v120 (224.50%, 14.95% DD, 15.0x ratio, gold standard)   |
//|   Changes vs v120:                                               |
//|     - BB_Dev: 2.5 (restored from v123's 2.2 — best quality)     |
//|     - H1 RSI widened: 48-78 buys / 22-52 sells (was 50-75/25-50)|
//|       Wider range → ~15% more qualifying signals                 |
//|     - Partial close: 75% at BB mid (was 50%) — lock profit early |
//|       25% trails with SL at BB_mid - Trail_ATR, low variance     |
//|     - Max_Daily_DD_Pct: 5.0% (was 15%) — tighter daily circuit  |
//|       breaker to limit cluster losses on bad days                |
//|   Why: v123 (BB=2.2) gave 34.87% DD & only 111.90% return.      |
//|   The tighter BB meant lower quality entries — definitively worse.|
//|   Back to 2.5 + wider RSI + tighter daily cap to reduce DD.     |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
...
input double H1_RSI_Min         = 48.0;           //H1 RSI min for BUY zone [was 50 in v120]
input double H1_RSI_Max         = 78.0;           //H1 RSI max for BUY [was 75 in v120]
input double H1_RSI_Min_Sell    = 22.0;           //H1 RSI min for SELL [was 25 in v120]
input double H1_RSI_Max_Sell    = 52.0;           //H1 RSI max for SELL [was 50 in v120]

//--- Bollinger Bands (M5)
input string BB_SET             = "============"; //====== Bollinger Band Settings ======
input int    BB_Period          = 20;             //BB period
input double BB_Dev             = 2.5;            //BB SD [restored to 2.5 — v120 optimal]
...
input double Partial_Close_Pct  = 0.75;           //Fraction to close at BB mid [was 0.5 in v120]
...
input int    Magic_Number       = 1342025;        //Magic Number
input string Order_Comment      = "BSv134";       //Order comment
input double Max_Daily_DD_Pct   = 5.0;            //Daily DD guard: stop day if equity drops >X%
...
void OnTick()
{
   ...
   //--- BUY: H4 bull + H1 above EMA + H1 RSI in wider bull zone (48-78)
   ...
   //--- SELL: H4 bear + H1 below EMA + H1 RSI in wider bear zone (22-52)
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(type == OP_BUY)
      {
         ...
         if(!partialDone && Bid >= bbMid)
         {
            ...
            double partLots   = MathFloor(OrderLots() * Partial_Close_Pct / stepLot) * stepLot;
