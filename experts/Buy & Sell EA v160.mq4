//+------------------------------------------------------------------+
//|   Buy & Sell EA v160 - WinStreak_High=2 (fastest scale onset)    |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry          |
//|   WinStreak breakthrough results (all Risk=6%, ATR=0.60%, TP=2x):|
//|     v152: WS=5, Scale=1.85 → 366.16% / 50.00% DD (ratio 7.32) ✓|
//|     v159: WS=3, Scale=1.85 → 484.01% / 52.00% DD (ratio 9.31)✓✓|
//|   BREAKTHROUGH: WS=3 gives ratio 9.31 vs 7.32 (+27%)            |
//|   Analysis: At 86.67% WR, P(3 consecutive wins) = 64.9%          |
//|     → 64% of trades at Scale_High=1.85× in v159                 |
//|     vs 48% at Scale_High in v152 (WS=5)                          |
//|     Result: +118% more return with only +2% more DD              |
//|   v160 Hypothesis: WinStreak_High=2 (scale at 2+ wins)          |
//|     P(2 consecutive wins) = 86.67%^2 = 75.1%                    |
//|     → ~75% of trades at Scale_High=1.85× (vs 65% at WS=3)       |
//|     Daily DD safety check:                                       |
//|       6% × 1.85 = 11.1% loss → then 0.5×6%=3% next trade       |
//|       Two losses: 11.1% + 3% = 14.1% < 15% guard ✓             |
//|     Set WinStreak_Mid=1 to maintain 3-tier scaling:              |
//|       1 win: Scale_Mid=1.25×                                     |
//|       2+ wins: Scale_High=1.85×                                  |
//|   Changes vs v159:                                               |
//|     1. WinStreak_High: 3 → 2                                   |
//|     2. WinStreak_Mid: 2 → 1                                    |
//|   All other params identical to v159:                            |
//|     Risk=6%, TP=2x, ATR=0.60%, Scale_High=1.85                 |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "160.0"
#property strict
...
input int    WinStreak_Mid      = 1;              //Consec wins for Scale_Mid: 2→1
input int    WinStreak_High     = 2;              //Consec wins for Scale_High: 3→2 (fastest onset)
input double Scale_Mid          = 1.25;           //Scale at 1 consecutive win
input double Scale_High         = 1.85;           //Scale at 2+ consecutive wins (proven safe)
...
input int    Magic_Number       = 1602025;        //Magic Number
input string Order_Comment      = "BSv160";       //Order comment
