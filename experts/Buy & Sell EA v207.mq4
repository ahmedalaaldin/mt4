//+------------------------------------------------------------------+
//|   Buy & Sell EA v207 - Max_Buys=2 Concurrent + Restore H1 RSI   |
...
//|       Restoring gate for v207.                                     |
//|   v207 HYPOTHESIS: Allow Max_Buys=2, Max_Sells=2                  |
//|     Currently capped at 1 concurrent position per side.            |
//|     With 2 allowed: during extended BB touches (multi-bar          |
//|     sell-offs), EA enters a second position on the next bar if     |
//|     conditions are still met. This:                                |
//|       - Increases total trade count                                |
//|       - Catches stronger mean-reversion moves with 2 entries       |
//|       - Risk managed by reducing Risk_Pct to 5% (from 7%)         |
//|         Max concurrent risk = 2 × 5% = 10% (vs v186's 7%)        |
//|     H1 RSI gate RESTORED to v186 optimal [45,75] / [25,55]        |
//|     All other parameters identical to v186 (proven optimal).       |
//|     Run with PERIOD = M5 in Strategy Tester.                       |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "207.0"
#property strict
...
//--- H1 RSI Gate (v207: RESTORED to v186 optimal values)
input string H1RSI_SET          = "============"; //====== H1 RSI Gate ======
input int    H1_RSI_Period      = 14;             //H1 RSI period
input double H1_RSI_Min         = 45.0;           //v207: RESTORED — was 0.0 in v206
input double H1_RSI_Max         = 75.0;           //v207: RESTORED — was 100.0 in v206
input double H1_RSI_Min_Sell    = 25.0;           //v207: RESTORED — was 0.0 in v206
input double H1_RSI_Max_Sell    = 55.0;           //v207: RESTORED — was 100.0 in v206
...
//--- Money Management (v207: Risk_Pct reduced to 5% for concurrent position safety)
input string MONEY_SET          = "============"; //====== Money Management ======
input double Risk_Pct           = 5.0;            //v207: 5% (from 7%) — max 10% concurrent risk
input double Fixed_Lot          = 0.0;            //Fixed lot (0 = use Risk_Pct)
input int    Max_Buys           = 2;              //v207: Allow 2 concurrent BUYs (from 1)
input int    Max_Sells          = 2;              //v207: Allow 2 concurrent SELLs (from 1)
...
input int    Magic_Number       = 2072037;        //Magic Number
input string Order_Comment      = "BSv207";       //Order comment
...
void OnTick()
{
   ...
   // H4 + H1 EMA direction filter + H1 RSI gate (v207: RESTORED to v186 optimal)
   ...
   // h1Bull: H1 close above H1 EMA + H1 RSI in [45,75] (restored v186 gate)
   ...
   //=== v207: Max_Buys=2 — enters second position if conditions re-fire next bar
   ...
   //=== v207: Max_Sells=2 — enters second position if conditions re-fire next bar
