//+------------------------------------------------------------------+
//|   Buy & Sell EA v213 - Remove Stoch Level Gate (Direction Only)   |
...
//|   v213 HYPOTHESIS: Remove Stochastic LEVEL gate (direction only)   |
//|     Current: BUY if Stoch K < 25 (oversold) AND K rising           |
//|     v213:    BUY if K rising ONLY (Stoch_OS=100 = always true)    |
//|              SELL if K falling ONLY (Stoch_OB=0 = always true)    |
//|     Rationale: The oversold/overbought LEVEL condition may be too  |
//|       restrictive. The DIRECTION (rising/falling) already confirms  |
//|       momentum alignment. Removing the level filter should:         |
//|       - Significantly increase trade count (K<25 is very rare)     |
//|       - Maintain direction confirmation quality                     |
//|       - More trades = better compounding = potentially higher Calmar|
//|     ALL other parameters = v186 GLOBAL OPTIMUM.                   |
//|     Run with PERIOD = M5 in Strategy Tester.                      |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "213.0"
#property strict
...
//--- Stochastic Gate (v213 CHANGE: Level filter removed, direction only)
...
input double Stoch_OS           = 100.0;          //v213 CHANGE: BUY if K < 100 (always true) = direction only
input double Stoch_OB           = 0.0;            //v213 CHANGE: SELL if K > 0 (always true) = direction only
...
input int    Magic_Number       = 2132038;        //Magic Number
input string Order_Comment      = "BSv213";       //Order comment
