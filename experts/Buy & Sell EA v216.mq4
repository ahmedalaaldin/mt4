//+------------------------------------------------------------------+
//|   Buy & Sell EA v216 - Limit Pending Orders (Better Entry Price) |
...
//|       69 bad trades. Stoch_OS=25, Stoch_OB=75 = GLOBAL OPTIMUM.   |
//|     v214: Max_Sells=0 (long-only) -> +188.82%, DD 22.39%, Cal 8.43|
//|       LESSON: Shorts contribute +49% extra return AND reduce DD!   |
//|       Sells HEDGE long losses during drops. Both sides = OPTIMAL.  |
//|     v215: CCI(14) filter -> +151.21%, DD 29.75%, Cal=5.08         |
//|       LESSON: CCI<-100 is a MOMENTUM indicator — selects for price |
//|       continuation, NOT reversal. Filters out mean-reversion wins,  |
//|       keeps the worst losers. Every extra oscillator = WORSE.      |
//|   v216 HYPOTHESIS: Limit pending orders for better entry fills     |
//|     Instead of market orders, use OP_BUYLIMIT / OP_SELLLIMIT.     |
//|     BUY LIMIT = Close[1] - LimitOffset_ATR*ATR (below signal bar) |
//|     SELL LIMIT = Close[1] + LimitOffset_ATR*ATR (above signal bar)|
//|     After BB breakout, price often dips briefly before reversing.  |
//|     A limit captures a 0.2-ATR better fill on each trade entry.   |
//|     Better entry = lower effective SL risk = better profit factor.  |
//|     Orders expire after Limit_ExpireBars (3 bars = 15 min).       |
//|     ALL filter parameters = v186 GLOBAL OPTIMUM (no CCI).         |
//|     If Calmar > 12.60: limit entries provide meaningful edge       |
//|     If Calmar < 12.60: price bounces too quickly, misses fills     |
//|     Run with PERIOD = M5 in Strategy Tester.                       |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "216.0"
#property strict
...
//--- Stochastic Gate (v186 GLOBAL OPTIMUM — K<25 buy, K>75 sell + direction)
...
//--- Limit Order Settings (v216 NEW — replaces market orders)
input string LIMIT_SET          = "============"; //====== Limit Order Settings ======
input double LimitOffset_ATR    = 0.2;            //Limit placed X ATR below/above signal bar close
input int    Limit_ExpireBars   = 3;              //Cancel pending order if unfilled after N bars
...
input int    Magic_Number       = 2162038;        //Magic Number
input string Order_Comment      = "BSv216";       //Order comment
...
//+------------------------------------------------------------------+
// Count open OR pending orders of a given type for this EA
...
void ManageExits()
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      // Skip pending orders — they have no open position to manage
...
void OnTick()
{
   ...
   // v186 GLOBAL OPTIMUM: K<25 oversold level AND K rising (direction) for buy
   bool stochBuy  = (stoch_k1 < Stoch_OS) && (stoch_k1 > stoch_k2);  // K<25 + K rising
   bool stochSell = (stoch_k1 > Stoch_OB) && (stoch_k1 < stoch_k2);  // K>75 + K falling

   double slDist    = ATR_SL_Multi * atr1;   // v186 optimum: 0.8 * ATR
   double tpDist    = ATR_TP_Multi * atr1;
   double limitOff  = LimitOffset_ATR * atr1; // v216: limit offset below/above signal bar close
   double riskScale = GetRiskScale();
   datetime expiry  = Time[0] + Limit_ExpireBars * PeriodSeconds();

   double candleRange = High[1] - Low[1];
   double candleBodyB = Close[1] - Open[1];
   double candleBodyS = Open[1] - Close[1];

   //=== Signal BUY: H4 bull + H1 bull(EMA+RSI[45,75]) + BB(20,2.5) + RSI<40 + Stoch<25 rising
   //    v216: Place BUY LIMIT at Close[1] - LimitOffset_ATR*ATR (better fill below signal bar)
   if(h4Bull && h1Bull && stochBuy)
   {
      ...
      if(bbTouch && rsiOS && rsiRising && bullBody)
      {
         ...
         int pendingBuys = CountTradesByMagic(OP_BUYLIMIT,  Magic_Number);
         int barsSince   = (int)((Time[0] - g_LastBuyTime) / PeriodSeconds());
         if((openBuys + pendingBuys) < Max_Buys && barsSince >= Min_Bar_Gap)
         {
            // v216: Limit price placed below the signal bar close
            double limitPrice = NormalizeDouble(Close[1] - limitOff, Digits);
            double sl         = NormalizeDouble(limitPrice - slDist, Digits);
            double tp         = NormalizeDouble(limitPrice + tpDist, Digits);
            double lots       = CalcLots(slDist, riskScale);
            // Validate: BUY LIMIT must be below current Ask
            if(lots > 0 && limitPrice < Ask)
            {
               int tkt = OrderSend(Symbol(), OP_BUYLIMIT, lots, limitPrice, 0,
   ...
   //=== Signal SELL: H4 bear + H1 bear(EMA+RSI[25,55]) + BB(20,2.5) + RSI>60 + Stoch>75 falling
   //    v216: Place SELL LIMIT at Close[1] + LimitOffset_ATR*ATR (better fill above signal bar)
   if(h4Bear && h1Bear && stochSell)
   {
      ...
      if(bbTouchUp && rsiOB && rsiFalling && bearBody)
      {
         ...
         int pendingSells  = CountTradesByMagic(OP_SELLLIMIT, Magic_Number);
         int barsSince     = (int)((Time[0] - g_LastSellTime) / PeriodSeconds());
         if((openSells + pendingSells) < Max_Sells && barsSince >= Min_Bar_Gap)
         {
            // v216: Limit price placed above the signal bar close
            double limitPrice = NormalizeDouble(Close[1] + limitOff, Digits);
            double sl         = NormalizeDouble(limitPrice + slDist, Digits);
            double tp         = NormalizeDouble(limitPrice - tpDist, Digits);
            double lots       = CalcLots(slDist, riskScale);
            // Validate: SELL LIMIT must be above current Bid
            if(lots > 0 && limitPrice > Bid)
            {
               int tkt = OrderSend(Symbol(), OP_SELLLIMIT, lots, limitPrice, 0,
                                   sl, tp, Order_Comment, Magic_Number, expiry, clrRed);
