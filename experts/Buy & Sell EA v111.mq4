//+------------------------------------------------------------------+
//|   Buy & Sell EA v111 - v104 Base + Max_Buys=3 Triple Compound  |
//|   Framework: 4H Direction / 1H Structure / 5M Entry             |
//|   Base: v104 AllBuysRiskFree gate (173.83% proven)              |
//|   Innovation:                                                    |
//|     - Max_Buys = 3: triple compounding via AllBuysRiskFree gate |
//|       Sequence: P1 opens → P1 at BE → P2 opens → P2 at BE →   |
//|       P3 opens. At any tick only 1 position at full risk (4.9%)|
//|     - H1_RSI_Min = 50: restored to v104's optimal value        |
//|       (v110 proved 45 lets in low-quality entries, -WR, +DD)   |
//|     - Min_Bar_Gap = 1: faster re-entry (tested OK in v108)      |
//|     - AllBuysRiskFree gate unchanged: ALL open buys must have  |
//|       SL >= openPrice before a new position can open            |
//|   Goal: Exceed 200% return by adding one more compounding layer |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "121.0"
#property strict
...
input double H1_RSI_Min         = 50.0;           //H1 RSI min (v104 optimal — 45 hurt WR)
...
input int    Max_Buys           = 3;              //Max concurrent BUY positions (triple compound)
...
input int    Magic_Number       = 1212025;        //Magic Number
input string Order_Comment      = "BSv121";       //Order comment
...
void OnTick()
{
   ...
   // 1H: Structure + RSI in zone (H1_RSI_Min=50 restored — proven optimal in v104)
   ...
   if(bbTouch && rsiOS && rsiRising && stochOS && bullBody)
   {
      ...
      if(tpDist >= minTP)
      {
         ...
         // Gate: only open 2nd/3rd position when ALL existing buys are risk-free (SL >= openPrice)
