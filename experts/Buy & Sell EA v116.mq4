//+------------------------------------------------------------------+
//|   Buy & Sell EA v116 - Break-Even Stop + Partial Close          |
//|   Framework: 4H Direction / 1H Structure / 5M Entry             |
//|   Base: v114 (217.78%, 14.93% DD)                               |
//|   Key change:                                                    |
//|     - Early break-even: when price moves BE_ATR_Multi×ATR       |
//|       in favor, SL moves to break-even BEFORE BB mid partial    |
//|       close — eliminates losses from "reversed after unrealized  |
//|       profit" scenario                                           |
//|     - Partial close (50% at BB mid) + trailing kept intact       |
//|     - Partial-close state tracked via circular ticket buffer     |
//|       (avoids clash with BE-triggered SL=openPrice)             |
//|   Hypothesis: lower DD by eliminating large losing trades that   |
//|     occurred after unrealized profit peaked                      |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "126.0"
#property strict
...
input double BE_ATR_Multi       = 0.5;            //Move SL to BE when price moves N×ATR in favor
...
input int    Magic_Number       = 1262025;        //Magic Number
input string Order_Comment      = "BSv126";       //Order comment
...
// Circular buffer to track which tickets have had partial close done
int g_PC_Tickets[20];
int g_PC_Index   = 0;
...
{
   for(int i = 0; i < 20; i++)
      if(g_PC_Tickets[i] == ticket) return true;
   return false;
}
...
void MarkPartialCloseDone(int ticket)
{
   ...
   {
      g_PC_Tickets[g_PC_Index % 20] = ticket;
      g_PC_Index++;
   }
}
...
int OnInit()
{
   ArrayInitialize(g_PC_Tickets, 0);
...
void ManageExits()
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(type == OP_BUY)
      {
         bool partialDone = IsPartialCloseDone(tkt);

         // Stage 1: Early break-even — move SL to openPrice when price moves BE_ATR_Multi×ATR in favor
         if(!partialDone && Bid >= openPrice + BE_ATR_Multi * atr && currentSL < openPrice)
            OrderModify(tkt, openPrice, NormalizeDouble(openPrice, Digits), 0, 0, clrBlue);

         // Stage 2: Partial close at BB mid
         ...
         // Stage 3: Trail remaining after partial close
         if(partialDone)
         {
      ...
      else if(type == OP_SELL)
      {
         ...
         // Stage 1: Early break-even — move SL to openPrice when price moves BE_ATR_Multi×ATR in favor
         if(!partialDone && Ask <= openPrice - BE_ATR_Multi * atr && currentSL > openPrice)
            OrderModify(tkt, openPrice, NormalizeDouble(openPrice, Digits), 0, 0, clrBlue);
