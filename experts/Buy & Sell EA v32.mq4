//+------------------------------------------------------------------+
//|   Buy & Sell EA v43 - M1 RSI Scalper (High Frequency)          |
//|   Run on M1 chart. H1 EMA200 trend filter + H1 RSI 50-75       |
//|   Entry: M1 RSI crosses up from below 30 + bullish bar         |
//|   TP = 1.0xATR, SL = 0.7xATR (tight, high frequency)          |
//|   Goal: 10-20+ trades/day to push toward 200% annual return    |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
...
input double H1_RSI_Min         = 50.0;           //H1 RSI min (bullish)
input double H1_RSI_Max         = 75.0;           //H1 RSI max (not overbought)

//--- Entry Signal
input string ENTRY_SET          = "============"; //====== Entry Settings ======
input int    RSI_Period         = 7;              //M1 RSI period (faster)
input double RSI_Oversold       = 30.0;           //RSI oversold threshold
input int    Min_Bar_Gap        = 5;              //Min M1 bars between entries
...
input double ATR_SL_Multi       = 0.7;            //SL = ATR x this
input int    Max_Bars           = 30;             //Force close after N M1 bars

//--- Money Management
input string MONEY_SET          = "============"; //====== Money Management ======
input double Risk_Pct           = 1.0;            //Risk % per trade (lower for M1 noise)
...
input int    Magic_Number       = 432025;         //Magic Number
input string Order_Comment      = "BSv43";        //Order comment
input double Max_Daily_DD_Pct   = 4.0;            //Daily drawdown guard (% of equity)
input int    Max_Trades_Day     = 200;            //Max trades per day
...
void OnTick()
{
   ...
   // M1 RSI crossover from oversold: bar[2] < 30, bar[1] > 30 (crossover on previous bar)
   double rsi1 = iRSI(NULL, 0, RSI_Period, PRICE_CLOSE, 1);
   double rsi2 = iRSI(NULL, 0, RSI_Period, PRICE_CLOSE, 2);

   bool rsiCross  = (rsi2 < RSI_Oversold) && (rsi1 >= RSI_Oversold);
   bool bullClose = (Close[1] > Open[1]);

   if(rsiCross && bullClose)
   {
