//+------------------------------------------------------------------+
//|   Buy & Sell EA v296 - Larger Partial Close (35% vs 20%)         |
//|   BASE: v293/v280 CHAMPION (Calmar=18.43, Return=220.62%, 637 T) |
//|   CHANGE: Partial_Pct = 0.35 (was 0.20)                         |
//|   RATIONALE:                                                      |
//|     The partial close structure is:                              |
//|       1. Close 20% of position at BB_mid → small guaranteed gain |
//|       2. Move SL to breakeven                                    |
//|       3. Trail remaining 80% at bbMid - 0.1×ATR (VERY tight)    |
//|     With Trail_ATR_Multi=0.1, the trail is only 10% of ATR      |
//|     below BB_mid — essentially exits very close to BB_mid.       |
//|     Analysis: For BB_Dev=2.5, BB_mid is ~2.1×ATR above entry.  |
//|     Trail profit per remaining lot ≈ 0.3-0.7×ATR (tight trail)  |
//|     Partial profit per lot ≈ 2.1×ATR >> trail profit.           |
//|     THEREFORE: Increasing partial from 20%→35% locks in more    |
//|     guaranteed profit (2.1×ATR per extra 15%) relative to the   |
//|     trail loss (0.5×ATR per reduced 15%). Net gain per winner.   |
//|   HYPOTHESIS: Same 637 trades, higher profit per winner,        |
//|     Return improves to ~250-260%, MaxDD similar ~12%,           |
//|     Calmar improves to ~21-22.                                  |
//|   SCOREBOARD:                                                    |
...
//|     v295: Calmar=10.11 (Longs only — DISASTER, lost diversif.)  |
//|     v296: Partial 35% — TARGET >20                              |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "296.0"
#property strict

#include <stdlib.mqh>

//--- Session (same as v280/v293: 9-21)
...
//--- Exit Settings (KEY CHANGE: Partial_Pct 0.20 → 0.35)
//    Lock in 35% of position at BB_mid (guaranteed partial gain).
//    Remaining 65% trails at bbMid - 0.1×ATR.
//    Since partial gain >> tight trail gain per unit, this improves
//    per-winner profit. Losing trades (SL hits) unaffected.
...
input double Partial_Pct        = 0.35;  // KEY CHANGE: was 0.20, now 0.35
...
input int    Magic_Number       = 2962057;
input string Order_Comment      = "BSv296";
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(type == OP_BUY)
      {
         ...
         if(!partialDone && Bid >= bbMid)
         {
            double stepLot  = MarketInfo(Symbol(), MODE_LOTSTEP);
            double minLot   = MarketInfo(Symbol(), MODE_MINLOT);
            // KEY CHANGE: partLots now uses 35% instead of 20%
