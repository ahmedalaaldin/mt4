//+------------------------------------------------------------------+
//|   Buy & Sell EA v217 - Early Breakeven Guard (0.5 ATR trigger)  |
...
//|       continuation, NOT reversal. Every extra oscillator = WORSE.  |
//|     v216: Limit orders 0.2 ATR offset -> +207.61%, DD 23.86%, Cal=8.70|
//|       LESSON: Limit orders miss 59 winning trades (258→199). Price |
//|       bounces immediately after BB break — limits don't fill.     |
//|       Market orders = OPTIMAL. Limits hurt both return AND DD.    |
//|   v217 HYPOTHESIS: Early Breakeven Guard (0.5 ATR trigger)        |
//|     After entry, when price moves 0.5*ATR in our favor, move SL   |
//|     to entry price IMMEDIATELY (before partial close at BB midline)|
//|     Rationale: Locks in risk-free status early. If trade reverses  |
//|     before hitting BB midline, exit at BE (not a full loss).      |
//|     MaxDD reduction target: losing trades that had initial float   |
//|     will now close at 0 instead of -0.8 ATR.                      |
//|     If Calmar > 12.60: early BE reduces DD more than return       |
//|     If Calmar < 12.60: early BE kills too many potential winners  |
//|     (trades that dip back to entry before rallying to BB midline)  |
//|     ALL other parameters = v186 GLOBAL OPTIMUM.                   |
//|     Run with PERIOD = M5 in Strategy Tester.                       |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "217.0"
#property strict
...
//--- Early Breakeven Guard (v217 NEW)
input string BE_SET             = "============"; //====== Early Breakeven Guard ======
input double EarlyBE_ATR        = 0.5;            //Move SL to entry when profit >= X * ATR
...
input int    Magic_Number       = 2172038;        //Magic Number
input string Order_Comment      = "BSv217";       //Order comment
...
void ManageExits()
{
   ...
   double earlyBE_Dist = EarlyBE_ATR * atr;  // v217: early BE trigger distance
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(type == OP_BUY)
      {
         // v217 NEW: Early BE guard — when price > entry + EarlyBE_ATR*ATR, move SL to entry
         if(currentSL < openPrice && Bid >= openPrice + earlyBE_Dist)
         {
            OrderModify(tkt, openPrice, NormalizeDouble(openPrice, Digits), 0, 0, clrBlue);
            currentSL = openPrice;  // update local copy
         }
      ...
      {
         // v217 NEW: Early BE guard — when price < entry - EarlyBE_ATR*ATR, move SL to entry
         if((currentSL > openPrice || currentSL == 0) && Ask <= openPrice - earlyBE_Dist)
         {
            OrderModify(tkt, openPrice, NormalizeDouble(openPrice, Digits), 0, 0, clrBlue);
            currentSL = openPrice;  // update local copy
         }
