//+------------------------------------------------------------------+
//|   Buy & Sell EA v289 - Bollinger Band Width Filter               |
//|   BASE: v280 CHAMPION (Calmar=18.36, Return=220%, 637 trades)    |
//|   CHANGE: BB Width filter - only enter when bands are narrow.    |
//|   RATIONALE:                                                      |
//|     v287 (M5 ADX<25)  → 85% trade reduction, too strict         |
//|     v288 (H1 ADX<30)  → results pending                         |
//|     BB Width filter is NATIVE to the BB mean-reversion strategy: |
//|     - Narrow bands (BB_Width < 5×ATR) = ranging market → good   |
//|     - Wide bands (BB_Width > 5×ATR) = trending market → bad     |
//|     When BB bands are expanded, any 2.5σ touch represents a      |
//|     momentum move, NOT a mean-reversion opportunity.             |
//|   HYPOTHESIS: BB_Width/ATR < 5.5 removes entries during sustained|
//|     trends (10-20% of trades), reducing MaxConsecLoss while      |
//|     keeping trade count in the 500-580 range.                    |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "289.0"
#property strict
...
input double BB_Width_ATR_Max   = 5.5;   // Skip when BB_Width > 5.5 × ATR(14)
...
//--- Anti-Martingale (v269 confirmed optimal)
...
input int    Magic_Number       = 2892057;
input string Order_Comment      = "BSv289";
