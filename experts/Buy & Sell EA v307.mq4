//+------------------------------------------------------------------+
//|   Buy & Sell EA v307 - Larger Partial + Wider Trail               |
//|   BASE: v300 CHAMPION (Calmar=20.84, Return=258.39%, 1040 T)      |
//|   CHANGES:                                                         |
//|     1) Partial_Pct: 0.20 → 0.50 (bank more at bbMid)             |
//|     2) Trail_ATR_Multi: 0.1 → 0.3 (more room for remaining 50%) |
//|   RATIONALE:                                                       |
//|     v300 closes only 20% at bbMid then trails 80% tightly.       |
//|     The tight trail (0.1×ATR) exits the remainder quickly after   |
//|     any minor pullback from bbMid, leaving most potential gain    |
//|     unrealized. By banking 50% at bbMid, we lock in more profit  |
//|     early — reducing MaxDD as more gains are secured. The wider  |
//|     trail (0.3×ATR) gives the remaining 50% more room to capture |
//|     larger moves without being stopped by normal volatility.      |
//|     Combined: higher per-trade expected value + lower MaxDD from  |
//|     banked profits.                                               |
//|   HYPOTHESIS: Return 250-280%, MaxDD ~8-10%, Calmar ~25-35       |
//|   DEAD ENDS SO FAR:                                               |
//|     - RSI_Buy=45/RSI_Sell=55 (v304): MaxDD=36.43%, Calmar=6.49  |
//|     - RSI_Buy=35/RSI_Sell=65 (v305): Return=99.75%, Calmar=4.39 |
//|     - BB_Period=14 (v306): Return=36.96%, Calmar=1.49            |
//|     - HWM scaling (v302): Calmar=5.75                            |
//|     - H4 EMA filter (v303): MaxDD=23.27%                        |
//|   SCOREBOARD:                                                     |
//|     v300: Calmar=20.84 (champion — stoch gate removed)           |
//|     v307: TARGET Calmar>25 via profit-banking + trail improvement |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "307.0"
#property strict
...
//--- Exit Settings — KEY CHANGES in v307
input string EXIT_SET           = "============"; //====== Exit Settings ======
input int    ATR_Period         = 14;
input double ATR_SL_Multi       = 0.8;
input double ATR_TP_Multi       = 5.0;
input double Trail_ATR_Multi    = 0.3;   // v307: 0.1 → 0.3 (wider trail for remaining position)
input double Partial_Pct        = 0.50;  // v307: 0.20 → 0.50 (bank more at bbMid)
...
input int    Magic_Number       = 3072057;
input string Order_Comment      = "BSv307";
...
void ManageExits()
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(type == OP_BUY)
      {
         ...
         if(partialDone)
         {
            // v307: Trail_ATR_Multi=0.3 gives wider trail buffer (was 0.1)
