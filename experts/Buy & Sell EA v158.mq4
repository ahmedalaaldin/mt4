//+------------------------------------------------------------------+
//|   Buy & Sell EA v158 - Scale_High=2.0 (vs 1.85) at Risk=6%      |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry          |
//|   Risk/Return table (all Risk=6%, TP=2x, ATR=0.60%):            |
//|     v152: Scale=1.85, WS=5 → 366.16% / 50.00% DD (ratio 7.32) ✓|
//|     v155: TP=3.0x        → 361.22% / 50.00% DD (ratio 7.22)    |
//|     v156: TP=1.5x        → 227.16% / 46.47% DD (ratio 4.89)    |
//|     v157: ATR=0.45%      → 366.14% / 50.00% DD (ratio 7.32) =  |
//|   Key findings:                                                   |
//|     - TP=2.0x is optimal exit (1.5x and 3.0x confirmed worse)   |
//|     - ATR filter saturated at 0.60% (tighter = identical result) |
//|     - Return/DD ceiling at 7.32 with current settings           |
//|   v158 Hypothesis: Scale_High 1.85 → 2.0 at Risk=6%            |
//|     Daily DD analysis: 6% × 2.0 = 12% per streak trade          |
//|     After loss from streak: next at 0.5×6%=3% (anti-martingale) |
//|     Two consecutive losses max: 12% + 3% = 15% → at guard limit |
//|     Expected: higher return on long win streaks, similar DD      |
//|     If streak compounding gain > boundary cost → ratio improves  |
//|   Changes vs v152:                                               |
//|     1. Scale_High: 1.85 → 2.0                                  |
//|   All other params identical to v152 (best performer):           |
//|     Risk=6%, ATR=0.60%, TP=2x, WinStreak_High=5                |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "158.0"
#property strict
...
input double Scale_High         = 2.0;            //Scale at high streak: 1.85→2.0 (higher compound)
...
input int    Magic_Number       = 1582025;        //Magic Number
input string Order_Comment      = "BSv158";       //Order comment
