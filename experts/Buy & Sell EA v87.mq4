//+------------------------------------------------------------------+
//|   Buy & Sell EA v98 - v97 + Fixed-$ risk sizing                 |
...
//|   Change: Fixed_Risk_USD replaces % sizing on equity            |
//|           Decouples compounding equity from loss size            |
//|   Goal: Keep DD small even at peak equity (target <5%)         |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "98.0"
#property strict
...
input double Risk_Pct           = 7.0;            //Risk % per trade (used if Fixed_Risk_USD=0)
input double Fixed_Risk_USD     = 700.0;          //Fixed $ risk per trade (0 = use Risk_Pct)
input double Loss_Risk_Scale    = 0.5;            //Risk multiplier after a loss (0.5 = half risk)
input double Fixed_Lot          = 0.0;            //Fixed lot (0 = use risk calculation)
...
input int    Magic_Number       = 982025;         //Magic Number
input string Order_Comment      = "BSv98";        //Order comment
...
double CalcLots(double slDist, double riskScale = 1.0)
{
   ...
   double riskAmt;
   if(Fixed_Risk_USD > 0)
      riskAmt = Fixed_Risk_USD * riskScale;
   else
      riskAmt = AccountEquity() * (Risk_Pct * riskScale) / 100.0;
