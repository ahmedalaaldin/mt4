//+------------------------------------------------------------------+
//|   Buy & Sell EA v336 - RSI_Period 14 → 7 (faster RSI)           |
//|   BASE: v328 (BEST BASELINE: +16.25%, MaxDD=4.99%, WR=83.47%)   |
//|                                                                   |
//|   v335 POST-MORTEM (367 trades, +4.96%, MaxDD=5.83%):            |
//|     FAILURE: WR=71.12% — RSI<45 filter is CRITICAL              |
//|     Raising RSI_Buy to 55 kills WR. RSI_Buy=45 is NON-NEGOTIABLE|
//|     Fewer trades with RSI_Buy=55 (367 vs 380) because trades at  |
//|     RSI=45-55 are less likely to meet rsiRising condition        |
//|                                                                   |
//|   v336 HYPOTHESIS:                                               |
//|   RSI(14) is SLOW — reacts in 14 bars. At XAUUSD M5 BB 2.5σ    |
//|   touches (sharp, fast moves), RSI(14) may not be <45 at the     |
//|   exact touch moment. RSI(7) reacts in 7 bars — 2x faster.      |
//|   RSI(7) should be BELOW 45 more often during BB touches AND    |
//|   should bounce (rsiRising) more decisively from extreme values.  |
//|   Goal: More signals that qualify, WR maintained near 83%.       |
//|                                                                   |
//|   v336 CHANGE (1 parameter only):                                |
//|     RSI_Period: 14 → 7                                           |
//|     RSI_Buy stays at 45.0 (NON-NEGOTIABLE from v335 lesson)     |
//|     All other parameters IDENTICAL to v328.                      |
//|                                                                   |
//|   SCOREBOARD:                                                     |
//|     v328: +16.25%, MaxDD=4.99%, WR=83.47%, 380 trades  BEST ✓  |
//|     v329: BB_Dev=2.0                                  FAIL ✗    |
//|     v331: Partial=0.40 → WR=77.47%                   FAIL ✗    |
//|     v332: SL=0.65 → WR=70.09%                        FAIL ✗    |
//|     v333: SL=1.0 → +8.08%, WR=85.45%                FAIL ✗    |
//|     v334: BB_Period=14 → WR=64.54%                   FAIL ✗    |
//|     v335: RSI_Buy=55 → WR=71.12%                    FAIL ✗    |
//|     v336: RSI_Period=7 → ??? (faster RSI, more signals?)        |
//|                                                                   |
...
//|     RSI_Buy=45.0 (CONFIRMED CRITICAL — v335 proved relaxing kills|
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "336.0"
#property strict
...
input int    RSI_Period         = 7;              // v336 CHANGE: 14 → 7 (faster RSI)
input double RSI_Buy            = 45.0;           // NON-NEGOTIABLE (v335 confirmed)
...
input double Max_ATR_Pct        = 0.0;            // disabled
...
input int    Magic_Number       = 3362057;
input string Order_Comment      = "BSv336";
...
void OnTick()
{
   ...
   double rsi1 = iRSI(NULL, 0, RSI_Period, PRICE_CLOSE, 1);  // RSI(7) in v336
   double rsi2 = iRSI(NULL, 0, RSI_Period, PRICE_CLOSE, 2);  // RSI(7) in v336
   ...
   if(h1Bull)
   {
      ...
      bool rsiOS       = (rsi1 < RSI_Buy);             // RSI(7) < 45
