//+------------------------------------------------------------------+
//|   Buy & Sell EA v333 - ATR_SL_Multi wider SL test               |
//|   BASE: v328 (best performing) + single change: ATR_SL_Multi     |
//|                                                                   |
//|   v332 POST-MORTEM (331 trades, +10.15%, MaxDD=5.45%):           |
//|     FAILURE: WR=70.09% collapsed 13pp below v328 ✗               |
//|     FAILURE: MaxDD=5.45% well above 5% limit ✗                   |
//|     LESSON: 0.65 ATR SL too tight for XAUUSD M5 noise           |
//|     WR CURVE: 0.65ATR=70% → 0.80ATR=83.47% → ?1.0ATR?          |
//|                                                                   |
//|   v333 HYPOTHESIS: Only change ATR_SL_Multi 0.80 → 1.0          |
//|     Rationale: Wider SL = more room for reversal to complete     |
//|       0.65 vs 0.80 ATR gap caused +13pp WR improvement           |
//|       0.80 vs 1.0 ATR gap may push WR above 83.47%               |
//|     BUT: Larger SL → smaller lots for same dollar risk           |
//|       Lots decrease to 0.80/1.0 = 80% of v328                   |
//|     If WR improves to ~88%+: expected profit may increase        |
//|     If WR stays at 83%: profit decreases (fewer lots, same WR)  |
//|                                                                   |
//|   v333 SINGLE CHANGE from v328:                                  |
//|     CHANGE: ATR_SL_Multi 0.80 → 1.0                             |
//|       Wider stop = less premature SL triggers                    |
//|       Potential: higher WR, smaller lots, same risk $            |
//|                                                                   |
//|   SUCCESS CRITERIA:                                               |
//|     WR > 83.47% AND MaxDD < 5% AND Return > 16.25% → IMPROVE    |
//|     Any metric worse than v328 → revert to v328                  |
//|                                                                   |
...
//|     ATR_SL_Multi≥0.80 ← LOCKED (v332 proved 0.65 too tight)     |
//|     TwoBar_Lookback=1, Session=9-21 (unchanged)                  |
//|     Max_Sells=0, Body_Pct=0.20, Hard_DD_Stop=0.0               |
//|                                                                   |
//|   SCOREBOARD:                                                     |
//|     v328: +16%, 4.99% MaxDD, 496 trades — BEST BASELINE ✓       |
//|     v329: +2%, 6.06% MaxDD — BB_Dev=2.0 fail ✗                  |
//|     v330: +13%, 5.14% MaxDD — exit combo fail ✗                 |
//|     v331: +17%, 5.01% MaxDD, 364 trades — Partial=0.40 fail ✗   |
//|     v332: +10%, 5.45% MaxDD, 331 trades — SL=0.65 too tight ✗   |
//|     v333: This version — ATR_SL_Multi=1.0, all else=v328        |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "333.0"
#property strict
...
//--- Exit Settings
// v333 SINGLE CHANGE: ATR_SL_Multi 0.80 → 1.0
// Rationale: Wider SL → more room for reversal → potentially higher WR
// Partial_Pct stays at 0.20 (v328 value, proven optimal)
input string EXIT_SET           = "============"; //====== Exit Settings ======
input int    ATR_Period         = 14;
input double ATR_SL_Multi       = 1.0;            // v333: 0.80 → 1.0 (SINGLE CHANGE)
...
input int    Magic_Number       = 3332057;
input string Order_Comment      = "BSv333";
...
void ManageExits()
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(type == OP_BUY)
      {
         ...
         if(partialDone)
         {
            // v333: BB_mid-based trail — IDENTICAL to v328 (NON-NEGOTIABLE)
