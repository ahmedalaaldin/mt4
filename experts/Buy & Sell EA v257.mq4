//+------------------------------------------------------------------+
//|   Buy & Sell EA v257 - No Partial Close (100% Rides BB_mid Trail)|
...
//|     When BB_mid hit: SL→entry only (breakeven lock), no partial  |
//|     100% of position rides BB_mid trail from breakeven           |
//|     vs v256: 30% locked at midline + 70% trailing               |
//|     If continuation after midline is strong → more upside (100%) |
//|     Breakeven protection preserved (SL still moves to entry)     |
//|     Code: partLots=0 → else branch → SL to entry → trail starts  |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "257.0"
#property strict
...
input double Partial_Pct        = 0.00;           // v257: no partial — 100% rides trail
...
input int    Magic_Number       = 2572057;
input string Order_Comment      = "BSv257";
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(type == OP_BUY)
      {
         ...
         if(!partialDone && Bid >= bbMid)
         {
            double stepLot  = MarketInfo(Symbol(), MODE_LOTSTEP);
            double minLot   = MarketInfo(Symbol(), MODE_MINLOT);
            // v257: Partial_Pct=0.00 → partLots=0 → falls to else branch
            // else branch: SL moves to entry (breakeven lock), no partial close
            // 100% of position then trails via BB_mid - 0.5×ATR
         ...
         if(partialDone)
         {
            // Full 100% position trails BB_mid - 0.5×ATR
