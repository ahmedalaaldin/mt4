//+------------------------------------------------------------------+
//|   Buy & Sell EA v109 - AllBuysRiskFree + Correct Near-BE Stop   |
//|   Framework: 4H Direction / 1H Structure / 5M Entry             |
//|   Base: v104 AllBuysRiskFree gate                               |
//|   Innovation:                                                    |
//|     - v107 near-BE fatal bug fixed: openPrice-2*Point ≈ $0.02   |
//|       = ABOVE current Bid (spread ~$0.20) → immediate stop-out  |
//|     - v109 fix: nearBE = openPrice-(spread+5)*Point             |
//|       safely below Bid at entry, no instant trigger             |
//|     - Once price gains BEMove_ATR (0.5 ATR), SL moves to       |
//|       ~entry-spread → near-zero loss on reversal                |
//|     - H1 RSI rising REMOVED: restores v104 trade frequency      |
//|     - H1_RSI_Min=45: slightly more entries than v104 (50)        |
//|     - AllBuysRiskFree + Max_Buys=2: proven compounding gate     |
//|   Goal: v104-level 173%+ return + near-BE reduces DD            |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "119.0"
#property strict
...
input double H1_RSI_Min         = 45.0;           //H1 RSI min (relaxed vs v104=50)
...
input double BEMove_ATR         = 0.5;            //Move SL near-BE after price gains this ATR
...
input int    Max_Buys           = 2;              //Max concurrent BUY positions (gated)
...
input int    Magic_Number       = 1192025;        //Magic Number
input string Order_Comment      = "BSv119";       //Order comment
...
//--- Returns true only when ALL open buys have SL at or above open price (risk-free)
...
void OnTick()
{
   ...
   // 4H: Direction (price above EMA, no candle filter)
   double h4_ema = iMA(NULL, PERIOD_H4, H4_EMA_Period, 0, MODE_EMA, PRICE_CLOSE, 0);
   bool   h4Bull = (iClose(NULL, PERIOD_H4, 0) > h4_ema);
   if(!h4Bull) return;

   // 1H: Structure + RSI in zone (no RSI rising requirement — restored trade frequency)
   ...
   if(bbTouch && rsiOS && rsiRising && stochOS && bullBody)
   {
      ...
      if(tpDist >= minTP)
      {
         ...
         // Gate: only open 2nd+ position when ALL existing buys are risk-free (SL >= openPrice)
...
void ManageExits()
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      // Corrected near-BE stop (v107's bug was 2*Point = ~$0.02 which is ABOVE Bid at entry
      // for XAUUSD with typical spread ~$0.20, causing immediate stop-out on every trade).
      // Fix: nearBE = openPrice - (spread+5)*Point, safely below Bid at entry.
      // Effect: once price gains 0.5 ATR, max loss on reversal = ~1 spread only.
      if(!partialDone && Bid >= openPrice + BEMove_ATR * atr)
      {
         double spread = MarketInfo(Symbol(), MODE_SPREAD);
         double nearBE = NormalizeDouble(openPrice - (spread + 5.0) * Point, Digits);
         if(currentSL < nearBE)
