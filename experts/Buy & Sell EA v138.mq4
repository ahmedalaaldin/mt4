//+------------------------------------------------------------------+
//|   Buy & Sell EA v138 - Risk=10%, MaxBuys/Sells=1 (vs v133 5%/2) |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry         |
//|   Base: v133                                                     |
//|   Changes vs v133:                                               |
//|     - Risk_Pct: 5.0 → 10.0                                      |
//|       Doubles position size → doubles gross profit AND gross loss |
//|       Expected: 2× net return = ~215% annual                     |
//|     - Max_Buys: 2 → 1, Max_Sells: 2 → 1                        |
//|       Net equity exposure stays identical:                        |
//|       v133: 2 positions × 5% = 10% max exposure                 |
//|       v138: 1 position × 10% = 10% max exposure                 |
//|       KEY BENEFIT: eliminates simultaneous double-losses.         |
//|       v133's 31% DD came partly from 2 positions losing at once. |
//|       With Max=1, only 1 position can lose at a time.            |
//|       GetRiskScale then reduces next entry before next loss.     |
//|       Expected DD: ~20-25% (vs v133's 31%)                      |
//|     - All other v133 logic preserved:                            |
//|       BB_Dev=2.5, RSI_Buy=40, RSI_Sell=60, ATR_SL_Multi=1.0    |
//|       Partial close 50% at BB midline + trail (Trail_ATR=0.8)   |
//|       H4/H1 gate, GetRiskScale loss protection                   |
//|   Expected: ~215% return, ~20-25% DD, 433+ trades, WR ~85%     |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "138.0"
#property strict
...
input double Risk_Pct           = 10.0;           //Risk % per trade (10% = 2x v133)
input double Fixed_Lot          = 0.0;            //Fixed lot (0 = use Risk_Pct)
input int    Max_Buys           = 1;              //Max concurrent BUY (1 = no double-loss)
input int    Max_Sells          = 1;              //Max concurrent SELL (1 = no double-loss)
...
input string Order_Comment      = "BSv138";       //Order comment
