//+------------------------------------------------------------------+
//|   Buy & Sell EA v388 - Dual magic: M5 vs M15+M30 independent (base v381)   |
...
input int    Magic_Number       = 3882057;        // v388: M5 magic
input int    Magic_M1530        = 3882058;        // v388: M15+M30 magic
input string Order_Comment      = "BSv388";
...
void CloseAllTrades()
{
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(OrderMagicNumber() != Magic_Number && OrderMagicNumber() != Magic_M1530) continue;
      if(OrderType() == OP_BUY)
...
void OnTick()
{
   ...
   if(Use_M15_Signals && isNewM15Bar && h1Bull)
   {
      ...
      {
         if(CountTradesByMagic(OP_BUY, Magic_M1530) < Max_Buys)
         {
            ...
            if(m15_atr > 0)
            {
               ...
               if(m15_lots > 0)
               {
                  int tkt = OrderSend(Symbol(), OP_BUY, m15_lots, entry, Slippage,
                                      sl, tp, Order_Comment, Magic_M1530, 0, clrLime);
                  if(tkt > 0) g_TradesToday++;
               }
            }
         }
            ...
            if(m30_atr > 0)
            {
               ...
               if(m30_lots > 0)
               {
                  int tkt = OrderSend(Symbol(), OP_BUY, m30_lots, entry, Slippage,
                                      sl, tp, Order_Comment, Magic_M1530, 0, clrAqua);
                  if(tkt > 0) g_TradesToday++;
               }
            }
         }
