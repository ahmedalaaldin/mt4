//+------------------------------------------------------------------+
//|   Buy & Sell EA v65 - v51 + Symmetric SELL side added          |
//|   BUY:  H1 EMA200 + H1 RSI 50-75 + BB 2.5SD lower touch       |
//|          + M5 RSI<40 + RSI rising + Stoch K&D<25 + Bull 20%   |
//|   SELL: H1 EMA200 + H1 RSI 50-75 + BB 2.5SD upper touch       |
//|          + M5 RSI>60 + RSI falling + Stoch K&D>75 + Bear 20%  |
//|   Exit: TP = BB mid, SL = 1xATR                                |
//|   Goal: Double trade count to ~100/year via symmetric signals   |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "65.0"
#property strict
...
input double RSI_Buy            = 40.0;           //M5 RSI buy threshold (below)
input double RSI_Sell           = 60.0;           //M5 RSI sell threshold (above)
...
input int    Magic_Number       = 652025;         //Magic Number
input string Order_Comment      = "BSv65";        //Order comment
...
void OnTick()
{
   ...
   bool h1Trend = (h1_rsi >= H1_RSI_Min) && (h1_rsi <= H1_RSI_Max);
   if(!h1Trend) return;
   ...
   //--- BUY signal
   bool bbTouchBuy = (Low[1] <= bbLower1) && (Close[1] > bbLower1);
   bool rsiOS      = (rsi1 < RSI_Buy);
   bool rsiRising  = (rsi1 > rsi2);
   bool stochBuy   = (stochK1 < Stoch_OS) && (stochD1 < Stoch_OS);
   bool strongBull = (candleBody > 0) && (candleRange > 0) && (candleBody >= Body_Pct * candleRange);

   if(bbTouchBuy && rsiOS && rsiRising && stochBuy && strongBull && Close[1] > h1_ema)
   {
   ...
   //--- SELL signal
   bool bbTouchSell = (High[1] >= bbUpper1) && (Close[1] < bbUpper1);
   bool rsiOB       = (rsi1 > RSI_Sell);
   bool rsiFalling  = (rsi1 < rsi2);
   bool stochSell   = (stochK1 > Stoch_OB) && (stochD1 > Stoch_OB);
   bool strongBear  = (candleBody < 0) && (candleRange > 0) && (MathAbs(candleBody) >= Body_Pct * candleRange);

   if(bbTouchSell && rsiOB && rsiFalling && stochSell && strongBear)
   {
