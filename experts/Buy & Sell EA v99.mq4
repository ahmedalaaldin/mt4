//+------------------------------------------------------------------+
//|   Buy & Sell EA v99 - BB Upper TP + Reduced Risk for DD < 5%    |
//|   Framework: 4H Direction / 1H Structure / 15M / 5M Entry       |
//|   Changes from v92:                                              |
//|     - TP changed from BB mid to BB upper (double the R:R)        |
//|     - Risk_Pct reduced from 7% to 3.5% (halved for DD control)  |
//|     - Max_Bars increased from 40 to 60 (time for upper BB reach) |
//|   Goal: 200%+ return AND <5% DD simultaneously via better R:R    |
//|   All quality gates preserved: BB 2.5SD + RSI<40 + Stoch<25     |
//|   + Bull body + H1 RSI 50-75 + M15 RSI<50 + H4 EMA              |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "109.0"
#property strict
...
input int    Max_Bars           = 60;             //Force close after N bars (60 = 5hrs on M5)

//--- Money Management
input string MONEY_SET          = "============"; //====== Money Management ======
input double Risk_Pct           = 3.5;            //Risk % per trade (halved for DD < 5%)
...
input int    Magic_Number       = 1092025;        //Magic Number
input string Order_Comment      = "BSv109";       //Order comment
...
void OnTick()
{
   ...
   if(bbTouch && rsiOS && rsiRising && stochOS && bullBody)
   {
      ...
      double tpDist = bbUpper0 - entry;   // TP at BB upper (full range, not just mid)
      if(tpDist >= minTP)
      {
