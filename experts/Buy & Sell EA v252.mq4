//+------------------------------------------------------------------+
//|   Buy & Sell EA v252 - Tiered Risk Reduction on Balance Milestones|
//|   Framework: H4+H1 Direction / M5 Entry (v186 CONFIRMED BEST)   |
//|   LESSONS LEARNED:                                               |
//|     v186: BB+RSI+Stoch(25/75)+H4+H1 -> Cal=12.60  *** BEST ***  |
//|     v247: EQUITY GUARD FAILED (fired on floating losses)          |
//|     v248: NO GUARD — Calmar=7.17, MaxDD=23.18%                   |
//|     v249: INITIAL BALANCE CAP — Calmar=7.40, MaxDD=16.12%        |
//|     v250: FLAT WIN SCALING — Calmar=10.33 *** BEST ***           |
//|       Return=107.11%, MaxDD=10.37%, 234 trades, Win=88.46%       |
//|       MaxDD root cause: each $700 loss at $22k peak = 3.1%       |
//|         Alternating L-W clusters create $2,312 cumulative DD      |
//|     v251: LOWER RISK (5%) + BIGGER TP (3.5×) FAILED             |
//|       Calmar=8.68, Return=76.51%, MaxDD=8.81%                    |
//|       Pure risk scaling doesn't improve Calmar — ratio unchanged  |
//|                                                                  |
//|   v252 HYPOTHESIS: TIERED RISK TIED TO REALIZED BALANCE          |
//|     Phase 1 (balance < 150% initial = $15k): 7% risk             |
//|       Max single loss $700 = 4.7% of $15k (good early on)        |
//|     Phase 2 (balance $15k-$20k): 5% risk                         |
//|       Max single loss $500 = 3.3% of $15k, 2.5% of $20k         |
//|       6 losses: $1,025 = 6.8% of $15k → early P2                |
//|     Phase 3 (balance > $20k): 4% risk                            |
//|       Max single loss $400 = 2.0% of $20k, 1.8% of $22k ✓       |
//|       6 losses: $820 = 4.1% of $20k, 3.7% of $22k peak ✓        |
//|     KEY DIFFERENCE from v247 equity guard:                       |
//|       Triggered by REALIZED BALANCE (closed P&L only)            |
//|       NOT by floating equity during open trades                   |
//|       → No false triggers on normal mean-reversion wiggles ✓     |
//|     Expected: Return ~90-105%, MaxDD <8%, Calmar >12             |
//|     All v186 entry conditions UNCHANGED (NON-NEGOTIABLE)         |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "252.0"
#property strict
...
input double ATR_TP_Multi       = 2.5;            // v252: back to v250 value (v251 3.5× didn't help)
...
input double Risk_Pct           = 7.0;            // Phase 1 base risk (< Tier1 balance)
input double Fixed_Lot          = 0.0;
input int    Max_Buys           = 1;
input int    Max_Sells          = 1;

//--- Tiered Risk Reduction (v252 KEY CHANGE)
input string TIER_SET           = "============"; //====== Tiered Risk ======
input double Tier1_Ratio        = 1.50;           // Phase 2 starts when balance >= 150% initial
input double Tier2_Ratio        = 2.00;           // Phase 3 starts when balance >= 200% initial
input double Risk_Pct_Tier1     = 5.0;            // Risk% once balance reaches Tier1 ($15k)
input double Risk_Pct_Tier2     = 4.0;            // Risk% once balance reaches Tier2 ($20k)

//--- Anti-Martingale (flat wins, only loss downscaling — from v250)
input string AM_SET             = "============"; //====== Anti-Martingale ======
input int    WinStreak_Mid      = 5;
input int    WinStreak_High     = 10;
input double Scale_Mid          = 1.0;            // flat (v250 confirmed)
input double Scale_High         = 1.0;            // flat (v250 confirmed)

//--- Safety
input string SAFETY_SET         = "============"; //====== Safety Settings ======
input int    MaxSpread          = 35;
input int    Slippage           = 30;
input int    Magic_Number       = 2522057;
input string Order_Comment      = "BSv252";
...
double   g_InitialBalance = 0;    // snapshot at startup for all balance-ratio calculations
...
//+------------------------------------------------------------------+
// v252 KEY FUNCTION: Returns effective risk% based on realized balance milestones.
// Uses AccountBalance() (realized P&L only) — NOT AccountEquity() (which includes floating).
// This ensures NO false triggers during open trades — only closed profits count.
// Thresholds: $15k → 5%, $20k → 4% (for $10k initial deposit)
double GetEffectiveRiskPct()
{
   if(g_InitialBalance <= 0) return Risk_Pct;
   double balanceRatio = AccountBalance() / g_InitialBalance;
   if(balanceRatio >= Tier2_Ratio) return Risk_Pct_Tier2;   // ≥200%: 4%
   if(balanceRatio >= Tier1_Ratio) return Risk_Pct_Tier1;   // ≥150%: 5%
   return Risk_Pct;                                          // <150%: 7%
}
...
//+------------------------------------------------------------------+
// v252: Uses tiered effective risk% + initial balance cap.
// Phase 1 (<$15k): 7%×$10k = $700 max loss (4.7% of $15k)
// Phase 2 ($15k-$20k): 5%×$10k = $500 max loss (3.3% of $15k, 2.5% of $20k)
// Phase 3 (>$20k): 4%×$10k = $400 max loss (2.0% of $20k)
double CalcLots(double slDist, double riskScale = 1.0)
{
   ...
   double effectiveRisk = GetEffectiveRiskPct();  // v252: tiered risk

   // Use current equity for normal sizing...
   double riskAmt = AccountEquity() * (effectiveRisk * riskScale) / 100.0;
   // ...but CAP to initial balance with tiered risk (prevents runaway sizing)
   double capAmt  = g_InitialBalance * (effectiveRisk * riskScale) / 100.0;
...
void OnTick()
{
   ...
   // Daily DD gate — balance-based (v247 fix confirmed)
