//+------------------------------------------------------------------+
//|   Buy & Sell EA v255 - Tighter RSI Filter (40→35 / 60→65)       |
...
//|     v252: TIERED RISK → Calmar=7.52 (worse)                     |
//|     v253: Trail 0.8→0.5 ATR below BB_mid → Calmar=10.39         |
//|       AvgWin=$111.55: only +$0.15 vs v250 — BB_mid trail slow   |
//|     v254: Bid-based trail 1.5×ATR → Calmar=6.96 (much worse)    |
//|       AvgWin=$101.91: LOWER — trail fires on tick noise          |
//|       LESSON: BB_mid trail is intentionally patient — correct    |
//|       for mean-reversion; Bid trail fires too early              |
//|                                                                  |
//|   v255 HYPOTHESIS: Improve ENTRY quality, not exit structure     |
//|     RSI_Buy:  40 → 35  (deeper oversold required)               |
//|     RSI_Sell: 60 → 65  (deeper overbought required)             |
//|     Effect: fewer trades but higher-conviction setups only       |
//|     Expected: Win rate 88%→91%+, fewer loss clusters,           |
//|       lower MaxDD%, similar or higher avg win → better Calmar    |
//|     All exit parameters: identical to v250/v253 (BB_mid trail)   |
//|     Trail_ATR_Multi = 0.5 (from v253, marginal improvement)      |
//|     All NON-NEGOTIABLE entry conditions retained                 |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "255.0"
#property strict
...
input double RSI_Buy            = 35.0;   // v255: 40 → 35 (deeper OS required)
input double RSI_Sell           = 65.0;   // v255: 60 → 65 (deeper OB required)
...
input double ATR_TP_Multi       = 2.5;            // v250 confirmed optimal
input double Trail_ATR_Multi    = 0.5;            // v253 marginal improvement retained
...
input int    Magic_Number       = 2552057;
input string Order_Comment      = "BSv255";
...
void ManageExits()
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(type == OP_BUY)
      {
         ...
         if(partialDone)
         {
            // BB_mid trail (v253 confirmed: 0.5 ATR — marginally better than 0.8)
...
void OnTick()
{
   ...
   //=== BUY: v186 FULL FILTER SET + v255 tighter RSI_Buy=35 ===
   if(h4Bull && h1Bull && stochBuy)
   {
      bool bbTouch   = (Low[1] <= bbLower1) && (Close[1] > bbLower1);
      bool rsiOS     = (rsi1 < RSI_Buy);      // v255: < 35 (was < 40)
   ...
   //=== SELL: v186 FULL FILTER SET + v255 tighter RSI_Sell=65 ===
   if(h4Bear && h1Bear && stochSell)
   {
      bool bbTouchUp  = (High[1] >= bbUpper1) && (Close[1] < bbUpper1);
      bool rsiOB      = (rsi1 > RSI_Sell);     // v255: > 65 (was > 60)
