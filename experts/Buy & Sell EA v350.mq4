//+------------------------------------------------------------------+
//|   Buy & Sell EA v350 - Add H1 signals (H4 trend filter)          |
...
//|     v348: ATR_TP_Multi=8.0 → +30.67%, IDENTICAL   NO EFFECT ✗  |
//|     v349: Risk_Pct=1.0% → +16.96%, MaxDD=5.01%❌  FAIL ✗       |
//|                                                                   |
//|   v350 HYPOTHESIS: Extend multi-TF from M5+M15+M30 to           |
//|   M5+M15+M30+H1. H1 BB lower touches are rare but strong        |
//|   mean-reversion setups. Use H4 EMA-200 trend filter for H1     |
//|   signals (consistent with multi-TF hierarchy:                  |
//|     M5/M15/M30: need H1 close > H1 EMA-200                      |
//|     H1: need H4 close > H4 EMA-200                              |
//|   Expected: +20-40 extra high-quality trades/year, similar DD.   |
//|                                                                   |
//|   v350 CHANGES from v343:                                        |
//|     Use_H1_Signals: false → true (NEW)                           |
//|     H4 EMA trend filter now active for H1 signals               |
//|                                                                   |
...
input int    H4_EMA_Period      = 200;            // Used for H1 signal trend filter
...
input bool   Use_H1_Signals     = true;           // v350: NEW - H1 signals
...
input int    Magic_Number       = 3502057;        // v350 unique magic
input string Order_Comment      = "BSv350";
...
datetime g_LastH1Bar      = 0;          // v350: H1 bar tracker
...
void OnTick()
{
   ...
   //--- H1 trend filter (for M5/M15/M30 signals)
   ...
   //--- H4 trend filter (for H1 signals) - v350 NEW
   ...
   //=== H1 BUY (v350 NEW - H4 trend filter) ===
   if(Use_H1_Signals && isNewH1Bar && h4Bull)
   {
      ...
      double h1s_rsi1    = iRSI(NULL, PERIOD_H1, RSI_Period, PRICE_CLOSE, 1);
      double h1s_rsi2    = iRSI(NULL, PERIOD_H1, RSI_Period, PRICE_CLOSE, 2);
      ...
      bool h1_rsiOS       = (h1s_rsi1 < RSI_Buy);
      bool h1_rsiRising   = (h1s_rsi1 > h1s_rsi2);
            ...
            if(h1_atr > 0)
            {
               ...
               if(h1_lots > 0)
               {
                  int tkt = OrderSend(Symbol(), OP_BUY, h1_lots, entry, Slippage,
                                      sl, tp, Order_Comment, Magic_Number, 0, clrOrange);
                  if(tkt > 0) g_TradesToday++;
               }
            }
         }
