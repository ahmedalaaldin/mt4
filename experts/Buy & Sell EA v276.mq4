//+------------------------------------------------------------------+
//|   Buy & Sell EA v276 - Aggressive anti-martingale + daily DD cap |
//|   Framework: H1 EMA direction + wider H1 RSI / M5 Entry         |
//|   HYPOTHESIS: MaxDD of 11.77% (v269) driven by 1-2 full-size   |
//|     losses before anti-martingale scaling kicks in.              |
//|     More aggressive scaling: 0.3x/0.15x/0.05x after 1/2/3+     |
//|     losses (vs current 0.5x/0.25x/0.1x)                        |
//|     ALSO: tighten Max_Daily_DD_Pct from 8% to 5%               |
//|     Both changes target MaxDD reduction without changing entry   |
//|     If MaxDD drops to <8%, return stays ~200%+ = higher Calmar   |
//|   BASE: v269 CHAMPION (Calmar=17.77, Return=209%, 707 trades)   |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "276.0"
#property strict
...
//--- Anti-Martingale - MORE AGGRESSIVE LOSS SCALING
...
// Loss scaling (more aggressive than v269):
// v269: 0.5x/0.25x/0.1x  →  v276: 0.3x/0.15x/0.05x
input double Loss1_Scale        = 0.30;  // after 1 loss (was 0.5)
input double Loss2_Scale        = 0.15;  // after 2 losses (was 0.25)
input double Loss3_Scale        = 0.05;  // after 3+ losses (was 0.1)

//--- Safety - TIGHTER DAILY DD CAP
input string SAFETY_SET        = "============"; //====== Safety Settings ======
input int    MaxSpread          = 35;
input int    Slippage           = 30;
input int    Magic_Number       = 2762057;
input string Order_Comment      = "BSv276";
input double Max_Daily_DD_Pct   = 5.0;   // v276: tightened from 8% to 5%
...
double GetRiskScale()
{
   ...
   int losses = -streak;
   if(losses == 1) return Loss1_Scale;   // 0.30x
   if(losses == 2) return Loss2_Scale;   // 0.15x
   return Loss3_Scale;                   // 0.05x for 3+ losses
}
