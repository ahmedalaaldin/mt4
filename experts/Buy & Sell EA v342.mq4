//+------------------------------------------------------------------+
//|   Buy & Sell EA v342 - Multi-Timeframe M5+M15 Signals            |
...
//|                                                                   |
//|   v342 HYPOTHESIS: Add M15 timeframe signals                     |
//|   Same BB breakdown-recovery pattern checked on BOTH M5 and M15. |
//|   M15 signals ≈ 0.5/day extra; M5 ≈ 1.5/day = ~2/day combined.  |
//|   M15 signals use M15 ATR for SL/TP (wider stops, fewer lots).   |
//|   Expected: WR slightly improves (M15 signals higher quality),   |
//|   trade count ↑ to ~500/year, total return ↑ above 20%.         |
//|   Addresses user request: "same strategies on different TFs"     |
//|                                                                   |
//|   v342 CHANGES from v328:                                        |
//|     Entry type: OP_BUY market (SAME as v328)                     |
//|     New: Use_M15_Signals parameter (default true)                |
//|     New: M15 BB pattern check fires once per new M15 bar         |
//|     M15 entry uses M15 ATR for SL/TP sizing                      |
//|     M15 and M5 trades share same Max_Buys limit                  |
//|                                                                   |
...
input int    Magic_Number       = 3422057;        // v342 unique magic
input string Order_Comment      = "BSv342";
...
void OnTick()
{
   ...
   //=== M15 BUY (v342 new: same pattern on M15 timeframe) ===
   // Fires once per new M15 bar only.
   // Uses M15 ATR for SL/TP; exits still managed by M5 ManageExits via M5 bbMid.
