//+------------------------------------------------------------------+
//|   Buy & Sell EA v192 - Guard: cap win-scaling only (not losses)  |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry         |
//|   LESSONS LEARNED:                                               |
//|     v186: 7.0%, Streak=5/10   -> 90.70% WR, 18.87% DD, Cal=12.60|
//|            BEST Calmar so far: delayed anti-martingale optimal    |
//|     v189: AccountEquity guard  -> 93.49% ret, 18.51% DD, Cal=5.05|
//|            FAIL: floating P&L inflates peak, false guard fires    |
//|     v190: AccountBalance guard -> 89.37% ret, 16.38% DD, Cal=5.46|
//|            FAIL: partial closes set new peaks (breakeven SL ok)   |
//|            but guard*anti-martingale = 0.25*0.5=0.125x lots.     |
//|     v191: Peak only when flat  -> 89.37% ret, 16.38% DD, Cal=5.46|
//|            FAIL: Identical to v190. Diagnosis was wrong: partial  |
//|            close + breakeven SL means net-trade is always         |
//|            positive, so peaks are same in v190 and v191.         |
//|   ROOT CAUSE FOUND: guard fires after 2 losses (balance drops 10%)|
//|     At that point anti-martingale already reduced riskScale to    |
//|     0.25. Guard then multiplies: 0.25 * 0.5 = 0.125x lots!      |
//|     Recovery at 0.125x lots takes dozens of wins, crushing        |
//|     compounding and halving the annual return.                    |
//|   FIX v192: Guard only caps UPWARD scaling during drawdowns.      |
//|     If riskScale > 1.0 (win streak) AND in drawdown: cap to 1.0  |
//|     If riskScale <= 1.0 (loss streak or neutral): guard inactive  |
//|     This prevents over-betting during recovery without stacking   |
//|     a second penalty on top of anti-martingale loss reductions.  |
//|   v192 = v191 base (flat-peak update) + new guard logic          |
//|   Parameters identical to v186 proven-best:                      |
//|     Risk=7%, Streak=5/10, Scale_Mid=1.10, Scale_High=1.25        |
//|     Guard: Pct=10%, new mode: cap-win-scaling-only               |
//|   Changes vs v191:                                               |
//|     1. Guard: riskScale=1.0 if in DD && riskScale>1.0 (not *=0.5)|
//|     2. Removed Equity_DD_Guard_Scale input (no longer needed)    |
//|     3. Magic_Number: 1912025 -> 1922025                          |
//|     4. Order_Comment: BSv191 -> BSv192                           |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "192.0"
#property strict
...
input double Risk_Pct           = 7.0;            //Risk%: 7.0 (v186 proven optimal)
...
input int    WinStreak_Mid      = 5;              //Wins for Scale_Mid (v186: 5)
input int    WinStreak_High     = 10;             //Wins for Scale_High (v186: 10)
input double Scale_Mid          = 1.10;           //Scale at mid streak
input double Scale_High         = 1.25;           //Scale at high streak

//--- Balance Peak Guard (NEW LOGIC v192)
input string GUARD_SET          = "============"; //====== Balance Peak Guard (v192) ======
input double Equity_DD_Guard_Pct = 10.0;          //Cap win-scaling if balance DD > X% from peak
// NEW v192 LOGIC: When guard fires AND riskScale > 1.0 (win streak scaling):
//   -> riskScale capped at 1.0 (no upward bet increase during drawdown)
// When guard fires AND riskScale <= 1.0 (loss streak or neutral):
//   -> guard inactive (anti-martingale handles this already)
// This prevents the v190/v191 failure: guard*anti-martingale = 0.125x lots
...
input int    Magic_Number       = 1922025;        //Magic Number
input string Order_Comment      = "BSv192";       //Order comment
...
double   g_EquityPeak   = 0;   // Balance peak (v191: only updated when flat)
...
void OnTick()
{
   ...
   // v191/v192: Only update peak when ALL positions are closed (flat)
   // Prevents partial-close from inflating peak mid-trade
   int openBuysNow  = CountTradesByMagic(OP_BUY,  Magic_Number);
   int openSellsNow = CountTradesByMagic(OP_SELL, Magic_Number);
   if((openBuysNow + openSellsNow) == 0 && curBalance > g_EquityPeak)
   ...
   // === v192: Balance Peak Guard — cap WIN-SCALING ONLY during drawdowns ===
   // CRITICAL DESIGN CHANGE from v189/v190/v191:
   //   OLD: riskScale *= 0.5 when guard fires (stacks with anti-martingale loss reductions)
   //        Result: 0.25 * 0.5 = 0.125x lots after 2 losses — crushes compounding
   //   NEW: riskScale = 1.0 ONLY IF guard fires AND riskScale > 1.0 (win-streak scaling)
   //        During loss streaks (riskScale <= 1.0): guard is inactive — let anti-martingale handle it
   //        During win streaks (riskScale > 1.0): guard prevents betting bigger while still in DD
   //   Effect: preserves anti-martingale loss-reduction AND prevents win-streak over-scaling in DD
   if(Equity_DD_Guard_Pct > 0 && g_EquityPeak > 0)
   {
      double ddFromPeak = (g_EquityPeak - curBalance) / g_EquityPeak * 100.0;
      if(ddFromPeak >= Equity_DD_Guard_Pct && riskScale > 1.0)
         riskScale = 1.0;   // Don't scale up during drawdowns — cap at base
   }
