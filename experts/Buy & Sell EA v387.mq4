//+------------------------------------------------------------------+
//|   Buy & Sell EA v387 - Add M1 signals (use M5 ATR for SL, base v381)   |
//|   BASE: v354 (ATR_Spike=2.0, M5+M15+M30) BEST ✓                 |
//|                                                                   |
...
input bool   Use_M1_Signals     = true;
...
input int    Magic_Number       = 3872057;        // v381 unique magic
input string Order_Comment      = "BSv387";
...
void OnTick()
{
   ...
   //=== M1 BUY (uses M5 ATR for SL to avoid tiny stops) ===
   if(Use_M1_Signals && isNewM1Bar && h1Bull)
   {
      ...
      if(m1_bbTouch && m1_rsiOS && m1_rsiRising && m1_bullBody && m1_bbPrevBreak)
      {
         if(CountTradesByMagic(OP_BUY, Magic_Number) < Max_Buys)
         {
            // Use M5 ATR for SL to avoid micro-stops on M1
            double m1_slDist = ATR_SL_Multi * atr1;
            double m1_tpDist = ATR_TP_Multi * atr1;
            ...
            if(m1_lots > 0)
            {
               int tkt = OrderSend(Symbol(), OP_BUY, m1_lots, entry, Slippage,
                                   sl, tp, Order_Comment, Magic_Number, 0, clrMagenta);
               if(tkt > 0) g_TradesToday++;
            }
         }
      }
