//+------------------------------------------------------------------+
//|   Buy & Sell EA v244 - High Frequency: BB(20,2.0) + No Stoch    |
...
//|       Cap let equity grow higher → bigger % crash from peak     |
//|       LESSON: dollar cap changes curve shape but not MaxDD %    |
//|                                                                 |
//|   v244 HYPOTHESIS: Increase trade frequency                    |
//|     Problem: current 258 trades/year = 0.71/day (need 20+)     |
//|     Approach: BB_Dev 2.5→2.0 (+4x signals) + Remove Stoch      |
//|     Keep: H1 RSI gate [45,75]/[25,55] for quality control      |
//|     Keep: H4+H1 EMA200 direction filter                        |
//|     Keep: RSI(14) level + rising/falling check                 |
//|     Reduce: Risk_Pct 7%→5% (more frequent = more variance)     |
//|     Expected: 3-8 trades/day, Calmar TBD                       |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "244.0"
#property strict
...
//--- H1 RSI Gate (v186 CONFIRMED OPTIMUM — KEPT for quality control)
...
//--- BB Entry Signal (v244: BB_Dev lowered to 2.0 for more signals)
input string S1_SET             = "============"; //====== BB Signal ======
input int    BB_Period          = 20;
input double BB_Dev             = 2.0;            // v244: 2.5→2.0 for ~4x more BB touches
input int    RSI_Period         = 14;
input double RSI_Buy            = 40.0;           // Keep v186 level
input double RSI_Sell           = 60.0;           // Keep v186 level
...
input double ATR_Spike_Multi    = 1.8;            // v186 CONFIRMED OPTIMUM

//--- Exit Settings (v186 CONFIRMED: ATR_SL_Multi=0.8)
input string EXIT_SET           = "============"; //====== Exit Settings ======
input int    ATR_Period         = 14;
input double ATR_SL_Multi       = 0.8;            // CONFIRMED OPTIMUM — NON-NEGOTIABLE
input double ATR_TP_Multi       = 2.5;
input double Trail_ATR_Multi    = 0.8;
input double Partial_Pct        = 0.50;
input int    Max_Bars           = 100;

//--- Money Management (v244: Risk reduced to 5% for higher-frequency strategy)
input string MONEY_SET          = "============"; //====== Money Management ======
input double Risk_Pct           = 5.0;            // v244: 7%→5% (more frequent trades, more variance)
...
input int    Magic_Number       = 2442054;
input string Order_Comment      = "BSv244";
...
void OnTick()
{
   ...
   //--- H1 RSI gate kept for quality control (v186 confirmed optimum)
   ...
   // NOTE v244: Stochastic gate REMOVED — it was the biggest signal frequency killer
   // With BB_Dev=2.0 + no Stoch, expected ~4-8x more trades than v186
   ...
   //=== BUY: H4Bull + H1Bull + BB(2.0) touch + RSI<40 rising + bullish body ===
   if(h4Bull && h1Bull)
   {
   ...
   //=== SELL: H4Bear + H1Bear + BB(2.0) upper touch + RSI>60 falling + bearish body ===
   if(h4Bear && h1Bear)
   {
