//+------------------------------------------------------------------+
//|   Buy & Sell EA v282 - Allow 2 Concurrent Positions per Direction |
//|   Framework: H1 Direction + H1 RSI / M5 Entry                    |
//|   HYPOTHESIS: Max_Buys=2 / Max_Sells=2 allows a second entry     |
//|     when conditions retrigger while the first trade is open.      |
//|     When first buy is open and price dips further to BB again,    |
//|     a second buy enters at a better (lower) price. This could:    |
//|     (a) Increase trade count 1.5-2x toward 20+/day target         |
//|     (b) Improve average entry (second entry deeper in oversold)    |
//|     Anti-martingale scaling still applies per GetRiskScale().      |
//|   BASE: v269 CHAMPION (Calmar=17.77, Return=209%, 707 trades)     |
//|   CHANGE: Max_Buys=2, Max_Sells=2 (was 1, 1 in v269)             |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "282.0"
#property strict
...
input int    Max_Buys           = 2;   // v282: increased from 1 to 2
input int    Max_Sells          = 2;   // v282: increased from 1 to 2
...
input int    Magic_Number       = 2822057;
input string Order_Comment      = "BSv282";
...
void OnTick()
{
   ...
   //=== BUY: Allow up to Max_Buys=2 concurrent buy positions ===
   if(h1Bull && stochBuy)
   {
   ...
   //=== SELL: Allow up to Max_Sells=2 concurrent sell positions ===
   if(h1Bear && stochSell)
   {
