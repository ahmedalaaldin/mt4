//+------------------------------------------------------------------+
//|   Buy & Sell EA v253 - Tighter Trailing Stop (0.8 -> 0.5 ATR)   |
...
//|       AvgWin=$111, AvgLoss=$457 → 4.1:1 ratio                   |
//|       Continuation trades (after partial close) exit at          |
//|         Trail = BB_midline - 0.8×ATR → can be near breakeven    |
//|         → average continuation profit is low                    |
//|     v251: LOWER RISK + BIGGER TP → Calmar=8.68 (worse)          |
//|     v252: TIERED RISK → Calmar=7.52 (worse, early DD unchanged) |
//|                                                                  |
//|   v253 HYPOTHESIS: Tighten trailing stop after partial close     |
//|     Trail_ATR_Multi: 0.8 → 0.5                                  |
//|     Effect: continuation trail fires at BB_midline - 0.5×ATR    |
//|       (vs current BB_midline - 0.8×ATR)                         |
//|     Tighter trail = fires at HIGHER price = LARGER avg win       |
//|       for continuation piece                                     |
//|     Logic: after partial close at BB_midline, price has already  |
//|       confirmed mean reversion. Tighter trail captures more of   |
//|       the confirmed move before volatility can claw it back.     |
//|     Also frees slot faster → potentially more total trades       |
//|     At entry point (higher uncertainty): SL still 0.8×ATR ✓     |
//|     At continuation (lower uncertainty near mean): 0.5×ATR ✓    |
//|     Expected: higher avg win, same/lower MaxDD, better Calmar    |
//|     All v186 entry conditions UNCHANGED (NON-NEGOTIABLE)         |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "253.0"
#property strict
...
input double Trail_ATR_Multi    = 0.5;            // v253: 0.8 → 0.5 (tighter trail = bigger avg win)
...
input int    Magic_Number       = 2532057;
input string Order_Comment      = "BSv253";
...
void ManageExits()
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(type == OP_BUY)
      {
         ...
         if(partialDone)
         {
            // v253: Trail_ATR_Multi=0.5 (was 0.8) — tighter trail captures more upside
      ...
      else if(type == OP_SELL)
      {
         ...
         if(partialDone)
         {
            // v253: Trail_ATR_Multi=0.5 — tighter trail for sells too
            double trailSL = NormalizeDouble(bbMid + Trail_ATR_Multi * atr, Digits);
            if(trailSL < currentSL)
               OrderModify(tkt, openPrice, trailSL, 0, 0, clrBlue);
         }
