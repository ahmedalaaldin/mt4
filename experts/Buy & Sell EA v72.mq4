//+------------------------------------------------------------------+
//|   Buy & Sell EA v83 - Full symmetric buy+sell BB bounce         |
//|   BUY:  H1 uptrend (RSI 50-75, >EMA200) + BB lower 2.5SD       |
//|         + M5 RSI<40 rising + Stoch K&D<25 + Bull body 20%      |
//|   SELL: H1 downtrend (RSI 25-50, <EMA200) + BB upper 2.5SD     |
//|         + M5 RSI>60 falling + Stoch K&D>75 + Bear body 20%     |
//|   Exit: TP = BB mid, SL = 1xATR                                 |
//|   Goal: Double signal count, maintain 56%+ WR on both sides     |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "83.0"
#property strict
...
input double H1_RSI_Bull_Min    = 50.0;           //H1 RSI min for BUY (uptrend)
input double H1_RSI_Bull_Max    = 75.0;           //H1 RSI max for BUY (not overextended)
input double H1_RSI_Bear_Min    = 25.0;           //H1 RSI min for SELL (not oversold)
input double H1_RSI_Bear_Max    = 50.0;           //H1 RSI max for SELL (downtrend)
...
input double RSI_Buy_Level      = 40.0;           //BUY: RSI must be below this
input double RSI_Sell_Level     = 60.0;           //SELL: RSI must be above this
...
input double Body_Pct           = 0.2;            //Min body as fraction of range
...
input int    Magic_Number       = 832025;         //Magic Number
input string Order_Comment      = "BSv83";        //Order comment
...
void OnTick()
{
   ...
   // ===== BUY LOGIC =====
   bool h1BullTrend = (Close[1] > h1_ema) && (h1_rsi >= H1_RSI_Bull_Min) && (h1_rsi <= H1_RSI_Bull_Max);
   if(h1BullTrend)
   {
      bool bbLowerTouch = (Low[1] <= bbLower1) && (Close[1] > bbLower1);
      bool rsiOS        = (rsi1 < RSI_Buy_Level);
      bool rsiRising    = (rsi1 > rsi2);
      bool stochBuyOS   = (stochK1 < Stoch_OS) && (stochD1 < Stoch_OS);
      bool strongBull   = (candleBody > 0) && (candleRange > 0) && (candleBody >= Body_Pct * candleRange);

      if(bbLowerTouch && rsiOS && rsiRising && stochBuyOS && strongBull)
      {
   ...
   // ===== SELL LOGIC =====
   bool h1BearTrend = (Close[1] < h1_ema) && (h1_rsi >= H1_RSI_Bear_Min) && (h1_rsi <= H1_RSI_Bear_Max);
   if(h1BearTrend)
   {
      bool bbUpperTouch = (High[1] >= bbUpper1) && (Close[1] < bbUpper1);
      bool rsiOB        = (rsi1 > RSI_Sell_Level);
      bool rsiFalling   = (rsi1 < rsi2);
      bool stochSellOB  = (stochK1 > Stoch_OB) && (stochD1 > Stoch_OB);
      bool strongBear   = (candleBody < 0) && (candleRange > 0) && ((-candleBody) >= Body_Pct * candleRange);

      if(bbUpperTouch && rsiOB && rsiFalling && stochSellOB && strongBear)
      {
