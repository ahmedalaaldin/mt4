//+------------------------------------------------------------------+
//|   Buy & Sell EA v337 - RSI_Period=7, RSI_Buy=42                  |
//|   BASE: v336 (RSI_Period=7 breakthrough: +11.62%, WR=82.38%)     |
//|                                                                   |
//|   v336 BREAKTHROUGH:                                             |
//|     RSI_Period=7 (faster RSI) gave +33% MORE TRADES: 505 vs 380 |
//|     WR only dropped 1.09% (83.47% → 82.38%) - excellent trade!  |
//|     MaxDD=4.97% — UNDER 5% threshold ✓                          |
//|     BUT: WR=82.38% is 0.59% below the 83% profitability floor   |
//|                                                                   |
//|   v337 HYPOTHESIS:                                               |
//|   With RSI(7), tightening RSI_Buy from 45 → 42 should filter    |
//|   out the marginal trades (RSI between 42-45) that are causing   |
//|   WR to dip below 83%. At RSI(7) < 42, entries are at more       |
//|   genuine oversold conditions vs transient RSI(7) dips.          |
//|   Goal: WR back above 83% while keeping 460-490 trades.         |
//|   If WR > 83% with 460+ trades → should beat v328's +16.25%.    |
//|                                                                   |
//|   v337 CHANGES (2 parameters from v328):                         |
//|     RSI_Period: 14 → 7  (from v336 breakthrough)                 |
//|     RSI_Buy: 45.0 → 42.0 (tighter threshold for RSI(7))         |
//|                                                                   |
//|   SCOREBOARD:                                                     |
//|     v328: +16.25%, MaxDD=4.99%, WR=83.47%, 380 trades  BEST ✓  |
//|     v335: RSI_Buy=55 → WR=71.12%                    FAIL ✗    |
//|     v336: RSI_Period=7 → +11.62%, WR=82.38%, 505t   NEAR ✓    |
//|     v337: RSI_Period=7 + RSI_Buy=42 → ??? (WR push)             |
//|                                                                   |
...
input double RSI_Buy            = 42.0;           // v337 CHANGE: 45 → 42 (tighter)
...
input int    Magic_Number       = 3372057;
input string Order_Comment      = "BSv337";
...
void OnTick()
{
   ...
   double rsi1 = iRSI(NULL, 0, RSI_Period, PRICE_CLOSE, 1);  // RSI(7)
   ...
   if(h1Bull)
   {
      ...
      bool rsiOS       = (rsi1 < RSI_Buy);             // RSI(7) < 42
