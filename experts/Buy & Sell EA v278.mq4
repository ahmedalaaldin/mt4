//+------------------------------------------------------------------+
//|   Buy & Sell EA v278 - H1 Candle Direction Confirmation          |
//|   Framework: H1 EMA direction + H1 bar confirmation / M5 Entry  |
//|   HYPOTHESIS: v269 loses when H1 bar is bullish on a buy entry  |
//|     (signal fires during H1 rally, not a pullback).             |
//|     Requiring H1 bar to be BEARISH for buys (pullback is visible |
//|     on H1) and BULLISH for sells ensures we enter only when     |
//|     H1 momentum confirms the M5 exhaustion signal.              |
//|     H1 EMA UP + H1 RSI 40-80 + H1 BAR BEARISH = pullback into  |
//|     uptrend support = high-probability M5 mean-reversion.       |
//|   BASE: v269 CHAMPION (Calmar=17.77, Return=209%, 707 trades)   |
//|   CHANGE: Add H1 bar direction filter (bearish H1 for buys,     |
//|     bullish H1 for sells)                                       |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "278.0"
#property strict
...
//--- H1 Bar Direction Filter (NEW in v278)
input string H1BAR_SET          = "============"; //====== H1 Bar Filter ======
input bool   Use_H1_Bar_Filter  = true;  // Require bearish H1 bar for buys, bullish for sells
...
input int    Magic_Number       = 2782057;
input string Order_Comment      = "BSv278";
...
void OnTick()
{
   ...
   //--- H1 bar direction: bearish H1 bar for buys (pullback), bullish for sells (rally into resistance)
   double h1_open1  = iOpen(NULL, PERIOD_H1, 1);
   double h1_close1 = iClose(NULL, PERIOD_H1, 1);
   bool h1BarBearish = (h1_close1 < h1_open1);  // H1 just closed down = pullback for buys
   bool h1BarBullish = (h1_close1 > h1_open1);  // H1 just closed up = rally into resistance for sells

   bool h1Bull = (iClose(NULL, PERIOD_H1, 1) > h1_ema)
              && (h1_rsi1 >= H1_RSI_Min) && (h1_rsi1 <= H1_RSI_Max)
              && (!Use_H1_Bar_Filter || h1BarBearish);   // H1 bar must be bearish for buys

   bool h1Bear = (iClose(NULL, PERIOD_H1, 1) < h1_ema)
              && (h1_rsi1 >= H1_RSI_Min_Sell) && (h1_rsi1 <= H1_RSI_Max_Sell)
              && (!Use_H1_Bar_Filter || h1BarBullish);   // H1 bar must be bullish for sells
