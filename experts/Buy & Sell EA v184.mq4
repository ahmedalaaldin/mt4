//+------------------------------------------------------------------+
//|   Buy & Sell EA v184 - Asymmetric Stochastic Gate               |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry         |
//|   LESSONS LEARNED:                                               |
//|     v181: Risk=5%, Max_Buys=1      -> 232% ret, 19.17% DD, Cal=12.12 |
//|            NEW BEST: 86.07% WR, 11 max consec losses            |
//|     v182: Daily DD guard 8%->5%   -> 182% ret, 26.06% DD, Cal=7.01  |
//|            FAIL: guard fires after 1 loss, blocks recovery       |
//|     v183: Stoch K<25 / K>75 gate  -> 172% ret, 15.50% DD, Cal=11.07 |
//|            MIXED: WR 86%->90.5% great! Consec losses 11->5!    |
//|            BUT: too restrictive, 40% fewer trades, return drops  |
//|            SELL improvement: 67.5%->87.5% WR (filter WORKS)     |
//|            BUY filter: over-filtered buys (86.5%->90.9% WR)     |
//|            Key: BUYs already had high WR, filter too aggressive  |
//|   ROOT CAUSE ANALYSIS v183->v184:                                |
//|     - BUY signals filtered 326->210 (-35%) unnecessarily        |
//|     - SELL signals filtered 40->32 (-20%), huge WR improvement   |
//|     - SELLs needed stricter filter, BUYs needed more relaxed     |
//|   APPROACH v184: ASYMMETRIC Stochastic Gate                      |
//|     BUY: Stoch K < 35 (relaxed from 25 -> restore trade volume) |
//|     SELL: Stoch K > 75 (keep strict -> maintain 87.5% WR)      |
//|     Expected: ~300 trades, 88-89% WR, 200%+ return, ~15% DD    |
//|     Calmar target: ~200/16 = 12.5 (beat v181's 12.12)           |
//|   Changes vs v183:                                                |
//|     1. Stoch_OS_Buy added (35.0 for BUY, was global 25.0)       |
//|     2. BUY condition: stochBuy uses Stoch_OS_Buy threshold       |
//|     3. SELL condition: stochSell uses Stoch_OB (75.0, unchanged) |
//|     4. Magic_Number: 1832025 -> 1842025                          |
//|     5. Order_Comment: BSv183 -> BSv184                           |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "184.0"
#property strict
...
//--- Asymmetric Stochastic Gate (NEW v184: separate BUY/SELL thresholds)
input string STOCH_SET          = "============"; //====== Stochastic Gate (v184 Asymmetric) ======
...
input double Stoch_OS_Buy       = 35.0;           //Stoch BUY level: K < this (relaxed: buys already 86.5% WR)
input double Stoch_OB_Sell      = 75.0;           //Stoch SELL level: K > this (strict: fixed sells 67.5%->87.5%)
...
input double Risk_Pct           = 5.0;            //Risk%: 5.0 (proven 200%+ return)
input double Fixed_Lot          = 0.0;            //Fixed lot (0 = use Risk_Pct)
input int    Max_Buys           = 1;              //Max concurrent BUY (no compounded concurrent losses)
...
input int    Magic_Number       = 1842025;        //Magic Number
input string Order_Comment      = "BSv184";       //Order comment
...
void OnTick()
{
   ...
   // === ASYMMETRIC STOCHASTIC GATE (NEW v184) ===
   // BUY: relaxed (K<35) - buys already had 86.5% WR in v181, don't over-filter
   // SELL: strict (K>75) - sells were only 67.5% WR, proved they needed filtering (v183: 87.5%!)
   ...
   bool stochBuy  = (stoch_k1 < Stoch_OS_Buy)  && (stoch_k1 > stoch_k2);  // oversold + turning up
   bool stochSell = (stoch_k1 > Stoch_OB_Sell) && (stoch_k1 < stoch_k2);  // overbought + turning down
   ...
   //=== Signal BUY === (relaxed Stochastic: K < 35)
   ...
   //=== Signal SELL === (strict Stochastic: K > 75, proved effective in v183)
