//+------------------------------------------------------------------+
//|   Buy & Sell EA v340 - RSI_Period=11 Sweet Spot                  |
//|   BASE: RSI_Period interpolation between v336 and v328           |
//|                                                                   |
//|   v339 POST-MORTEM (508 trades, MaxDD=5.04% ❌):                |
//|     rsiTrough filter (rsi[2]<rsi[3]) had near-zero effect       |
//|     RSI was already declining into bar[2] in virtually every    |
//|     case. MaxDD pushed fractionally above 5% threshold           |
//|     v336 remains BEST with RSI_Period=7 (MaxDD=4.97%✓)         |
//|                                                                   |
//|   RSI_PERIOD INTERPOLATION ANALYSIS:                             |
//|     Period=7:  505 trades, WR=82.38% (below 83% floor)          |
//|     Period=14: 380 trades, WR=83.47% (above 83% floor - v328)   |
//|     Slope: +0.156% WR per +1 RSI period                         |
//|     Crossover to WR=83%: at Period ~ 7 + (83%-82.38%)/0.156% ≈ 11|
//|                                                                   |
//|   v340 HYPOTHESIS:                                               |
//|   RSI_Period=11 should be the sweet spot:                        |
//|   - More trades than v328 (estimated 430-470 vs 380)             |
//|   - WR above 83% threshold (estimated ~83.0-83.1%)               |
//|   - Higher annual return than v328 (+16.25%) if both hold        |
//|   RSI(11) is faster than RSI(14) so catches more signals but    |
//|   still slow enough to maintain the 83%+ WR quality gate.       |
//|                                                                   |
//|   v340 CHANGES from v328:                                        |
//|     RSI_Period: 14 → 11 (sweet-spot interpolation)               |
//|     RSI_Buy: 45.0 (UNCHANGED - NON-NEGOTIABLE)                   |
//|   Expected: ~430-470 trades, WR ~83.0%+                         |
//|                                                                   |
//|   SCOREBOARD:                                                     |
//|     v328: +16.25%, MaxDD=4.99%, WR=83.47%, 380t   BEST ✓       |
//|     v336: RSI_Period=7 → +11.62%, WR=82.38%, 505t  NEAR ✓      |
//|     v337: RSI_Per=7+Buy=42 → WR=81.15%, 435t        FAIL ✗     |
//|     v338: Dual RSI(14)<55 → identical to v336       SKIP ~      |
//|     v339: rsiTrough → WR=82.48%, MaxDD=5.04%❌      FAIL ✗     |
//|     v340: RSI_Period=11 → ??? (sweet spot test)                  |
//|                                                                   |
...
input int    RSI_Period         = 11;             // v340: sweet-spot between 7 (82.38%) and 14 (83.47%)
...
input int    Magic_Number       = 3402057;
input string Order_Comment      = "BSv340";
...
void OnTick()
{
   ...
   double rsi1 = iRSI(NULL, 0, RSI_Period, PRICE_CLOSE, 1);  // RSI(11)
   double rsi2 = iRSI(NULL, 0, RSI_Period, PRICE_CLOSE, 2);  // RSI(11)
   ...
   if(h1Bull)
   {
      ...
      bool rsiOS       = (rsi1 < RSI_Buy);              // RSI(11) < 45
