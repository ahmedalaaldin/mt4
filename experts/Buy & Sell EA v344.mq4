//+------------------------------------------------------------------+
//|   Buy & Sell EA v344 - No TwoBar Filter (single pinbar entry)    |
//|   BASE: v343 (M5+M15+M30, WR=85.14%, +30.67%, 814t)             |
//|                                                                   |
...
//|                                                                   |
//|   v344 HYPOTHESIS: Remove TwoBar pattern requirement             |
//|   Current: bar[2] must close BELOW BB_Lower (2-bar move).        |
//|   v344: Allow 1-bar pinbar recovery at BB_Lower.                 |
//|   A single wick below BB_Lower + close inside = valid signal.    |
//|   These "clean pinbar" setups may have similar or better WR.     |
//|   Expected: +30-60% more trades, WR stays above 76.8% breakeven.|
//|                                                                   |
//|   v344 CHANGES from v343:                                        |
//|     Use_TwoBar_Pattern = false (single-bar recovery allowed)     |
//|     All other settings identical to v343                         |
//|                                                                   |
//|   NON-NEGOTIABLES (ALL unchanged from v343):                     |
//|     BB_Dev=2.5, BB_Period=20, Session=9-21, Max_Sells=0          |
//|     ATR_SL_Multi=0.80, Partial_Pct=0.20, Trail_ATR_Multi=0.5    |
//|     Hard_DD_Stop=0.0, RSI_Buy=45.0, RSI_Period=14               |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "344.0"
#property strict
...
//--- Two-bar pattern (v344: DISABLED - single pinbar recovery allowed)
input string TWOBAR_SET         = "============"; //====== Two-Bar Pattern ======
input bool   Use_TwoBar_Pattern = false;          // v344: disabled, allow 1-bar signals
...
input int    Magic_Number       = 3442057;        // v344 unique magic
input string Order_Comment      = "BSv344";
...
void OnTick()
{
   ...
   //=== M5 BUY (Use_TwoBar_Pattern=false → single pinbar allowed) ===
   if(h1Bull)
   {
      ...
      // v344: Use_TwoBar_Pattern=false → bbPrevBreak always true (single pinbar OK)
   ...
   //=== M15 BUY (v342+: same pattern on M15, TwoBar also disabled) ===
   if(Use_M15_Signals && isNewM15Bar && h1Bull)
   {
      ...
      // v344: TwoBar disabled on M15 too
   ...
   //=== M30 BUY (v343+: same pattern on M30, TwoBar also disabled) ===
   if(Use_M30_Signals && isNewM30Bar && h1Bull)
   {
      ...
      // v344: TwoBar disabled on M30 too
