//+------------------------------------------------------------------+
//|   Buy & Sell EA v224 - Bell-Curve Anti-Martingale Win Scaling     |
...
//|     v215: CCI filter       -> Cal=5.08   WORSE (momentum≠reversal)|
//|     v216: Limit orders     -> Cal=8.70   WORSE (misses fills)     |
//|     v217: Early BE 0.5 ATR -> BUGGY      (partialDone state broken)|
//|     v218: 75% partial      -> Cal=6.87   WORSE (trail 25% too small)|
//|     v219: Daily DD 4%      -> Cal=2.09   WORSE (cuts recoveries)  |
//|     v220: Peak guard T1=5% -> Cal=3.85   WORSE (too sensitive)    |
//|     v221: Scale 1.05/1.10  -> Cal=7.06   WORSE (lower peak ->     |
//|       smaller equity peak but trough unchanged -> %DD worsens)    |
//|       LESSON: Scaling 1.10/1.25 confirmed GLOBAL OPTIMUM in BOTH  |
//|       directions (gentler AND bolder both worse).                  |
//|     v222: ADX<25 filter    -> Cal=2.05   WORSE (removes good       |
//|       setups, MaxDD events occur across all ADX regimes)           |
//|     v223: Dual M5+M1 TF    -> Return -49.15% (NET LOSS) MaxDD 64%  |
//|       M1 BB signals too noisy (WR 71% vs 87%+ on M5 alone).       |
//|       M5 is the ONLY viable timeframe — NEVER add M1 again.       |
//|                                                                    |
//|   v224 HYPOTHESIS: Bell-Curve Anti-Martingale Win Scaling         |
//|     ROOT CAUSE of MaxDD: win streaks of 30-56 observed across     |
//|     versions. At streak peak, Risk_Pct × Scale_High = 7% × 1.25  |
//|     = 8.75% equity per loss. First reversal loss at peak          |
//|     = massive hit to equity, driving the MaxDD event.             |
//|     FIX: Bell-curve that REDUCES size as streak becomes extreme:  |
//|       Wins  0-4:  1.00x (base)                                    |
//|       Wins  5-9:  1.10x (building — same as v186 Scale_Mid)       |
//|       Wins 10-19: 1.20x (peak — slightly less than v186 1.25x)    |
//|       Wins 20-29: 1.10x (reducing — streak too long, be cautious) |
//|       Wins 30+:   1.00x (back to base — high reversal risk)       |
//|     Expected: Same Calmar through wins 0-19. Then BETTER Calmar   |
//|     by capping extreme-streak positions that cause MaxDD events.  |
//|     Loss downscaling unchanged: 0.50x → 0.25x → 0.10x            |
//|     ALL other parameters = v186 GLOBAL OPTIMA.                    |
//|     Run with PERIOD = M5 in Strategy Tester.                       |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "224.0"
#property strict
...
//--- Anti-Martingale (v224: Bell-Curve Win Scaling)
//--- Rationale: Positions shrink back at extreme streaks (20-29: 1.10x, 30+: 1.00x)
//--- to reduce first-reversal loss at streak peaks — root cause of MaxDD events.
input string AM_SET             = "============"; //====== Anti-Martingale ======
input int    WinStreak_Low      = 5;              //Scale up threshold: 5 wins (Wins 5-9)
input int    WinStreak_Mid      = 10;             //Peak threshold: 10 wins (Wins 10-19)
input int    WinStreak_High     = 20;             //Scale down threshold: 20 wins (Wins 20-29)
input int    WinStreak_Max      = 30;             //Back-to-base threshold: 30+ wins
input double Scale_Low          = 1.10;           //1.10x after 5 wins (building up)
input double Scale_Peak         = 1.20;           //1.20x after 10 wins (peak boost)
input double Scale_High_Down    = 1.10;           //1.10x after 20 wins (reducing — streak risky)
input double Scale_Max          = 1.00;           //1.00x after 30 wins (base — high reversal risk)
...
input int    Magic_Number       = 2242038;        //Magic Number
input string Order_Comment      = "BSv224";       //Order comment
...
double GetRiskScale()
{
   ...
   // v224: Bell-curve anti-martingale
   // Builds up to peak at wins 10-19, then reduces back at 20-29, returns to base at 30+
   // Key insight: streaks of 30-56 observed — reversal at peak = massive loss
   if(streak >= WinStreak_Max)  return Scale_Max;       // 1.00x: 30+ wins (high reversal risk — base)
   if(streak >= WinStreak_High) return Scale_High_Down; // 1.10x: 20-29 wins (reducing)
   if(streak >= WinStreak_Mid)  return Scale_Peak;      // 1.20x: 10-19 wins (peak boost)
   if(streak >= WinStreak_Low)  return Scale_Low;       // 1.10x:  5-9 wins (building up)
   if(streak > 0)               return 1.0;             // 1.00x:  1-4 wins (base)
   if(streak == 0)              return 1.0;             // 1.00x:  no history
   // Loss downscaling: unchanged from v186
   int losses = -streak;
