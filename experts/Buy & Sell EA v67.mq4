//+------------------------------------------------------------------+
//|   Buy & Sell EA v78 - BB upper band TP + breakeven stop         |
//|   Entry: H1 EMA200 + H1 RSI 50-75 + BB 2.5SD lower touch       |
//|          + M5 RSI<40 + RSI rising + Stoch K&D<25 + Bull 20%    |
//|   Exit: TP = BB upper band (3-5xATR target), BE after 1xATR    |
//|   Changes: Extended TP to BB upper, breakeven locks in profit   |
//|   Goal: Bigger avg win with same quality entry as v58            |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "78.0"
#property strict
...
input double ATR_SL_Multi       = 1.0;            //Initial SL = ATR x this
input double ATR_BE_Multi       = 1.0;            //Move SL to breakeven after ATR x this profit
input int    Max_Bars           = 60;             //Force close after N bars (longer for BB upper target)
...
input int    Magic_Number       = 782025;         //Magic Number
input string Order_Comment      = "BSv78";        //Order comment
...
void OnTick()
{
   ...
   // Breakeven management runs every tick
   double atrCurrent = iATR(NULL, 0, ATR_Period, 0);
   ManageBreakeven(atrCurrent);
   ...
   if(bbTouch && rsiOS && rsiRising && stochOS && strongBull)
   {
      ...
      double tpDist = bbUpper0 - entry; // TP at BB upper band
...
{
   if(atr <= 0) return;
   double beTrigger = ATR_BE_Multi * atr;
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(currentSL >= openPrice) continue; // Already at or above breakeven

      if(Bid - openPrice >= beTrigger)
      {
         double beSL = NormalizeDouble(openPrice, Digits);
         OrderModify(OrderTicket(), openPrice, beSL, OrderTakeProfit(), 0, clrCyan);
      }
   }
}
