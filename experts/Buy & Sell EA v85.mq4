//+------------------------------------------------------------------+
//|   Buy & Sell EA v96 - v77 + H1 ADX ranging-market filter        |
//|   Entry: H4 EMA200 + H1 EMA200 + H1 RSI 50-75                  |
//|          + BB 2.5SD lower touch + M5 RSI<40 + RSI rising        |
//|          + Stoch K&D<25 + Bull body 20%                         |
//|          + H1 ADX < 25 (range filter on hourly regime)          |
//|   Exit: TP = BB mid, SL = 1xATR                                 |
//|   Goal: H1 ADX smoother than M5 ADX, better regime detection    |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "96.0"
#property strict
...
//--- ADX Range Filter
input string ADX_SET            = "============"; //====== ADX Range Filter ======
input int    ADX_Period         = 14;             //ADX period
input double ADX_Max            = 25.0;           //H1 ADX max (skip when H1 trending >this)
...
input int    Magic_Number       = 962025;         //Magic Number
input string Order_Comment      = "BSv96";        //Order comment
...
void OnTick()
{
   ...
   // H1 ADX ranging-market filter: skip if hourly regime is strongly trending
   double h1adx = iADX(NULL, PERIOD_H1, ADX_Period, PRICE_CLOSE, MODE_MAIN, 0);
   if(h1adx > ADX_Max) return;
