//+------------------------------------------------------------------+
//|   Buy & Sell EA v254 - Bid/Ask-Based Trailing (vs BB_mid Trail)  |
...
//|       AvgWin=$111, AvgLoss=$457 → trail based on BB_mid (SMA20) |
//|       BB_mid trail is SLOW: SMA barely moves intra-bar           |
//|     v251: LOWER RISK + BIGGER TP → Calmar=8.68 (worse)          |
//|     v252: TIERED RISK → Calmar=7.52 (worse, early DD unchanged) |
//|     v253: Trail 0.8→0.5 ATR below BB_mid → Calmar=10.39         |
//|       Only +$0.15 avg win improvement — BB_mid still too slow    |
//|       Root cause: BB_mid (20-SMA) moves very slowly per tick     |
//|       Trail fires at nearly same price regardless of ATR_Multi   |
//|                                                                  |
//|   v254 HYPOTHESIS: Switch to Bid/Ask-based trailing              |
//|     After partial close at BB_midline:                           |
//|       OLD trail: bbMid - Trail_ATR_Multi × ATR (sluggish SMA)   |
//|       NEW trail: Bid   - Trail_ATR_Multi × ATR (per-tick price) |
//|     Bid-based trail updates EVERY TICK, not just each new bar    |
//|     When price runs after partial close:                         |
//|       Bid trail moves UP immediately with price                  |
//|       BB_mid trail barely moves (20-bar SMA lag)                 |
//|     Expected: Bid trail locks in significantly more profit        |
//|       from continuation moves after partial close                |
//|     Trail_ATR_Multi=1.5 (wider than BB_mid-based 0.5 to account |
//|       for tick-level noise without a 20-bar smoothing buffer)    |
//|     All v186 entry conditions UNCHANGED (NON-NEGOTIABLE)         |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "254.0"
#property strict
...
input double ATR_TP_Multi       = 2.5;            // initial hard TP (removed on partial)
input double Trail_ATR_Multi    = 1.5;            // v254: Bid/Ask-based trail (was BB_mid-0.5)
...
input int    Magic_Number       = 2542057;
input string Order_Comment      = "BSv254";
...
void ManageExits()
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(type == OP_BUY)
      {
         ...
         if(partialDone)
         {
            // v254: Bid-based trailing — updates every tick (vs BB_mid SMA which barely moves)
            // Trail_ATR_Multi=1.5 accounts for tick noise without the 20-bar SMA smoothing buffer
            double trailSL = NormalizeDouble(Bid - Trail_ATR_Multi * atr, Digits);
      ...
      else if(type == OP_SELL)
      {
         ...
         if(partialDone)
         {
            // v254: Ask-based trailing for sells — same per-tick logic
            double trailSL = NormalizeDouble(Ask + Trail_ATR_Multi * atr, Digits);
            if(trailSL < currentSL)
               OrderModify(tkt, openPrice, trailSL, 0, 0, clrBlue);
         }
