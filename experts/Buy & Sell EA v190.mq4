//+------------------------------------------------------------------+
//|   Buy & Sell EA v190 - Balance Peak Guard (bug fix)              |
...
//|            FAIL: guard used AccountEquity() which includes float- |
//|            ing P&L -- peaks inflated by open trades, guard fires  |
//|            during normal intraday swings -> return crushed 93%   |
//|   ROOT CAUSE OF v189 BUG: AccountEquity() includes floating P&L |
//|   FIX v190: Switch to AccountBalance() for peak tracking.        |
//|     Balance only rises when trades CLOSE profitably.             |
//|     Guard now triggers only on REALIZED losses (closed trades),  |
//|     not transient floating equity swings. Open trades' unrealized |
//|     gains/losses no longer pollute the peak or the DD calc.      |
//|   v190 = v189 + AccountBalance() peak fix + Magic/Comment update |
//|   All other parameters identical to v186/v189:                   |
//|     Risk=7%, Streak=5/10, Scale_Mid=1.10, Scale_High=1.25        |
//|     Guard: Pct=10%, Scale=0.5                                    |
//|   Changes vs v189:                                               |
//|     1. g_EquityPeak now tracks AccountBalance() not AccountEquity |
//|     2. Guard DD check uses curBalance not curEquity              |
//|     3. Magic_Number: 1892025 -> 1902025                          |
//|     4. Order_Comment: BSv189 -> BSv190                           |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "190.0"
#property strict
...
//--- Equity Peak Guard (NEW v189)
input string EQUITY_GUARD_SET   = "============"; //====== Equity Peak Guard (v189) ======
input double Equity_DD_Guard_Pct   = 10.0;        //Reduce lots if equity drops >X% from peak
...
input int    Magic_Number       = 1902025;        //Magic Number
input string Order_Comment      = "BSv190";       //Order comment
...
double   g_EquityPeak   = 0;        // NEW v189: rolling equity peak tracker

//+------------------------------------------------------------------+
int OnInit()
{
   g_EquityPeak = AccountBalance();  // v190: seed with BALANCE (closed-only) not equity
...
void OnTick()
{
   ...
   // === v190 FIX: Track BALANCE peak (closed trades only) ===
   // AccountBalance() excludes floating P&L from open trades.
   // This prevents unrealized gains from inflating the peak and
   // triggering the guard during normal intraday fluctuations.
   double curEquity  = AccountEquity();   // still used for daily DD guard + lots
   double curBalance = AccountBalance();  // used ONLY for peak guard (closed trades)
   if(curBalance > g_EquityPeak) g_EquityPeak = curBalance;
   ...
   // === v190: Balance Peak Guard (FIXED from v189) ===
   // Uses curBalance (closed trades only) vs g_EquityPeak (balance peak).
   // Guard fires only when REALIZED balance drops >X% from its peak.
   // Open trades' floating P&L cannot trigger false positives.
