//+------------------------------------------------------------------+
//|   Buy & Sell EA v154 - Risk 7% + ATR 0.60% (Peak Return Test)   |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry          |
//|   Results table so far (all with ATR=0.60%):                     |
//|     v150: Risk=5%, Scale=1.85 → 280.79% / 43.70% DD            |
//|     v152: Risk=6%, Scale=1.85 → 366.16% / 50.00% DD            |
//|     v153: Risk=5%, Scale=2.5  → 334.25% / 51.43% DD            |
//|   Key insight: Higher Risk_Pct >> higher Scale_High for           |
//|     return/DD efficiency. Risk=6% beats Scale=2.5 in BOTH        |
//|     return AND DD. v152 return/DD ratio = 7.32 (best so far).   |
//|   v154 Hypothesis: Risk=7% continues the trend                   |
//|     Expected: ~470-500% return, ~56-60% DD                       |
//|     Return/DD ratio target: 8.0+                                 |
//|     Daily DD guard (15%) may auto-limit DD: at 7% risk,          |
//|     2 consecutive losses at 1.85x scale hits ~15% daily limit,  |
//|     forcing stop — this could organically cap the DD impact.     |
//|   Changes vs v152:                                               |
//|     1. Risk_Pct: 6.0% → 7.0%                                   |
//|   All other params identical to v152 (proven best):              |
//|     BB_Dev=2.5, RSI_Buy=40, ATR=0.60%, Scale_High=1.85         |
//|     WinStreak_High=5, Max_Daily_DD_Pct=15%                      |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "154.0"
#property strict
...
input double Risk_Pct           = 7.0;            //Base risk %: 6%→7% (peak test)
...
input int    Magic_Number       = 1542025;        //Magic Number
input string Order_Comment      = "BSv154";       //Order comment
input double Max_Daily_DD_Pct   = 15.0;           //Daily DD guard (key at 7% risk)
