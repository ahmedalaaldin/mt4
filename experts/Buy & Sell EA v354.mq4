//+------------------------------------------------------------------+
//|   Buy & Sell EA v354 - ATR Spike filter (Multi=2.0, Period=50)   |
...
//|     v352: H1_RSI_Max=70 → +24.91%, MaxDD=5.23%❌   FAIL ✗      |
//|     v353: No H1_EMA → +11.86%, WR=70.20%❌         FAIL ✗      |
//|                                                                   |
//|   CONFIRMED: H1_200_EMA filter is CRITICAL (#1 quality gate)     |
//|                                                                   |
//|   v354 HYPOTHESIS: ATR_Spike_Multi=2.0, ATR_Spike_MA_Period=50.  |
//|   When current M5 ATR > 2× the 50-bar rolling ATR average,      |
//|   skip entry. High-volatility spikes (news events, stop hunts)   |
//|   may lead to trend continuation rather than mean reversion.     |
//|   These "spike" entries could be the low-quality subset pulling  |
//|   WR down from its theoretical peak. Filtering them out should   |
//|   improve WR with minimal trade count reduction (~5-15%).        |
//|                                                                   |
//|   v354 CHANGES from v343:                                        |
//|     ATR_Spike_Multi: 0.0 → 2.0 (enabled)                        |
//|     ATR_Spike_MA_Period: 50 (rolling 50-bar ATR average = ~4h)   |
//|                                                                   |
...
input int    ATR_Spike_MA_Period = 50;            // v354: 50-bar rolling ATR average (~4h)
input double ATR_Spike_Multi    = 2.0;            // v354: block when ATR > 2× average
...
input int    Magic_Number       = 3542057;        // v354 unique magic
input string Order_Comment      = "BSv354";
...
void OnTick()
{
   ...
   // v354: ATR spike filter — skip when ATR > 2× rolling average
