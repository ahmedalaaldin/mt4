//+------------------------------------------------------------------+
//|   Buy & Sell EA v221 - Gentler Anti-Martingale Win Scaling        |
...
//|       single 7% loss already triggers T1, reducing size during    |
//|       recovery; average profit/trade drops from $328 to $108;     |
//|       50 consec wins but tiny positions during recovery phases)   |
//|       LESSON: Peak equity guard threshold 5% = too sensitive for  |
//|       7% risk strategy. One loss = 7% equity drop = guard triggers.|
//|   v221 HYPOTHESIS: Gentler Anti-Martingale Win Scaling            |
//|     Scale_Mid = 1.05 (was 1.10) — 5% boost after 5 wins          |
//|     Scale_High = 1.10 (was 1.25) — 10% boost after 10 wins       |
//|     Loss downscaling unchanged: 0.50x → 0.25x → 0.10x            |
//|     Rationale: The 1.25x upscale at win streak peaks means         |
//|     positions are oversized exactly when a streak reversal hits.  |
//|     Reducing peak scale from 1.25x to 1.10x means:               |
//|     - Peak positions are 12% smaller when streak reverses         |
//|     - MaxDD should decrease as first-loss impact is smaller       |
//|     - Return decreases modestly (less amplification of long wins) |
//|     - Net Calmar: improves if MaxDD drop > return drop            |
//|     ALL other parameters = v186 GLOBAL OPTIMA.                    |
//|     Run with PERIOD = M5 in Strategy Tester.                       |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "221.0"
#property strict
...
//--- Anti-Martingale (v221: gentler win scaling)
...
input double Scale_Mid          = 1.05;           //v221: 1.05x after 5 wins (was 1.10 in v186)
input double Scale_High         = 1.10;           //v221: 1.10x after 10 wins (was 1.25 in v186)
...
input int    Magic_Number       = 2212038;        //Magic Number
input string Order_Comment      = "BSv221";       //Order comment
...
double GetRiskScale()
{
   ...
   // v221: gentler upscaling — 1.05x at 5 wins, 1.10x at 10 wins
