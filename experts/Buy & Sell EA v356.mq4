//+------------------------------------------------------------------+
//|   Buy & Sell EA v356 - Extended Session 7-23 (was 9-21)          |
//|   BASE: v354 (ATR_Spike_Multi=2.0) BEST ✓                        |
//|                                                                   |
//|   SCOREBOARD (all M5 unless noted):                              |
//|     v342: M5+M15 → +29.51%, MaxDD=4.74%, WR=86.90%, 756t ✓    |
//|     v343: M5+M15+M30 → +30.67%, MaxDD=4.87%, WR=85.14%, 814t ✓|
//|     v354: ATR_Spike=2.0 → +30.33%, MaxDD=4.65%✅   BEST ✓      |
//|     v355: ATR_Spike=1.5 → +0.73%, WR=70.16%, 305t  FAIL ✗      |
//|                                                                   |
//|   v355 FINDING: ATR_Spike_Multi=1.5 is too tight — BB touches   |
//|   ARE moderate ATR spikes by nature. 1.5x cuts 62% of signals.  |
//|   2.0x is confirmed optimal (only removes ~4 extreme outliers).  |
//|                                                                   |
//|   v356 HYPOTHESIS: Extend session from 9-21 to 7-23 (+4 hours). |
//|   - Pre-market 7-9: London open — strong directional moves but   |
//|     H1 EMA filter protects against raw trend following.          |
//|   - After-market 21-23: NY close wind-down — typically ranging   |
//|     = ideal for mean-reversion.                                  |
//|   Goal: +20-30% more trades while maintaining WR above 83%.     |
//|                                                                   |
//|   v356 CHANGES from v354:                                        |
//|     Session_Start_Hour: 9 → 7 (London open included)            |
//|     Session_End_Hour: 21 → 23 (NY close included)               |
//|                                                                   |
//|   NON-NEGOTIABLES (ALL unchanged):                               |
//|     BB_Dev=2.5, BB_Period=20, TwoBar_Lookback=1                  |
//|     Max_Sells=0, ATR_SL_Multi=0.80, H1_EMA_Period=200            |
//|     Partial_Pct=0.20, Trail_ATR_Multi=0.5, Hard_DD_Stop=0.0     |
//|     RSI_Buy=45.0, RSI_Period=14, Body_Pct=0.20                   |
//|     Risk_Pct=0.85, DD_Budget=3.8, ATR_Spike_Multi=2.0            |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "356.0"
#property strict

#include <stdlib.mqh>

//--- Session (EXTENDED to 7-23)
input string SESSION_SET        = "============"; //====== Session Settings ======
input int    Session_Start_Hour = 7;              // v356: extended from 9 → 7
input int    Session_End_Hour   = 23;             // v356: extended from 21 → 23
...
input int    Magic_Number       = 3562057;        // v356 unique magic
input string Order_Comment      = "BSv356";
