//+------------------------------------------------------------------+
//|   Buy & Sell EA v152 - Risk 6% + ATR Filter 0.60%               |
//|   Framework: H4 Direction / H1 RSI Structure / M5 Entry          |
//|   Base: v150 (280.79% / 43.70% DD / 86.03% WR / 458 trades)    |
//|   v151 result: 256.63% / 46.06% DD - WinStreak=7 hypothesis     |
//|     failed: harder-to-reach high scale hurts performance         |
//|   Reverted to v150 params: WinStreak_High=5, Scale_High=1.85    |
//|   Changes vs v150:                                               |
//|     1. Risk_Pct: 5.0% → 6.0%                                   |
//|        v147 tested 6% + NO ATR filter → 260%, 50% DD            |
//|        v150 tested 5% + ATR 0.60% → 281%, 43.7% DD             |
//|        Hypothesis: 6% + ATR 0.60% = higher return (~330%?)       |
//|        Does ATR filter contain DD at 6% risk? Target: DD<48%     |
//|        If ATR filter scales DD proportionally:                   |
//|        5%→280% returns, 6%→ ~336% returns (x1.2 from risk)      |
//|        5%→43.7% DD, 6%→ ~47-50% DD (x1.05-1.15 from risk)      |
//|   All other params identical to v150 (proven best config):        |
//|     BB_Dev=2.5, RSI_Buy=40, ATR=0.60%, Max_Buys=2              |
//|     WinStreak_High=5, Scale_High=1.85                           |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "152.0"
#property strict
...
input double Risk_Pct           = 6.0;            //Base risk % (raised from v150's 5%)
...
input int    Magic_Number       = 1522025;        //Magic Number
input string Order_Comment      = "BSv152";       //Order comment
