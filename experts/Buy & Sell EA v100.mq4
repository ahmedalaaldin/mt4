//+------------------------------------------------------------------+
//|   Buy & Sell EA v100 - Breakeven at BB Mid + TP at BB Upper     |
//|   Framework: 4H Direction / 1H Structure / 15M / 5M Entry       |
//|   v92 quality gates + dynamic breakeven management              |
//|   Strategy:                                                      |
//|     - Entry: same as v92 (all quality gates preserved)          |
//|     - TP: BB upper at entry time (bigger R:R)                   |
//|     - SL: 1xATR below entry                                     |
//|     - When Bid reaches BB mid: move SL to breakeven             |
//|     - Trades that reach BB mid become risk-free                 |
//|     - Avoids WR collapse seen in v99 (no "must hit upper" rule) |
//|   Goal: High R:R without WR collapse via breakeven protection   |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "110.0"
#property strict
...
input double H1_RSI_Min         = 50.0;           //H1 RSI min (uptrend zone)
input double H1_RSI_Max         = 75.0;           //H1 RSI max (not overextended)

//--- M15 Setup Filter
input string M15_SET            = "============"; //====== M15 Setup Filter ======
input int    M15_RSI_Period     = 14;             //M15 RSI period
input double M15_RSI_Max        = 50.0;           //M15 RSI max (pullback zone)

//--- Bollinger Bands (M5)
input string BB_SET             = "============"; //====== Bollinger Band Settings ======
input int    BB_Period          = 20;             //BB period
input double BB_Dev             = 2.5;            //BB SD (proven quality gate)
...
input int    Max_Bars           = 60;             //Force close after N bars (more time for BB upper)

//--- Money Management
input string MONEY_SET          = "============"; //====== Money Management ======
input double Risk_Pct           = 5.5;            //Risk % per trade (lower since SL rarely hits)
input double Loss_Risk_Scale    = 0.5;            //Risk multiplier after a loss
...
input int    Magic_Number       = 1102025;        //Magic Number
input string Order_Comment      = "BSv110";       //Order comment
...
bool WasLastTradeLoss()
{
   ...
   for(int i = OrdersHistoryTotal() - 1; i >= 0; i--)
   {
      ...
      if(OrderCloseTime() >= latestTime)
      {
         ...
         lastLoss   = (OrderProfit() + OrderCommission() + OrderSwap() < 0);
      }
   }
   return lastLoss;
}
...
void OnTick()
{
   ...
   bool h1Bull = (Close[1] > h1_ema) && (h1_rsi >= H1_RSI_Min) && (h1_rsi <= H1_RSI_Max);
   if(!h1Bull) return;

   // 15M: Setup
   double m15_rsi1 = iRSI(NULL, PERIOD_M15, M15_RSI_Period, PRICE_CLOSE, 1);
   bool m15Setup = (m15_rsi1 < M15_RSI_Max);
   if(!m15Setup) return;

   // 5M: Entry (all v92 quality gates)
   double bbUpper0  = iBands(NULL, 0, BB_Period, BB_Dev, 0, PRICE_CLOSE, MODE_UPPER, 0);
   ...
   if(bbTouch && rsiOS && rsiRising && stochOS && bullBody)
   {
      ...
      if(tpDist >= minTP)
      {
         ...
         if(openBuys < Max_Buys && barsSince >= Min_Bar_Gap)
         {
            ...
            {
               double riskScale = WasLastTradeLoss() ? Loss_Risk_Scale : 1.0;
               ...
               if(lots > 0)
               {
                  int tkt = OrderSend(Symbol(), OP_BUY, lots, entry, Slippage,
                                      ...
                                      NormalizeDouble(bbUpper0, Digits),
...
void ManageExits()
{
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      double bbMid = iBands(Symbol(), 0, BB_Period, BB_Dev, 0, PRICE_CLOSE, MODE_MAIN, 0);

      // Move SL to breakeven once price reaches BB mid
      double breakEvenSL = NormalizeDouble(OrderOpenPrice() + 1 * Point, Digits);
      if(Bid >= bbMid && OrderStopLoss() < OrderOpenPrice())
         OrderModify(OrderTicket(), OrderOpenPrice(), breakEvenSL, OrderTakeProfit(), 0, clrBlue);
