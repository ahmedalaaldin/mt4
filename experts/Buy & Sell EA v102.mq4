//+------------------------------------------------------------------+
//|   Buy & Sell EA v102 - Partial Exit + Trailing Stop             |
//|   Framework: 4H Direction / 1H Structure / 5M Entry             |
//|   Base: v101 streak-scaling + partial close at BB mid            |
//|   Strategy:                                                      |
//|     - Entry: v97 quality gates (no M15, full M5 gates)          |
//|     - No fixed TP                                                |
//|     - SL: 1xATR below entry                                      |
//|     - When Bid reaches BB mid: close 50%, move SL to breakeven  |
//|     - Remaining 50%: trail SL = BB mid - Trail_ATR_Multi * ATR  |
//|     - Risk scaling: streak-based from v101                       |
//|   Goal: Bank profit at BB mid, let winners run further           |
//|   Note: ManageExits runs every tick for real-time trail/partial  |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "112.0"
#property strict
...
input double Risk_Pct           = 7.0;            //Base risk % per trade
...
input int    Magic_Number       = 1122025;        //Magic Number
input string Order_Comment      = "BSv112";       //Order comment
...
void OnTick()
{
   ...
   // Manage exits every tick (for real-time partial close and trailing)
   ManageExits();
   ...
   // No M15 filter (same as v97/v101 — more trades, streak scaling compensates)

   // 5M: Entry (all quality gates)
            ...
            if(openBuys == 0 || HasEnoughDistance(OP_BUY, atr1))
            {
               ...
               if(lots > 0)
               {
                  int tkt = OrderSend(Symbol(), OP_BUY, lots, entry, Slippage,
                                      ...
                                      0,  // No fixed TP — trailing manages exit
...
void ManageExits()
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      // SL >= openPrice means partial close already done (SL moved to breakeven)
      ...
      if(!partialDone && Bid >= bbMid)
      {
         ...
         {
            // Lot too small to split — just move SL to breakeven and trail
            double beSL = NormalizeDouble(openPrice, Digits);
      ...
      // Phase 2: Trail remaining position (SL = bbMid - offset, only moves up)
      if(partialDone)
      {
