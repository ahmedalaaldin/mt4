//+------------------------------------------------------------------+
//|   Buy & Sell EA v220 - Peak Equity Drawdown Guard                 |
...
//|       LESSON: CCI is momentum (not reversal). Extra filters=WORSE. |
//|     v216: Limit orders 0.2 ATR -> +207.61%, DD 23.86%, Cal=8.70  |
//|       LESSON: Market orders optimal. Limits miss fills on bounces. |
//|     v217: Early BE (0.5 ATR) -> BUGGY - only 53 trades, 37.7% WR |
//|       LESSON: Early BE breaks the partialDone state machine.      |
//|     v218: 75% partial -> +154.55%, DD 22.51%, Cal=6.87, 160 tr   |
//|       LESSON: 75% partial WORSE. Trailing 25% too small.          |
//|       50% partial (v186) confirmed optimal.                       |
//|     v219: Daily DD 4% -> +56.56%, DD 27.03%, Cal=2.09, 192 tr    |
//|       LESSON: Tighter daily DD hurts badly. Cuts recovery trades  |
//|       while INCREASING MaxDD (misses intraday reversals).         |
//|       8% daily DD (v186) confirmed optimal.                       |
//|   v220 HYPOTHESIS: Peak Equity Drawdown Guard                     |
//|     Track all-time peak equity. When current equity drops below   |
//|     peak by threshold %, reduce lot size proportionally:          |
//|       0-5% below peak: full lots (1.0x)                           |
//|       5-10% below peak: 0.70x lots                                |
//|       10-15% below peak: 0.50x lots                               |
//|       >15% below peak: 0.30x lots                                 |
//|     Rationale: MaxDD 22-25% persists because positions are sized  |
//|     at full equity even during active drawdowns. Reducing size    |
//|     automatically during drawdowns prevents compounding losses.   |
//|     Recovery is slower but the drawdown ceiling is lower.        |
//|     Parameters: Peak_Guard = true, Peak_Guard_T1=5%, T2=10%, T3=15%|
//|     ALL other parameters = v186 GLOBAL OPTIMA.                    |
//|     Run with PERIOD = M5 in Strategy Tester.                       |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "220.0"
#property strict
...
//--- Peak Equity Guard (v220 NEW)
input string PEAK_SET           = "============"; //====== Peak Equity Guard ======
input bool   Peak_Guard         = true;           //v220: Enable peak equity drawdown guard
input double Peak_Guard_T1      = 5.0;            //Threshold 1: reduce to 70% lots when >5% below peak
input double Peak_Guard_T2      = 10.0;           //Threshold 2: reduce to 50% lots when >10% below peak
input double Peak_Guard_T3      = 15.0;           //Threshold 3: reduce to 30% lots when >15% below peak
...
input int    Magic_Number       = 2202038;        //Magic Number
input string Order_Comment      = "BSv220";       //Order comment
...
double   g_PeakEquity   = 0;   // v220: all-time peak equity tracker
...
//+------------------------------------------------------------------+
// v220: Calculate peak equity guard multiplier
double GetPeakGuard()
{
   if(!Peak_Guard || g_PeakEquity <= 0) return 1.0;
   double eq = AccountEquity();
   // Update peak
   if(eq > g_PeakEquity) g_PeakEquity = eq;
   double ddPct = (g_PeakEquity - eq) / g_PeakEquity * 100.0;
   if(ddPct >= Peak_Guard_T3) return 0.30;
   if(ddPct >= Peak_Guard_T2) return 0.50;
   if(ddPct >= Peak_Guard_T1) return 0.70;
   return 1.0;
}
...
void OnTick()
{
   ...
   // v220: Update peak equity on every tick
   double eq = AccountEquity();
   if(eq > g_PeakEquity) g_PeakEquity = eq;
   ...
   double peakGuard = GetPeakGuard();   // v220: peak equity guard multiplier
   ...
   if(h4Bull && h1Bull && stochBuy)
   {
      ...
      if(bbTouch && rsiOS && rsiRising && bullBody)
      {
         ...
         if(openBuys < Max_Buys && barsSince >= Min_Bar_Gap)
         {
            ...
            double lots  = CalcLots(slDist, riskScale * peakGuard);
            if(lots > 0)
            {
