//+------------------------------------------------------------------+
//|   Buy & Sell EA v113 - Cross-Direction Risk Gate                 |
//|   Framework: 4H Direction / 1H Structure / 5M Entry             |
//|   Base: v112 Dual Direction (253.80% proven)                     |
//|   Innovation:                                                    |
//|     - Cross-direction gate: new position (buy OR sell) only      |
//|       opens when ALL positions in BOTH directions are risk-free  |
//|       (SL >= openPrice for buys, SL <= openPrice for sells)      |
//|     - Prevents simultaneous buy+sell at full risk (was causing   |
//|       16.26% DD in v112 from combined 11% concurrent exposure)   |
//|     - At most 1 position at full risk at any time                |
//|     - Risk_Pct = 5.5% maintained for 200%+ return target        |
//|   Goal: Maintain 200%+ return, cut DD from 16% toward <10%      |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "123.0"
#property strict
...
input double Risk_Pct           = 5.5;            //Risk % per trade
...
input int    Magic_Number       = 1232025;        //Magic Number
input string Order_Comment      = "BSv123";       //Order comment
...
//--- All buys risk-free (SL >= openPrice)
...
//--- All sells risk-free (SL <= openPrice)
...
//--- ALL positions (both buys and sells) are risk-free — cross-direction gate
bool AllPositionsRiskFree()
{
   return AllBuysRiskFree() && AllSellsRiskFree();
}
...
void OnTick()
{
   ...
   //--- BUY logic: H4 bull + H1 bull + cross-direction gate
   ...
   if(h4Bull && h1Bull)
   {
      ...
      if(bbTouch && rsiOS && rsiRising && stochOS && bullBody)
      {
         ...
         if(tpDist >= minTP)
         {
            ...
            // Cross-direction gate: all positions (buys AND sells) must be risk-free
            bool canOpen = (openBuys == 0 && AllPositionsRiskFree())
                        || (openBuys > 0 && openBuys < Max_Buys && AllBuysRiskFree() && AllSellsRiskFree());
   ...
   //--- SELL logic: H4 bear + H1 bear + cross-direction gate
   ...
   if(h4Bear && h1Bear)
   {
      ...
      if(bbTouchUp && rsiOB && rsiFalling && stochOB && bearBody)
      {
         ...
         if(tpDist >= minTP)
         {
            ...
            // Cross-direction gate: all positions (buys AND sells) must be risk-free
            bool canOpen = (openSells == 0 && AllPositionsRiskFree())
                        || (openSells > 0 && openSells < Max_Sells && AllSellsRiskFree() && AllBuysRiskFree());
