//+------------------------------------------------------------------+
//|   Buy & Sell EA v305 - Tighter RSI Thresholds (35/65)           |
//|   BASE: v300 CHAMPION (Calmar=20.84, Return=258.39%, 1040 T)    |
//|   CHANGE: RSI_Buy 40→35, RSI_Sell 60→65 (TIGHTER)              |
//|   RATIONALE:                                                      |
//|     v304 tested RSI relaxation (40→45 / 60→55) and FAILED:     |
//|     1543 trades but MaxDD exploded 12.40%→36.43% (Calmar 6.49).|
//|     Relaxed RSI allows low-quality "early" setups that reverse  |
//|     = larger losses, worse MaxDD.                               |
//|     Opposite direction: TIGHTER RSI = require MORE extreme      |
//|     oversold/overbought conditions for entry.                   |
//|     Higher quality signals → fewer false breakdowns →           |
//|     lower MaxDD → better Calmar (even if fewer trades).         |
//|   HYPOTHESIS: ~700-900 trades, win rate ~95%+,                 |
//|     return ~220-260%, MaxDD ~8-10%, Calmar ~25-32             |
//|   DEAD ENDS SO FAR (20 confirmed):                             |
//|     - BB_Dev=2.0/3.0, ADX<25/30, BBW<5.5/8.0                  |
//|     - SL=0.6×ATR, Max_Bars=20, Session 7-22                    |
//|     - Longs only (v295): MaxDD=20.43%                           |
//|     - Partial_Pct=35% (v296), Body_Pct=40% (v297)              |
//|     - Binary HWM circuit breaker (v301): 1 trade only           |
//|     - Graduated HWM scaling (v302): Calmar=5.75                 |
//|     - H4 EMA200 filter (v303): kills shorts, MaxDD=23.27%       |
//|     - RSI_Buy=45/Sell=55 RELAX (v304): MaxDD=36.43%, WORST     |
//|   SCOREBOARD:                                                    |
//|     v300: Calmar=20.84 (CHAMPION — no stoch gate)              |
//|     v305: Tighter RSI 35/65 — TARGET lower MaxDD + Calmar>20   |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "305.0"
#property strict
...
//--- BB Entry Signal (BB_Dev=2.5 as per champion v300)
input string S1_SET             = "============"; //====== BB Signal ======
input int    BB_Period          = 20;
input double BB_Dev             = 2.5;
input int    RSI_Period         = 14;
input double RSI_Buy            = 35.0;   // v305 KEY CHANGE: tightened from 40 → 35
input double RSI_Sell           = 65.0;   // v305 KEY CHANGE: tightened from 60 → 65
...
//--- Stochastic settings (KEPT as inputs but NOT used as gate — v300 KEY CHANGE)
...
input int    Magic_Number       = 3052057;
input string Order_Comment      = "BSv305";
...
void OnTick()
{
   ...
   //=== BUY === v305: RSI_Buy=35 (tighter than v300's 40)
   if(h1Bull)
   {
      bool bbTouch   = (Low[1] <= bbLower1) && (Close[1] > bbLower1);
      bool rsiOS     = (rsi1 < RSI_Buy);      // v305: requires rsi < 35 (more extreme OS)
   ...
   //=== SELL === v305: RSI_Sell=65 (tighter than v300's 60)
   if(h1Bear)
   {
      bool bbTouchUp  = (High[1] >= bbUpper1) && (Close[1] < bbUpper1);
      bool rsiOB      = (rsi1 > RSI_Sell);    // v305: requires rsi > 65 (more extreme OB)
