//+------------------------------------------------------------------+
//|   Buy & Sell EA v19 - BB Bounce + RSI Dip Scalper               |
//|   BUY: dip to BB lower band OR RSI<40, recovery close           |
//|   SELL: spike to BB upper band OR RSI>60, recovery close        |
//|   TP = BB midline (SMA20) — 1:1 RR, needs only 50%+ WR         |
//|   Direction: H1 EMA200 bias                                     |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
...
input double RSI_Dip            = 40.0;           //BUY if RSI dips below this (oversold)
input double RSI_Spike          = 60.0;           //SELL if RSI spikes above this (overbought)
input double RSI_Skip_OB        = 68.0;           //Skip BUY if RSI above this (too overbought)
input double RSI_Skip_OS        = 32.0;           //Skip SELL if RSI below this (too oversold)
...
input int    ATR_Period         = 14;             //ATR Period (for SL sizing)
input double ATR_SL_Multi       = 1.0;            //Stop Loss = ATR x this
input int    Max_Bars           = 30;             //Force close after N bars
...
input int    Max_Buys           = 4;              //Max concurrent BUY positions
input int    Max_Sells          = 4;              //Max concurrent SELL positions
...
input int    Magic_Number       = 192025;         //Magic Number
input string Order_Comment      = "BSv19";        //Order comment
...
void OnTick()
{
   ...
   //--- BUY conditions (above H1 EMA200 = macro uptrend)
   if(Close[1] > h1_ema)
   {
      //  Signal A: dip to/below BB lower band then bullish recovery close
      bool sigBB  = (Low[1] <= bbLower1) && (Close[1] > bbLower1) && (Close[1] > Open[1]);
      //  Signal B: RSI dips below oversold level then crosses back up (bullish RSI recovery)
      bool sigRSI = (rsi2 < RSI_Dip) && (rsi1 >= RSI_Dip) && (Close[1] > Open[1]);

      bool buySignal = (sigBB || sigRSI) && (rsi1 < RSI_Skip_OB);
      ...
      if(buySignal)
      {
         ...
         if(openBuys < Max_Buys && barsSince >= Min_Bar_Gap)
         {
            if(openBuys == 0 || HasEnoughDistance(OP_BUY, atr1))
            {
               ...
               double tp     = NormalizeDouble(bbMid0, Digits);    // TP = BB midline
               if(tp <= entry) tp = NormalizeDouble(entry + slDist, Digits); // fallback 1:1
               double lots   = CalcLots(slDist);
               if(lots > 0)
               {
                  int tkt = OrderSend(Symbol(), OP_BUY, lots, entry, Slippage,
                                      NormalizeDouble(entry - slDist, Digits),
                                      tp, Order_Comment, Magic_Number, 0, clrGreen);
   ...
   //--- SELL conditions (below H1 EMA200 = macro downtrend)
   if(Close[1] < h1_ema)
   {
      //  Signal A: spike to/above BB upper band then bearish recovery close
      bool sigBB  = (High[1] >= bbUpper1) && (Close[1] < bbUpper1) && (Close[1] < Open[1]);
      //  Signal B: RSI spikes above overbought then crosses back down
      bool sigRSI = (rsi2 > RSI_Spike) && (rsi1 <= RSI_Spike) && (Close[1] < Open[1]);

      bool sellSignal = (sigBB || sigRSI) && (rsi1 > RSI_Skip_OS);
            ...
            if(openSells == 0 || HasEnoughDistance(OP_SELL, atr1))
            {
               ...
               if(lots > 0)
               {
                  int tkt = OrderSend(Symbol(), OP_SELL, lots, entry, Slippage,
                                      NormalizeDouble(entry + slDist, Digits),
                                      tp, Order_Comment, Magic_Number, 0, clrRed);
