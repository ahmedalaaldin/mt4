//+------------------------------------------------------------------+
//|   Buy & Sell EA v84 - BB 2.5SD + RSI confirmed cross above 40  |
//|   Entry: H1 EMA200 + H1 RSI 50-75 + BB 2.5SD lower touch       |
//|          + M5 RSI crosses FROM below 40 TO above 40             |
//|          + Stoch K&D<25 + Bull body 20%                         |
//|   Exit: TP = BB mid, SL = 1xATR                                 |
//|   Changes: RSI cross confirmed (was below, now above 40)        |
//|   Goal: Improve WR from 56% to 60%+ with cleaner entry timing  |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "84.0"
#property strict
...
input double RSI_Cross_Level    = 40.0;           //RSI crosses from below to above this level
...
input int    Magic_Number       = 842025;         //Magic Number
input string Order_Comment      = "BSv84";        //Order comment
...
void OnTick()
{
   ...
   // RSI on bars 1 and 2 to detect the cross
   ...
   // BB touch on bar 1 OR bar 2 (RSI might cross the bar AFTER the touch)
   bool bbTouch    = (Low[1] <= bbLower1 && Close[1] > bbLower1) ||
                     (Low[2] <= bbLower2 && Close[2] > bbLower2);
   // RSI confirmed cross: was below 40 two bars ago, now above 40
   bool rsiCrossUp = (rsi2 < RSI_Cross_Level) && (rsi1 >= RSI_Cross_Level);
   bool stochOS    = (stochK1 < Stoch_OB) && (stochD1 < Stoch_OB);
   bool strongBull = (candleBody > 0) && (candleRange > 0) && (candleBody >= Body_Pct * candleRange);

   if(bbTouch && rsiCrossUp && stochOS && strongBull)
   {
