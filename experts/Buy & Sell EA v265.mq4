//+------------------------------------------------------------------+
//|   Buy & Sell EA v265 - Loosen entry: Body=0, Spike=2.5          |
//|   Framework: H4+H1 Direction / M5 Entry (v186 CONFIRMED BEST)   |
//|   CONFIRMED OPTIMAL (from v263):                                 |
//|     Partial_Pct=0.20, Trail_ATR_Multi=0.1, ATR_TP_Multi=5.0+    |
//|     Risk_Pct=7.0, ATR_SL_Multi=0.8, BB_Period=20, BB_Dev=2.5    |
//|     RSI_Buy=40, RSI_Sell=60, Stoch_OS=25, Stoch_OB=75           |
//|                                                                  |
//|   v263: ATR_TP_Multi=5.0 -> Calmar=10.98 *** CHAMPION ***       |
//|     Return=111.58%, MaxDD=10.16%, 472 trades, Win=94.28%         |
//|   v264: TP=10.0 + Max_Bars fix -> Calmar=10.98 (no change)      |
//|     Proves: bbMid trail is the ONLY real exit, TP never fires    |
//|                                                                  |
//|   v265 HYPOTHESIS: Loosen entry filters to get more trades       |
//|     1) Body_Pct=0.0: remove candle body requirement              |
//|        Doji/indecisive candles also qualify if RSI+Stoch align   |
//|        BB touch + RSI + Stoch are the real quality filters       |
//|     2) ATR_Spike_Multi=2.5: allow slightly more volatile periods |
//|        Was 1.8; raising to 2.5 permits mild momentum entries     |
//|        while still blocking extreme spike bars                   |
//|     Target: more trades per day while preserving win rate        |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "265.0"
#property strict
...
input double Body_Pct           = 0.0;    // v265: remove body filter, BB+RSI+Stoch are quality filters
...
input double ATR_Spike_Multi    = 2.5;    // v265: 1.8->2.5, allow mild volatility entries
...
input double ATR_TP_Multi       = 5.0;            // v263 CONFIRMED: trail exits, TP rarely fires
...
input int    Magic_Number       = 2652057;
input string Order_Comment      = "BSv265";
...
void OnTick()
{
   ...
   //=== BUY: v265 - Body filter removed (Body_Pct=0.0) ===
   if(h4Bull && h1Bull && stochBuy)
   {
      bool bbTouch   = (Low[1] <= bbLower1) && (Close[1] > bbLower1);
      bool rsiOS     = (rsi1 < RSI_Buy);
      bool rsiRising = (rsi1 > rsi2);
      // Body_Pct=0.0: bullBody always true (any close direction qualifies)
      bool bullBody  = (Body_Pct <= 0.0) ? true :
                       ((candleBodyB > 0) && (candleRange > 0)
   ...
   //=== SELL: v265 - Body filter removed (Body_Pct=0.0) ===
   if(h4Bear && h1Bear && stochSell)
   {
      ...
      // Body_Pct=0.0: bearBody always true
      bool bearBody   = (Body_Pct <= 0.0) ? true :
                        ((candleBodyS > 0) && (candleRange > 0)
