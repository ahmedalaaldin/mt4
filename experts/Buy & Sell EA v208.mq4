//+------------------------------------------------------------------+
//|   Buy & Sell EA v208 - Scale_High=1.50 (Aggressive Compounding)  |
...
//|   v208 HYPOTHESIS: Scale_High=1.50 (from 1.25)                   |
//|     When we hit a streak of 10+ consecutive wins, we currently    |
//|     scale up to 1.25x risk. This version tries 1.50x instead.    |
//|     Rationale: during high-win streaks (10+), the system is in    |
//|     its optimal condition. More aggressive compounding at that     |
//|     point could meaningfully boost returns without proportionally  |
//|     increasing DD (since we're in peak performance mode).         |
//|     ALL other parameters = v186 GLOBAL OPTIMUM:                   |
//|       Max_Buys=1, Max_Sells=1, Risk_Pct=7.0%, H1 RSI gate ON     |
//|       Scale_Mid=1.10 at 5+, Scale_High=1.50 at 10+ (changed)     |
//|     Run with PERIOD = M5 in Strategy Tester.                      |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "208.0"
#property strict
...
//--- H1 RSI Gate (v208: v186 optimal values, same as v207)
...
//--- Money Management (v208: restored to v186 optimal 7%)
input string MONEY_SET          = "============"; //====== Money Management ======
input double Risk_Pct           = 7.0;            //v208: 7% — restored to v186 GLOBAL OPTIMUM
input double Fixed_Lot          = 0.0;            //Fixed lot (0 = use Risk_Pct)
input int    Max_Buys           = 1;              //v208: 1 concurrent BUY — restored to v186 GLOBAL OPTIMUM
input int    Max_Sells          = 1;              //v208: 1 concurrent SELL — restored to v186 GLOBAL OPTIMUM

//--- Anti-Martingale (v208 HYPOTHESIS: Scale_High=1.50 at 10+ wins)
input string AM_SET             = "============"; //====== Anti-Martingale ======
input int    WinStreak_Mid      = 5;              //Wins before mid scaling (v186 optimal)
input int    WinStreak_High     = 10;             //Wins before high scaling (v186 optimal)
input double Scale_Mid          = 1.10;           //v186 optimal: 1.10x at 5+ wins (unchanged)
input double Scale_High         = 1.50;           //v208 CHANGE: 1.50x at 10+ wins (was 1.25)
...
input int    Magic_Number       = 2082038;        //Magic Number
input string Order_Comment      = "BSv208";       //Order comment
...
double GetRiskScale()
{
   ...
   if(streak >= WinStreak_High) return Scale_High;  // 1.50x at 10+ wins (v208)
...
void OnTick()
{
   ...
   // H4 + H1 EMA direction filter + H1 RSI gate (v208: same as v186/v207 optimal)
