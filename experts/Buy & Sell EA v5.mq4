//+------------------------------------------------------------------+
//|   Buy & Sell EA v16 - Momentum Candle Scalper                   |
//|   Signal: Strong body bar (body/range > threshold) continuation  |
...
input double Body_Ratio_Min     = 0.65;           //Min body/range ratio for signal bar
input double Range_ATR_Min      = 0.5;            //Min bar range as multiple of ATR
...
input int    Magic_Number       = 162025;         //Magic Number
input string Order_Comment      = "BSv16";        //Order comment
...
void OnTick()
{
   ...
   bool bullBar = strongBar && (Close[1] > Open[1]);
   bool bearBar = strongBar && (Close[1] < Open[1]);

   //--- BUY: above H1 EMA200, strong bull bar, RSI not overbought
   ...
   //--- SELL: below H1 EMA200, strong bear bar, RSI not oversold
