//+------------------------------------------------------------------+
//|   Buy & Sell EA v403 - SELL signals enabled (Max_Sells=1)        |
//|   BASE: v393 (ALL-TIME BEST ratio=8.83) + full SELL logic        |
//|   CHANGE: Enable SELL signals (M5+M15+M30), add stage-2 partial  |
//|   for SELL exits (mirror of BUY stage 2: 37.5% at bbMid-0.5ATR) |
//|                                                                   |
...
//|     v355: ATR_Spike=1.5 → +0.73%, WR=70.16%, 305t  FAIL ✗      |
//|     v356: Session 7-23 → +8.32%, MaxDD=5.33%❌, 619t FAIL ✗     |
//|     v357: M5+M15 only → +28.66%, WR=87.05%✓, MaxDD=4.84% ✗    |
//|                                                                   |
//|   v357 FINDING: M30 signals lower WR but reduce MaxDD and add   |
//|   recovery volume. Removing M30 → higher WR but worse overall.  |
//|   v354 (M5+M15+M30+Spike=2.0) remains the best configuration.  |
//|                                                                   |
//|   v358 HYPOTHESIS: Add H4_EMA(50) as additional medium-term     |
//|   trend gate on top of v354. The H4 50-period EMA (~200 hours   |
//|   = ~8 trading days) tracks the medium-term trend. When H4      |
//|   price dips below H4_EMA(50) but H1 is still bullish, those   |
//|   entries may be fighting a medium-term downswing. Filtering    |
//|   them out could improve WR and reduce MaxDD with minimal        |
//|   trade count impact (H4 EMA(50) filters rarely in bull mkt).  |
//|                                                                   |
//|   v358 CHANGES from v354:                                        |
//|     Added: H4_EMA_Period_Fast = 50 filter (Close[H4,1]>EMA)     |
//|     All M5+M15+M30 sessions require BOTH H1 AND H4 to be        |
//|     bullish. H4_EMA_Period=200 (existing, unused) left as-is.   |
//|                                                                   |
//|   NON-NEGOTIABLES (ALL unchanged):                               |
...
//|     Risk_Pct=0.85, DD_Budget=3.8, H1_EMA_Period=200              |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "403.0"
#property strict
...
input int    H4_EMA_Fast_Period = 60;             // v374: confirmed optimal
input int    H1_EMA_Period      = 200;            // NON-NEGOTIABLE: H1 EMA filter CRITICAL
...
input int    ATR_Spike_MA_Period = 50;            // 50-bar rolling ATR average (~4h)
input double ATR_Spike_Multi    = 2.0;            // v354 confirmed optimal
...
input int    Max_Bars           = 22;             // v381: fine-tune above 20
...
input int    Max_Sells          = 1;              // v403: SELL signals enabled
...
input int    Magic_Number       = 4032057;        // v403 unique magic
input string Order_Comment      = "BSv403";
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(type == OP_BUY)
      {
         ...
         if(partialDone && !partial2Done && Bid >= bbMid + Trail_ATR_Multi * atr)
         {
            double stepLot   = MarketInfo(Symbol(), MODE_LOTSTEP);
            double minLot    = MarketInfo(Symbol(), MODE_MINLOT);
            double partLots2 = MathFloor(OrderLots() * 0.375 / stepLot) * stepLot;
      ...
      else if(type == OP_SELL)
      {
         ...
         bool partial2Done = (currentSL > 0 && currentSL < openPrice - atr * 0.05);
         ...
         if(partialDone && !partial2Done && Ask <= bbMid - Trail_ATR_Multi * atr)
         {
            // Stage 2 SELL: close 37.5% of remaining, lock SL to openPrice - 0.5*ATR
            double stepLot   = MarketInfo(Symbol(), MODE_LOTSTEP);
            double minLot    = MarketInfo(Symbol(), MODE_MINLOT);
            double partLots2 = MathFloor(OrderLots() * 0.375 / stepLot) * stepLot;
            if(partLots2 >= minLot && partLots2 < OrderLots())
            {
               double newSL2 = NormalizeDouble(openPrice - Trail_ATR_Multi * atr, Digits);
               if(OrderClose(tkt, partLots2, Ask, Slippage, clrMagenta))
                  if(OrderSelect(tkt, SELECT_BY_TICKET, MODE_TRADES))
                     OrderModify(tkt, openPrice, newSL2, 0, 0, clrBlue);
            }
            continue;
...
void OnTick()
{
   ...
   // v358: H4 EMA(50) medium-term trend gate
   ...
   // ATR spike filter — skip when ATR > 2× rolling average (v354 confirmed optimal)
   ...
   //=== M15 SELL ===
   if(Use_M15_Signals && isNewM15Bar && Max_Sells > 0 && h1Bear)
   {
      double m15_bbUpper1s = iBands(NULL, PERIOD_M15, BB_Period, BB_Dev, 0, PRICE_CLOSE, MODE_UPPER, 1);
      double m15_bbUpper2s = iBands(NULL, PERIOD_M15, BB_Period, BB_Dev, 0, PRICE_CLOSE, MODE_UPPER, 2);
      double m15s_rsi1     = iRSI(NULL, PERIOD_M15, RSI_Period, PRICE_CLOSE, 1);
      double m15s_rsi2     = iRSI(NULL, PERIOD_M15, RSI_Period, PRICE_CLOSE, 2);

      double m15s_high1  = iHigh(NULL, PERIOD_M15, 1);
      double m15s_close1 = iClose(NULL, PERIOD_M15, 1);
      double m15s_open1  = iOpen(NULL, PERIOD_M15, 1);
      double m15s_low1   = iLow(NULL, PERIOD_M15, 1);
      double m15s_close2 = iClose(NULL, PERIOD_M15, 2);

      double m15s_range = m15s_high1 - m15s_low1;
      double m15s_bodyS = m15s_open1 - m15s_close1;

      bool m15_bbTouchUp    = (m15s_high1 >= m15_bbUpper1s) && (m15s_close1 < m15_bbUpper1s);
      bool m15_rsiOB        = (m15s_rsi1 > RSI_Sell);
      bool m15_rsiFallingS  = (m15s_rsi1 < m15s_rsi2);
      bool m15_bearBody     = (m15s_bodyS > 0) && (m15s_range > 0) && (m15s_bodyS >= Body_Pct * m15s_range);
      bool m15_bbPrevBreakS = (!Use_TwoBar_Pattern) || (m15s_close2 >= m15_bbUpper2s);

      if(m15_bbTouchUp && m15_rsiOB && m15_rsiFallingS && m15_bearBody && m15_bbPrevBreakS)
      {
         if(CountTradesByMagic(OP_SELL, Magic_Number) < Max_Sells)
         {
            double m15s_atr = iATR(NULL, PERIOD_M15, ATR_Period, 1);
            if(m15s_atr > 0)
            {
               double m15s_slDist = ATR_SL_Multi * m15s_atr;
               double m15s_tpDist = ATR_TP_Multi * m15s_atr;
               double m15s_lots   = CalcLots(m15s_slDist, riskScale);
               double entry       = NormalizeDouble(Bid, Digits);
               double sl          = NormalizeDouble(entry + m15s_slDist, Digits);
               double tp          = NormalizeDouble(entry - m15s_tpDist, Digits);
               if(m15s_lots > 0)
               {
                  int tkt = OrderSend(Symbol(), OP_SELL, m15s_lots, entry, Slippage,
                                      sl, tp, Order_Comment, Magic_Number, 0, clrOrangeRed);
                  if(tkt > 0) g_TradesToday++;
               }
            }
         }
   ...
   //=== M30 SELL ===
   if(Use_M30_Signals && isNewM30Bar && Max_Sells > 0 && h1Bear)
   {
      double m30_bbUpper1s = iBands(NULL, PERIOD_M30, BB_Period, BB_Dev, 0, PRICE_CLOSE, MODE_UPPER, 1);
      double m30_bbUpper2s = iBands(NULL, PERIOD_M30, BB_Period, BB_Dev, 0, PRICE_CLOSE, MODE_UPPER, 2);
      double m30s_rsi1     = iRSI(NULL, PERIOD_M30, RSI_Period, PRICE_CLOSE, 1);
      double m30s_rsi2     = iRSI(NULL, PERIOD_M30, RSI_Period, PRICE_CLOSE, 2);

      double m30s_high1  = iHigh(NULL, PERIOD_M30, 1);
      double m30s_close1 = iClose(NULL, PERIOD_M30, 1);
      double m30s_open1  = iOpen(NULL, PERIOD_M30, 1);
      double m30s_low1   = iLow(NULL, PERIOD_M30, 1);
      double m30s_close2 = iClose(NULL, PERIOD_M30, 2);

      double m30s_range = m30s_high1 - m30s_low1;
      double m30s_bodyS = m30s_open1 - m30s_close1;

      bool m30_bbTouchUp    = (m30s_high1 >= m30_bbUpper1s) && (m30s_close1 < m30_bbUpper1s);
      bool m30_rsiOB        = (m30s_rsi1 > RSI_Sell);
      bool m30_rsiFallingS  = (m30s_rsi1 < m30s_rsi2);
      bool m30_bearBody     = (m30s_bodyS > 0) && (m30s_range > 0) && (m30s_bodyS >= Body_Pct * m30s_range);
      bool m30_bbPrevBreakS = (!Use_TwoBar_Pattern) || (m30s_close2 >= m30_bbUpper2s);

      if(m30_bbTouchUp && m30_rsiOB && m30_rsiFallingS && m30_bearBody && m30_bbPrevBreakS)
      {
         if(CountTradesByMagic(OP_SELL, Magic_Number) < Max_Sells)
         {
            double m30s_atr = iATR(NULL, PERIOD_M30, ATR_Period, 1);
            if(m30s_atr > 0)
            {
               double m30s_slDist = ATR_SL_Multi * m30s_atr;
               double m30s_tpDist = ATR_TP_Multi * m30s_atr;
               double m30s_lots   = CalcLots(m30s_slDist, riskScale);
               double entry       = NormalizeDouble(Bid, Digits);
               double sl          = NormalizeDouble(entry + m30s_slDist, Digits);
               double tp          = NormalizeDouble(entry - m30s_tpDist, Digits);
               if(m30s_lots > 0)
               {
                  int tkt = OrderSend(Symbol(), OP_SELL, m30s_lots, entry, Slippage,
                                      sl, tp, Order_Comment, Magic_Number, 0, clrTomato);
                  if(tkt > 0) g_TradesToday++;
               }
            }
         }
