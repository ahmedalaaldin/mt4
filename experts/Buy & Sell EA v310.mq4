//+------------------------------------------------------------------+
//|   Buy & Sell EA v310 - M1 Timeframe: Higher Signal Frequency      |
//|   BASE: v300 CHAMPION (Calmar=20.84, Return=258.39%, 1040 T)      |
//|   CHANGES from v300:                                              |
//|     BB_Dev   = 2.0  (was 2.5) — lower threshold → more M1 signals |
//|     Risk_Pct = 3.0  (was 7.0) — reduced for high-frequency trades |
//|     Max_Buys = 1, Max_Sells = 1 (reverted from v309's 2)          |
//|     Max_Bars = 30   (unchanged, but on M1 = 30 min max hold)       |
//|   TESTED ON: M1 TIMEFRAME (vs M5 for all prior versions)           |
//|   RATIONALE:                                                       |
//|     On M1, BB_Dev=2.0 triggers ~15-20 entries per day vs ~3/day   |
//|     on M5 with Dev=2.5. High-frequency scalping with tighter       |
//|     risk (3% per trade) targets 20+ trades/day requirement.        |
//|     30-bar hold on M1 = 30-minute max, allowing rapid turnover.   |
//|     ATR-based SL/TP automatically scales to M1 volatility.        |
//|   HYPOTHESIS: ~20-30 trades/day (7000-11000/year),                |
//|     WR ~75-85%, MaxDD ~8-12%, Return 150-300%, Calmar 15-25       |
//|   DEAD ENDS SO FAR (27 confirmed):                                |
//|     v304: RSI_Buy=45/RSI_Sell=55 → MaxDD=36.43%, Calmar=6.49    |
//|     v305: RSI_Buy=35/RSI_Sell=65 → Return=99.75%, Calmar=4.39   |
//|     v306: BB_Period=14 → Return=36.96%, Calmar=1.49              |
//|     v307: Partial=50%+Trail=0.3 → Trades halved, WR=87%          |
//|     v308: BB_Period=30 → Return=170.38%, MaxDD=25.53%, Calmar=6.67|
//|     v309: Max_Buys=2/Max_Sells=2 → Return=128.73%, Calmar=13.07  |
//|   SCOREBOARD:                                                      |
//|     v300: Calmar=20.84 (M5 champion)                              |
//|     v310: M1 timeframe with BB_Dev=2.0 targeting 20+/day          |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
...
//--- BB Entry Signal — v310: BB_Dev=2.0 on M1 for higher signal frequency
input string S1_SET             = "============"; //====== BB Signal ======
input int    BB_Period          = 20;             // v310: unchanged (v300 champion value)
input double BB_Dev             = 2.0;            // v310: 2.5 → 2.0 (more signals on M1)
...
//--- Money Management — v310: Risk_Pct=3.0 for high-frequency M1 trading
input string MONEY_SET          = "============"; //====== Money Management ======
input double Risk_Pct           = 3.0;            // v310: 3.5 → 3.0 (lower for M1 higher trade count)
input double Fixed_Lot          = 0.0;
input int    Max_Buys           = 1;              // v310: reverted to 1 (no scale-in)
input int    Max_Sells          = 1;              // v310: reverted to 1 (no scale-in)
...
input int    Magic_Number       = 3102057;
input string Order_Comment      = "BSv310";
...
void OnTick()
{
   ...
   // v309: BB_Period=20 (v300 champion) — DO NOT change
