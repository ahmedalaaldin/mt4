//+------------------------------------------------------------------+
//|   Buy & Sell EA v171 - Remove H4/H1 EMA filter, H1-RSI gate only |
//|   Framework: H1 RSI Gate Only / M5 BB+RSI Entry                  |
//|   ROOT CAUSE ANALYSIS (v170 → v171):                             |
//|     v170: 100% return, 14.34% DD — both targets missed           |
//|     Root cause: H4+H1 EMA direction filter blocks most signals   |
//|       - Only 59 sells vs 299 buys (gold bull market blocked H4   |
//|         bear condition almost entirely)                           |
//|       - H4/H1 EMA direction adds trend-following bias while the  |
//|         underlying strategy IS a mean-reversion (counter-trend)  |
//|       - H1 RSI gate (45-75 / 25-55) already ensures we're not   |
//|         fighting extreme momentum; EMA direction is redundant    |
//|   FIX:                                                            |
//|     Remove H4 and H1 EMA direction requirements entirely         |
//|     Keep ONLY the proven H1 RSI structural gate                  |
//|     Expand H1 RSI ranges slightly (45→40, 75→80, 25→20, 55→60) |
//|     Lower Risk_Pct to 1.0% (more trades → DD stays controlled)  |
//|   Expected: 2-3x more trades, DD proportionally lower, return up |
//|   Changes vs v170:                                                |
//|     1. Removed H4_EMA and H1_EMA direction filter from signals  |
//|     2. H1 RSI Buy gate: 45-75 → 40-80 (slightly wider)          |
//|     3. H1 RSI Sell gate: 25-55 → 20-60 (slightly wider)         |
//|     4. Risk_Pct: 2.0 → 1.0 (controls DD as trades increase)     |
//|     5. Magic_Number: 1702025 → 1712025                           |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "171.0"
#property strict
...
//--- H1 RSI Gate (ONLY higher-TF filter — EMA direction removed)
input string H1RSI_SET          = "============"; //====== H1 RSI Gate ======
input int    H1_RSI_Period      = 14;             //H1 RSI period
input double H1_RSI_Min         = 40.0;           //H1 RSI min for BUY (was 45, wider)
input double H1_RSI_Max         = 80.0;           //H1 RSI max for BUY (was 75, wider)
input double H1_RSI_Min_Sell    = 20.0;           //H1 RSI min for SELL (was 25, wider)
input double H1_RSI_Max_Sell    = 60.0;           //H1 RSI max for SELL (was 55, wider)
...
input double Risk_Pct           = 1.0;            //Risk%: 2.0→1.0 (lower as trade count rises)
...
input int    Magic_Number       = 1712025;        //Magic Number
input string Order_Comment      = "BSv171";       //Order comment
...
void OnTick()
{
   ...
   // === H1 RSI Gate ONLY (H4/H1 EMA direction removed — redundant for mean-reversion) ===
   double h1_rsi1 = iRSI(NULL, PERIOD_H1, H1_RSI_Period, PRICE_CLOSE, 1);

   bool h1Bull = (h1_rsi1 >= H1_RSI_Min) && (h1_rsi1 <= H1_RSI_Max);
   bool h1Bear = (h1_rsi1 >= H1_RSI_Min_Sell) && (h1_rsi1 <= H1_RSI_Max_Sell);
   ...
   //=== Signal BUY: H1 RSI gate only (no H4/H1 EMA direction) ===
   if(h1Bull)
   {
   ...
   //=== Signal SELL: H1 RSI gate only (no H4/H1 EMA direction) ===
   if(h1Bear)
   {
