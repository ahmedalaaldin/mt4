//+------------------------------------------------------------------+
//|   Buy & Sell EA v288 - H1 ADX Trend Filter                       |
//|   BASE: v280 CHAMPION (Calmar=18.36, Return=220%, 637 trades)    |
//|   CHANGE: H1 ADX < ADX_Max filter (use PERIOD_H1, not M5).       |
//|   v287 LESSON: M5 ADX<25 removed 85% of trades - too strict.    |
//|     Gold on M5 is almost always above ADX=25. Must use H1 ADX   |
//|     which reflects genuine hourly trend strength. H1 ADX>30     |
//|     means a real multi-hour trend → mean reversion on M5 fails. |
//|   HYPOTHESIS: H1 ADX filter removes ~15-25% of trades (those    |
//|     entered during H1 trending), reducing MaxConsecLoss from     |
//|     8 to ~3-5, improving Calmar while keeping trade count       |
//|     in the acceptable 480-540 range.                             |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "288.0"
#property strict
...
//--- H1 ADX Trend Strength Filter (NEW in v288)
//    v287 used M5 ADX<25 → removed 85% of trades (too strict)
//    v288 uses H1 ADX<30 → reflects genuine hourly trend strength
//    H1 ADX>30 = multi-hour trend active = skip M5 mean-reversion entries
input string ADX_SET            = "============"; //====== H1 ADX Filter ======
input bool   Use_ADX_Filter     = true;    // Block entries when H1 is strongly trending
input int    ADX_Period         = 14;      // ADX calculation period
input double ADX_Max            = 30.0;   // H1 ADX max (>30 = trending hour = skip)
...
input int    Magic_Number       = 2882057;
input string Order_Comment      = "BSv288";
...
void OnTick()
{
   ...
   //--- H1 ADX Trend Strength Filter (NEW v288)
   //    Uses PERIOD_H1 for ADX - reflects genuine hourly trend strength
   //    When H1 ADX > ADX_Max: sustained hourly trend active, M5 mean-reversion likely to fail
   double adx_h1 = iADX(NULL, PERIOD_H1, ADX_Period, PRICE_CLOSE, MODE_MAIN, 1);
   bool   adxOK  = (!Use_ADX_Filter) || (adx_h1 > 0 && adx_h1 < ADX_Max);
   if(!adxOK) return;
