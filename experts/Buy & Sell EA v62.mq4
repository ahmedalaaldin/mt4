//+------------------------------------------------------------------+
//|   Buy & Sell EA v73 - High-frequency RSI bounce                  |
//|   Entry: H1 EMA200 + H1 RSI 50-75 + M5 RSI crosses above 35    |
//|          (previously <35, now >35 = oversold bounce signal)      |
//|   Exit: TP = 0.4xATR, SL = 0.3xATR                              |
//|   Changes: No BB/Stoch/body filter, RSI crossover only           |
//|   Goal: 10-20 trades/day by catching every RSI bounce in trend   |
//+------------------------------------------------------------------+
#property copyright ""
#property link      ""
#property version   "73.0"
#property strict
...
input double RSI_Cross_Level    = 35.0;           //RSI crossover level (was below, now above)
input int    Min_Bar_Gap        = 3;              //Min bars between entries
input double Price_Gap_ATR      = 0.15;           //Min ATR gap between same-dir entries

//--- Exit
input string EXIT_SET           = "============"; //====== Exit Settings ======
input int    ATR_Period         = 14;             //ATR Period
input double ATR_TP_Multi       = 0.4;            //TP = ATR x this
input double ATR_SL_Multi       = 0.3;            //SL = ATR x this
input int    Max_Bars           = 20;             //Force close after N bars (100 min)

//--- Money Management
input string MONEY_SET          = "============"; //====== Money Management ======
input double Risk_Pct           = 2.0;            //Risk % per trade (low, many trades expected)
input double Fixed_Lot          = 0.0;            //Fixed lot (0 = use Risk_Pct)
input int    Max_Buys           = 8;              //Max concurrent BUY positions
...
input int    Magic_Number       = 732025;         //Magic Number
input string Order_Comment      = "BSv73";        //Order comment
...
void OnTick()
{
   ...
   // RSI crossover: previous bar was below the level, current bar is above
   bool rsiCross = (rsi2 < RSI_Cross_Level) && (rsi1 >= RSI_Cross_Level);
   ...
   if(rsiCross)
   {
      ...
      if(tpDist > slDist * 0.5) // minimal TP/SL filter
      {
